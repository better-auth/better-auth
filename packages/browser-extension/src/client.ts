...
interface BrowserExtensionClientOptions {
	scheme?: string;
	storage: {
...
export const browserExtensionClient = (opts: BrowserExtensionClientOptions) => {
	...
	if (!storage) {
		throw new Error(
			"Storage not found. Please provide a `storage` parameter in the browserExtensionClient options. Read more about the storage options here: https://www.better-auth.com/docs/integrations/browser-extension.mdx",
		);
	}
	return {
