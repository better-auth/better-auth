...
// A 1.6-shape user/account pair. The required `tier` additional field is
// either absent, present as a nullable column (added by hand without the
// NOT NULL step), or present as NOT NULL.
function createAccountDb({
	tier,
	seeded = true,
}: {
	tier?: "nullable" | "notNull";
	seeded?: boolean;
}) {
	const db = new DatabaseSync(":memory:");
	...
	const tierTarget = tier ? `, "tier"` : "";
	db.exec(
		`INSERT INTO "user" ("id", "name", "email", "emailVerified", "createdAt", "updatedAt"${tierTarget})
		 VALUES ('u1', 'Ada', 'ada@example.com', 1, '2020-01-01', '2020-01-01'${tierValue})`,
	);
	db.exec(
		`INSERT INTO "account" ("id", "accountId", "providerId", "userId", "createdAt", "updatedAt")
		 VALUES ('a1', '10769150350006150715113082367', 'google', 'u1', '2020-01-01', '2020-01-01')`,
	);
	return db;
}
...
describe("get-migration: unsafe schema changes on populated tables", () => {
	...
	it("refuses to add a required core additional field to a populated table", async () => {
		const failure = await captureFailure(
			getMigrations({ database: createAccountDb({}), ...tierField }),
		);

		expect(failure).toBeInstanceOf(BetterAuthError);
		expect(String(failure)).toContain(
			'Cannot add required column "tier" to populated table "user"',
		);
	});
	...
	it("plans a required column without a default when the table is empty", async () => {
		...
		expect(toBeAdded.find((t) => t.table === "user")?.fields).toHaveProperty(
			"tier",
		);
...
describe("get-migration: nullable columns for required fields", () => {
	it("warns and proceeds when a required field's live column is nullable", async () => {
		const db = createAccountDb({ tier: "nullable" });
		const warnings: string[] = [];
	...
	it("accepts a required field whose live column is not null", async () => {
		...
		expect(toBeAdded.find((t) => t.table === "user")).toBeUndefined();
		expect((await compileMigrations()).toLowerCase()).not.toContain(
			'alter table "user"',
		);
		expect(warnings.some((warning) => warning.includes('"tier"'))).toBe(false);
	});
...
describe("get-migration: inspecting a migration that cannot be applied", () => {
	it("reports the unsafe column change and still compiles the statements", async () => {
		const { compileMigrations, unsafeChanges } = await getMigrations(
			{ database: createAccountDb({}), ...tierField },
			{ throwOnUnsafe: false },
		);
		...
		expect((await compileMigrations()).toLowerCase()).toContain(
			'alter table "user" add column "tier" text not null',
		);
	});
