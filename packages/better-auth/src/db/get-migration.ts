...
export async function getMigrations(config: BetterAuthOptions) {
	...
	const isNumberIdModel = createIsNumberIdModel(config);
	...
	function getType(
		...
		> = {
			...
			// These are placeholders — id/foreignKeyId resolution is handled
			// per-model in the if-block below the typeMap.
			id: {
				postgres: "text",
				mysql: "varchar(36)",
				mssql: "varchar(36)",
		...
		if (fieldName === "id" || field.references?.field === "id") {
			// Resolve which model this id/FK belongs to for per-model number ID checks
			let resolvedModel: string | undefined;
			if (fieldName === "id" && tableName) {
				try {
					resolvedModel = getDefaultModelName(tableName);
				} catch {
					resolvedModel = tableName;
				}
			} else if (field.references?.model) {
				try {
					resolvedModel = getDefaultModelName(field.references.model);
				} catch {
					resolvedModel = field.references.model;
				}
			}
			const useNumberIdForModel = resolvedModel
				? isNumberIdModel(resolvedModel)
				: false;

			const numberId: Record<
				KyselyDatabaseType,
				ColumnDataType | RawBuilder<unknown>
			> = {
				postgres: sql`integer GENERATED BY DEFAULT AS IDENTITY`,
				mysql: "integer",
				mssql: "integer",
				sqlite: "integer",
			};
			const numberFkId: Record<
				KyselyDatabaseType,
				ColumnDataType | RawBuilder<unknown>
			> = {
				postgres: "integer",
			...
			if (fieldName === "id") {
				if (useNumberIdForModel) return numberId[provider];
				if (useUUIDs) return uuidId[provider];
				return typeMap.id[provider];
			}
			// foreignKeyId
			if (useNumberIdForModel) return numberFkId[provider];
			if (useUUIDs) return uuidId[provider];
	...
	if (toBeCreated.length) {
		for (const table of toBeCreated) {
			let resolvedTableModel: string;
			try {
				resolvedTableModel = getDefaultModelName(table.table);
			} catch {
				resolvedTableModel = table.table;
			}
			const useNumberIdForTable = isNumberIdModel(resolvedTableModel);
			const idType = getType(
				{ type: useNumberIdForTable ? "number" : "string" },
				"id",
				table.table,
			);
			let dbT = db.schema
