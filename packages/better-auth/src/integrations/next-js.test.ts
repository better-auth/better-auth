...
describe("next-js integration", () => {
	...
	it("runs every registerBeforeSend callback even if a sibling throws", async () => {
		vi.doMock("next/headers.js", () => ({
			cookies: vi.fn(async () => ({
				set: vi.fn(),
				delete: vi.fn(),
		...
		const [{ getTestInstance }, { createAuthMiddleware }] = await Promise.all([
			import("../test-utils/test-instance"),
			import("@better-auth/core/api"),
		]);
		...
		const survivingPlugin = {
			id: "surviving-before-send",
			hooks: {
				before: [
					{
						matcher: (ctx) => ctx.path === "/sign-in/email",
		...
		const { auth, testUser } = await getTestInstance({
			plugins: [failingPlugin, survivingPlugin],
		});
		...
		expect(ran).toContain("failing");
		expect(ran).toContain("surviving");
	});
});
