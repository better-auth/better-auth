...
describe("generateDictWithT", () => {
	it("should generate dict using tFuncServer and simpleDictWithTFunc", async () => {
		const simpleDictWithTFunc: SimpleDictWithTFunc = (t) => ({
			TEST: t("TEST"),
		});
		const tFuncServer = async (locale: string) => (key: string) =>
			`[${locale}]${key}`;
		const dict = await generateDictWithT(
			simpleDictWithTFunc,
			tFuncServer,
			"ja",
		);
