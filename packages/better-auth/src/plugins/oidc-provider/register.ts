...
import { OauthClient, OIDCOptions, SchemaClient } from "./types";
...
export async function registerEndpoint(
	...
	if (body.type) {
		if (
			...
			throw new APIError("BAD_REQUEST", {
				...
				error_description: `Type be 'native' or 'user-agent-based' for public applications`,
			});
		} else if (!isPublic && !(body.type === "web")) {
			throw new APIError("BAD_REQUEST", {
				error: "invalid_client_metadata",
				error_description: `Type be 'web' for confidential applications`,
			});
		}
	}
	...
	return ctx.json(schemaToOauth(client, true), {
		status: 201,
		headers: {
