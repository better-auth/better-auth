 ...
 */
async function decodeRefreshToken(opts: OIDCOptions, token: string) {
...
async function createUserTokens(
	...
	const [accessToken, sessionRefresh, idToken] = await Promise.allSettled([
		...
		scopes.includes("openid")
			? createIdToken(ctx, opts, user, client.clientId, scopes, nonce)
			: undefined,
	]).then((val) =>
		val.map((v) => (v.status === "fulfilled" ? v.value : undefined)),
	);
