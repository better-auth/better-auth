...
describe("oidc register", async (it) => {
	...
	it("should check that certain fields are overwritten", async () => {
		...
		expect(response.data?.software_version).toEqual(
			applicationRequest.software_version,
		);
		expect(response.data?.software_statement).toEqual(
			applicationRequest.software_statement,
		);

		expect(response.data?.redirect_uris).toEqual(
			applicationRequest.redirect_uris,
		);
		expect(response.data?.token_endpoint_auth_method).toEqual(
			applicationRequest.token_endpoint_auth_method,
		);
		expect(response.data?.grant_types).toEqual(applicationRequest.grant_types);
		expect(response.data?.response_types).toEqual(
			applicationRequest.response_types,
		);
