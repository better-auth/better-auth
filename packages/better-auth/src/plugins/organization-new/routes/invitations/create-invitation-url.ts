...
const baseInvitationUrlSchema = z.object({
	email: z.string().meta({
	...
	resend: z
		...
		.meta({
			description:
				"Resend the invitation, if the user is already invited. Eg: true",
		})
		.optional(),
	teamId: z.union([
		z
...
export type CreateInvitationURL<O extends OrganizationOptions> = ReturnType<
	typeof createInvitationURL<O>
>;

export const createInvitationURL = <O extends OrganizationOptions>(
	_options: O,
) => {
	...
	return createAuthEndpoint(
		"/organization/create-invitation-url",
		{
			method: "POST",
			requireHeaders: true,
			...
			metadata: {
				...
				openapi: {
					operationId: "createOrganizationInvitationURL",
					description:
						"Create an invitation to an organization and return the invitation URL instead of sending an email",
					responses: {
						"200": {
							description: "Success",
							content: {
		...
		async (ctx) => {
			...
			const url = generateInvitationURL(invitation.id);
