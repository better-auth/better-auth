...
export const admin = <O extends AdminOptions>(options?: O) => {
  ...
  const apiKeyMiddleware = (apiKey: string, prefix?: string) =>
    createAuthMiddleware(async (ctx) => {
      const providedApiKey = ctx.headers.get("x-api-key") || ctx.query.apiKey;
  ...
  const authMiddleware = opts.apiKey
    ? apiKeyMiddleware(opts.apiKey)
    : adminMiddleware;
