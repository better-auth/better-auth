	...
	({
		...
		onRequest: async (request, ctx) => {
			try {
				...
				const captchaResponseHeader =
					options.captchaResponseHeader &&
					options.captchaResponseHeader.trim() !== ""
						? options.captchaResponseHeader
						: "x-captcha-response";

				const remoteUserIPHeader =
					options.remoteUserIPHeader && options.remoteUserIPHeader.trim() !== ""
				...
				const remoteUserIP =
					request.headers.get(remoteUserIPHeader) ?? undefined;
