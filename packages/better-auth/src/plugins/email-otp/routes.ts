	...
	createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (!verificationValue) {
				throw APIError.from("BAD_REQUEST", ERROR_CODES.ERR_INVALID_OTP);
			}
			if (verificationValue.expiresAt < new Date()) {
				await ctx.context.internalAdapter.deleteVerificationValue(
					verificationValue.id,
				);
				throw APIError.from("BAD_REQUEST", ERROR_CODES.ERR_OTP_EXPIRED);
			}
			...
			if (attempts && parseInt(attempts) >= allowedAttempts) {
				...
				throw APIError.from("FORBIDDEN", ERROR_CODES.ERR_TOO_MANY_ATTEMPTS);
			}
