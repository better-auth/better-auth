...
/**
 * @see https://github.com/better-auth/better-auth/issues/11181
 */
describe("email-otp concurrent sends on a unique verification identifier", async () => {
	// Schemas generated by earlier releases (and many hand-written ones) put a
	// unique constraint on `verification.identifier`, so the second of two
	// concurrent inserts fails. Emulate that here through a plugin-declared index.
	const uniqueVerificationIdentifier = {
	...
	it("should email the same code to both requests when the stored code is recoverable", async () => {
		const otps: string[] = [];
		const { client, auth } = await getTestInstance(
			{
				plugins: [
		...
		const email = "concurrent-send@example.com";
		...
		const rows = await (await auth.$context).adapter.findMany({
			model: "verification",
