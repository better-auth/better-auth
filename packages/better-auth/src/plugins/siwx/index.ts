			...
			siwxCallback: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if ("errorCode" in ctx.query) {
						...
						throw ctx.redirect(
							`${errorRedirect}${errorRedirect.includes("?") ? "&" : "?"}provider=${provider}&errorCode=${encodeURIComponent(errorCode)}&errorMessage=${encodeURIComponent(errorMessage)}`,
						);
					}
					...
					try {
						...
						throw ctx.redirect(
							`${successRedirect}${successRedirect.includes("?") ? "&" : "?"}provider=${provider}`,
						);
					} catch (error: unknown) {
						if (error instanceof APIError) throw error;
						if (
