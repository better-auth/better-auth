...
describe("siwe", async (it) => {
  const publicKey = "0x1EBfa830CEcf15b8B3b3832C0a2F997386C0c1A7";
  ...
  const { data } = await client.siwe.nonce({ publicKey });
  ...
  it("should reject invalid public key", async () => {
    const { error } = await client.siwe.nonce({ publicKey: "invalid" });
  ...
  it("should reject invalid publicKey format", async () => {
    const { error } = await client.siwe.nonce({ publicKey: "not_a_valid_key" });
    expect(error).toBeDefined();
