...
import { isPrivateKeyEncrypted, parseJwk, toJwtTime } from "./utils";
...
describe("jwt", async () => {
	...
	describe("Default settings", async () => {
		...
		it("Should fail to import JWK with the same ID", async () => {
			...
			).rejects.toThrow("A JWK with the same ID exists already");
		});
	...
	describe.each([
		...
		describe.each([
			...
			describe("Default endpoints and custom plugins'", async () => {
				it("Should sign JWT and verify it", async () => {
					...
					async function testJwtVerification(jwt: string): Promise<JWTPayload> {
				...
				async function verifyJwt(
					...
					optional?: {
						...
						options?: JwtVerifyOptions; // Setting options to null, forces defaults so we can test them
					},
				): Promise<{
				...
				it("Should sign JWT with claims removed and verify it", async () => {
					...
					async function testJwtVerification(
						...
						const { payload, payloadCustom, payloadManual } = await verifyJwt(
							...
							jwt,
							claims,
							{ now, localJwks, options: { requiredClaims: null } },
						);
				...
				it(`Private keys should be ${decrypted ? "decrypted" : "encrypted"} and re-${decrypted ? "encrypt" : "decrypt"}able`, async () => {
					...
					for (const key of keys)
						expect(isPrivateKeyEncrypted(key.privateKey)).toBe(!decrypted);

					await auth.api.reencrypt({ body: { decrypted: !decrypted } });
