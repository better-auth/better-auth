...
import type { RbacHookContext, RoleAssignmentHookData, PermissionCheckHookData } from "../rbac-hooks";
    ...
    organizationRbac({
      ...
      rbac: {
        ...
        hooks: {
          ...
          onMemberJoin: async (data: { userId: string; organizationId: string; teamId?: string; invitedBy?: string }, context: RbacHookContext) => {
            ...
            if (rbacAdapter) {
              const viewerRole = await rbacAdapter.findRoleByName("Viewer", data.organizationId);
              if (viewerRole) {
          ...
          onUnauthorizedAccess: async (data: PermissionCheckHookData, context: RbacHookContext) => {
            console.warn(`Unauthorized access attempt: User ${data.userId} tried ${data.action} on ${data.resource}`);
            
            // Could integrate with monitoring/alerting systems here
          },
          
          // Organization-specific setup
          onOrganizationCreate: async (data: { organizationId: string; creatorId: string }, context: RbacHookContext) => {
            console.log(`Setting up RBAC for organization ${data.organizationId}`);
      ...
      organizationCreation: {
        ...
        afterCreate: async ({ organization, member, user }: any) => {
          console.log(`Organization ${organization.name} created with RBAC enabled`);
