...
export type OrganizationSchema<O extends OrganizationOptions> =
	O["dynamicAccessControl"] extends { enabled: true }
		...
		: {} & (O["teams"] extends { enabled: true }
				...
				} & (O extends { keepActiveOrganization: true } | { teams: { enabled: true; keepActiveTeam: true }} ? {
				  user: {
						fields: InferSchema<O["schema"] extends BetterAuthPluginDBSchema ? O["schema"] : {}, "user", {
						  lastOrganizationId: {
								type: "string";
								required: false;
								input: false;
							}
						}>["fields"] & (O extends { teams: { enabled: true; keepActiveTeam: true }} ? {
						  lastTeamId: {
								type: "string";
								required: false;
								input: false;
