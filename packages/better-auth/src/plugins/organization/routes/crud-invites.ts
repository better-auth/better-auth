	...
	return createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (!organizationId) {
				throw APIError.from(
					"BAD_REQUEST",
					ORGANIZATION_ERROR_CODES.ERR_ORGANIZATION_NOT_FOUND,
				);
			}
			...
			if (!member) {
				throw APIError.from(
					"BAD_REQUEST",
					ORGANIZATION_ERROR_CODES.ERR_MEMBER_NOT_FOUND,
				);
			}
			...
			if (!canInvite) {
				throw APIError.from(
					"FORBIDDEN",
					ORGANIZATION_ERROR_CODES.ERR_YOU_ARE_NOT_ALLOWED_TO_INVITE_USERS_TO_THIS_ORGANIZATION,
				);
			}
				...
				if (ctx.context.orgOptions.dynamicAccessControl?.enabled) {
					...
					if (stillInvalid.length > 0) {
						throw new APIError("BAD_REQUEST", {
							message: `${ORGANIZATION_ERROR_CODES.ERR_ROLE_NOT_FOUND}: ${stillInvalid.join(", ")}`,
						});
					}
				} else {
					throw new APIError("BAD_REQUEST", {
						message: `${ORGANIZATION_ERROR_CODES.ERR_ROLE_NOT_FOUND}: ${unknownRoles.join(", ")}`,
					});
				}
			}
			...
			if (pendingInvitations.length >= invitationLimit) {
				throw APIError.from(
					"FORBIDDEN",
					ORGANIZATION_ERROR_CODES.ERR_INVITATION_LIMIT_REACHED,
				);
			}
			...
			if (
				...
				for (const teamId of teamIds) {
					...
					if (!team) {
						throw APIError.from(
							"BAD_REQUEST",
							ORGANIZATION_ERROR_CODES.ERR_TEAM_NOT_FOUND,
						);
					}
					...
					if (team.members.length >= maximumMembersPerTeam) {
						throw APIError.from(
							"FORBIDDEN",
							ORGANIZATION_ERROR_CODES.ERR_TEAM_MEMBER_LIMIT_REACHED,
						);
					}
				}
			}
	...
	createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (
				...
				throw APIError.from(
					"BAD_REQUEST",
					ORGANIZATION_ERROR_CODES.ERR_INVITATION_NOT_FOUND,
				);
			}
			...
			if (invitation.email.toLowerCase() !== session.user.email.toLowerCase()) {
				throw APIError.from(
					"FORBIDDEN",
					ORGANIZATION_ERROR_CODES.ERR_YOU_ARE_NOT_THE_RECIPIENT_OF_THE_INVITATION,
				);
			}
			...
			if (
				...
				throw APIError.from(
					"FORBIDDEN",
					ORGANIZATION_ERROR_CODES.ERR_EMAIL_VERIFICATION_REQUIRED_BEFORE_ACCEPTING_OR_REJECTING_INVITATION,
				);
			}
			...
			if (!acceptedI) {
				throw APIError.from(
					"BAD_REQUEST",
					ORGANIZATION_ERROR_CODES.ERR_FAILED_TO_RETRIEVE_INVITATION,
				);
			}
			if (
				ctx.context.orgOptions.teams &&
			...
			if (!acceptedI) {
				return ctx.json(null, {
					status: 400,
					body: {
						message: ORGANIZATION_ERROR_CODES.ERR_INVITATION_NOT_FOUND.message,
					},
				});
			}
	...
	createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (!canCancel) {
				throw APIError.from(
					"FORBIDDEN",
					ORGANIZATION_ERROR_CODES.ERR_YOU_ARE_NOT_ALLOWED_TO_CANCEL_THIS_INVITATION,
				);
			}
	...
	createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (!member) {
				throw APIError.from(
					"BAD_REQUEST",
					ORGANIZATION_ERROR_CODES.ERR_INVITER_IS_NO_LONGER_A_MEMBER_OF_THE_ORGANIZATION,
				);
			}
