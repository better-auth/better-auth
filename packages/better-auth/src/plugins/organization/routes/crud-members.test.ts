...
describe("transferOwnership", async () => {
	...
	it("should reject transfer when requirePasswordToTransferOwnership is true and no password provided", async () => {
		const { auth, signInWithTestUser } = await getTestInstance({
			plugins: [
				organization({
					requirePasswordToTransferOwnership: true,
				}),
			],
		});
		const { headers: ownerHeaders } = await signInWithTestUser();
		...
		const org = await auth.api.createOrganization({
			body: { name: "require-pw-no-pw", slug: "require-pw-no-pw" },
			headers: ownerHeaders,
		});

		const newOwnerUser = await auth.api.signUpEmail({
			body: {
				email: "newowner-require-pw@test.com",
				password: "password",
				name: "New Owner RequirePw",
			},
		});
		await auth.api.addMember({
			body: {
	...
	it("should allow transfer when requirePasswordToTransferOwnership is true and correct password is provided", async () => {
		const { auth, signInWithTestUser } = await getTestInstance({
			plugins: [
				organization({
					requirePasswordToTransferOwnership: true,
		...
		const org = await auth.api.createOrganization({
			body: { name: "require-pw-with-pw", slug: "require-pw-with-pw" },
			headers: ownerHeaders,
		});

		const newOwnerUser = await auth.api.signUpEmail({
			body: {
				email: "newowner-with-pw@test.com",
				password: "password",
				name: "New Owner WithPw",
			},
		});
		await auth.api.addMember({
			body: {
		...
		const res = await auth.api.transferOwnership({
			body: {
				organizationId: org?.id as string,
				memberId: newOwnerMember?.id as string,
				password: "test123456", // correct password
			},
			headers: ownerHeaders,
		});
		expect(res).toMatchObject({
	...
	it("should verify password even when sendTransferOwnershipEmail is not configured", async () => {
		const { auth, signInWithTestUser } = await getTestInstance({
			plugins: [organization()],
		});
		const { headers: ownerHeaders } = await signInWithTestUser();
		...
		const org = await auth.api.createOrganization({
			body: { name: "pw-no-email", slug: "pw-no-email" },
			headers: ownerHeaders,
		});

		const newOwnerUser = await auth.api.signUpEmail({
			body: {
				email: "newowner-pw-no-email@test.com",
				password: "password",
				name: "New Owner PwNoEmail",
			},
		});
		await auth.api.addMember({
			body: {
		...
		// Wrong password should be rejected even without email configured
		await expect(
			auth.api.transferOwnership({
				body: {
					organizationId: org?.id as string,
