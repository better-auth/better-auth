...
export const steamAuthPlugin = (
	...
	endpoints: {
		signInWithSteam: createAuthEndpoint(
			...
			async (ctx) => {
                const encodedCallbackURL = encodeURIComponent(ctx.body.callbackURL || ctx.context.baseURL)
		...
		steamCallback: createAuthEndpoint(
			...
			async (ctx) => {
				...
				if (!steamid) {
					throw ctx.redirect(
						`${errorURL}?error=steamid_missing`,
					);
				}
				...
				if (profileRes.error) {
					...
					throw ctx.redirect(
						`${errorURL}?error=steam_profile_fetch_failed`,
					);
				}
				...
				if (!account) {
                    const userDetails = (await config.mapProfileToUser?.(profile)) || {} as User
                    ...
                    if(!user){
                        ...
                        throw ctx.redirect(
                            `${errorURL}?error=user_creation_failed`,
                        );
                    }
                    ...
                    if(!account){
                        ...
                        throw ctx.redirect(
                            `${errorURL}?error=account_creation_failed`,
                        );
                    }
				} else {
