...
export const getMcpPlugin = (ctx: AuthContext): Omit<BetterAuthPlugin, 'options'> & { options: MCPOptions } => {
  const plugin = ctx.options.plugins?.find(
    (plugin): plugin is Omit<BetterAuthPlugin, 'options'> & { options: MCPOptions } =>
      plugin.id === 'mcp'
  );
			...
			getMcpOAuthConfig: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if (scopes?.includes("openid")) {
						const oidcPluginOptions: OIDCOptions & { claims?: string[] } = getOidcPlugin(ctx.context).options
						metadata = oidcMetadata(
							ctx, {
								...oidcPluginOptions,
								advertisedMetadata: scopes_supported && {
									claims_supported:
										oidcPluginOptions.advertisedMetadata?.claims_supported ?? oidcPluginOptions.claims ?? [],
									scopes_supported,
								},
							}
						)
...
>({
	...
	const jwtPluginOptions = auth?.options?.plugins?.find(
		(plugin): plugin is Omit<BetterAuthPlugin, 'options'> & { options: JwtPluginOptions } =>
			plugin.id === "jwt"
	)?.options;
	...
	try {
		console.log({
				...
				issuer: jwtPluginOptions?.jwt?.issuer ??
					auth.options.baseURL
		})
...
>(
	...
	return async (req: Request) => {
		const mcpMetadata = metadata ?? auth?.options?.plugins?.find(
			(plugin): plugin is Omit<BetterAuthPlugin, 'options'> & { options: MCPOptions } =>
				plugin.id === "mcp"
		...
		return new Response(JSON.stringify(mcpOverridesMetadata), {
			...
			headers: {
				...
				"Cache-Control": "public, max-age=15, stale-while-revalidate=15, stale-if-error=86400", // 15 sec
			},
		})
	}
};
