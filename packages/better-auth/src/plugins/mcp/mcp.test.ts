...
describe("mcp - no scopes", async () => {
	...
	it("should obtain auth server metadata", async () => {
		const response = await unauthenticatedClient.$fetch<AuthServerMetadata>(
		...
		expect(response.data?.authorization_endpoint).toBe(
			`${auth.options.baseURL}/api/auth/oauth2/authorize`,
		);
		expect(response.data?.token_endpoint).toBe(
			`${auth.options.baseURL}/api/auth/oauth2/token`,
		);
		expect(response.data?.jwks_uri).toBe(
			`${auth.options.baseURL}/api/auth/jwks`,
		);
		expect(response.data?.registration_endpoint).toBe(
			`${auth.options.baseURL}/api/auth/oauth2/register`,
		);
		...
		expect(response.data?.grant_types_supported).toMatchObject([
			"authorization_code",
			"refresh_token",
			"client_credentials",
		]);
		expect(
			response.data?.token_endpoint_auth_signing_alg_values_supported,
		).toMatchObject(["EdDSA"]);
		expect(response.data?.token_endpoint_auth_methods_supported).toMatchObject([
			"client_secret_basic",
			"client_secret_post",
		]);
		expect(response.data?.code_challenge_methods_supported).toMatchObject([
			"s256",
		]);
	});
});
...
describe("mcp - oidc", async () => {
	...
	it("should obtain oidc-like metadata", async () => {
		const response = await unauthenticatedClient.$fetch<OIDCMetadata>(
			"/.well-known/oauth-authorization-server",
			{
				method: "GET",
			},
		...
		expect(response.data?.userinfo_endpoint).toBe(
			`${auth.options.baseURL}/api/auth/oauth2/userinfo`,
		);
		expect(response.data?.subject_types_supported).toBeDefined();
		expect(response.data?.id_token_signing_alg_values_supported).toMatchObject([
			"EdDSA",
		]);
...
describe("mcp - checkMcp", async () => {
	...
	const client = createAuthClient({
		plugins: [testJwtHelperClient(), mcpClient()],
		baseURL: authServerBaseUrl,
		fetchOptions: {
			customFetchImpl,
			headers,
