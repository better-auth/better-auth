...
async function adapterTest(
	...
	return new Promise((resolveTest) => {
		...
		test
			.skipIf(disabledTests?.CREATE_MODEL)
			.sequential(
		...
		test
			.skipIf(disabledTests?.CREATE_MODEL_SHOULD_ALWAYS_RETURN_AN_ID)
			.sequential(
		...
		test
			.skipIf(disabledTests?.FIND_MODEL)
			.sequential(
		...
		test
			.skipIf(disabledTests?.FIND_MODEL_WITHOUT_ID)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.FIND_MODEL_WITH_MODIFIED_FIELD_NAME)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.FIND_MODEL_WITH_SELECT)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.UPDATE_MODEL)
			.sequential(
		...
		test
			.skipIf(disabledTests?.SHOULD_FIND_MANY)
			.sequential(
		...
		test
			.skipIf(disabledTests?.SHOULD_FIND_MANY_WITH_WHERE)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.SHOULD_FIND_MANY_WITH_OPERATORS)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.SHOULD_WORK_WITH_REFERENCE_FIELDS)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
		...
		test
			.skipIf(disabledTests?.SHOULD_PREFER_GENERATE_ID_IF_PROVIDED)
			.sequential(
				`${testPrefix ? `${testPrefix} - ` : ""}${
...
export async function runNumberIdAdapterTest(opts: NumberIdAdapterTestOptions) {
	...
	describe("Should run number id specific tests", async () => {
		...
		test
			.skipIf(opts.disableTests?.SHOULD_RETURN_A_NUMBER_ID_AS_A_RESULT)
			.sequential(
		...
		test
			.skipIf(opts.disableTests?.SHOULD_INCREMENT_THE_ID_BY_1)
			.sequential(
				`${opts.testPrefix ? `${opts.testPrefix} - ` : ""}${
