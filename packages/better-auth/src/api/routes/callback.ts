...
export const callbackOAuth = createAuthEndpoint(
	...
	async (c) => {
		...
		if (link) {
			// If account linking is enabled, check if the account is already linked
			const existingAccount = await c.context.internalAdapter.findAccount(
				userInfo.id,
			);

			if (existingAccount) {
				// If the account already exists and belongs to a different user, show an error
				if (existingAccount.userId !== link.userId) {
					return redirectOnError("account_already_linked_to_different_user");
				}
				// If the account is already linked, return an error
				// return redirectOnError("account_already_linked");
			}

			// If no account is linked yet, allow linking multiple accounts for the same provider
			const newAccount = await c.context.internalAdapter.createAccount(
				{
					userId: link.userId,
					providerId: provider.id,
