...
describe("account", async () => {
	...
	it("should reject linking when account linking is disabled", async () => {
		...
		const { headers } = await disabledSignIn();

		const result = await disabledClient.linkSocial(
			{
				provider: "google",
				idToken: {
					token: "mock-token",
	...
	it("should reject linking accounts with different emails when not allowed", async () => {
		...
		const { headers } = await strictSignIn();

		const result = await strictClient.linkSocial(
			{
				provider: "google",
				idToken: {
					token: "mock-token",
