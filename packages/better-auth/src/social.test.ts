...
describe("Threads Provider", async () => {
	...
	it("should upgrade to a long-lived token and key the synthetic email on the stable id", async () => {
		...
		// via the shared createPlaceholderEmail helper, which uses the RFC 2606
		// reserved `.invalid` TLD so it can never route mail.
		expect(session.data?.user.email).toBe(
			"threads_user_123@threads.placeholder.invalid",
		);
		...
		expect(accounts[0]?.accountId).toBe("threads_user_123");
	...
	it("should fall back to the username when the profile has no name", async () => {
		...
		expect(session.data?.user.email).toBe(
			"threads_user_456@threads.placeholder.invalid",
		);
	});
	...
	it("should abort sign-in when the long-lived token exchange fails", async () => {
		// If the th_exchange_token upgrade fails, the provider must throw rather
		// than persist the raw short-lived token (which has no expiry and no
		// refresh credential, silently locking the user out after ~1h).
		mswServer.use(
			http.get("https://graph.threads.net/access_token", async () => {
				return HttpResponse.json(
		...
		await expect(
			provider.validateAuthorizationCode({
				code: "threads_test_code",
				redirectURI: "https://app.example/api/auth/callback/threads",
			}),
		).rejects.toBeDefined();
	});
});
