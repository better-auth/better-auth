...
describe("audience admin CRUD", () => {
	it("create + read round-trips", async () => {
		const instance = await boot();
		const headers = await signedInHeaders(instance);
		const created = await instance.auth.api.adminCreateAudience({
			body: {
		...
		expect((created as OAuthAudience).identifier).toBe(
		...
		expect((fetched as OAuthAudience).name).toBe("Created Audience");
		expect((fetched as OAuthAudience).accessTokenTtl).toBe(300);
	});
	...
	it("rejects identifiers that aren't absolute URIs", async () => {
		...
		const headers = await signedInHeaders(instance);
		await expect(
			instance.auth.api.adminCreateAudience({
	...
	it("update on a missing identifier returns 404", async () => {
		...
		const headers = await signedInHeaders(instance);
		await expect(
			instance.auth.api.adminUpdateAudience({
	...
	it("delete removes the row and 404s on subsequent reads", async () => {
		...
		await instance.auth.api.adminDeleteAudience({
			params: { identifier: "https://api.example.com/delete-me" },
			headers,
		});
		await expect(
			instance.auth.api.adminGetAudience({
				params: { identifier: "https://api.example.com/delete-me" },
				headers,
			}),
		).rejects.toMatchObject({
	...
	it("list returns all audiences", async () => {
		const instance = await boot();
		const headers = await signedInHeaders(instance);
		await instance.auth.api.adminCreateAudience({
		...
		const list = (await instance.auth.api.adminListAudiences({
			headers,
		})) as OAuthAudience[];
...
describe("audiencePrivileges gate", () => {
	it("blocks create when audiencePrivileges returns false", async () => {
		const instance = await boot({
			audiencePrivileges: async ({ action }) => action !== "create",
		});
		const headers = await signedInHeaders(instance);
		await expect(
			instance.auth.api.adminCreateAudience({
	...
	it("allows actions the callback approves", async () => {
		const instance = await boot({
			audiencePrivileges: async ({ action }) =>
	...
	it("blocks list independently of read", async () => {
		const instance = await boot({
			audiencePrivileges: async ({ action }) => action !== "list",
		});
		const headers = await signedInHeaders(instance);
		await expect(
			instance.auth.api.adminListAudiences({ headers }),
		).rejects.toMatchObject({ status: "UNAUTHORIZED" });
	});
});

describe("client/audience linking", () => {
	...
	it("link creates the join row and unlink removes it", async () => {
		...
		expect(links?.[0]?.audienceId).toBe("https://api.example.com/linked");

		await instance.auth.api.adminUnlinkClientAudience({
			params: {
				identifier: "https://api.example.com/linked",
				client_id: "test-client",
			},
		...
		const afterUnlink =
			await instance.ctx.adapter.findMany<OAuthClientAudience>({
				model: "oauthClientAudience",
				where: [{ field: "clientId", value: "test-client" }],
			});
	...
	it("link is idempotent on repeat calls", async () => {
		...
		const second = await instance.auth.api.adminLinkClientAudience({
			params: {
				identifier: "https://api.example.com/idempotent",
				client_id: "idempotent-client",
			},
	...
	it("link rejects an unknown audience", async () => {
		const instance = await boot();
		const headers = await signedInHeaders(instance);
		await seedClient(instance.ctx, "client-x");
		await expect(
			instance.auth.api.adminLinkClientAudience({
				params: {
	...
	// Composite uniqueness on (clientId, audienceId).
	//
	// The schema layer can't declare `UNIQUE(clientId, audienceId)` directly;
	// the row's `id` is a deterministic `${clientId}::${audienceId}` so the
...
describe("path-param decoding for URI-valued identifiers", () => {
	...
	// wired into cookie parsing). The protected-resource metadata endpoint
	// in oauth.ts already calls decodeURIComponent on ctx.params.identifier;
	// these tests pin the parallel behavior for the admin CRUD + link/unlink
	// handlers. Even though the admin endpoints are SERVER_ONLY today, callers
	// that wrap them (e.g. a hand-rolled HTTP exposure that forwards raw URL
	// segments, or a UI router that pre-encodes) should not lose rows just
	// because the identifier traveled through a percent-encoded path. We
	// drive auth.api.* with explicitly-encoded params to exercise the
	// handler-side decode without depending on the HTTP router.
	...
	it("link / unlink endpoints decode both percent-encoded path segments", async () => {
		...
		const ctx = await instance.auth.$context;
		const after = await ctx.adapter.findMany<OAuthClientAudience>({
			model: "oauthClientAudience",
			where: [{ field: "audienceId", value: raw }],
		});
		...
		const afterUnlink = await ctx.adapter.findMany<OAuthClientAudience>({
			model: "oauthClientAudience",
			where: [{ field: "audienceId", value: raw }],
		});
