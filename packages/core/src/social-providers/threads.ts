	...
	return {
		...
		validateAuthorizationCode: async ({ code, redirectURI }) => {
			...
			if (tokens.accessToken) {
				const { data: longLived, error } = await betterFetch<{
					access_token: string;
					token_type?: string;
					expires_in?: number;
				}>(
				...
				// Fail loudly if the upgrade does not succeed. Returning the raw
				// short-lived token would persist an account with a ~1h access
				// token, no expiry, and no refresh credential, silently locking the
				// user out once it lapses with no way to recover the session.
				if (error || !longLived?.access_token) {
					throw (
						error ||
						new Error(
							"Failed to exchange the Threads token for a long-lived token",
						)
					);
				}
				tokens.accessToken = longLived.access_token;
				// Threads has no OAuth refresh token: a long-lived token is its own
				// refresh credential. Persist it as the refresh token so better-auth's
				// refresh machinery can drive the self-refresh below.
		...
		async getUserInfo(token) {
			...
			return {
				user: {
					...
					// unique email constraint and breaking sign-in. Override with
					// `mapProfileToUser` to supply a real email.
