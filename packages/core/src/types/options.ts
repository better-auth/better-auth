import type { BetterAuthAdvancedOptions } from "./init-options";
import type { LiteralUnion } from "./helper";
import type { DBFieldAttribute } from "../db";
import type { DBAdapterDebugLogOption } from "../db/adapter";
import type {
	User,
	Account,
	Session,
...
// External database types (from kysely, better-sqlite3, bun, etc.)
// These are kept minimal to avoid importing heavy dependencies
// Using object type to allow type narrowing with 'in' operator
type Dialect = Record<string, unknown>;
type Kysely<T = unknown> = Record<string, unknown>;
type MysqlPool = Record<string, unknown>;
type PostgresPool = Record<string, unknown>;
type Database = Record<string, unknown>;
type BunDatabase = Record<string, unknown>;
type DatabaseSync = Record<string, unknown>;

// Forward declarations for types that will be provided by better-auth package
// These will be resolved when better-auth re-exports this type with concrete implementations
type GenericEndpointContext = Record<string, unknown>;
type BetterAuthPlugin = Record<string, unknown>;
type SocialProviders = Record<string, unknown>;
type SocialProviderList = readonly unknown[];
type AdapterInstance = Record<string, unknown>;
type SecondaryStorage = Record<string, unknown>;
type KyselyDatabaseType = string;
...
type OmitId<T> = T extends { id: unknown } ? Omit<T, "id"> : T;
