...
describe("seat-based billing", () => {
	...
	describe("seat sync on member changes", () => {
		...
		test("should choose the newest eligible subscription when history has duplicates", async ({
			stripeMock,
		}) => {
			const { auth, signInWithTestUser } = await getTestInstance({
				plugins: [organization(), stripe(buildSeatPlanOptions(stripeMock))],
			...
			const ctx = await auth.$context;
			const org = await auth.api.createOrganization({
				body: { name: "Duplicate Subscription Org", slug: "duplicate-sub-org" },
				headers,
			});
			const orgId = org!.id;
			await ctx.adapter.update({
			...
			const user = await ctx.adapter.create({
				model: "user",
				data: { email: "duplicate-sub-member@test.com", name: "New Member" },
			});
			...
			expect(stripeMock.subscriptions.retrieve).toHaveBeenCalledExactlyOnceWith(
				"sub_newer",
			);
			expect(stripeMock.subscriptions.update).toHaveBeenCalledExactlyOnceWith(
				"sub_newer",
