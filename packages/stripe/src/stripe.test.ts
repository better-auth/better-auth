...
describe.only("stripe", () => {
	...
	const getMockStripeClient = (stripeClient?: DeepPartial<Stripe>) =>
		({
			..._stripeClient,
			customers: {
				..._stripeClient.customers,
				create: vi.fn().mockImplementation(async (res) => {
					return {
						id: "cus_mock123",
						url: "https://api.stripe.com/v1/customers/cus_mock123",
					};
				}),
			},
			subscriptions: {
				..._stripeClient.subscriptions,
				create: vi.fn().mockResolvedValue({ id: "sub_mock123" }),
				list: vi.fn().mockResolvedValue({ data: [] }),
			},
			checkout: {
				sessions: {
					create: vi.fn().mockImplementation(async (param) => {
						return {
							id: "cs_mock123",
							url: "https://api.stripe.com/v1/checkout/sessions/cs_mock123",
							...param,
						};
					}),
				},
	...
	const defaultPlans = [
		{
			...
			priceId: "price_123",
			lookupKey: "price_123",
			annualDiscountPriceId: "price_123_annual",
			annualDiscountLookupKey: "price_123_annual",
			limits: {
				tokens: 1000000,
				users: 10,
			},
		},
		{
			...
			priceId: "price_456",
			lookupKey: "price_456",
			annualDiscountPriceId: "price_456_annual",
			annualDiscountLookupKey: "price_456_annual",
			limits: {
				tokens: 2000000,
				users: 100,
			},
	...
	const getAuth = (pluginOptions?: Partial<StripeOptions>) =>
		betterAuth({
			database: db,
			baseURL: "http://localhost:3000",
			emailAndPassword: {
			...
			plugins: [
				bearer(),
				stripe({
					stripeClient: getMockStripeClient(),
					stripeWebhookSecret: process.env.STRIPE_WEBHOOK_SECRET!,
					subscription: {
						enabled: true,
						plans: defaultPlans,
	...
	async function getTestInstance(options?: Partial<StripeOptions>) {
		const auth = getAuth(options);
		const client = getClient(auth);
		const ctx = await auth.$context;
		const userRes = await client.signIn.email(testUser.one, {
			throw: true,
		});
		const headers = new Headers({
			Authorization: `Bearer ${userRes.token}`,
		});
		return { client, headers, ctx, auth, user: userRes.user };
	}
	...
	it("should create a customer on sign up", async () => {
		...
		const client = getClient(auth);
		const userRes = await client.signUp.email(testUser.one, {
			throw: true,
		});
		...
		expect(session?.user.stripeCustomerId?.length).toBeGreaterThan(0);
	});

	it("should not create a customer on sign up", async () => {
		const auth = getAuth();
		const client = getClient(auth);
		const userRes = await client.signUp.email(testUser.two, {
			throw: true,
		});
		const session = await auth.api.getSession({
			headers: new Headers({
		...
		expect(session?.user.stripeCustomerId).toBeNull();
	});

	describe("subscription", () => {
		let successURL = "";
		it("should create a subscription", async () => {
			const { client, headers, ctx, user } = await getTestInstance();
			...
			expect(data.success_url).toBe(
				`${ctx.baseURL}/subscription/success?callbackURL=${encodeURIComponent(
		...
		it("should handle subscription success callback", async () => {
			const { client, headers, user } = await getTestInstance({
				stripeClient: getMockStripeClient({
					subscriptions: {
							...
							data: [
								{
									...
									current_period_end: Date.now() / 1000 + 31536000,
									items: {
										data: [
											{
												id: "si_mock123",
