  ...
  try {
    ...
    if (!subscription) {
      ...
      if (subs.length > 1) {
        ...
        if (!activeSub) {
          logger.warn(
            `Stripe webhook error: Multiple subscriptions found for customerId: ${customerId}, but no active subscription found.`,
          );
          return;
        }
    ...
    if (
      ...
      logger.warn(
        `Stripe webhook error: Subscription mismatch. Expected stripeSubscriptionId: ${subscription?.stripeSubscriptionId}, Received: ${subscriptionUpdated.id}`,
      );
      return;
    }

    const subscriptionLastUpdated = subscription.updatedAt?.getTime();
    const eventCreatedAt = subscriptionUpdated.created * 1000;

    if (eventCreatedAt <= subscriptionLastUpdated) {
      logger.info(
        `Stripe webhook: Skipping outdated update for subscriptionId: ${subscription.id}.`,
      );
      return;
    }
