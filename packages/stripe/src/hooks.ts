	...
	try {
		...
		await ctx.context.adapter.update({
			...
			update: {
				...
				periodStart: new Date(subscriptionUpdated.items.data?.[0]?.current_period_start * 1000),
				periodEnd: new Date(subscriptionUpdated.items.data?.[0]?.current_period_end * 1000),
