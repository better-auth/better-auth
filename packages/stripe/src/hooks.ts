				...
				let dbSubscription =
					await ctx.context.adapter.update<InputSubscription>({
						...
						update: {
							...
							updatedAt: new Date(),
							periodStart: subscription.items.data?.[0]?.current_period_start ? new Date(subscription.items.data?.[0]?.current_period_start  * 1000 ) : undefined,
							periodEnd: subscription.items.data?.[0]?.current_period_end ? new Date(subscription.items.data?.[0]?.current_period_end * 1000) : undefined,
	...
	try {
		...
		await ctx.context.adapter.update({
			...
			update: {
				...
				periodStart: subscriptionUpdated.items.data?.[0]?.current_period_start ? new Date(subscriptionUpdated.items.data?.[0]?.current_period_start * 1000) : undefined,
				periodEnd: subscriptionUpdated.items.data?.[0]?.current_period_end ? new Date(subscriptionUpdated.items.data?.[0]?.current_period_end * 1000) : undefined,
