...
export const redisAdapter = (
	...
	// Get or create maxIds for this client connection
	const clientKey =
	...
	async function loadModel(model: string) {
		...
		if (keys.length === 0) {
			// Reset maxIds for this model when table is empty (e.g., after migrations)
			maxIds[model] = 0;
			return [];
		}
		...
		// Update maxIds based on existing records for numeric ID tracking
	...
	function applyWhere(records: any[], where: CleanedWhere[]) {
		...
		const filtered = records.filter((record) => {
			...
			for (const clause of where) {
				...
				if (v === null) {
					// Explicit null value comparison
			...
			// Fixed AND/OR logic: handle mixed conditions properly
	...
	const adapterCreator = createAdapterFactory({
		...
		adapter: ({ getModelName, options }) => {
			return {
				async create({ model, data }) {
					await ensureConnection();
					// Generate numeric IDs for serial mode (for when disableTransformInput is true)
	...
	return (options) => adapterCreator(options);
};
