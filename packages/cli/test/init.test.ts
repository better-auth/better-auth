...
describe("initAction", () => {
	...
	testWithTmpDir(
		 ...
		 */
		"should generate a working Kysely PostgreSQL auth config",
		async ({ tmp }) => {
			await fs.writeFile(
				path.join(tmp, "package.json"),
				JSON.stringify({ name: "test-project", version: "1.0.0" }),
			...
			expect(authConfigContent).toContain("const database = new Pool");
			...
			expect(authConfigContent).not.toContain('type: "postgresql"');
		},
	);
