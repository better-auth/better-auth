...
type GetArgumentsOption = PrettifyDeep<
 ...
 */
function extractJSDocTags(jsDocNodes: any[]): {
	hasCliTag: boolean;
	shouldSkip: boolean;
	isRequired: boolean;
	isSelect: boolean;
...
function generateQuestion(
	...
	switch (actualType) {
		case "boolean":
			return `Would you like to ${formattedName}?`;
		case "number":
			return `What is the ${formattedName}?`;
		default:
			return `What is the ${formattedName}?`;
	}
}
...
function processProperty(
	...
	const question =
		tags.question || generateQuestion(propertyName, type, tags.type);
	...
	// Generate schema
	const schema = generateZodSchema(type, tags.type, tags.enumValues);
 ...
 */
function generateIndividualPluginFile(
	pluginName: string,
 ...
 */
function generateIndexFile(pluginNames: string[]): string {
...
function generateArgumentCode(
	...
	if (arg.defaultValue !== undefined) {
		...
		if (typeof arg.defaultValue === "string") {
			if (arg.defaultValue === "true" || arg.defaultValue === "false") {
