...
describe("ciba", async () => {
	...
	describe("approval flow", () => {
		it("should prevent approving another user's request", async () => {
			// This test requires a second user, which getTestInstance doesn't easily provide
			// We test that the session user must match the request user
			const bcRes = await auth.api.bcAuthorize({
				body: {
					client_id: oauthClient.client_id,
					client_secret: oauthClient.client_secret!,
