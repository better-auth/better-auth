 ...
 *
 * The same test exists for `relations-v2`, which has its own copy of the code.
 */
...
const schema = pgSchema("increment_one_contention");
...
describe("drizzle adapter (pg): incrementOne under contention", () => {
	const pool = new Pool({
		connectionString: "postgres://user:password@localhost:5432/better_auth",
		max: CONCURRENT_CALLS,
	});
	const adapter = drizzleAdapter(drizzle(pool, { schema: { counters } }), {
		provider: "pg",
		schema: { counters },
	})({
		plugins: [
	...
	const seed = (ids: string[]) =>
		pool.query(
			`INSERT INTO "increment_one_contention"."counters" ("id", "used", "status")
	...
	const rows = async () =>
		(
			await pool.query<{ id: string; used: number; status: string }>(
				`SELECT "id", "used", "status" FROM "increment_one_contention"."counters" ORDER BY "id"`,
			)
		).rows;
	...
	beforeAll(async () => {
		...
		await pool.query(`
			CREATE SCHEMA IF NOT EXISTS "increment_one_contention";
			DROP TABLE IF EXISTS "increment_one_contention"."counters";
			CREATE TABLE "increment_one_contention"."counters" (
				"id" text PRIMARY KEY,
	...
	beforeEach(async () => {
		...
		await pool.query(`DELETE FROM "increment_one_contention"."counters"`);
	});

	afterAll(async () => {
		await pool.query(
			`DROP SCHEMA IF EXISTS "increment_one_contention" CASCADE`,
		);
		await pool.end();
	});
