...
description: "MCP Auth, Electron integration, i18n, adapter extraction, dynamic base URL, CLI init wizard, seat-based billing, SAML Single Logout, test utilities, and more!"
date: 2025-02-28
author:
  name: "Alex Yang"
...
tags: ["1.5", "CLI", "MCP", "electron", "i18n", "adapters", "stripe", "SSO", "SCIM", "CLI", "test-utils"]
---
...
Sign up now: [https://better-auth.com/sign-in](https://beta.better-auth.com/sign-in) 🚀
...
We’re announcing a new better auth cli command. `npx auth` As we’ll soon be deprecating the previous`@better-auth/cli` package.

```jsx
npx auth init
```

You can now initialize better auth with one interactive command `npx auth init`

You should also be able to run `migrate` `generate` and other better auth commands with it.
