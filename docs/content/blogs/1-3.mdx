...
We've added providers for **[Notion](https://notion.com), [Slack](https://slack.com), [Linear](https://linear.app), and [Faceit](https://faceit.com)**.
...
<Callout type="warn">
  Breaking change: `building` and `getRequestEvent()` must now be passed in as
  props.
</Callout>
...
Setting `sendOnSignIn` to true will send a verification email on sign-in if the user isn't verified.

```ts title="auth.ts"
export const auth = betterAuth({
  emailVerification: {
    sendOnSignIn: true, // set to true to send verification email on sign-in
  },
});
```
...
<Callout type="warn">
  Breaking change: `teamId` has been removed from the `member` table. A new
  `teamMembers` table is required.
</Callout>
