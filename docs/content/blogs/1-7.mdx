---
title: "Better Auth 1.7: OAuth, SSO, and Provisioning Come Together"
...
# Better Auth 1.7: OAuth, SSO, and Provisioning Come Together

Better Auth 1.7 is a minor version by number, but it is one of the largest expansions of what you can build with Better Auth.

The existing OAuth and OpenID Connect provider can now protect APIs with stronger tokens and support more kinds of clients. MCP gains its own package, clearer resource protection, and support for the latest authorization profile. SCIM keeps its User lifecycle features and adds Groups, direct memberships, role projections, and stronger connection management. A new OAuth device grant lets command-line tools, TVs, and other devices ask for API access through a browser.

This release also brings the OAuth, SSO, and provisioning paths closer together. Applications can now explicitly connect a provisioned SCIM identity to a verified OpenID or SAML identity without relying on email matching.

We introduced the first set of changes in the [1.7 release candidate](/blog/1-7-rc), and the release continued to grow through the candidate cycle. This post covers the final highlights. For every migration step, use the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide).
...
### A much stronger OAuth and OpenID provider
...
If you adopted a 1.7 beta or release candidate, also read the prerelease section. Some intermediate schemas and APIs changed again before the stable release, especially for SCIM, MCP, CIMD, OAuth clients, and account identity.
