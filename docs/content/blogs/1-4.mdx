...
### Pre-Configured Social Providers for OAuth
...
The JWT plugin now supports token rotation, allowing you to rotate your JWKS keys periodically with a grace period.

[👉 Read more about JWT rotation](/docs/plugins/jwt#key-rotation)
...
export const auth = betterAuth({
    plugins: [
        jwt({
            jwks: {
                rotationInterval: 60 * 60 * 24 * 30, // 30 days // [!code highlight]
                gracePeriod: 60 * 60 * 24 * 30 // 30 days // [!code highlight]
            }
        }),
    ],
});
...
<Accordions type="single">
  ...
  <Accordion title="Cloudflare Runtime">
    Breaking change for cloudflare worker, you will be required to add `nodejs_als`, or `nodejs_compat` in the wrangler config.
    See [cloudflare docs](https://developers.cloudflare.com/workers/configuration/compatibility-flags/#nodejs-compatibility-flag).
  </Accordion>
...
- Last-Login-Method plugin now supports tracking SIWE.
