...
function getIdentityIssuer(providerId: string): string {
  const issuer = identityIssuers[providerId];
  if (!issuer) {
    throw new Error(`Add a trusted Identity issuer for ${providerId}`);
  }
  return issuer;
}

function getAccountLinkKey(
  issuer: string,
  providerAccountId: string,
  providerInstanceId: string,
): string {
  return `${issuer}\u0000${providerAccountId}\u0000${providerInstanceId}`;
}
...
const migrateFromSupabase = async () => {
  ...
  for (const user of users) {
    ...
    await ctx.adapter
      ...
      .catch(() => {});
    const linkedAccounts = await ctx.internalAdapter.listUserAccounts(user.id);
    const accountLinkKeys = new Set(
      linkedAccounts.map(({ account, identity }) =>
        getAccountLinkKey(
          identity.issuer,
    ...
    for (const identity of user.identities) {
      if (identity.provider === "email") {
        ...
        const accountLinkKey = getAccountLinkKey(
          issuer,
          providerAccountId,
          "credential",
        );
        if (!accountLinkKeys.has(accountLinkKey)) {
          ...
          accountLinkKeys.add(accountLinkKey);
        }
      }
      ...
      if (supportedProviders.includes(identity.provider)) {
        ...
        const providerAccountId = identity.identity_data?.sub;
        if (!providerAccountId) {
          throw new Error(`Missing provider subject for ${identity.provider}`);
        }
        const accountLinkKey = getAccountLinkKey(
          issuer,
          providerAccountId,
