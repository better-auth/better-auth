...
<APIMethod path="/admin/create-user" method="POST" resultVariable="newUser">
  ...
  type createUser = {
      ...
      /**
       * The password of the user. If not provided, the user is created without a credential account.
       */
      password?: string = "some-secure-password"
      /**
       * Send a passwordless-enrollment email so the user can set their own password and sign in.
       * Only meaningful when `password` is omitted; requires `user.enrollment` to be configured.
       * @see /docs/concepts/users-accounts#enrollment
       */
      sendEnrollmentEmail?: boolean = false
      /**
