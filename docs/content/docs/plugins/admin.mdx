...
<Callout type="warning">
    ...
    Please use the [userHasPermission](#access-control-usage) APIs to include checks for any dynamic roles & permissions.
</Callout>

## Dynamic Access Control

Dynamic access control allows you to create roles at runtime. This is achieved by storing the created roles and permissions in a database table.
...
<Callout type="info">
    This will require you to run migrations to add the new `role` table to the database. See the [Schema](#role-optional).
</Callout>

<Callout type="warning">
    The `authClient.admin.checkRolePermission` function will not include any dynamic roles as everything is ran synchronously on the client side.
    Please use the [userHasPermission](#access-control-usage) APIs to include checks for any dynamic roles.
</Callout>

### Creating a role

To create a new role at runtime, you can use the `createRole` function.

Only users with roles which contain the `ac` resource with the `create` permission can create a new role.
By default, only the `admin` role has this permission. You also cannot add permissions that your current role can't already access.
...
// make sure they are defined in the `ac` instance of your admin config.
const permission = {
...
Now you can freely call [`setRole`](#set-user-role) to update the role of a user with your newly created role.

### Deleting a role

To delete a role, you can use the `deleteRole` function, then provide either a `roleName` or `roleId` parameter.
...
type deleteRole = {
   ...
   * Alternatively, you can pass a `roleId`
   * parameter instead.
   */
   roleName?: string = "my-role"
  /**
   * The id of the role to delete.
   * Alternatively, you can pass a `roleName`
   * parameter instead.
   */
...
To list roles, you can use the `listRoles` function.
This requires the `ac` resource with the `read` permission to list all roles or the `read-own` permission to only list roles the user is assigned to.
By default, admins have the `read` and users the `read-own` permission.
...
To get a specific role, you can use the `getRole` function and pass either a `roleName` or `roleId` parameter.
This requires the `ac` resource with either the `read` permission to get any role or the `read-own` permission to only get roles the user is assigned to.
...
To update a role, you can use the `updateRole` function and pass either a `roleName` or `roleId`. This requires the `ac` resource with the `update` permission.
...
type updateRole = {
  ...
  data: {
    /**
     * Optionally update the permissions of
     * the role.
     */
...
To add additional fields to the `role` table, you can pass the `additionalFields` configuration option to the `admin` plugin.
...
Then you can use `inferAdminAdditionalFields` to infer the additional fields.

```ts title="auth-client.ts"
import { createAuthClient } from "better-auth/client";
import { adminClient, inferAdminAdditionalFields } from "better-auth/client/plugins";
...
Otherwise, you can pass the schema values directly, the same way as on the server plugin.
...
When [Dynamic Access Control](#dynamic-access-control) is enabled, add the following fields to the `role` table:
