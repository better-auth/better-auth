...
Hard deletes a user from the database. Before the deletion occurs, the `beforeRemoveUser` hook is called if one is configured — throw from it to abort the operation. See [Hooks](#hooks) for details.
...
Lifecycle callbacks that let you inject custom logic around admin user operations.

#### `beforeRemoveUser`

Called before a user is deleted via the [`/admin/remove-user`](#remove-user) endpoint. Throw any error (e.g. `APIError`) to abort the deletion — the user record will not be touched.
...
export const auth = betterAuth({
  plugins: [
    admin({
      hooks: {
        async beforeRemoveUser(user) {
          // Example: block deletion if the user is the sole owner of an org
          const soloOrgs = await db.query.member.findMany({
            where: and(
              eq(member.userId, user.id),
              eq(member.role, "owner"),
            ),
          });
          for (const membership of soloOrgs) {
...
Called after a user has been successfully deleted. Use this for post-deletion side effects such as cleaning up external records or sending notifications.

<Callout type="warning">
  Deletion and the `afterRemoveUser` hook are **not atomic**. If the hook throws, the user has already been removed from the database but the endpoint will return an error to the caller. Do not treat external cleanup and user deletion as a single transaction — make your hook idempotent or handle failures independently (e.g. a background retry queue).
</Callout>

```ts title="auth.ts"
admin({
  hooks: {
    async afterRemoveUser(user) {
      await externalService.deleteAccount(user.id);
    },
  },
});
```
