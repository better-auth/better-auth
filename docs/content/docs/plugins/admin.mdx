...
The admin plugin provides event callbacks that are triggered when specific admin operations occur.
These events are useful for logging, auditing, notifications, and other side effects.

```ts title="auth.ts"
admin({
  events: {
    impersonateStart: async (adminSession, impersonatedUser) => {
      // Called when admin.impersonateUser is called
      await logAuditEvent({
    ...
    impersonateEnd: async (adminSession) => {
      // Called when admin.stopImpersonating is called
      await logAuditEvent({
        type: "impersonation_ended",
        adminId: adminSession.user.id,
      });
    },
    ban: async (adminSession, bannedUser) => {
      // Called when admin.banUser is called
      await sendNotification({
        userId: bannedUser.id,
        message: `You have been banned: ${bannedUser.banReason}`,
      });
    },
    unban: async (adminSession, unbannedUser) => {
      // Called when admin.unbanUser is called
      await sendNotification({
        userId: unbannedUser.id,
        message: "Your ban has been lifted",
      });
    },
    userCreate: async (adminSession, createdUser) => {
      // Called when admin.createUser is called
      await sendWelcomeEmail(createdUser.email);
    },
    userRemove: async (adminSession, removedUser) => {
      // Called when admin.removeUser is called
      await cleanupUserData(removedUser.id);
    },
  },
});
```
...
Triggered when `admin.impersonateUser` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Always defined (not null) — `impersonateUser` requires an active admin session.
* `impersonatedUser`: The user object that is being impersonated.

**Example:**

```ts
impersonateStart: async (adminSession, impersonatedUser) => {
  console.log(`Admin ${adminSession.user.email} started impersonating ${impersonatedUser.email}`);
}
```

##### `impersonateEnd`

Triggered when `admin.stopImpersonating` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Always defined (not null).

**Example:**

```ts
impersonateEnd: async (adminSession) => {
  console.log(`Admin ${adminSession.user.email} stopped impersonating`);
}
```

##### `ban`

Triggered when `admin.banUser` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Always defined (not null).
* `bannedUser`: The user object that was banned (includes `banned`, `banReason`, and `banExpires` fields).
...
Triggered when `admin.unbanUser` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Always defined (not null).
* `unbannedUser`: The user object that was unbanned (the `banned` field will be `false`).
...
Triggered when `admin.createUser` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Will be `null` if the request is made from the server (without a session).
* `createdUser`: The user object that was created.
...
Triggered when `admin.removeUser` is called.

**Parameters:**

* `adminSession`: The admin's session object (contains `user` and `session`). Always defined (not null).
* `removedUser`: The user object that was removed (this is the user object before deletion).

**Example:**

```ts
userRemove: async (adminSession, removedUser) => {
  // Archive user data after deletion
  await archiveUserData(removedUser.id);
}
```

<Callout>
  **Note:** All event callbacks are `async` functions and should return a `Promise<void>`.
  If an event callback throws an error, it will not prevent the admin operation from completing. Errors are logged via the auth logger.
</Callout>
