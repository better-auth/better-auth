...
By default, there is one resource with eight actions.

**user**:

    `create` `list` `set-role` `ban` `list-sessions` `revoke-sessions` `impersonate` `delete`
...
<Steps>
    ...
    const statement = {
        ...admin.defaultStatements, // [!code highlight]
        project: ["create", "share", "update", "delete"],
    } as const;
    ...
    const admin = ac.newRole({
        ...
        ...admin.adminAc.statements, // [!code highlight]
    });
    ```
...
You can use the `adminHasPermission` action provided by the `api` to check the permission of the user.
