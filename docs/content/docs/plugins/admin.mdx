...
Allows you to search for users by their email or name adding the possibility to make more complex queries.

```ts title="admin.ts"
const users = await authClient.admin.searchUsers({
    search: {
        field: "email",
        operator: "contains",
        value: "@example.com"
