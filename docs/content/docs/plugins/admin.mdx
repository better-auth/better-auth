...
When `signInTracking` is enabled, the following field is added (optional):
  ...
  fields={[
    {
      name: "latestSignInAt",
      type: "date",
      description: "The date of the user's most recent successfull sign-in.",
      isOptional: true,
    }
  ]}
/>
...
Enables tracking of the user's last successful sign-in. When enabled, a `latestSignInAt` field is added to the user table and updated each time the user signs in.
