...
Use the `checkRolePermission` function on the client side to verify if a specific role has a given permission. This is useful after roles and their permissions have been defined, as it avoids the need to perform permission checks through the server. The function is synchronous, so there's no need to use `await` when calling it.
...
// You can also check multiple resource permissions at the same time
const canCreateProjectAndRevokeSession = authClient.admin.checkRolePermission({
  permissions: {
    user: ["delete"],
    session: ["revoke"]
  },
