...
- `role`: The user's role. Defaults to `user`. Admins will have the `admin` role.
- `banned`: Indicates whether the user is banned.
- `banReason`: The reason for the user's ban.
- `banExpires`: The date when the user's ban will expire.

### `session` table fields:

- `impersonatedBy`: The ID of the admin that is impersonating this session.
