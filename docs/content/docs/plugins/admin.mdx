...
Allows an admin to list all the roles in the system.
...
Allows an admin to list the permissions for a role.

```ts title="admin.ts"
const permissions = await authClient.admin.listRolePermissions({
  query: {
    role: "admin",
  },
});
