...
When you define custom roles with access control, the `adminRoles` option is **not required**. The custom role definitions will determine what permissions each role has.
...
export const auth = betterAuth({
  plugins: [
    adminPlugin({
      ...
      roles: {
        admin,
        manager, // Can have full admin permissions via custom AC
        user,
        }
        // adminRoles is NOT needed here
      }),
    ],
  });
```

In this configuration, if your `manager` role includes admin permissions (like `impersonate`, `ban`, etc.), users with the `manager` role can perform those operations without being in an `adminRoles` list.
...
This option only applies when you are **not** using custom access control. In that case, any role that isn't in the `adminRoles` list will not be able to perform admin operations.

<Callout type="warning">
  When **not** using custom access control, any role that isn't in the `adminRoles` list 
  will not be able to perform admin operations.
