...
For multi-field search, call the same endpoint with a `POST` body. The legacy `GET` query parameters above are still supported.

<APIMethod path="/admin/list-users" method="POST" requireSession note={"Recommended when searching across multiple fields or custom user fields."} resultVariable={"users"}>
  ```ts
  type listUsers = {
    /**
     * Search conditions to apply to the user table.
     */
    search?: {
      /**
       * The field to search in. Can be a built-in field or a custom user field.
       */
      field: string = "email"
      /**
       * The operator to use for the search. Defaults to `eq` when omitted.
       */
      operator?: "eq" | "ne" | "lt" | "lte" | "gt" | "gte" | "in" | "not_in" | "contains" | "starts_with" | "ends_with" = "eq"
      /**
       * The value to search for.
       */
      value: string | number | boolean | string[] | number[] = "hello@example.com"
      /**
       * How this condition is combined with the other search conditions. Defaults to `AND` when omitted.
       */
      connector?: "AND" | "OR" = "AND"
      /**
       * Case sensitivity for string comparisons. Defaults to `sensitive` when omitted.
       */
...
Use `search` to match users across multiple fields. Set `connector: "OR"` on each condition for a search bar that matches any field.
...
Custom fields must exist in your user schema. `mode: "insensitive"` support depends on the configured database adapter.

When calling the `POST` request from a browser with cookie-based auth, add your frontend origin to [`trustedOrigins`](/docs/reference/options#trustedorigins), like other authenticated `POST` endpoints.
