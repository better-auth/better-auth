...
If you want to use dynamic registration, you can define the `dynamicRegistration` property using following configuration values:

```ts
interface GenericOAuthConfig {
  // other configurations...
  dynamicRegistration: {
    registration_endpoint: string; // The register endpoint to use for dynamic registration.
    client_name: string; // The name you want to register the client with.
    client_uri?: string; // A URL which represents the client.
  }
}
```
...
**dynamicRegistration**: (Required, unless `clientId` and `clientSecret` are provided) If true, the plugin will dynamically register the client with the provider based on provided configuration values in this property.
