...
await authClient.signIn.social({
    ...
    scopes: ["my-scope"], // optional, additive
})
```
...
await authClient.linkSocial({
    ...
    scopes: ["repo"], // optional, additive
})
```

Additional scopes requested when linking accumulate into the account's `grantedScopes`; existing granted scopes are never removed.
...
Use [`user.validateUserInfo`](/docs/concepts/users-accounts#callbacks) to reject Generic OAuth users before Better Auth creates a user, links an account, or creates a session. Check `source.providerId` to scope validation to a specific Generic OAuth provider, and use `source.profile` for provider-specific profile fields.
...
export const auth = betterAuth({
  user: {
    validateUserInfo: ({ user, source }) => {
      if (source.type !== "oauth" || source.providerId !== "company-oauth") {
        return;
      }
...
Return nothing to allow the OAuth flow. Return an object with `error` to reject it. Redirect-based OAuth flows send the rejection to the configured error URL, while API-based flows return a structured API error.
...
```ts
import type { TokenEndpointAuth } from "better-auth/oauth2";
...
**scopes**: (Optional) An array of scopes to request from the provider (e.g., `["openid", "email", "profile"]`). These are the default scopes requested at sign-in; the `scopes` passed to a sign-in or link request are added on top. Whatever the user consents to accumulates into the account's `grantedScopes`, and re-authenticating or refreshing tokens never narrows that set.
