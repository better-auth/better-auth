...
Use `tokenEndpointAuth` when your provider requires an explicit token endpoint client authentication method, or when you want to avoid storing a long-lived client secret.
  ...
  config: [
    {
      ...
      tokenEndpointAuth: {
        ...
        getClientAssertion: createPrivateKeyJwtClientAssertionGetter({
          privateKeyPem: process.env.IDP_PRIVATE_KEY!,
          kid: process.env.IDP_PRIVATE_KEY_ID,
        }),
      },
      pkce: true,
    },
...
### Example: Assertion From Vercel OIDC

For an application running on Vercel, the assertion can come from Vercel's OIDC federation token instead of a private key stored in your app.
  ...
  config: [
    {
      ...
      tokenEndpointAuth: {
        method: "private_key_jwt",
        getClientAssertion: async () => getVercelOidcToken(),
      },
      pkce: true,
    },
  ],
...
Configure your IdP to trust the Vercel issuer mode used by your project. For team issuer mode, that is `https://oidc.vercel.com/<team-slug>`. The audience is typically `https://vercel.com/<team-slug>`, and the subject includes the owner, project, and environment, for example `owner:<team-slug>:project:<project-name>:environment:production`.
