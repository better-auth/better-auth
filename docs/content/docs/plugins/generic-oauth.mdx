...
You can use `validateUser` on a Generic OAuth provider to reject a user before Better Auth creates a user, links an account, or creates a session. The callback receives the mapped `user` and the raw OAuth `profile`, which is useful for email/domain allowlists or provider-specific profile checks.
...
Return nothing to allow the OAuth flow. Return an object with `error` or `errorDescription` to reject it. Redirect-based OAuth flows send the rejection to the configured error URL, while API-based flows such as ID-token sign-in return a 401 API error. Avoid putting sensitive details in `errorDescription` because it is returned to the client.
