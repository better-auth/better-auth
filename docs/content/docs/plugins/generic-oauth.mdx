...
**discoveryUrl**: (Optional) URL to fetch the provider's OAuth 2.0/OIDC configuration. If provided, endpoints like `authorizationUrl`, `tokenUrl`, and `userInfoUrl` will be auto-discovered at server startup. When the discovery document publishes a `jwks_uri`, `id_token`s returned by the provider are verified against it (signature, issuer, audience, and advertised signing algorithms) before their claims are used; a token that fails verification rejects the sign-in. Discovery providers also bind the `id_token` to the authorization request with a server-generated OIDC `nonce` and reject a callback whose `id_token` does not echo it. When discovery is unavailable at startup, the provider stays registered and discovery is retried on first use: the first request waits for that retry, and sign-in fails with `503 OAUTH_PROVIDER_UNAVAILABLE` instead of `PROVIDER_NOT_FOUND` only if the provider still has no usable endpoints afterwards. The provider self-heals once the identity provider is reachable again. When explicit endpoints are configured and verified ID-token metadata is not required (see `requireIdTokenVerification`), they act as a fallback while discovery is temporarily unavailable: the provider stays usable on them and keeps retrying discovery until the OIDC metadata is restored.

**discoveryTimeout**: (Optional) Milliseconds before a discovery fetch is aborted. Defaults to `5000`. A stalled identity provider must not hang sign-in while discovery is retried on use.
