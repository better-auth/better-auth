...
### OAuth 2.1 Provider Integration (Two-App Pattern)

If you are using Better Auth as an OAuth 2.1 Provider in one app, and the Generic OAuth client in another, you must ensure the client supplies the OAuth 2.1 `resource` indicator. This is required by the OAuth 2.1 provider to know which resource server an access token is valid for.

You can configure the `resource` field to be passed during the authorization, token exchange, and refresh token requests using the `*UrlParams` options:

```ts
genericOAuth({
  config: [
    {
      providerId: "scheduler-backend",
      clientId: process.env.SCHEDULER_CLIENT_ID,
      clientSecret: process.env.SCHEDULER_CLIENT_SECRET,
      authorizationUrl: `${process.env.SCHEDULER_URL}/api/auth/oauth2/authorize`,
      tokenUrl: `${process.env.SCHEDULER_URL}/api/auth/oauth2/token`,
      userInfoUrl: `${process.env.SCHEDULER_URL}/api/auth/oauth2/userinfo`,
      // Add 'resource' indicator required by Better Auth's OAuth 2.1 Provider
      authorizationUrlParams: { resource: process.env.SCHEDULER_URL },
      tokenUrlParams: { resource: process.env.SCHEDULER_URL },
      refreshTokenParams: { resource: process.env.SCHEDULER_URL },
      // Better Auth OAuth 2.1 Providers issue JWTs 
      // which can be verified manually if required on backend
      getUserInfo: async (tokens) => {
        ...
        // Fallback to standard provider UserInfo Endpoint
        const res = await fetch(`${process.env.SCHEDULER_URL}/api/auth/oauth2/userinfo`, {
