...
| Provider configurations share one identity namespace           | Set the same `identityIssuer`                                |
| Provider uses a non-standard immutable user identifier         | Set `identitySubject`                                        |
...
interface GenericOAuthConfig {
  ...
  identityIssuer?:
    | string
    | ((context: OAuthIdentityKeyContext<GenericOAuthUserInfo>) => string | Promise<string>);
  discoveryUrl?: string;
...
Provider configurations with the same `identityIssuer` and subject resolve to one Identity. Each provider configuration gets its own Account, so aliases keep independent tokens, grants, and lifecycle records.
...
**mapProfileToUser**: (Optional) A function to map the provider profile to mutable fields on your app's user. It cannot set provider identity; use `identitySubject` for a non-standard immutable identifier.
