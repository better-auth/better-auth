...
The `:providerId` parameter identifies which provider configuration to use, unlike built-in providers that have fixed redirect URLs.
...
The redirect URI must include `:providerId` which is used to identify provider configuration. For example, if your providerId is "my-provider", the full redirect URI would be `https://example.com/api/auth/oauth2/callback/my-provider`.
