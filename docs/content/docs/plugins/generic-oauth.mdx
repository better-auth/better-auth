...
### Multi-Channel Support (LINE)

LINE requires separate OAuth channels for different countries (Japan, Thailand, Taiwan, etc.), each with its own `clientId` and `clientSecret`. To support multiple countries, you can configure multiple LINE providers with different `providerId`s:
...
When users sign in, they'll use the appropriate `providerId` (e.g., `"line-jp"`, `"line-th"`, `"line-tw"`) to identify which channel to use.
