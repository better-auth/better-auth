...
* `clientAssertionProvider?: () => Promise<string>` - Returns a client assertion for token endpoint authentication
...
interface GenericOAuthConfig {
  ...
  clientId: string;
  clientSecret?: string;
  clientAssertionProvider?: () => Promise<string>;
  scopes?: string[];
  redirectURI?: string;
  responseType?: string;
  prompt?: string;
...
**clientAssertionProvider**: (Optional) An async function that returns a client assertion for token endpoint authentication. When configured, Better Auth calls this function for each token request and sends `client_assertion_type` plus `client_assertion` instead of `client_secret`. The assertion can come from any source, such as a signed JWT, a cloud identity token, or a workload identity provider.
...
**authentication**: (Optional) The authentication method for secret-based token requests (including when using client assertions). Can be `'basic'` or `'post'`. Defaults to `'post'`.

For `private_key_jwt`, pass a `clientAssertionProvider`. You can provide your own function, or use `createPrivateKeyJwtClientAssertionProvider` to sign an RFC 7523 JWT assertion from a private key:
