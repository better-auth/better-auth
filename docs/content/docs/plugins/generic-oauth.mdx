...
<Callout>
The Generic OAuth plugin is fully compatible with Better Auth's configurable [OAuth state management](/docs/concepts/oauth#oauth-state-management). Custom state management functions configured at the root level will be used by all OAuth providers, including those configured through this plugin.
</Callout>
