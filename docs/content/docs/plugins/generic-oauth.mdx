...
```ts title="auth.ts"
import { betterAuth } from "better-auth"
import { genericOAuth, auth0, keycloak, okta, microsoftEntraId, slack } from "better-auth/plugins"
...
interface GenericOAuthConfig {
  ...
  tokenUrl?: string;
  userInfoUrl?: string;
  clientId: string; // Required, unless `dynamicRegistration` is used
  clientSecret: string; // Required, unless `dynamicRegistration` is used
  scopes?: string[];
  redirectURI?: string;
  responseType?: string;
  prompt?: string;
...
If you want to use dynamic registration, you can define the `dynamicRegistration` property using the following configuration values:

```ts
interface GenericOAuthConfig {
  // other configurations...
  dynamicRegistration: {
    registrationEndpoint: string; // The register endpoint to use for dynamic registration.
    clientName: string; // The name you want to register the client with.
    clientUri?: string; // A URL which represents the client.
  }
}
```

When using dynamic registration, the plugin will automatically register the client with the provider based on the provided configuration values
in this property. This allows you to not have to manually register the client with the provider and let the plugin
handle fetching the `clientId` and `clientSecret` for you.
...
**clientId**: (Required, unless `dynamicRegistration` is used) The OAuth client ID issued by your provider.

**clientSecret**: (Required, unless `dynamicRegistration` is used) The OAuth client secret issued by your provider. 

**dynamicRegistration**: (Required, unless `clientId` and `clientSecret` are provided) Provide this object to let the plugin dynamically register the client with the provider based on its configuration values.
