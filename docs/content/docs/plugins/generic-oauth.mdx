...
For OAuth providers that return ID tokens, use `verifyIdToken` to verify the token and extract user info from the JWT claims.
  ...
  config: [
    {
      ...
      // Default behavior requires `sub` and `email` claims, but can be customized with this function
      getUserInfo: async (tokens) => {
...
To sign in with an OAuth provider using an ID Token, you can use the `signIn.social` function to pass the ID Token directly.

<Callout>
 If an ID token is provided no redirection will happen, and the user will be signed in directly.
</Callout>
