...
**getToken**: (Optional) A custom function to exchange authorization code for tokens. If provided, this function will be used instead of the default token exchange logic. This is useful for providers with non-standard token endpoints (e.g., WeChat).
...
For providers with non-standard token endpoints (e.g., WeChat), you can provide a custom `getToken` function:
  ...
  config: [
    {
      ...
      scopes: ["snsapi_login"],
      // Custom token exchange for WeChat
      getToken: async ({ code, redirectURI }) => {
        ...
        return {
          ...
          // Preserve WeChat-specific fields in raw
          raw: data,
        };
      },
      getUserInfo: async (tokens) => {
        // Access WeChat-specific fields from raw token data
...
The `OAuth2Tokens` object includes a `raw` field that preserves the original token response from the provider. This is useful for accessing provider-specific fields:

```ts
getUserInfo: async (tokens) => {
  // Access provider-specific fields
  const customField = tokens.raw?.custom_provider_field;
  
  // Use in your logic
  return {
    id: tokens.raw?.provider_user_id,
    // ...
  };
}
```
