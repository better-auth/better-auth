...
interface GenericOAuthConfig {
  ...
  pkce?: boolean;
  accessType?: string;
  getUserInfo?: boolean | ((tokens: OAuth2Tokens) => Promise<User | null>);
}
```
...
- `getUserInfo`: A boolean to always fetch from the userInfoUrl or custom function to fetch user info (optional).
...
Or a boolean to always fetch from the `userInfoUrl`.
  ...
  config: [
    {
      providerId: "custom-provider",
      // ... other config options
      getUserInfo: true // Always fetch from userInfoUrl
    }
  ]
})
```
