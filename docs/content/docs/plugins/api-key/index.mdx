...
<Callout>
  ...
  If you'd rather manage cleanup yourself, set `keyExpiration.autoCleanup` to
  `false` and call `auth.api.deleteAllExpiredApiKeys()` from a cron job or other
  external maintenance flow.
</Callout>
