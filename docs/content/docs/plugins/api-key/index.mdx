...
<Callout type="warn">
  `x-forwarded-for` is trusted based on your Better Auth `advanced.ipAddress` config.
  If your deployment sits behind an additional proxy or gateway that terminates
  the API-key request, the IP Better Auth sees will be the gateway's — always
  pass `clientIp` explicitly in that case, or the allowlist will be
