...
`keyExpiration` <span className="opacity-70">`{ defaultExpiresIn?: number | null; disableCustomExpiresTime?: boolean; minExpiresIn?: number; maxExpiresIn?: number; autoCleanup?: boolean; }`</span>
...
<Accordions>
  <Accordion title="keyExpiration options">
    ...
    `autoCleanup` <span className="opacity-70">`boolean`</span>

    Whether to automatically clean up expired API keys inline during API key requests.
    If `false`, inline cleanup is disabled and you can call `auth.api.deleteAllExpiredApiKeys()`
    from a cron job or another external maintenance task instead.
    Default is `true`.
  </Accordion>
</Accordions>
