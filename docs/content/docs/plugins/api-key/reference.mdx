...
`disableCustomPrefix` <span className="opacity-70">`boolean`</span>

Whether to disable custom prefixes passed from the client during API key creation. If `true`, the prefix will always use `defaultPrefix` and any client-provided prefix will be ignored. Default is `false`.
