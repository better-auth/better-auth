...
`schema` <span className="opacity-70">`ApiKeySchema`</span>

Custom schema for the API key plugin, including additional fields for the `apikey` table.
...
You can extend the table with additional fields:
