...
<Accordions>
  <Accordion title="rateLimit options">
    ...
    Length of each fixed rate-limit window in milliseconds (`[rateLimitWindowStart, rateLimitWindowStart + timeWindow)`).

    `maxRequests` <span className="opacity-70">`number`</span>

    Maximum successful validations allowed in each fixed window before the next window boundary (`rateLimitWindowStart + timeWindow`).
...
<DatabaseTable
  fields={[
    ...
    },
    {
      ...
      type: "number",
      description:
        "Length of each fixed rate-limit window in milliseconds (`[rateLimitWindowStart, rateLimitWindowStart + rateLimitTimeWindow)`).",
      isOptional: true,
    },
    {
      name: "rateLimitMax",
      type: "number",
      description:
        "Maximum successful validations allowed within each fixed window (see `rateLimitWindowStart`).",
      isOptional: true,
    },
    {
      name: "requestCount",
      type: "number",
      description:
        "Successful validations counted in the current fixed window (since `rateLimitWindowStart`).",
      isOptional: true,
    },
    {
      ...
      type: "Date",
      description:
        "Start time (inclusive) of the current fixed rate-limit window; `null` until the first validation in a new period.",
      isOptional: true,
    },
    {
      name: "remaining",
