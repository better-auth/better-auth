...
If the server pauses sign-in behind 2FA, the One Tap client redirects to that callback URL with `challenge=two-factor`, `attemptId=...`, and `methods=...` instead of navigating as if login had already finished.
...
* `onTwoFactorRedirect`: Called when the server pauses the One Tap sign-in behind a 2FA challenge. Receives `attemptId` plus `availableMethods`. If omitted, the client falls back to redirecting to `callbackURL` (or `/`) with `challenge=two-factor`, `attemptId`, and `methods` query params.
