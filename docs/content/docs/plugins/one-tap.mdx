...
Prompt mode is passive: the browser decides when to show the prompt and renders it in a corner of the page. Active mode instead opens the browser's centered account chooser, so you can trigger it from your own sign-in button and keep your own styling:

```ts
import { authClient } from "@/lib/auth-client";

await authClient.oneTap({ mode: "active" }); // [!code highlight]
```

Active mode requires [FedCM](https://developers.google.com/privacy-sandbox/cookies/fedcm) and a user gesture, so call it from a click handler. When the browser has no FedCM support the plugin logs a warning and falls back to the passive prompt.

If the user closes the chooser, or the browser refuses to open it (for example when third-party sign-in is disabled in the browser settings), the promise resolves and `onPromptNotification` receives the `DOMException` the browser rejected with. Use it to fall back to your regular Google sign-in:

```ts
await authClient.oneTap({
  mode: "active",
  onPromptNotification: async () => {
    await authClient.signIn.social({ provider: "google" });
  },
});
```

If the browser resolves the chooser without handing over a credential instead of rejecting, `onPromptNotification` is called with no argument, so a dismissal is never silent. Only the chooser itself is reported this way. Once it returns a credential, a failing sign-in request rejects the promise as it does in passive mode, so a failed login is never mistaken for a dismissal.
...
* `mode`: The UI mode for the flow. `"passive"` shows the One Tap prompt in a corner of the page, `"active"` opens the browser's centered account chooser and needs a user gesture. Can be set per call on the `oneTap` action. Default is "passive".
