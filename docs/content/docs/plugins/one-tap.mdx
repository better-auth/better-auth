...
If the server pauses sign-in behind 2FA, the One Tap client redirects to that callback URL with `challenge=two-factor` and `methods=...` instead of navigating as if login had already finished. The `attemptId` is intentionally omitted from the URL: it is carried on the signed `two_factor` cookie set by the server, because query parameters leak through `Referer` headers and proxy/CDN logs.

#### Handling 2FA Challenges Explicitly

If you want to intercept that paused-sign-in branch yourself, use `onTwoFactorRedirect`:
...
await authClient.oneTap({
  ...
  onTwoFactorRedirect: ({ attemptId, availableMethods }) => {
    ...
    // Show your own 2FA UI or route to a dedicated verification page.
...
* `onTwoFactorRedirect`: Called when the server pauses the One Tap sign-in behind a 2FA challenge. Receives `attemptId` plus `availableMethods`. If omitted, the client falls back to redirecting to `callbackURL` (or `/`) with `challenge=two-factor` and `methods` query params. The `attemptId` is not placed in the URL (it travels on the signed `two_factor` cookie) to avoid Referer / proxy-log leakage.
