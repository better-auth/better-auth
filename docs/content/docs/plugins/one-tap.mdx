...
Active mode only applies together with the `button` option, because the chooser is opened by the button Google renders. Calling `oneTap({ mode: "active" })` without a container logs a warning and shows the passive prompt.
