...
* `mapProfileToUser`: Optional mapper from the **verified Google ID token payload** to fields passed to `createUser` on first sign-up (same contract as `socialProviders.google.mapProfileToUser`). If omitted, Better Auth reuses `socialProviders.google.mapProfileToUser` when configured, so structured claims like `given_name` and `family_name` behave the same as the standard Google OAuth redirect flow and you avoid splitting `name` heuristically. If you define both, the One Tap plugin option wins.

Example – map structured name fields onto your schema (often via `user.additionalFields`; see [extending the user schema](/docs/concepts/database#extending-core-schema)):
...
export const auth = betterAuth({
  ...
  plugins: [
    oneTap(), // uses socialProviders.google.mapProfileToUser
  ],
});
```

Override only for One Tap (e.g. different rules than redirect OAuth):
