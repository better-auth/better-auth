...
By default the prompt is passive: the browser decides when to show it and renders it in a corner of the page. Active mode is Google's [FedCM button flow](https://developers.google.com/identity/gsi/web/guides/fedcm-migration) — clicking the rendered Sign in with Google button opens the browser's centered account chooser instead of a popup window:
...
await authClient.oneTap({
  mode: "active", // [!code highlight]
  button: { container: "#google-signin" },
});
```

Active mode only applies together with the `button` option, because the chooser is opened by the button Google renders. Calling `oneTap({ mode: "active" })` without a container logs a warning and shows the passive prompt. Setting `promptOptions: { fedCM: false }` opts out of FedCM everywhere, so the button keeps the popup flow even in active mode.

`autoSelect` carries over to the button flow: a returning user with an active Google session is signed in straight away, skipping the account chooser.

The flow needs Chrome 125+ on desktop or Chrome 128+ on Android. Elsewhere the button keeps the existing non-FedCM popup flow, so there is nothing to feature-detect.
...
* `mode`: The UI mode for the flow. `"passive"` shows the One Tap prompt in a corner of the page, `"active"` opts the rendered button into the FedCM button flow so it opens the browser's centered account chooser. Only applies with the `button` option. Can be set per call on the `oneTap` action. Default is "passive".
...
* `promptOptions`: Configuration for the prompt behavior and exponential backoff:
  ...
  * `fedCM`: When enabled, calls [`navigator.credentials.preventSilentAccess()`](https://developer.mozilla.org/en-US/docs/Web/API/CredentialsContainer/preventSilentAccess) on sign-out to clear the browser's FedCM credential state, and lets `mode: "active"` opt the rendered button into the FedCM button flow. Setting it to false keeps that button on the popup flow. FedCM for the One Tap prompt itself is managed by the Google Identity Services library and cannot be disabled. Default is true.
