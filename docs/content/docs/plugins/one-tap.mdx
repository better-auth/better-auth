...
If the server pauses sign-in behind 2FA, the One Tap client redirects to that callback URL with `challenge=two-factor` instead of navigating as if login had already finished. The `attemptId` is intentionally omitted from the URL: it is carried on the signed `two_factor_challenge` cookie set by the server, because query parameters leak through `Referer` headers and proxy/CDN logs.
...
* `onTwoFactorRedirect`: Called when the server pauses the One Tap sign-in behind a 2FA challenge. Receives `attemptId` plus the available `methods`. If omitted, the client falls back to redirecting to `callbackURL` (or `/`) with `challenge=two-factor`. The `attemptId` is not placed in the URL (it travels on the signed `two_factor_challenge` cookie) to avoid Referer / proxy-log leakage.
