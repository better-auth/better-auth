...
<Steps>
    ...
    export const auth = betterAuth({
        plugins: [
            captcha({ // [!code highlight]
                provider: "cloudflare-turnstile", // or google-recaptcha, hcaptcha, captchafox, yandex-smart-captcha // [!code highlight]
  ...
  <Step>
    ...
    * To implement Yandex SmartCaptcha on the client side, follow the official [Yandex SmartCaptcha documentation](https://yandex.cloud/en/docs/smartcaptcha/quickstart)
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    it validates the captcha token on the server, by calling the captcha provider's verification endpoint.
  </Step>

  <Step>
    * if the token is missing, gets rejected by the captcha provider, or if the verification endpoint is
