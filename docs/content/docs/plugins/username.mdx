...
When set to `true`, users will be required to provide a username during sign up. When set to `false` (default), the username field is optional during registration.

### Client

**requiredUsername**: Whether the username is required during user registration. Affects `signUp.email` function. Default is `false`.
