...
const data = await authClient.signUp.email({
    ...
    password: "password1234",
    username: "test",
    displayUsername: "Test_User123" 
})
```

<Callout type="info">
    If both `username` and `displayUsername` are provided, the `username` will be normalized and the `displayUsername` will be preserved as-is. If only `username` is provided, the `displayUsername` will be set to the pre normalized version of the `username`. You can see the [Username Normalization](#username-normalization) and [Display Username Normalization](#display-username-normalization) sections for more details.
</Callout>
...
By default, the username is normalized after validation. You can disable this behavior by setting `validatePostNormalization` to `true`.
