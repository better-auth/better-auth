...
When `username()` is enabled, Better Auth UI reads the username capability and updates the built-in auth pages:

* `/auth/sign-up` adds a required `username` field and an optional `displayUsername` field labeled "Display name".
* `/auth/sign-in` links to the username sign-in flow.
* `/auth/sign-in/username` renders a dedicated username and password sign-in form backed by `POST /sign-in/username`.

The username field respects `minUsernameLength` and `maxUsernameLength` from the username plugin options.
