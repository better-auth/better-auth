...
- SCIM bearer token: See [generating a SCIM token](#genering-a-scim-token)

### Generating a SCIM token

Before your identity provider can start syncing information to your SCIM server
you need to generate a SCIM token which your identity provider will use to authenticate against it.
