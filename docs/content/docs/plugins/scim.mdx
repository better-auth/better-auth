...
<APIMethod path="/scim/v2/ServiceProviderConfig" method="GET" isServerOnly note="Standard SCIM metadata endpoint used by identity providers. See https://datatracker.ietf.org/doc/html/rfc7644#section-4">
```ts
...
<APIMethod path="/scim/v2/Schemas" method="GET" isServerOnly note="Standard SCIM metadata endpoint used by identity providers to acquire information about supported schemas. See https://datatracker.ietf.org/doc/html/rfc7644#section-4">
```ts
type getSCIMSchemas = {
}
```
...
<APIMethod path="/scim/v2/Schemas/:schemaId" method="GET" isServerOnly note="Standard SCIM metadata endpoint used by identity providers to acquire information about a given schema. See https://datatracker.ietf.org/doc/html/rfc7644#section-4">
```ts
type getSCIMSchema = {
}
```
...
<APIMethod path="/scim/v2/ResourceTypes" method="GET" isServerOnly note="Standard SCIM metadata endpoint used by identity providers to get a list of server supported types. See https://datatracker.ietf.org/doc/html/rfc7644#section-4">
```ts
...
<APIMethod path="/scim/v2/ResourceTypes/:resourceTypeId" method="GET" isServerOnly note="Standard SCIM metadata endpoint used by identity providers to get a server supported type. See https://datatracker.ietf.org/doc/html/rfc7644#section-4">
```ts
...
<Callout>
**Note:** When the `organization` plugin is enabled, only users with an admin role (`adminRoles`) will be able to generate a SCIM token.
In all other cases, you can use the `adminUserids` option to restrict who can generate a SCIM token
</Callout>
...
- `adminRoles`: The list of roles that are considered admin roles. Only users with these roles will be able to sync information through SCIM. Defaults to `["admin", "owner"]`
- `adminUserIds`: The list of user ids that are considered admins. Only users with these ids will be able to sync information through SCIM. Defaults to `[]`
