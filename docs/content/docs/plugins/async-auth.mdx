...
<Steps>
    ...
    <Step>
        ...
        Register an OAuth client that will be used by your agent/application. CIBA requires a **confidential client** (with `client_secret`), so you must be authenticated with a session cookie when registering.

        ```bash
        # First sign in to get a session cookie, then register:
        ...
        <Callout type="info">
          The `token_endpoint_auth_method: "client_secret_post"` means the agent must send `client_id` and `client_secret` in the request body when calling token endpoints. This is required for CIBA since the agent needs to prove its identity.
        </Callout>
    </Step>
...
asyncAuth({
  ...
  // How long requests are valid as TimeString (default: "5m")
  requestLifetime: "5m",

  // Minimum polling interval as TimeString (default: "5s")
  pollingInterval: "5s",

  // Custom user resolver (default: looks up by email)
  resolveUser: async (loginHint, ctx) => {
    // Return user or null if not found
    return await ctx.context.internalAdapter.findUserByEmail(loginHint);
  },
});
```

<Callout type="info">
  Token expiration settings (`accessTokenExpiresIn`, `refreshTokenExpiresIn`) and client configuration (`trustedClients`) are configured on the `oauthProvider()` or `oidcProvider()` plugin, not on `asyncAuth()`.
</Callout>
