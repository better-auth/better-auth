...
- `customOTP`: An object with the following properties to customize OTP generation:
    - `phoneNumbers`: A record or an function that returns a record of phone numbers mapped to custom OTP codes. This is useful for testing purposes.
    - `skipSendOTP`: A boolean to skip sending the OTP code for phone numbers listed in `customOTP.phoneNumbers`. Default is `false`. 

```ts
// Example usage of custom OTPs using a static record
export const auth = betterAuth({
    plugins: [
        phoneNumber({
            sendOTP: ({ phoneNumber, code }, request) => {
            ...
            customOTP: {
                ...
                skipSendOTP: true // this will avoid calling sendOTP() for custom OTPs
            }
        })
    ]
})
...
```ts
// Example usage of custom OTPs with async function
export const auth = betterAuth({
    plugins: [
        phoneNumber({
            sendOTP: ({ phoneNumber, code }, request) => {
