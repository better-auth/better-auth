    ...
    plugins: [
        phoneNumber({
            ...
            generateOTP: ({ phoneNumber, type }, ctx) => { // [!code highlight]
                if (phoneNumber === process.env.REVIEWER_PHONE_NUMBER) { // [!code highlight]
...
<Callout type="warn">
  A fixed code is only as secret as the value you configure. Keep it out of source control and restrict it to dedicated test accounts.
</Callout>
