...
- `signUpOnVerification`: An object with the following properties:
    - `fields`: Optional object containing default values for user fields:
      - `name`: Default name to use if not provided during verification
      - `email`: Default email to use if not provided during verification
    - `getTempEmail`: A function that generates a temporary email for the user if no email is provided. It takes the phone number as an argument and returns the temporary email.
    - `getTempName`: A function that generates a temporary name for the user if no name is provided. It takes the phone number as an argument and returns the temporary name.
