...
A custom function to generate the OTP code. It receives an object with the `phoneNumber` and the `type` of code being sent (`"phone-number-verification"` or `"forget-password"`) and a request object, and returns the code to send. Return `undefined` to fall back to the default generator (a random code of `otpLength` digits).

Only the generated code changes: the code is stored, expires, counts attempts and is consumed exactly like a generated one. This is useful to give a fixed code to test accounts that cannot receive SMS, such as an app store reviewer account.
...
export const auth = betterAuth({
    plugins: [
        phoneNumber({
            ...
            generateOTP: ({ phoneNumber, type }, ctx) => { // [!code highlight]
                // Only use the fixed code to sign in, not to reset the password // [!code highlight]
                if ( // [!code highlight]
                    phoneNumber === process.env.REVIEWER_PHONE_NUMBER && // [!code highlight]
                    type === "phone-number-verification" // [!code highlight]
                ) { // [!code highlight]
                    return process.env.REVIEWER_OTP; // [!code highlight]
                } // [!code highlight]
                // Return undefined to use the default generator // [!code highlight]
            } // [!code highlight]
        })
    ]
})
...
<Callout type="warn">
  A fixed code is only as secret as the value you configure. Keep it out of source control, restrict it to dedicated test accounts, and check `type` so it cannot be used to reset their password.
</Callout>
