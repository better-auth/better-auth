...
export const cimdOptionsType = {
	...
	metadataFetchPolicy: {
		...
		description:
			"Reject-immediately request-amplification limits. Numeric minimumFetchInterval values are seconds and pace retries after failed fetches. Successful fetches remain governed by HTTP freshness and the origin/global budgets. Minute limits use a rolling 60-second window. Set minimumFetchInterval to 0 only to disable failed-fetch pacing.",
	},
	originBoundFields: {
		type: "string[]",
