...
Protected dynamic client registration allows machine callers to register public or confidential clients without a Better Auth user session. Issue an RFC 7591 initial access token out of band, then validate the token from the `Authorization: Bearer <token>` header.

```ts title="auth.ts"
oauthProvider({
  allowDynamicClientRegistration: true,
  validateInitialClientRegistrationAccessToken: async ({
    initialClientRegistrationAccessToken,
    clientMetadata,
  }) => {
    if (initialClientRegistrationAccessToken !== process.env.CLIENT_REGISTRATION_TOKEN) {
      return false
    }
...
The `referenceId` value attaches application ownership metadata to the created OAuth client. Token issuance, expiration, and revocation stay in your application because RFC 7591 leaves initial access token lifecycle policy to the authorization server. This is separate from RFC 7592 registration management tokens.
