...
Configured resources are seeded during auth initialization. If a database operation fails during seeding, the provider logs a warning and retries on the first resource access. A resource seed failure does not permanently disable unrelated session or login requests. Resource-dependent OAuth requests still fail if seeding or the resource lookup fails. Exceptions from a custom `identifierValidator` still reject initialization. Check startup warnings for database connectivity, permissions, and schema problems, even if browser sign-in works.
