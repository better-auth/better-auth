...
```ts title="client.ts"
import { auth } from "@/lib/auth"; // optional
import { createAuthClient } from "better-auth/client";
import { oauthProviderResourceClient } from "better-auth/server/plugins"
const authClient = createAuthClient({
...
The majority of these endpoints are server-only to allow implementers to describe their own RBAC checks through their own api.

#### Create

There are two types of OAuth Clients:
- **Public Clients**: Applications such as native mobile clients and user-agent clients (like AI) which are unable to hold client secrets.
- **Confidential Clients**: Applications such as web clients able to hold client secrets.

An example confidential client. Note that the client secret is displayed once and cannot be seen again:
```ts
await auth.api.createOAuthClient({
  headers,
  body: {
    name: "Internal Dashboard",
    redirectUris: ["https://dashboard.company.com/auth/callback"], // [!code highlight]
    type: "web",
    disabled: false,
...
An example public client:
```ts
await auth.api.createOAuthClient({
  headers,
  body: {
    token_endpoint_auth_method: "none", // [!code highlight]
    name: "Company Mobile App",
    redirectUris: ["com.company.app://auth"], // [!code highlight]
    type: "native",
    disabled: false,
...
Can obtain OAuth clients from their `user_id` or `reference_id`.
...
Can obtain information about a specified client.

```ts
await auth.api.getOAuthClient({
  headers,
  params: {
    id: client_id,
  },
...
Obtains public information about a client. Useful for login flow pages such as consent where you'd need to display public client information.

```ts
await client.oauth2.clients.public({
  query: {
    client_id: client_id,
  },
});
...
Can update fields for a specific client. The combination must still be a valid OAuthClient configuration. Converting between public and confidential clients cannot be performed and client_secrets cannot be changed.
...
To rotate a secret, use the rotate function. Note that the secret updates immediately and is displayed only once.
...
<APIMethod path="/oauth2/continue" method="POST">
```ts
type oauth2Continue = {
  /**
...
oauthProvider({
  ...
  selectedAccount: async ({ headers }) => { // [!code highlight]
...
The plugin will redirect the user to the `selectAccountPage`. This page should prompt for account selection and upon completion of selection, should call `oauth2Continue`. Note the `oauth_login_prompt` is automatically stored in a signed cookie.
...
oauthProvider({
  ...
  postLogin: {
    ...
    shouldRedirect: async ({ session }) => { // [!code highlight]
    ...
    consentReferenceId: ({ session, scopes }) => {
      if (scopes.includes("read:organization")) {
        const activeOrganizationId = (context.session?.activeOrganizationId ?? undefined) as string | undefined;
...
The plugin will redirect the user to the `postLoginPage`. This page should prompt for account selection and upon completion of selection, should call `oauth2Continue`. Note the `oauth_login_prompt` is automatically stored in a signed cookie.
...
You can choose to format your session tokens in a different string format using the `storeRefreshToken`.
...
<DatabaseTable
  fields={[
    ...
    },
    {
      name: "used",
      type: "Date",
      description: "Timestamp when the token was used",
      isOptional: true,
    },
    {
      name: "revoked",
