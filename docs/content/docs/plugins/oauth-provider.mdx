...
import { createAuthClient } from "better-auth/client";
import { oauthProviderResourceClient } from "@better-auth/oauth-provider/client"
const authClient = createAuthClient({
...
The plugin will redirect the user to the `postLoginPage`. This page should prompt for account selection and upon completion of selection, should call `oauth2Continue`.
...
To determine whether a logged in has the ability to perform specific action in client creation, you can utilize the `clientPrivileges` configuration setting. By default, CRUD actions are allowed for users with matching `userId` or through `clientReference`.
...
```ts title="auth.ts"
oauthProvider({
  clientPrivileges: ({ action, headers, session }) => {
