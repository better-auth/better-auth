...
OAuth companion plugins can extend the provider without changing OAuth Provider core. Use `extendOAuthProvider()` from a plugin `init()` hook to add token grants, assertion-based client authentication methods, additive discovery metadata, and token or UserInfo claims.
      ...
      extendOAuthProvider(ctx, {
        grants: {
          "urn:example:params:oauth:grant-type:custom": async ({
            ...
            const { client } = await tools.authenticateClient({ grantType });
...
Extension metadata is additive: existing authorization-server metadata fields such as `issuer`, `token_endpoint`, `grant_types_supported`, and authentication method lists remain owned by the OAuth Provider. Extension grants and client authentication methods are advertised automatically when registered. Client authentication strategies are keyed by `token_endpoint_auth_method`, consume matching `client_assertion_type` values, and must return the verified registered client.
