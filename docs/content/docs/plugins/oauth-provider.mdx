...
<Callout type="warn">
  A resolver must resolve to the issuer's key set or throw. Resolving to `undefined` surfaces as `Error: No jwks found`; a resolver that throws surfaces its own error unchanged. Either way this is not an authorization failure, so it is not answered with a `401` and an RFC 6750 challenge: it propagates as an unexpected error, which a route handler reports as an HTTP `500`. Treat a resolver as "the key set is available" or "something is broken", never as a "not ready yet" state for the caller to handle.
</Callout>
...
<Callout type="warn">
  A function source runs in your own process, so its failures are yours. A `TypeError` thrown anywhere inside a resolver is read as "probably an opaque token" and swallowed, and verification then falls through to `remoteVerify` when one is configured, instead of surfacing the bug. With no `remoteVerify` configured there is no fallthrough, so the token is still rejected. Do not signal an expected condition by throwing a `TypeError` from a resolver, and make sure a resolver cannot throw one for a reason unrelated to token format.
</Callout>
...
The cache is keyed by that object alone and records no issuer or audience of its own, so `jwksCacheKey` must be stable **per issuer (and audience)**, not merely stable across requests. Do not share one key across issuers: a cached set is reused whenever it contains the token's `kid`, so a `kid` that both issuers publish verifies against the wrong key and is rejected for the rest of the TTL. That surfaces as a spurious `UNAUTHORIZED` that looks like a key rotation bug, and the `kid`-miss refetch cannot rescue it, because the `kid` was found. Key it per issuer, and per audience when one issuer serves several resources, from a module-scoped map whose *values* are the objects you pass:

```ts title="lib/jwks-cache-key.ts"
const jwksCacheKeys = new Map<string, object>();

export function jwksCacheKeyFor(issuer: string, audience: string) {
  const entry = `${issuer}|${audience}`;
  let cacheKey = jwksCacheKeys.get(entry);
  if (!cacheKey) {
    cacheKey = {};
    jwksCacheKeys.set(entry, cacheKey);
  }
  return cacheKey;
}
```

The string is only the map's own key; the object stored under it is what you pass as `jwksCacheKey`, which is why the function returns an `object` and not the `entry` string. Two mistakes type-check silently here, so avoid both. Forwarding `jwksCacheKeys.get(entry)` directly yields `object | undefined`, which still satisfies `jwksCacheKey?: object` and quietly turns caching off on every miss. Declaring the map inside a request handler makes every request a fresh key, degrading to the per-verification read this option exists to remove. Declare it at module scope and use the get-or-create above.

`jwksCacheKey` is ignored for string `jwksUrl` values, which are already cached by url.
