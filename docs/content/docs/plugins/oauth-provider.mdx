...
description: A Better Auth OAuth authorization-server plugin with selected OAuth 2.1 draft-aligned behavior.
---
...
<Callout type="info">
  OAuth 2.1 is still an [IETF Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/). This page documents Better Auth's current OAuth authorization-server behavior and selected draft-aligned practices; it does not claim final OAuth 2.1 compliance.
</Callout>
...
* **OAuth security profile:** aligns with selected practices from the current [OAuth 2.1 Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/) and includes the RFC 9207 `iss` parameter to prevent [authorization-server mix-up attacks](https://datatracker.ietf.org/doc/html/rfc9207).
...
The plugin operates as an OAuth authorization server with OIDC-capable endpoints and JWT-verifiable access tokens. The following provides more detailed information about each endpoint.
...
An OAuth authorization endpoint aligned with selected guidance in the current [OAuth 2.1 Internet-Draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/). Since the draft is not final and many details may change, parts are adapted from the legacy [OAuth 2.0 Authorization Endpoint Section](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1).

The Authorization Endpoint is the entry point for initiating an OAuth authorization flow.

Important notes:

* This provider supports only `response_type: "code"`, consistent with the current OAuth 2.1 Internet-Draft.
...
The device authorization grant ([RFC 8628](https://datatracker.ietf.org/doc/html/rfc8628)) lets limited-input clients (CLIs, smart TVs, IoT) obtain an OAuth access token. Add `oauthDeviceAuthorization()` alongside `oauthProvider()` to register the `urn:ietf:params:oauth:grant-type:device_code` token grant and advertise `device_authorization_endpoint`. The integration adds the OAuth client binding and the OAuth Provider integration's profile of RFC 8707: the `resource` parameter on `/device/code` is the same standardized parameter RFC 8707 defines for authorization requests and token requests, and the approved resource set is checked again at `/oauth2/token`. Standalone Device Authorization installations remain unchanged. The device requests codes at `/device/code`, the user approves them, and the client polls `/oauth2/token` for a first-class OAuth token. This is an OAuth device grant; it does not make a general OIDC device-grant conformance claim. See [Authorize a CLI to call an API](/docs/plugins/device-authorization#authorize-a-cli-to-call-an-api).

At `/device/code`, confidential clients using `client_secret_basic` may authenticate with an `Authorization: Basic ...` header and omit the body `client_id`; public clients using `none` must send `client_id`. Empty authentication parameters are treated as omitted. Repeated non-empty authentication parameters or multiple authentication methods return `invalid_request`, while repeated `resource` parameters remain supported.
...
The scopes configuration can contain as many or as few scopes as you wish! Note that `openid` is required to be considered an OIDC server; without it, this is an OAuth server without OIDC identity features. All supported scopes must be in this array.
...
PKCE (Proof Key for Code Exchange) is a security mechanism that prevents authorization code interception attacks. This plugin requires PKCE by default for all authorization code flows, consistent with the current OAuth 2.1 Internet-Draft and OAuth security guidance.

#### Default Behavior

By default, PKCE is required for all clients. This provides maximum security and matches current OAuth 2.1 draft guidance.
...
* **Future-proof**: Aligns with current OAuth 2.1 draft guidance
