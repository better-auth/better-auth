...
oauthProvider({
  ...
  postLogin: {
    ...
    shouldRedirect: async ({ session, scopes }) => { // [!code highlight]
...
oauthProvider({
  ...
  customIdTokenClaims: ({ user, scopes, referenceId, metadata }) => {
    return {
      locale: "en-GB",
  ...
  customAccessTokenClaims: ({ user, scopes, referenceId, metadata }) => {
    return {
      "https://example.com/org": referenceId,
