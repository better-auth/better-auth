...
const res = await client.oauth2.oauth2Continue({
  confirm: true,
})
```
...
oauthProvider({
  ...
  postLoginPage: "/select-organization", // [!code highlight]
  postLogin: async ({ session }) => { // [!code highlight]
  ...
  postLoginConsentReferenceId: ({ session, scopes }) => {
