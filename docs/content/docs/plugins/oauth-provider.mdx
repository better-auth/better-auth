...
* **Dynamic Client Registration**: Allow clients to register clients dynamically.
  * **Public Clients**: Support clients that cannot keep a secret by using `token_endpoint_auth_method: "none"`
  * **Confidential Clients**: Support clients that authenticate at the token endpoint with a registered method
...
`application_type` ([OIDC Registration §2](https://openid.net/specs/openid-connect-registration-1_0.html#ClientMetadata)) defaults to `web` when omitted. A `web` client requires `https` redirect URIs on non-loopback hosts. A `native` client may use claimed `https` links, loopback `http` redirects on exact `localhost`, `127.0.0.0/8`, or `::1`, or an authority-free private-use URI with a well-formed reverse-domain scheme such as `com.example.app:/callback` ([RFC 8252 §7](https://datatracker.ietf.org/doc/html/rfc8252#section-7)). Credentials, fragments, malformed or reserved schemes such as `file:` and `mailto:`, loopback `https`, trailing-dot localhost names, and routable-host `http` redirects are rejected.
...
* **[`@better-auth/cimd`](/docs/plugins/cimd)**: an optional plugin that lets clients identify themselves by hosting a metadata document at an HTTPS URL. The URL itself becomes the `client_id`; the server fetches and validates the document. This is the pattern MCP adopts from [Client ID Metadata Document draft-00](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document-00).
