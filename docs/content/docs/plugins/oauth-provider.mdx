...
<Callout type="warn">
Support for `allowUnauthenticatedClientRegistration` **is deprecated** and is considered insecure. MCP has added [Client ID Metadata Documents](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/991) which is [available](#cimd). Additionally, [`software_statement` and `jwks_uri`](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1032) may be added as an alternative.
</Callout>
...
[ClientId Metadata Documents](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) provides a way for clients to identify themselves without prior registration by utilizing a metadata document as their `client_id`.

Dynamic Client Registration can be utilized with CIMD providing a useful fallback for clients who may not yet support CIMD.

To enable CIMD, please add the following configuration:
...
To set the rate at which client metadata documents should be updated, set `refreshRate` (defaults: "60m"):

```ts title="auth.ts"
oauthProvider({
  cimd: {
    enable: true,
    refreshRate: "10m", // [!code highlight]
  },
})
```

To change restrictions set of the metadata document, set `restrictOrigins` to its respective client metadata field (defaults: "redirect_uris", "post_logout_redirect_uris", "client_uri").

```ts title="auth.ts"
oauthProvider({
  cimd: {
    enable: true,
    restrictOrigins: ["redirect_uris", "post_logout_redirect_uris"], // [!code highlight]
  },
})
```
...
<Steps>
  ...
  <Step>
  If you use `cimd` or `allowUnauthenticatedClientRegistration`, you must ensure that your API Server is a confidential client itself:
  ...
  <Callout type="info">
  If you choose to not support `cimd` or `allowUnauthenticatedClientRegistration` (and only `allowDynamicClientRegistration`), the MCP client (ie. ChatGPT, Anthropic, Gemini) would need to allow you to put in a public client_id in their UI or at runtime while chatting with the AI.
  </Callout>
  </Step>
