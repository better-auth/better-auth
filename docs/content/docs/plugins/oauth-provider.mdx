...
* Requires `nonce` when `offline_access` is requested without PKCE
...
This only applies to confidential clients created through dynamic client registration. Public clients still require PKCE. Confidential OIDC clients that request `offline_access` without PKCE must send `nonce`.
