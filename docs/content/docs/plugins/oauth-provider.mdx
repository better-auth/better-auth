...
oauthProvider({
  ...
  postLogin: {
    ...
    shouldRedirect: async ({ session, scopes, headers, isContinue }) => { // [!code highlight]
      ...
      return !hasActiveOrganization || (!isContinue && organizations.length > 1);
    },
...
The plugin will redirect the user to the `postLogin.page` to provide a prompt for account selection. Upon completion, call `authClient.oauth2.continue`.

`shouldRedirect` runs again on continuation with `isContinue: true`. Use this value to check the selected organization without asking multi-organization users to select again. It is not proof of selection: check the current session and organization membership before returning `false`. Each new authorization request starts with `isContinue: false`.
