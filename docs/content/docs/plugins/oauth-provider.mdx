...
2. Native loopback port variance for `localhost`, `127.0.0.0/8`, and `[::1]` ([RFC 8252 §7.3](https://www.rfc-editor.org/rfc/rfc8252#section-7.3))
...
The callback does not change registration: clients created through the built-in endpoints still need a concrete redirect URI. It applies to every client of this OAuth provider, so use `registeredUris` when an exception should apply only to particular clients.

```ts title="auth.ts"
oauthProvider({
  // Extend: add preview deployment support while preserving defaults (recommended)
  validateRedirectUri: (uri, _registeredUris, defaultResult) => {
