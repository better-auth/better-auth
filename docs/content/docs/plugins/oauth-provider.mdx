...
<Callout type="warn">
  Custom acceptance relaxes the [exact redirect URI matching required by OAuth security guidance](https://www.rfc-editor.org/rfc/rfc9700.html#section-2.1). Use it only for callback hosts you control, and ensure untrusted contributors cannot deploy a callback under an accepted hostname. The example assumes only trusted clients can register its production callback URI. If untrusted clients can register arbitrary URIs, membership in `registeredUris` does not prove client ownership.

  For newly accepted web callbacks, pin the HTTPS scheme, hostname suffix, port, path, and query. Avoid broad patterns such as `*` or `*.com`. Preserve the built-in checks for native callbacks unless you intend to replace them. The provider rejects malformed URIs, fragments, credentials, and `javascript:`, `data:`, or `vbscript:` schemes before the callback can accept them.

  The callback also decides whether early authorization errors may be redirected to the requested `redirect_uri` ([RFC 6749 §4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749#section-4.1.2.1)). On rejection or exception, errors go to the server error page.
</Callout>

<Callout type="info">
  If you use [pairwise subject identifiers](/docs/plugins/oauth-provider#pairwise-subject-identifiers), the sector identifier comes from the client's first registered redirect URI. Every preview or tenant host accepted for that client therefore uses the same `sub`. `sector_identifier_uri` is not supported. If tenants need distinct subject identifiers, register separate clients with distinct redirect URI hosts.
</Callout>
