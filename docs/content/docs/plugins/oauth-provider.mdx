...
<Callout type="warn">
  ...
  * **Replay protection:** `verifyAccessTokenRequest` defaults to an in-memory `jti` store that is safe only for a single instance. For multi-instance or serverless resource servers, pass a shared `dpop.replayStore` (the provider's own endpoints already use a database-backed store).
</Callout>
...
export const oauthAccessTokenTableFields = [
  ...
  },
  {
    ...
    type: "string",
    description:
      "JWK SHA-256 thumbprint that sender-constrains this opaque access token via DPoP",
    isOptional: true,
  },
  {
    name: "revoked",
...
Table Name: `oauthDpopProof`

export const oauthDpopProofTableFields = [
  {
    name: "id",
    type: "string",
    description: "Database ID of the DPoP proof replay reservation",
    isPrimaryKey: true,
  },
  {
    name: "replayId",
    type: "string",
    description:
      "Unique replay key derived from the DPoP proof key, method, URL, and jti",
    isUnique: true,
  },
  {
    name: "expiresAt",
    type: "Date",
    description: "Timestamp when this replay reservation can be discarded",
  },
  {
    name: "createdAt",
    type: "Date",
    description: "Timestamp when this proof reservation was created",
  },
];

<DatabaseTable name="oauthDpopProof" fields={oauthDpopProofTableFields} />
