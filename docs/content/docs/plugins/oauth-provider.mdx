...
| ID token     | base identity claims \< `customIdTokenClaims`; extension and per-issuance `idTokenClaims` are additive (fill absent keys only) | reserved OIDC/JWT names (`iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, `jti`, `nonce`, `sid`, `at_hash`, `c_hash`, `s_hash`, `auth_time`, `acr`, `amr`, `azp`) |
