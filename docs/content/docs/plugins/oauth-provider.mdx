...
<Steps>
  ...
  <Step>
    ...
    To create a `private_key_jwt` client, register key material with `jwks` or `jwks_uri`. The client is confidential, but no `client_secret` is issued or used.
...
For `private_key_jwt` clients, include exactly one of `jwks` or `jwks_uri`.
