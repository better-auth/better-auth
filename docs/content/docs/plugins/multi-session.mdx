...
When a user logs out, the plugin will revoke the current session and set the next active session as the current session. If there are no active sessions, the user will be logged out. You can change this behaviour by passing the `signOutAll` option to the plugin.
