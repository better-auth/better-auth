...
**verificationUri** (required): The URL (absolute or relative) where users verify their device code. It is typically the page where users enter the code. Returned as `verification_uri` in the response. Can be an absolute URL (e.g., `https://example.com/device`) or relative path (e.g., `/device`).
