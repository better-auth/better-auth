...
| Let a registered CLI, TV app, or other public client call an OAuth-protected API | Scoped OAuth access token | `jwt()`, `deviceAuthorization({ grant })`, and `oauthProvider({ extensions: [grant] })` | `/oauth2/token` |
...
const grant = deviceCodeGrant(); // [!code highlight]
...
export const auth = betterAuth({
  plugins: [
    ...
    deviceAuthorization({
      ...
      grant, // [!code highlight]
    }),
    oauthProvider({
      loginPage: "/sign-in",
      consentPage: "/consent",
      extensions: [grant], // [!code highlight]
...
`deviceCodeGrant()` returns one shared extension object. Passing it to `deviceAuthorization({ grant })` adds the OAuth-only request, storage, verification, and routing behavior; passing the same object to `oauthProvider({ extensions: [grant] })` registers the `urn:ietf:params:oauth:grant-type:device_code` token grant and advertises `/device/code` as `device_authorization_endpoint`. This explicit two-sided composition keeps either plugin from silently configuring the other. The provider validates the registered client, scopes, and [RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707) resource before creating the request. The approval page can show the client, scopes, and resource to the signed-in user.
...
When the same `deviceCodeGrant()` object is passed to `deviceAuthorization({ grant })` and `oauthProvider({ extensions: [grant] })`, the OAuth grant extends this table with optional `oauthClientId` and `resources` fields. Standalone Device Authorization installations do not add those fields and do not expose the RFC 8707 `resource` request parameter.
