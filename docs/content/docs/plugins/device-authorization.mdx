...
1. The client requests codes at `/device/code` (the advertised `device_authorization_endpoint`).
2. The user approves at the verification URL, exactly as with the first-party flow.
3. The client polls **`/oauth2/token`** (not `/device/token`) with `grant_type=urn:ietf:params:oauth:grant-type:device_code`, `device_code`, and `client_id`. It receives an OAuth token set governed by the provider's scope, resource, and token policy.
