...
The rest of this page describes the first-party path, where `/device/token` returns a Better Auth session token. This is a Better Auth-specific session-token extension, not the RFC 8628 OAuth token endpoint; clients that need an OAuth access token must use `deviceCodeGrant()` with the OAuth Provider and poll `/oauth2/token`.
