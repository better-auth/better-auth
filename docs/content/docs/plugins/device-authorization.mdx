...
2. The user approves at the verification URL. The `GET /device` response includes `client_id`, `scope`, and `resource` so the approval page can show exactly what is requesting access.
