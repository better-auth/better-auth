...
The wrapper passes the verified access-token claims as a second callback argument, not as a database record. The example keeps authentication at the route boundary; if a tool needs token context, convert those claims to the SDK's `AuthInfo` and pass it through `mcpServerHandler.fetch(request, { authInfo })`. `requireMcpAuth` never exposes a refresh token, and it checks the access token locally against the JWKS, resolved in-process through your auth instance's JWT plugin and cached — no HTTP self-fetch of the server's own origin, which some runtimes (such as Cloudflare Workers by default) cannot perform. DPoP replay protection uses your auth instance's database adapter by default so it works across server instances; pass `dpop.replayStore` only when you need a different shared store.

By default, `requireMcpAuth` reads the server's resolved Better Auth URL from the auth context and uses it as the expected issuer and resource, and resolves the JWKS in-process through the JWT plugin (honoring a configured `jwks.remoteUrl`). If `mcp({ resource })` uses a different identifier, pass that same value to `requireMcpAuth`, as shown above. Override the issuer when `jwt.issuer` is custom, and pass `jwksUrl` — a URL string or a `() => Promise<JSONWebKeySet | undefined>` function — when the authorization server runs separately:
