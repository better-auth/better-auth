...
<Callout type="warn">
	This plugin is based on OIDC Provider plugin. It's currently not ready for
	production use. We are working on it and will update this documentation when
	it's ready.
</Callout>
...
<TypeTable
	type={{
		loginPage: {
			description:
				'Path to the login page where users will be redirected for authentication',
			type: 'string',
			required: true,
		},
		resource: {
