...
<Callout>
  Install `@better-auth/mcp` and the recommended `@better-auth/cimd` companion package.
</Callout>
...
<Steps>
  <Step>
    ...
    import { fetchClientMetadataResource } from "./oauth-network"; // [!code highlight]
    ...
    The injected metadata-resource transport must resolve each hostname once, reject RFC 6890 special-use addresses, pin the approved address for the connection, and refuse redirects. It is used for both the Client ID Metadata Document and discovery-owned resources such as `jwks_uri`. See the [CIMD security boundary](/docs/plugins/cimd#security-boundary) for the complete contract.
...
<Callout type="info">
  Better Auth owns OAuth discovery, client registration and discovery, PKCE authorization, resource binding, token issuance and refresh, and protected-resource challenges. Use the official MCP TypeScript SDK v2 for MCP protocol negotiation and transport behavior. Pin its version negotiation to `2026-07-28` when adopting this profile.
</Callout>
...
`mcpHandler` returns the same RFC 9728 `WWW-Authenticate` response for unauthenticated requests. Set `requiredScopes` in the first argument to require scopes; a token missing any of them receives the same `403` `insufficient_scope` challenge as `requireMcpAuth`. Use `challengeScopes` in the third argument to advertise a scope hint on unauthenticated challenges.
