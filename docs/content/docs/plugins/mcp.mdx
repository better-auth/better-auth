...
`mcp()` configures the OAuth provider with MCP-appropriate resource binding and serves the [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) protected resource metadata. For the MCP 2026-07-28 profile, compose it with [Client ID Metadata Documents](/docs/plugins/cimd). Dynamic Client Registration (DCR) is deprecated by MCP and is never enabled implicitly.
