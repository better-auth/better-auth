...
The OAuth Proxy plugin is particularly useful in conjunction with the [`oauth.stateManagement` configuration](/docs/reference/options#statemanagement), especially in situations where the proxy does not share the same datastore as the reliant services. By default, OAuth state is stored in the database for security, but this can cause issues in proxy scenarios where the proxy server and your application have separate databases.
