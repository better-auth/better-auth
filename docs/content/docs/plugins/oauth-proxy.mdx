...
<Callout type="warn">
  The request URL is only used when its origin is listed in `trustedOrigins` (wildcard patterns are supported). Because `request.url` is derived from the `Host`/forwarded-host headers on many platforms, an un-allowlisted origin is ignored and the plugin falls back to your configured `baseURL`/vendor URL. Make sure your preview/development origins are in `trustedOrigins` (see Step 3 above), or set `currentURL` explicitly.
</Callout>
