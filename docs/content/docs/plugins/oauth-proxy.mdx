...
The plugin includes built-in protection against replay attacks by adding timestamps to encrypted payloads. If an encrypted URL is intercepted, it can only be used within a short time window (default: 60 seconds) before it expires. This prevents attackers from reusing leaked URLs to gain unauthorized access.
