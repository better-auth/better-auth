...
export const auth = betterAuth({
    account: {
        accountLinking: {
            enabled: true,
            // Option 1: Trust specific providers
            trustedProviders: ["my-enterprise-sso"],
            // Option 2: Use existingUserMode to control policy
            existingUserMode: "trusted_providers_only", // default
        }
    },
    plugins: [
        sso({
...
<Callout>
Domain verification is the recommended approach for enterprise SSO. Once a provider's domain is verified, users with matching email domains can be auto-linked securely without adding every provider to `trustedProviders`.
</Callout>
