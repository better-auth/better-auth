...
You can automatically trust a new domain by enabling the domain verification feature:
...
When domain verification is enabled, every new SSO provider domain will be marked as unverified at first.
Upon registration, a **verification token** will be issued to the provider,
which you need to verify it to prove that you own it. To do this, you'll need to add a `TXT` record to your domain's DNS settings.
...
2.  **Create a new `TXT` record in your DNS provider's dashboard.**
    *   **Host:** `ba-domain-verification-{your-provider-id}`
...
4.  **Once the record has propagated, you can submit a validation request
...
This request will either result in a rejection (could not your ownership over the domain)
or if the verification is successful, your SSO provider domain will be validated as verified.

```ts title="domain-validation-request.ts"
...
After that time, the token will expire cannot longer be used. When that happens,
you can create a new verification token:

```ts title="create-domain-verification-token.ts"
...
console.log("Token: ", response.domainVerificationToken);
```
...
- **Domain verification submission**: `/api/auth/sso/domain-verfication`
- **Domain verification validation**: `/api/auth/sso/domain-verfication/verify`
...
<DatabaseTable
    fields={[
        ...
        { name: "domainVerified", type: "boolean", description: "A flag indicating whether the provider domain has been verified. Only available when domain verification is enabled", isRequired: false },
    ]}
/>
