...
<DatabaseTable
    fields={[
        ...
        { name: "useHttps", type: "boolean", description: "Whether to use HTTPS for the provider domain", isRequired: false },
...
With `useProviderDomain: true`:
- **Authorization URLs** are resolved against `https://{provider.domain}/api/auth`
- **Callback URIs** use the provider domain: `https://{provider.domain}/api/auth/sso/callback/{providerId}`
- **Token requests** are made to endpoints on the provider domain

### Configuration Options

- **`domain`**: The provider's domain.
- **`useHttps`** (optional): Explicitly control whether to use HTTPS or HTTP for the provider domain. If not specified, preserves the protocol from your base URL
...
const auth = betterAuth({
    plugins: [
        sso({
            useProviderDomain: true, // Enable provider domain resolution
        })
    ]
})
...
await authClient.sso.register({
    providerId: "tenant-sso",
    issuer: "https://idp.tenant.com",
    domain: "tenant.example.com",
    useHttps: true, // Optional - force HTTPS
    oidcConfig: {
        // ... OIDC configuration
    }
});
...
**useProviderDomain**: Use the provider domain to build callback URLs instead of the base URL.
...
<TypeTable
  type={{
    useProviderDomain: {
        description: "Use the provider domain to build callback URLs instead of the base URL. When enabled, authorization URLs, callback URIs, and token requests will be resolved against the provider's domain.",
        type: "boolean",
        default: false,
    },
    provisionUser: {
