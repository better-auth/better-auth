...
Better Auth keeps the durable SSO Identity separate from the provider configuration used to authenticate it. Persisted, dynamically registered providers scope both the Identity and Account to their immutable provider instance, so one tenant-managed provider cannot claim an Identity created through another provider. Only OIDC providers declared by the application in `defaultSSO` may share an Identity through the same verified issuer and `sub` pair.

An SSO Account keeps the provider's public alias in `providerId`. Account APIs such as `listAccounts` return that alias, so it can be matched to the provider used for sign-in. The server-only `providerInstanceId` identifies the exact configuration: persisted providers use `sso:provider:<recordId>`, while providers declared in `defaultSSO` use `sso:config:<providerId>`. Authentication state binds the flow to that exact provider instance, so another provider record cannot finish the flow or receive its credentials.

Deleting a persisted provider deletes only the Accounts attached to that provider instance. Better Auth retains the Identity record so provider deletion does not discard identity ownership. The User and records associated with other provider instances remain unchanged. Both OIDC and SAML use the provider-instance Identity namespace for persisted providers; OIDC uses the verified `sub` as its subject, while SAML uses the signed `NameID`.

<Callout type="warning">
  Dynamically registered providers require a database adapter with native transaction support. Better Auth coordinates provider updates and deletion with in-flight authentication inside one transaction; a sequential fallback or `commitAtomicWrites` implementation alone cannot protect that boundary. Providers declared in `defaultSSO` do not have this requirement because their configuration is immutable at runtime.
</Callout>

Provider IDs are used as callback-path segments. They must be 1 to 128 characters and contain only letters, numbers, `.`, `_`, `~`, or `-`; `.` and `..` are not valid IDs. Domains may be comma-separated. Better Auth normalizes them to lowercase hostnames, removes duplicates, and requires every entry to resolve to a hostname, so one unusable entry rejects the whole provider. These rules apply to registration, updates, and `defaultSSO`; invalid `defaultSSO` configuration fails during startup.

An in-progress OIDC or SAML flow is bound to the provider configuration that started it. If a dynamically registered provider changes or is deleted before the callback completes, Better Auth rejects the callback and the user must start sign-in again.
...
Better Auth uses the verified `sub` claim as the OIDC Identity subject. Persisted providers pair it with their immutable provider-instance namespace, even when 2 provider records use the same upstream issuer. Server-configured providers in `defaultSSO` can instead share an Identity when their verified ID tokens contain the same issuer and subject. The `mapping` option maps profile fields such as email, name, and image, but it cannot replace the Identity subject or namespace.
...
Better Auth identifies a SAML Identity within its provider instance by the signed `NameID`. The `issuer` field identifies your service provider, while the IdP entity ID comes from `idpMetadata.metadata` or an explicit `idpMetadata.entityID` in a manual configuration. Attribute mappings can populate profile fields, but they cannot replace the signed `NameID` identity subject.
...
The SSO plugin validates SAML request correlation and rejects unsolicited responses by default.

### AuthnRequest / InResponseTo Validation

InResponseTo validation is enabled by default for service provider (SP)-initiated SAML flows. The plugin tracks AuthnRequest IDs and validates the `InResponseTo` attribute in SAML responses. This prevents:
...
#### Configuring Validation (Single Instance)

For a single-instance deployment, you can keep the defaults or set them explicitly alongside a custom request lifetime:
    ...
    plugins: [
        sso({
            saml: {
                ...
                // IdP-initiated SSO is rejected by default
...
| `allowIdpInitiated`            | `boolean` | `false`          | Allow IdP-initiated SSO responses that do not contain `InResponseTo`. Only applies when validation is enabled. |
...
<Callout type="info">
  Better Auth registers both the shared endpoint (`/sso/callback`) and the per-provider endpoints (`/sso/callback/:providerId`) regardless of this setting.
</Callout>
...
When a provider's domain is verified, it is also trusted for **automatic account linking**. If a User signs in through an OIDC or SAML provider and a Better Auth User with the same email exists, Better Auth attaches the new SSO Identity and Account to that User only when the existing User's local email is verified and the email domain matches the provider's verified domain.
...
The SAML ACS endpoint (`/api/auth/sso/saml2/sp/acs/{providerId}`) always handles **SP-initiated** SSO and handles **IdP-initiated** SSO when `saml.allowIdpInitiated` is `true`:
...
Better Auth supports **IdP-initiated SSO flows** when you explicitly set `saml.allowIdpInitiated` to `true`. In this flow, users access your application from their identity provider (IdP) dashboard, which is common when IT administrators manage application access centrally.
...
After you enable the flow, the callback route handles both GET and POST requests.
...
export const ssoOptionsType = {
	...
	saml: {
		...
		properties: {
			...
			allowIdpInitiated: {
				description: "Allow IdP-initiated SSO responses that do not contain InResponseTo. Only applies when validation is enabled.",
				type: "boolean",
				default: false,
			},
			requestTTL: {
