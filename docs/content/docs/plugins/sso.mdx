...
<Callout type="info">
This is a stronger trust mechanism than `trustEmailVerified` (deprecated) because it requires proof of domain ownership rather than relying on the provider's `email_verified` claim.
</Callout>
