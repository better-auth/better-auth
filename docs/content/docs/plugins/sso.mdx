...
You can also attach arbitrary key/value data to a sign-in request with `additionalData`. The data is carried through the auth flow and delivered to the [`provisionUser`](#user-provisioning) callback on the server. This is useful for things like referral codes, source attribution, or tenant context that originate on the client:

```ts title="sign-in-with-additional-data.ts"
import { authClient } from "@/lib/auth-client"
...
type signInSSO = {
    ...
    requestSignUp?: boolean = true
     /**
     * Arbitrary key/value data to carry through the auth flow and deliver to the `provisionUser` callback.
     */
...
- Updating user information from additional state data sent from client
...
- **additionalData**: The `additionalData` object passed by the client at sign-in time (see [Sign In with SSO](#sign-in-with-sso))

### Accessing additionalData in Hooks

You can also access `additionalData` and other SSO state from server-side hooks using `getSSOState`. This works for both OIDC and SAML flows:

```ts title="auth.ts"
import { betterAuth } from "better-auth";
import { sso, getSSOState } from "@better-auth/sso";
...
const auth = betterAuth({
    ...
    hooks: {
        after: createAuthMiddleware(async (ctx) => {
            ...
            if (ssoState?.additionalData?.referralCode) {
                // handle referral
            }
        }),
    },
...
`getSSOState` is a request-scoped accessor — it returns `undefined` outside of an SSO callback request.
