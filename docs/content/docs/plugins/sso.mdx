...
<DatabaseTable
    fields={[
        ...
        { name: "useHttps", type: "boolean", description: "Whether to use HTTPS for the provider domain (defaults to true)", isRequired: false },
...
- **`useHttps`** (optional): Explicitly control whether to use HTTPS or HTTP for the provider domain. Defaults to `true` (HTTPS) if not specified.
