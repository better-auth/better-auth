...
Provider IDs are used as callback-path segments. They must be 1 to 128 characters and contain only letters, numbers, `.`, `_`, `~`, or `-`; `.` and `..` are not valid IDs. Domains may be comma-separated. Better Auth normalizes them to lowercase hostnames, removes duplicates, and rejects a provider with no valid domain. These rules apply to registration, updates, and `defaultSSO`; invalid `defaultSSO` configuration fails during startup.
