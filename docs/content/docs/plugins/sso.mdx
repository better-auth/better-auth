...
<Callout type="warn">
**Important**: SSO account linking is stricter than regular OAuth linking. Even if `linkingPolicy` is set to `"email_match_any"` in your core config, SSO providers **still require a trust signal** before auto-linking to existing accounts.
</Callout>
