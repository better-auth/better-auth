...
The mapper runs before Better Auth writes the session cookie. It assigns roles to new provider-linked members and synchronizes existing provider-linked members on every SSO login, so manual role changes may be overwritten. Set `syncRoleOnLogin: false` to map only new memberships. Domain-based organization assignment does not call `mapClaimsToRoles` because it has no SSO claims.
...
export const ssoOptionsType = {
	...
	organizationProvisioning: {
		...
		properties: {
			...
			syncRoleOnLogin: {
				description: "Synchronize existing provider-linked member roles during SSO login without removing creator roles. Defaults to true when mapClaimsToRoles is configured and false otherwise.",
				type: "boolean",
			},
		},
	},
