...
Use `mapClaimsToRoles` when the identity provider carries the authoritative organization role. For OIDC providers, `claims` contains the userinfo response when a userinfo endpoint is configured; otherwise, it contains the verified ID token payload. For SAML providers, `claims` contains the assertion attributes.
            ...
            organizationProvisioning: {
                ...
                mapClaimsToRoles: ({ claims }) => {
                    ...
                    return Array.isArray(groups) &&
                        groups.includes("engineering-admins")
                        ? "admin"
                        : "member";
                },
            },
...
The mapper runs before Better Auth writes the session cookie. It assigns roles to new provider-linked members and synchronizes existing provider-linked members on every SSO login, so manual role changes may be overwritten. Set `syncRoleOnLogin: false` to map only new memberships. Synchronization requires `mapClaimsToRoles` or `getRole`; `defaultRole` remains a default for new memberships even when `syncRoleOnLogin` is `true`. Domain-based organization assignment does not call `mapClaimsToRoles` because it has no SSO claims.

Automatic SSO synchronization never removes an organization's creator role. Better Auth skips the update and logs a warning with the organization, member, and provider IDs; remove the creator role through the organization member-role API when that change is intentional.

Errors from `mapClaimsToRoles` stop the SSO callback, and Better Auth does not write a session cookie. Handle recoverable errors inside the mapper when sign-in should continue.
...
export const ssoOptionsType = {
	...
	organizationProvisioning: {
		...
		properties: {
			...
			defaultRole: {
				description: "The default role for new organization members.",
				type: "string",
				default: "member",
			},
			getRole: {
				description: "A custom function to determine the role for new organization members. Set syncRoleOnLogin to true to use it for existing provider-linked members.",
				type: "function",
			},
			mapClaimsToRoles: {
				description: "Maps OIDC claims or SAML attributes to an organization role. Existing provider-linked members are synchronized by default.",
				type: "function",
			},
			syncRoleOnLogin: {
				description: "Synchronize existing provider-linked member roles through mapClaimsToRoles or getRole during SSO login without removing creator roles. Has no effect without a role resolver. Defaults to true when mapClaimsToRoles is configured and false otherwise.",
				type: "boolean",
			},
		},
	},
