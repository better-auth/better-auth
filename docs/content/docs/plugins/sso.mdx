...
Better Auth identifies an OIDC account by the provider configuration and verified `sub` claim by default. With `account.identityStrategy: "issuer"`, it uses the configured `issuer` and verified `sub` instead. The `mapping` option maps profile fields such as email, name, and image, but it cannot replace the account subject. Under issuer-scoped identity, provider aliases that return the same issuer and subject resolve to one external identity; this deduplication does not create independent grant or provider lifecycle records for each alias.
    ...
    plugins: [
        sso({
            resolveUser: async (input, context) => {
                ...
                const accountIdentity = "issuer" in input.accountKey
                    ? {
                        issuer: input.accountKey.issuer,
                        subject: input.accountKey.accountId,
                    }
                    : {
                        providerId: input.accountKey.providerId,
                        subject: input.accountKey.accountId,
                    };
...
`accountKey` follows the configured account identity strategy. The default form contains `providerId` and `accountId`; the issuer-scoped form contains `issuer` and `accountId`. For OIDC, the issuer-scoped values come from the validated ID Token's exact `iss` and `sub` claims. For SAML, they come from the verified identity provider entity ID and signed `NameID`. `verifiedIdTokenClaims` contains the complete verified ID Token payload, while `providerClaims` and `providerAttributes` contain the protocol-accepted claims used for the profile. Subject and `NameID` matching is case-sensitive.
...
`findProvisionedUserByAccountKey` is application-owned in this example. It should resolve the exact provider-and-subject or issuer-and-subject tuple through the transaction-scoped `database` adapter. Do not fall back to matching an email address when the exact provisioned subject is absent.
