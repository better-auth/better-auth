...
## Account Linking for SSO

When a user signs in via SSO and an account with the same email already exists, Better Auth applies a trust check before auto-linking. This prevents account takeover attacks where a malicious SSO provider could claim any email address.

<Callout type="info">
**SSO Linking Policies**: The SSO plugin only supports `"trusted_providers_only"` and `"never"` linking policies. If your core config uses `"verified_email_trusted"`, SSO will automatically default to `"trusted_providers_only"`. This is intentional—enterprise SSO providers should prove domain ownership or be explicitly trusted before linking to existing accounts.
</Callout>

For SSO providers, auto-linking is allowed when **any** of the following conditions are met:

1. **Trusted Provider**: The provider is listed in `account.accountLinking.trustedProviders`
2. **Domain Verified**: The SSO provider's domain is verified AND the user's email matches that domain

If neither condition is met (or if `linkingPolicy` is set to `"never"`), the user will be redirected with `?error=account_not_linked`.
...
export const auth = betterAuth({
    account: {
        accountLinking: {
            enabled: true,
            // Trust specific SSO providers
            trustedProviders: ["my-enterprise-sso"],
        }
    },
    plugins: [
        sso({
            // Enable domain verification for trust via domain ownership
            domainVerification: {
                enabled: true
            }
        })
...
<Callout>
**Recommended approach**: Enable domain verification. Once a provider's domain is verified, users with matching email domains can be auto-linked securely without adding every provider to `trustedProviders`.
</Callout>

### Error Handling

When SSO sign-in fails due to account linking policy, the user is redirected to the callback URL with an error parameter:

- `?error=account_not_linked` — User exists but the provider isn't trusted for auto-linking
- `?error=account_not_found` — No user exists (when `disableImplicitSignUp` is enabled)

You can handle these errors in your callback page to show appropriate messages or offer manual account linking.
...
This means that auto-linking to existing accounts will be blocked until the domain ownership has been verified. Once verified, users whose email matches the provider's domain can be auto-linked securely.
