...
Upon SSO provider registration, a **verification token** will be issued to the provider (it will be returned as part of the response),
which you need to prove that you own it. To do this, you'll need to add a `TXT` record to your domain's DNS settings.
...
2.  **Create a new `TXT` record in your DNS provider's dashboard.**
    *   **Host:** `better-auth-token-{your-provider-id}`
...
<Tabs items={["client", "server"]}>
    <Tab value="client">
```ts title="domain-validation-request-client.ts"
const res = await authClient.sso.verifyDomain({
    providerId: "acme-corp",
});
```
    </Tab>
...
const res = await auth.api.verifyDomain({
    body: {
        providerId: "acme-corp",
    },
    headers,
...
<Tabs items={["client", "server"]}>
    <Tab value="client">
```ts title="create-domain-verification-token-client.ts"
const res = await authClient.sso.requestDomainVerification({
    providerId: "acme-corp"
})
```
    </Tab>
...
### Schema for domain verification

When the domain verification is enabled, the `ssoProvider` schema is extended as follows:
