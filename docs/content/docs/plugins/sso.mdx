...
For environments requiring stricter security, you can reduce the clock skew and require timestamps:

```ts title="auth.ts"
sso({
    saml: {
        clockSkew: 60 * 1000,      // 1 minute tolerance
        requireTimestamps: true,   // Reject assertions without timestamps
    },
})
```
