...
The SSO plugin includes assertion replay protection to prevent attackers from capturing and resubmitting valid SAML responses. When enabled (default), each SAML Assertion ID is tracked and rejected if reused.

<Callout type="info">
Replay protection is **enabled by default**. This is a critical security feature that prevents attackers from reusing intercepted SAML responses.
</Callout>
...
<Callout type="info">
Unlike InResponseTo validation, replay protection uses the database verification table by default, so it works correctly in multi-instance deployments without additional configuration.
</Callout>
    ...
    plugins: [
        sso({
            saml: {
                ...
                // Set to false only if you have a specific reason
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `enableReplayProtection` | `boolean` | `true` | Track assertion IDs to prevent replay attacks. Disable only if you have a specific requirement. |
...
<Callout type="warning">
Disabling replay protection (`enableReplayProtection: false`) is **not recommended** as it leaves your application vulnerable to replay attacks where attackers can reuse captured SAML responses.
</Callout>
    ...
    saml: {
        ...
        properties: {
            ...
            enableReplayProtection: {
                description: "Enable assertion replay protection. When enabled, each SAML Assertion ID is tracked and rejected if reused. This prevents replay attacks where attackers resubmit captured SAML responses.",
                type: "boolean",
                default: true,
            },
        },
