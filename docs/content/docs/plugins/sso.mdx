...
<Tabs items={["client", "server"]}>
    <Tab value="client">
```ts title="minimal-oidc-registration.ts"
import { authClient } from "@/lib/auth-client";

// Minimal OIDC configuration — endpoints discovered automatically
await authClient.sso.register({
    providerId: "okta",
...
| `discovery_incomplete` | Required fields (`issuer`, `authorization_endpoint`, `token_endpoint`, `jwks_uri`) are missing |
...
- Legacy providers missing required metadata are handled automatically at sign-in time
