...
<Callout>
  SAML support is coming soon. Upvote the feature request on our
  [GitHub](https://github.com/better-auth/better-auth/issues/96)
</Callout>
  ...
  schema={{
    ssoProvider: {
      fields: {
        ...
        providerId: { type: "string", required: true, isUnique: true },
        organizationId: {
          type: "string",
          required: false,
...
### `ssoProvider` table fields:

- `issuer`: The issuer identifier.
- `domain`: The domain of the provider.
- `oidcConfig`: The OIDC configuration.
- `userId`: The user id.
- `providerId`: The provider id. Used to identify a provider and to generate a redirect URL.
- `organizationId`: The organization Id. If provider is linked to an organization.
