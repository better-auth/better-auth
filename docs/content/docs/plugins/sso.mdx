...
Note: To encrypt the OIDC clientSecret at rest, enable encryptionEnabled in the SSO plugin configuration. When enabled, the clientSecret is encrypted with BETTER_AUTH_SECRET before storing and decrypted on read.

```ts title="server-setup.ts"
import { sso } from "@better-auth/sso";
import { betterAuth } from "better-auth";
...
Security note: When `encryptionEnabled` is set in the SSO plugin, the OIDC `clientSecret` is encrypted at rest using `BETTER_AUTH_SECRET` and decrypted on read. Ensure only trusted backends can call registration endpoints that return provider configuration. Do not enable this on databases that already contain plaintext client secrets without first migrating them to the encrypted format.

Quick migration example (one-time):

```ts
import { symmetricEncrypt } from "better-auth/crypto";

// Pseudocode: iterate ssoProvider rows, parse oidcConfig JSON,
// encrypt the plaintext clientSecret, and save it back.
const encrypted = await symmetricEncrypt({
  key: process.env.BETTER_AUTH_SECRET!,
  data: plainClientSecret,
});
// write `encrypted` to `oidcConfig.clientSecret` in your DB
```
...
**encryptionEnabled**: Encrypt the OIDC clientSecret at rest using BETTER_AUTH_SECRET. When enabled, API responses include the decrypted clientSecret; restrict access to registration endpoints accordingly. This cannot be turned on if existing client secrets are stored in plaintext; perform a one-time migration first.
...
<TypeTable
  type={{
    ...
    encryptionEnabled: {
        description: "Encrypt the OIDC clientSecret at rest using BETTER_AUTH_SECRET. API responses return decrypted clientSecret; restrict access to registration endpoints. This cannot be enabled if existing client secrets are stored in plaintext; migrate those records first.",
        type: "boolean",
        default: false,
    },
    defaultSSO: {
