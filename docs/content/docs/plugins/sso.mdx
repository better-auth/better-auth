...
Note: To control how the OIDC clientSecret is stored at rest, use the storeSecretAs option in the SSO plugin. Supported values are "plain" (default), "encrypted" (uses BETTER_AUTH_SECRET), or a custom { encrypt, decrypt } implementation.
...
export const auth = betterAuth({
  plugins: [
    sso({
      // Store OIDC clientSecret encrypted at rest (uses BETTER_AUTH_SECRET)
...
Security note: When storeSecretAs is set to "encrypted" in the SSO plugin, the OIDC clientSecret is encrypted at rest using BETTER_AUTH_SECRET and decrypted on read. Ensure only trusted backends can call registration endpoints that return provider configuration. Do not enable this on databases that already contain plaintext client secrets without first migrating them to the encrypted format.

Custom encryptor example:

```ts title="server-setup.ts (custom secret storage)"
import { sso } from "@better-auth/sso";
import { betterAuth } from "better-auth";
    ...
    sso({
      storeSecretAs: {
        encrypt: async (secret: string) => {
          return myCustomEncryptor(secret);
        },
        decrypt: async (stored: string) => {
          return myCustomDecryptor(secret);
        },
      },
      // ...other SSO options
    }),
...
**storeSecretAs**: How to store sensitive secrets such as the OIDC clientSecret. Supported values: plain (default), encrypted (uses BETTER_AUTH_SECRET), or a custom { encrypt, decrypt } implementation.
...
<TypeTable
  type={{
    ...
    storeSecretAs: {
        description: "How to store sensitive secrets such as the OIDC clientSecret. Supported values: plain (default), encrypted (uses BETTER_AUTH_SECRET), or a custom { encrypt, decrypt } implementation.",
        type: "string | object",
        default: "plain",
    },
    defaultSSO: {
