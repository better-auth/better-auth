...
<APIMethod path="/sso/register" method="POST" requireSession note="Minimal OIDC configuration — endpoints are discovered automatically from the issuer.">
  ...
  type registerSSOProvider = {
      ...
      oidcConfig: {
           ...
           * Required for client_secret_basic/client_secret_post.
           */
...
For `private_key_jwt`, configure `tokenEndpointAuthentication: "private_key_jwt"` and provide private key material at runtime. Database-backed providers should store a `privateKeyId` and resolve the key with the plugin's `resolvePrivateKey` option. `defaultSSO` providers can include `privateKey` directly because they are configured in code and are not stored in the database.
...
export const ssoOptionsType = {
	...
	resolvePrivateKey: {
		description: "Resolve private key material for OIDC providers that use private_key_jwt token endpoint authentication. The key is resolved during token exchange and is not stored in the database.",
		type: "function",
		required: false,
	},
	domainVerification: {
	...
	defaultSSO: {
		...
		items: {
			...
			properties: {
				...
				privateKey: {
					description: "Private key material for default OIDC providers that use private_key_jwt. This is only used for defaultSSO providers configured in code.",
					type: "object",
					required: false,
				},
			},
