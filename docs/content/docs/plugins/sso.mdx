...
Better Auth identifies an OIDC account by the provider ID and the verified `sub` claim. The `mapping` option maps profile fields such as email, name, and image, but it cannot replace the account subject.
...
Better Auth identifies a SAML account by the provider ID and the signed `NameID`. The `issuer` field identifies your service provider, while the IdP entity ID comes from `idpMetadata.metadata` or an explicit `idpMetadata.entityID` in a manual configuration. Attribute mappings can populate profile fields, but they cannot replace the signed `NameID` account subject.
...
## Encrypting client secrets at rest

By default, an OIDC provider's `clientSecret` is stored as plaintext in the `ssoProvider.oidcConfig` column. You can have Better Auth encrypt it at rest with the `storeSecretAs` option.
...
const auth = betterAuth({
    plugins: [
        sso({
            // "plain" (default) | "encrypted" | custom encryptor
            storeSecretAs: "encrypted", // [!code highlight]
        }),
    ],
});
```

* **`"plain"`** (default) — the client secret is stored as-is.
* **`"encrypted"`** — the client secret is encrypted with your `BETTER_AUTH_SECRET` using `symmetricEncrypt`. The secret is decrypted only when needed (sign-in, token exchange).
* **Custom encryptor** — bring your own KMS/HSM by providing `encrypt`/`decrypt` functions:

  ```ts
  sso({
      storeSecretAs: {
          encrypt: async (clientSecret) => myKms.encrypt(clientSecret),
          decrypt: async (ciphertext) => myKms.decrypt(ciphertext),
      },
  });
  ```

<Callout type="warn">
  Encryption is keyed on `BETTER_AUTH_SECRET` (or your custom encryptor). If that key changes, previously encrypted secrets can no longer be decrypted. Rotate deliberately.
</Callout>

### Migrating existing providers

Enabling `storeSecretAs` does not require a database migration — reads are backward compatible. A provider registered before encryption was enabled keeps its plaintext secret and is still read correctly, so plaintext and encrypted providers can coexist in the same table.

New and updated providers are encrypted automatically. Existing plaintext providers are converted to encrypted-at-rest in one of two ways:

1. **Lazily, on use.** The first time a provider is used for sign-in after encryption is enabled, its stored secret is re-encrypted in place. Over time every actively-used provider converges to encrypted-at-rest with no action required. (This best-effort re-encryption never blocks the sign-in flow.)
2. **Eagerly, via the migration endpoint.** To encrypt every existing provider immediately (e.g. for compliance), call `migrateSSOSecretEncryption`. It re-encrypts every provider the caller can manage that is still stored in plaintext and is idempotent.

   ```ts title="server.ts"
   const summary = await auth.api.migrateSSOSecretEncryption({
       headers, // an authenticated session
   });
   // => { total, migrated, alreadyEncrypted, skipped, failed }
   ```

   The endpoint requires `storeSecretAs` to be configured for encryption; otherwise it returns a `400`.
...
<Callout type="info">
  ...
  invited user must complete the existing invitation flow before Better Auth
...
provider. Better Auth uses the stored user's verified email and assigns an
...
<Callout type="info">
  With domain verification disabled, Better Auth does not create organization
