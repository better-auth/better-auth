...
When your IdP rotates its signing certificate, list both the old and the new PEM in `idpMetadata.cert` as an array. Better Auth accepts SAML responses signed by either, so users keep signing in while the rotation completes.
...
`idpMetadata.cert` takes a single PEM string or an array. A single string is just the no-rotation case.
