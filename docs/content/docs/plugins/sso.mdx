...
When your IdP rotates its signing certificate, list both the old and the new PEM in an array. Better Auth accepts SAML responses signed by either, so users keep signing in while the rotation completes.
...
Certificates can sit at the top level (`samlConfig.cert`) or inside `samlConfig.idpMetadata.cert`. Either form takes a single PEM or an array. If you set both, Better Auth uses the IdP metadata value and ignores the top-level one.

If your IdP publishes a metadata XML document instead, pass it as `idpMetadata.metadata` and leave `cert` unset. The certificates come from the document, and rotation happens when the IdP republishes it. Setting both metadata XML and an explicit cert fails at registration, since the explicit value would never be used.
