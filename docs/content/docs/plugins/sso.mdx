...
Discovery, token, userinfo, and JWKS endpoints must use public hosts. Public IdPs such as Okta, Google, Microsoft Entra, and Auth0 do not need `trustedOrigins` entries.

Add private, VPN-only, or local IdPs to `trustedOrigins`:

```ts
trustedOrigins: ["https://idp.internal.example.com"],
```

You can compute private IdP origins dynamically:
...
See [`trustedOrigins`](/docs/reference/security#trusted-origins) for the full option contract.

#### Redirecting OIDC endpoints

Better Auth does not follow redirects for server-side OIDC requests. Configure the final discovery, token, userinfo, and JWKS URLs.

If an endpoint returns `3xx`, registration or sign-in fails with `oidc_endpoint_redirect`.
...
| `discovery_private_host`        | The discovery URL or one of the discovered endpoints points to a private, link-local, or otherwise non-public host |
