...
const client = await db.create({
  ...
  data: {
    ...
    type: 'web',
    redirectUrls: 'https://financial.example.com/callback',
    jwks: JSON.stringify({
      keys: [{
        ...publicKey,
        kid: 'key-1',
...
const response = await fetch('https://your-domain.com/api/auth/oauth2/token', {
  ...
  body: new URLSearchParams({
    grant_type: 'authorization_code',
    code: authorizationCode,
    redirect_uri: 'https://financial.example.com/callback',
...
<Callout type="info">
The `private_key_jwt` method is recommended for financial-grade APIs and scenarios requiring enhanced security. It's used by providers like Okta and is part of the FAPI (Financial-grade API) security profile.
</Callout>
