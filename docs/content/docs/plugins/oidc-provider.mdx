...
This endpoint is only available if you have enabled dynamic client registration by setting the `allowDynamicClientRegistration` option to `true`.
You can configure the `canUserRegisterClient` option to control which users can register new clients.
...
#### Controlling Client Registration
You can control which users can register new clients by providing a `canUserRegisterClient` function. This function receives the user object and should return `true` if the user is allowed to register new clients, or `false` otherwise.

```ts title="auth.ts"
const auth = betterAuth({
    plugins: [oidcProvider({
        allowDynamicClientRegistration: true,
        canUserRegisterClient: (user) => {
            // Only allow users with the "admin" role to register new clients
