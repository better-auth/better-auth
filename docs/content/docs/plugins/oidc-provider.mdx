...
type registerOAuthApplication = {
     ...
     * Supports wildcard patterns in the host part (e.g., `https://*.example.com/callback`).
     */
...
<Callout type="info">
**Wildcard Redirect URIs**: You can use wildcard patterns in redirect URIs to match multiple subdomains. For example, `https://*.example.com/callback` will match `https://app.example.com/callback`, `https://api.example.com/callback`, etc. Wildcards are only supported in the host part of the URL (not in the path or protocol).
</Callout>
        ...
        trustedClients: [
            {
                ...
                redirectURLs: [
                    ...
                    "https://*.company.com/auth/callback" // Wildcard pattern for subdomains
                ],
                disabled: false,
