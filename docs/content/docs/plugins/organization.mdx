...
As of Better Auth v1.3 we support adding additional fields to the `organization`, `invitation`, `member` and `team` tables.

Take a look at this [example](#customizing-the-schema) to see how to add additional fields to the schema.

Upon adding additional fields, depending on which model it's for, will introduce new properties to the relevant endpoints.
For example, if you add a new field to the `organization` table,
the `createOrganization` endpoint will have a new properties to pass for the new field if required.
