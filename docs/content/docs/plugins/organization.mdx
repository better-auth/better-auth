...
<Callout type="warn">
  Role names cannot contain a comma. Members store their roles comma-joined, so a name like `owner,admin`
  would create a role that could never be assigned. Requests with such names are rejected with the
  `INVALID_ROLE_NAME` error code. The same rule applies when renaming a role.
</Callout>
...
The new name cannot contain a comma; such requests are rejected with the `INVALID_ROLE_NAME` error code.
