...
<Callout type="info">
  Auto-creation only works when `enableCustomResources` is `true`. The resource names must still pass validation (lowercase alphanumeric with underscores, not reserved names).
</Callout>
