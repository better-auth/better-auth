...
<Steps>
    ...
    const statement = { 
        ...organization.defaultStatements, // [!code highlight]
        project: ["create", "share", "update", "delete"],
    } as const;
    ...
    const admin = ac.newRole({
        ...
        ...organization.adminAc.statements, // [!code highlight]
    });
    ```
...
Teams allow you to group members within an organization. You can enable teams by passing `teams.enabled` to the organization plugin.

### Enabling Teams

#### Server Setup
To enable teams on the server, update your authentication configuration:
...
#### Client Setup
For client-side support, pass `teams.enabled` to the client plugin:
...
### Database Migration
Once teams are enabled, run the appropriate command to update your database schema.
...
### Creating a Team
Use the `createTeam` function to add a new team to an organization:
...
> **Note:** Once teams are enabled, `getFullOrganization` will return all teams within an organization.

### Updating a Team
Modify a team's details using `updateTeam`:
...
### Listing Teams
Retrieve all teams within an organization using `listTeams`:

```ts
await authClient.organization.listTeams()
```

To fetch teams for a specific organization, pass an `organizationId`:
