...
For personal invitations to work, provide the `sendInvitationEmail` function to the `better-auth` instance. This function is responsible for sending invitation emails to specific users.

<Callout type="info">
This function is only called for personal invitations. Public invitations don't send emails since they don't have a specific recipient.
</Callout>
...
To invite users to an organization, you can use the `invite` function provided by the client. You can create personal invitations (sent to specific emails) or public invitations (reusable links).

<APIMethod path="/organization/invite-member" method="POST" requireSession>
```ts
type createInvitation = {
    /**
     * The email address of the user to invite. Omit to create public invitations.  
     */
    ...
    /**
     * Comma-separated list of domains allowed to accept a public invitation.
     */
...
Public invitations allow anyone to join an organization without specifying a recipient email. They're reusable and ideal for open signups.

Create a public invitation by omitting the `email` field:
...
<Callout>
Public invitations remain `pending` after acceptance and can be used by multiple users. They cannot be rejected, only canceled.
</Callout>
...
<DatabaseTable
  fields={[
    ...
    },
    {
      name: "email",
      type: "string",
      description: "The email address of the user. Omit for public invitations",
      isOptional: true
    },
    {
      name: "inviterId",
