...
**globalOrganizationAccess**: `boolean` | `string[]` | `((data: { user, organizationId, session }) => Promise<boolean> | boolean)` - Allow certain users to manage all organizations without being a member. If `true`, users with `'admin'` role can update and delete any organization. If an array, users with any of those roles can manage all organizations. If a function, custom logic determines access. By default, it's `false`. Requires the [Admin plugin](/docs/plugins/admin) for role-based access.
