...
For personal invitations (those sent to specific email addresses) to work, we first need to provide `sendInvitationEmail` to the `better-auth` instance. This function is responsible for sending the invitation email to the user.

<Callout type="info">
**Note:** This function is only called for personal invitations. Public invitations don't trigger email notifications since they don't have a specific recipient.
</Callout>
...
To invite users to an organization, you can use the `inviteMember` function provided by the client. You can create two types of invitations:

1. **Personal invitations** - Sent to a specific email address
2. **Public invitations** - Reusable invitation links that anyone can use

<APIMethod path="/organization/invite-member" method="POST">
```ts
type createInvitation = {
    /**
     * The email address of the user to invite. Optional for public invitations.  
     */
     ...
     * Only applies to public invitations (when email is not provided).
     */
    domainWhitelist?: string = "example.com,company.org"
}
```
</APIMethod>
...
Public invitations are reusable invitation links that don't require a specific email address. They're perfect for:
- Open team signups
- Onboarding multiple users with a single link
- Self-service organization joining

#### Creating Public Invitations

To create a public invitation, simply omit the `email` field:

```ts
// Unrestricted public invitation - anyone can join
const publicInvite = await authClient.organization.inviteMember({
    organizationId: "org-id",
    role: "member"
})

// Domain-restricted public invitation
const restrictedInvite = await authClient.organization.inviteMember({
    organizationId: "org-id",
    role: "member",
    domainWhitelist: "company.com,partner.org"
})
```

#### Key Differences from Personal Invitations

1. **No email notifications** - Since there's no specific recipient, no email is sent when creating public invitations
2. **Reusable** - Public invitations remain `pending` after being accepted and can be used by multiple users
3. **Cannot be rejected** - Only personal invitations can be rejected. Public invitations can only be canceled by authorized members
4. **Domain restrictions** - You can limit who can use the invitation based on email domains

#### Domain Whitelisting

When creating a public invitation with `domainWhitelist`, only users whose email addresses match one of the specified domains can accept the invitation:

```ts
const domainRestrictedInvite = await authClient.organization.inviteMember({
    role: "member",
    domainWhitelist: "acme.com,partner.com" // Only @acme.com and @partner.com emails
})
```

#### Example: Building a Self-Service Signup Flow

Here's a complete example of implementing self-service organization joining:

```ts
// 1. Admin creates a public invitation link
async function createPublicInviteLink(organizationId: string) {
    const invite = await authClient.organization.inviteMember({
        organizationId,
        role: "member",
        domainWhitelist: "company.com" // Optional: restrict to company emails
    })
    
    // Generate a shareable link
    const inviteLink = `https://app.example.com/join?inviteId=${invite.data.id}`
    return inviteLink
}

// 2. New user visits the link and signs up
async function handleJoinOrganization(inviteId: string) {
    // First, ensure the user is logged in
    const session = await authClient.session.get()
    if (!session) {
        // Redirect to login/signup with return URL
        window.location.href = `/auth?return=/join?inviteId=${inviteId}`
        return
    }
    ...
    try {
        ...
        console.log("Successfully joined organization:", result.data.member)
        ...
        // Handle errors (e.g., domain not allowed, invitation expired)
...
Users can accept both personal and public invitations using the same method. The invitation link should include the invitation ID, which will be used to accept the invitation.
...
- **Personal invitations**: Status changes to `accepted` and cannot be reused
- **Public invitations**: Remain `pending` and can be accepted by multiple users
- **Domain validation**: For public invitations with `domainWhitelist`, the user's email domain is validated before acceptance
</Callout>
...
Users can reject personal invitations they have received. This method only works for personal invitations (those sent to a specific email address).
...
**Public invitations cannot be rejected** - Since public invitations are meant to be reusable and don't have a specific recipient, they cannot be rejected. Only the organization's authorized members can cancel public invitations.
</Callout>
...
<DatabaseTable
  fields={[
    ...
    },
    {
      name: "email",
      type: "string",
      description: "The email address of the user. Optional for public invitations",
      isOptional: true
    },
    {
      name: "inviterId",
    ...
    },
    {
      ...
      type: "string",
      description: "Comma-separated list of allowed email domains for public invitations",
      isOptional: true
    },
  ]}
/>
