...
// You can also check multiple resource permissions at the same time
const canCreateProjectAndCreateSale =
  await authClient.organization.checkRolePermission({
    permissions: {
      organization: ["delete"],
      member: ["delete"],
    },
...
<Callout type="warn">
  The `team` table is optional. Only required when teams is enabled.
</Callout>
...
- `metadata`: Additional metadata for the organization
- `createdAt`: The timestamp of when the organization was created
...
- `createdAt`: The timestamp of when the member was added to the organization
- `teamId`: The id of the team (only if `teams` is enabled)
...
- `expiresAt`: The timestamp of when the invitation expires
- `createdAt`: The timestamp of when the invitation was created
- `teamId`: The id of the team (only if `teams` is enabled)

### `session` table fields:

- `activeOrganizationId`: The id of the active organization

### `team` table fields: (optional)

- `name`: The name of the team
- `organizationId`: The id of the organization
- `createdAt`: The timestamp of when the team was created
- `updatedAt`: The timestamp of when the team was last updated
