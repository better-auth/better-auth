  ...
  plugins: [
    organization({
      organizationHooks: {
        ...
        // Before a member leaves an organization (self-removal)
        beforeLeaveOrganization: async ({ member, user, organization }) => {
          // Validate if member can leave (e.g., check for pending tasks)
          const pendingTasks = await getPendingTasks(user.id, organization.id);
          if (pendingTasks.length > 0) {
            throw new Error("Cannot leave: you have pending tasks");
          }

          // Cleanup user's resources before leaving
          await cleanupUserData(user.id, organization.id);
        },

        // After a member leaves an organization
        afterLeaveOrganization: async ({ member, user, organization }) => {
          // Send departure notification
          await sendEmail(user.email, `You left ${organization.name}`);
