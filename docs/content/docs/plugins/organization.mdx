...
**requireEmailVerificationOnInvitation**: `boolean` - Whether to require email verification before token-bearing recipient invitation calls (`acceptInvitation`, `rejectInvitation`, `getInvitation`). By default, it's `false`. Client-side `listUserInvitations` always requires a verified session email because it enumerates invitation IDs from the session email.
