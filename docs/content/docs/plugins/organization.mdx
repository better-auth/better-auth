...
```ts title="client.ts"
//revoke invitation
await client.organization.revokeInvitation({
    invitationId: "invitation-id"
})
...
Like with the core schema, the Organization plugin allows you to customize the table names and column names.

### Custom Table Names

You can configure this using the `schema` option when initializing the plugin:
...
export const auth = betterAuth({
    plugins: [
        organization({
            schema: {
                organization: {
                    modelName: "tenants", // Changes table name to "tenants"
                },
                member: {
                ...
                session: {
                    fields: {
                        activeOrganizationId: "active_tenant_id"
                    }
                }
            }
        })
...
All organization plugin tables and columns listed in the [Schema](#schema) section can be customized. The column `activeOrganizationId` in the `session` table can also be customized like in the example above.
...
<DatabaseTable
  fields={[
    ...
    { 
      name: "status", 
      type: "string", 
      description: "The status of the invitation. Enum with 4 possible options: 'pending', 'accepted', 'rejected', 'canceled'" 
    },
    { 
      name: "expiresAt", 
      type: "Date", 
...
The Organization plugin tracks the `activeOrganizationId` in the `Session` table. Make sure to add this column to the table.
...
<DatabaseTable
  fields={[
    { 
      ...
      type: "string", 
      description: "The active organization id for the session",
      isForeignKey: true
    },
  ]}
  />
...
To customize table and column names in the Organization plugin, you can use the schema option during the plugin's initialization. This feature allows you to map the default schema to your preferred table and column names:
