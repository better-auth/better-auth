...
By default, accepting an invitation requires the invitation ID from the email link and a logged-in session whose email matches the invitation. If you enable `requireEmailVerificationOnInvitation`, users must also verify their email address before they can accept invitations.
...
<Callout type="info">
  Like accepting and rejecting invitations, getting an invitation by ID requires
  email verification when the `requireEmailVerificationOnInvitation` option is
  enabled.
</Callout>
...
**requireEmailVerificationOnInvitation**: `boolean` - Whether to require email verification before recipient invitation calls that carry an invitation ID (`acceptInvitation`, `rejectInvitation`, `getInvitation`). When unset, Better Auth requires verification for database-generated or custom-generated invitation IDs and skips it for built-in opaque invitation IDs so emailed invitation links can remain the ownership signal. Client-side `listUserInvitations` always requires a verified session email because it enumerates invitation IDs from the session email.
