...
To invite multiple users to an organization at once, you can use the `inviteMembers` function provided by the client. This allows you to send invitations to multiple users in a single API call, with detailed success and failure reporting.

<APIMethod path="/organization/invite-members" method="POST" requireSession>
```ts
type createBulkInvitation = {
    /**
     * The organization ID to invite users to. Defaults to the active organization.  
     */
    organizationId?: string = "org-id"
    /**
    ...
    invitations: Array<{
      ...
      resend?: boolean = true
      /**
       * The team ID(s) to invite the user to. Can be a string or array of strings.
       */
      teamId?: string | string[] = "team-id"
    }>
}
```
...
<Callout>
  **Bulk Invitation Behavior:**
  
  - **Duplicate emails** within the request are automatically filtered out
  - **Invalid emails** are skipped and reported in the `failed` array
  - **Already members** are skipped and reported in the `failed` array
  - **Already invited** users are handled based on the `resend` flag:
    - If `resend: true`, the invitation expiration is updated and email is resent
    - If `resend: false` or not set, the invitation is skipped and reported in `failed`
  - If `cancelPendingInvitationsOnReInvite` is enabled, old invitations are canceled before creating new ones
  - **Role validation** is performed - users without permission to invite certain roles will have those invitations marked as failed
  - **Invitation limit** is checked against the total number of valid invitations
  - **Team member limits** are validated if teams are enabled
  - Each successful invitation triggers the `beforeCreateInvitation` and `afterCreateInvitation` hooks
  - Each invitation email is sent individually via `sendInvitationEmail`
</Callout>

**Example Usage:**

<Tabs items={["Client-side", "Server-side", "React Component"]}>
<Tab value="Client-side">
```ts
// Invite multiple users from the client
...
if (result.totalSuccessful > 0) {
  console.log(`✅ Successfully invited ${result.totalSuccessful} users`);
  result.success.forEach((invitation) => {
    console.log(`  - ${invitation.email} (${invitation.role})`);
  });
}

if (result.totalFailed > 0) {
  console.log(`❌ Failed to invite ${result.totalFailed} users`);
...
```tsx
// React component example with form handling
import { useState } from "react";
import { authClient } from "@/lib/auth-client";

export function BulkInviteForm({ organizationId }: { organizationId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authClient.organization.inviteMembers({
        organizationId,
      ...
      if (response.totalSuccessful > 0) {
        alert(`Successfully invited ${response.totalSuccessful} users!`);
      }
      
      if (response.totalFailed > 0) {
        console.warn("Some invitations failed:", response.failed);
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {result && (
        <div>
          <p>✅ Successfully invited: {result.totalSuccessful}</p>
          {result.totalFailed > 0 && (
            <div>
              <p>❌ Failed: {result.totalFailed}</p>
              <ul>
                {result.failed.map((failure: any, idx: number) => (
                  <li key={idx}>
...
<Callout type="warn">
	This will not include any dynamic roles as everything is ran synchronously on
	the client side. Please use the [hasPermission](#access-control-usage) APIs to
	include checks for any dynamic roles & permissions.
</Callout>
...
<Callout>
	This will require you to run migrations to add the new `organizationRole`
	table to the database.
</Callout>

<Callout type="warn">
	The `authClient.organization.checkRolePermission` function will not include
	any dynamic roles as everything is ran synchronously on the client side.
	Please use the [hasPermission](#access-control-usage) APIs to include checks
	for any dynamic roles.
</Callout>
...
type createOrgRole = {
/**
_ A unique name of the role to create.
_/
role: string = "my-unique-role"
/**
_ The permissions to assign to the role.
_/
permission?: Record<string, string[]> = permission,
/\*\*
_ The organization ID which the role will be created in. Defaults to the active organization.
_/
organizationId?: string = "organization-id"
}
