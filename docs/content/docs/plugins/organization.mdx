...
<APIMethod
  ...
  method="POST"
  note={<>You can choose between <code>organizationId</code> or <code>organizationSlug</code>.</>}
  requireSession
>
  ```ts
...
<APIMethod
  ...
  message="You can choose to provide an organization ID, or an organization slug. If no properties are provided, it will default to the active organization."
  requireSession
  >
    ```ts
    type getFullOrganization = {
        /**
        * The id of the organization. By default, it will use the active organization.
        */
