...
By default, accepting an invitation requires the invitation ID from the email link and a logged-in session whose email matches the invitation. When Better Auth generates built-in opaque invitation IDs, that is enough for the normal emailed-invitation flow. If invitation IDs are database-generated or custom-generated, Better Auth also requires verified email unless you explicitly set `requireEmailVerificationOnInvitation` to `false`. Set the option to `true` to always require verified email before accepting invitations.
...
**requireEmailVerificationOnInvitation**: `boolean | undefined` - Whether to require email verification before recipient invitation calls that carry an invitation ID (`acceptInvitation`, `rejectInvitation`, `getInvitation`). When unset, Better Auth requires verification for database-generated or custom-generated invitation IDs and skips it for built-in opaque invitation IDs so emailed invitation links can remain the ownership signal. Client-side `listUserInvitations` always requires a verified session email because it enumerates invitation IDs from the session email.
