...
To transfer ownership of an organization to another member, use `organization.transferOwnership`. It is callable by the current owner (or a member with permission to update other members), and swaps the `creatorRole` between the current owner and the target member atomically — the organization always has an owner, before and after.
...
<APIMethod path="/organization/transfer-ownership" method="POST" requireSession>
  ...
  type transferOwnership = {
      ...
      callbackURL?: string = "/dashboard"
      /**
       * The token to confirm the ownership transfer request. If provided, ownership is transferred immediately and the other fields are ignored.
       */
      token?: string = "token"
  }
  ```
...
owner's membership, and their permission, re-checks the target hasn't already become the
owner, consumes the single-use token, then performs the swap.
