...
[Flowglad](https://flowglad.com) is an open source payments and billing platform that provides a zero-webhook stateless SDK for managing feature gating and usage access based on customer billing state. The Better Auth plugin seamlessly integrates Flowglad with your authentication setup.

<Callout>
  This plugin is maintained by the Flowglad team. For support,
  please join our [Discord](https://app.flowglad.com/invite-discord). For bug fixes or feature requests, open an issue in our [Github](https://github.com/flowglad/flowglad/issues/new).
</Callout>
