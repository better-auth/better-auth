...
Expo stores cookies that match `cookieNamespace`. The default `better-auth-passkey` challenge cookie works without configuration. Set a matching namespace when you customize `webAuthnChallengeCookie`.
...
Configure the matching namespace in the Expo client:
...
export const authClient = createAuthClient({
    ...
    plugins: [
        expoClient({
            storage: SecureStore,
            cookieNamespace: "my-app" // Must match the namespace of your custom cookie name
        }),
        passkeyClient()
    ]
});
...
If you're using multiple authentication systems or custom cookie names, you can provide an array of namespaces:
...
<Callout type="warn">
  A mismatched namespace prevents Expo from storing the challenge cookie.
</Callout>
