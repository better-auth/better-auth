...
**attestation**: Attestation conveyance preference for passkey registration. Forwarded as `attestationType` to `generateRegistrationOptions` from `@simplewebauthn/server`. Set this to `"direct"` or `"enterprise"` when you need an attestation statement from the authenticator — for example, to verify a hardware-backed passkey against the FIDO Metadata Service. Mirrors the subset of the WebAuthn [`AttestationConveyancePreference`](https://www.w3.org/TR/webauthn-3/#enum-attestation-convey) IDL accepted by `@simplewebauthn/server` (`"indirect"` is not exposed because the underlying library does not accept it).

* `none`: The relying party is not interested in authenticator attestation. (Default)
* `direct`: The relying party wants the attestation statement generated by the authenticator. Required for FIDO MDS verification of hardware-backed passkeys.
* `enterprise`: The relying party wants an attestation statement that may include uniquely identifying information. Only honored by authenticators configured for enterprise attestation.
...
export const auth = betterAuth({
    plugins: [
        passkey({
            attestation: "direct", // request an attestation statement
        }),
    ],
});
```
