...
**attestation**: Attestation conveyance preference for passkey registration. Mirrors the WebAuthn [`AttestationConveyancePreference`](https://www.w3.org/TR/webauthn-3/#enum-attestation-convey) IDL and is forwarded to `generateRegistrationOptions`. Set this to `"direct"` or `"enterprise"` when you need an attestation statement from the authenticator — for example, to verify a hardware-backed passkey against the FIDO Metadata Service.

* `none`: The relying party is not interested in authenticator attestation. (Default)
* `indirect`: The relying party prefers an attestation statement, but allows the client to decide how to obtain it (e.g. an anonymization CA).
