...
It gives AI agents a standard way to discover your service, register themselves, request approval, and execute scoped capabilities using short-lived signed JWTs.

## Features

- Discovery document at `/.well-known/agent-configuration` (including `default_location` for execute)
