...
<Tabs items={["server", "client"]} defaultValue="server">
    ...
    **Available options:**
    ...
    - **excludeEndpoints?**: `string[]` — Endpoints that should not be prefixed with the alias.
...
The compat plugin wraps another client plugin to preserve interoperability
...
To use the compat plugin

```ts title="auth-client.ts"
import { createAuthClient } from "better-auth/client";
import { aliasClient, aliasCompatClient } from "better-auth/client/plugins";

const aliasedPlugin = aliasClient("/v1", somePlugin()); // [!code highlight]

export const authClient = createAuthClient({
    plugins: [
        aliasedPlugin,
        // Always pass the compat plugin after the aliased plugin
        aliasCompatClient(aliasedPlugin, otherPlugin()) // [!code highlight]
...
1. **Prefixing specific endpoint paths** - Only included endpoints are prefixed to match the aliased plugin
2. **Modifying specific atoms** - Atom keys are adjusted to match aliased signals
