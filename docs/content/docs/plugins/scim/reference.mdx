...
| `listSCIMManagedConnectionEvents`   | `connectionId`, `provisioningDomainId`, optional `limit`, optional `offset`, optional `sortDirection` | Returns connection and credential lifecycle events sorted by `sequence`. Defaults to `limit: 10`, `offset: 0`, sorted newest first (`sortDirection: "desc"`). |
