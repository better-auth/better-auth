...
| `listSCIMManagedConnectionEvents`   | `connectionId`, `provisioningDomainId`, optional `limit`, optional `offset`, optional `sortBy`, optional `sortDirection` | Returns connection and credential lifecycle events. Defaults to `limit: 10`, `offset: 0`, `sortBy: "sequence"`, sorted newest first (`sortDirection: "desc"`). |
