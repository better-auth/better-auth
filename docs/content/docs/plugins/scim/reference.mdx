...
| `projection`  | `SCIMProjection`                   | Optional. Maps Groups to roles and applies access state.           |
...
| `credentials`          | `readonly SCIMBearerCredentialOptions[]` | Required. Contains at least one bearer credential.                                                                                                                         |
...
`POST` and `PUT` bodies must include the matching core schema URN:

* User: `urn:ietf:params:scim:schemas:core:2.0:User`
* Group: `urn:ietf:params:scim:schemas:core:2.0:Group`

PATCH bodies must include `urn:ietf:params:scim:api:messages:2.0:PatchOp`. Extra schema URNs do not enable extensions.
...
* OAuth client credentials, Basic authentication, or mutual TLS
