...
If you are making your system OIDC compatible, you must set this parameter to disable the /token endpoint and the ability to obtain tokens via the `set-auth-jwt`.
...
NOTE: If you are also using the OIDC plugin, you must specify which algorithm is used in signing as well.
...
jwt({
  ...
  jwt: {
    sign: async (jwtPayload: JWTPayload) => {
      ...
      const encHeaders = Buffer.from(headers).toString('base64url')
      const encPayload = Buffer.from(payload).toString('base64url')
      const hash = createHash('sha256')
      const data = `${encHeaders}.${encPayload}`
