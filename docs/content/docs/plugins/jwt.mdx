...
<Callout type="info">
  ...
  If you enable `session.cookieCache.strategy = "jwt"`, the `session_data` cookie still uses HS256 with your Better Auth secret by default. To make that cookie-cache JWT verifiable with the JWT plugin's JWKS too, set `sessionCookieCache: true` on `jwt()`.
</Callout>
...
export const jwksTableFields = [
	...
	},
    {
		name: "alg",
		type: "string",
		description: "The JWS algorithm this web key signs with",
        isOptional: true,
	},
    {
		name: "crv",
		type: "string",
		description: "The elliptic curve this web key was generated on",
        isOptional: true,
	},
	{
		name: "createdAt",
