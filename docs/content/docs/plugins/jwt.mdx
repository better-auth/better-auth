...
The JWT plugin automatically adds the `set-auth-jwt` header to the `access-control-expose-headers` response header when calling `getSession`, allowing clients to read the JWT token from response headers in cross-origin scenarios.

<Callout type="warning">
**CORS `exposeHeaders` Configuration**: If you're using CORS middleware (such as Hono's `cors`, Express's `cors`, or similar) and need to configure the `exposeHeaders` option, you **must** include `set-auth-jwt` in the list. Otherwise, the JWT plugin's automatic header exposure will be overridden and clients won't be able to read the token.
...
app.use(
	...
	cors({
		...
		exposeHeaders: ["set-auth-jwt"], // Required for JWT plugin
		credentials: true,
	}),
);
```
...
app.use(
	cors({
		origin: "http://localhost:3001",
		credentials: true,
		exposedHeaders: ["set-auth-jwt"], // Required for JWT plugin
	}),
);
```

If you omit `exposeHeaders` from your CORS configuration, Better Auth will automatically handle exposing the `set-auth-jwt` header.
</Callout>
