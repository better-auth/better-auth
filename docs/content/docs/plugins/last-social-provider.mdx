...
```ts title="auth.ts"
const lastProvider = await authClient.lastUsedSocial(); // [!code highlight]
...
Used alongside the enabled social providers. The plugin will only track and return a last-used provider if its ID is present in either the enabled providers or this trusted list. (Default: `[]`)
...
const auth = betterAuth({
  plugins: [
    lastSocialProvider({
      trustedProviderIds: ["google", "github"] // [!code highlight]
    })
  ]
})
```
