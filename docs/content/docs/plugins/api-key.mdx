...
1. **Start with `"secondary-storage-with-fallback"`**:
   ```ts
   apiKey({
     storage: "secondary-storage-with-fallback",
   })
...
3. **Switch to `"secondary-storage"`** once all keys are migrated:
   ```ts
   apiKey({
     storage: "secondary-storage",
   })
...
#### Using Cache Mode for Performance
...
`storage` <span className="opacity-70">`"database" | "secondary-storage" | "secondary-storage-with-fallback" | "cache"`</span>
...
<Callout>
  When `storage` is set to `"secondary-storage"`, `"secondary-storage-with-fallback"`, or `"cache"`, you must configure `secondaryStorage` in your Better Auth options. API keys will be stored using key-value patterns:
...
Cache TTL in seconds for cache mode. If not provided, the API key's expiration date will be used to calculate TTL. Only applies when `storage` is set to `"cache"`.
