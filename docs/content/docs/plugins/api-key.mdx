...
<SchemaDemo plugin="api-key" focus="apikey" height="large" includeDefaultUser schema={{
  apikey: {
    fields: {
      name: {
        type: "string",
...
- `name`: The name of the API key.
- `start`: The starting characters of the API key.
- `prefix`: The prefix of the API key.
- `key`: The hashed API key itself.
- `userId`: The id of the user who created the API key.
- `refillInterval`: The interval to refill the key in milliseconds.
- `refillAmount`: The amount to refill the remaining count of the key.
- `lastRefillAt`: The date and time when the key was last refilled.
- `enabled`: Whether the API key is enabled.
- `rateLimitEnabled`: Whether the API key has rate limiting enabled.
- `rateLimitTimeWindow`: The time window in milliseconds for the rate limit.
- `rateLimitMax`: The maximum number of requests allowed within the `rateLimitTimeWindow`.
- `requestCount`: The number of requests made within the rate limit time window.
- `remaining`: The number of requests remaining.
- `lastRequest`: The date and time of the last request made to the key.
- `expiresAt`: The date and time when the key will expire.
- `createdAt`: The date and time the API key was created.
- `updatedAt`: The date and time the API key was updated.
- `permissions`: The permissions of the key.
- `metadata`: Any additional metadata you want to store with the key.
