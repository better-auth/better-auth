...
Check secondary storage first, then fallback to database if not found. Ideal for gradual migration from database to secondary storage.

**Read behavior:**
- Checks secondary storage first
- If not found, queries the database
- Does not automatically populate secondary storage on fallback

**Write behavior:**
- Writes to secondary storage only
...
Use secondary storage as a write-through cache. API keys are stored in both the database and cache, with cache serving as a fast lookup layer.

**Read behavior:**
- Checks cache first
- On cache miss, fetches from database and automatically populates cache
- Ensures frequently accessed keys stay in cache

**Write behavior:**
- Writes to both database and cache (write-through)
- Updates both on modifications

**Delete behavior:**
- Deletes from both database and cache
...
export const auth = betterAuth({
  ...
  plugins: [
    apiKey({
      storage: "database",
      cacheEnabled: true,
      cacheTTL: 3600, // Optional: Custom TTL in seconds
    }),
  ],
});
```
...
When using `"cache"` mode, you can configure a custom TTL for cache entries:
...
If `cacheTTL` is not provided, the API key's expiration date (`expiresAt`) will be used to calculate the TTL automatically.

### How Storage Works

All storage modes (except `"database"`) use the following key patterns in secondary storage:

- `api-key:${hashedKey}` - Primary lookup by hashed key (for validation)
- `api-key:by-id:${id}` - Lookup by ID (for get/update/delete operations)
- `api-key:by-user:${userId}` - User's API key list (for listing operations)

### Migration Guide

#### Migrating from Database to Secondary Storage
...
1. **Start with fallback enabled**:
   ...
   ```
   This allows existing keys in the database to be accessible while new keys are stored in secondary storage.

2. **Migrate existing keys** (optional): Write a migration script to copy existing keys from database to secondary storage.

3. **Disable fallback** once all keys are migrated:
   ```ts
   apiKey({
     storage: "secondary-storage",
     fallbackToDatabase: false, // or omit, false is default
   })
   ```

#### Using Cache Layer for Performance

If you want to keep database as the source of truth but improve performance:
...
When `storage` is `"secondary-storage"`, enable fallback to database if key is not found in secondary storage. Useful for gradual migration from database to secondary storage.

Default is `false`.

`cacheEnabled` <span className="opacity-70">`boolean`</span>

Use secondary storage as a write-through cache layer. When enabled:
...
<Callout>
  When `storage` is set to `"secondary-storage"` or `cacheEnabled` is `true`, you must configure `secondaryStorage` in your Better Auth options. API keys will be stored using key-value patterns:
...
`storageMethods` <span className="opacity-70">`{ get: (key: string) => Promise<unknown> | unknown; set: (key: string, value: string, ttl?: number) => Promise<void | null | unknown> | void; delete: (key: string) => Promise<void | null | string> | void; }`</span>
...
`cacheTTL` <span className="opacity-70">`number | null`</span>

Cache TTL in seconds when using `cacheEnabled`. If not provided, the API key's expiration date will be used to calculate TTL. Only applies when `cacheEnabled` is `true`.
