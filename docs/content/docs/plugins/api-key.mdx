...
The API Key plugin provides lifecycle hooks that allow you to intercept and customize the behavior of create, update, and delete operations.
      ...
      hooks: {
        create: {
          ...
          after: async (apiKey) => {
            ...
            await logApiKeyCreation(apiKey);
          },
        },
      },
    }),
...
**`before`** - Called before an API key is created.
- Return `false` to prevent the creation
- Return `{ data: modifiedApiKey }` to use modified data instead of the original
- Return `void` to proceed normally with the original data

**`after`** - Called after an API key is successfully created. Receives the created API key.
      ...
      hooks: {
        update: {
          before: async (apiKey, ctx) => {
            ...
            if (apiKey.name && apiKey.name.startsWith("some_prefix")) {
              return false;
            }
          },
          after: async (updatedApiKey, ctx) => {
            await notifyApiKeyUpdate(updatedApiKey);
          },
        },
      },
    }),
...
**`before`** - Called before an API key is updated.
- Return `false` to prevent the update
- Return `{ data: modifiedData }` to use modified data instead of the original
- Return `void` to proceed normally with the original data

**`after`** - Called after an API key is successfully updated. Receives the updated API key data.
    ...
    apiKey({
      hooks: {
        delete: {
          ...
          after: async (apiKey, ctx) => {
            await cleanupRelatedResources(apiKey.id);
          },
        },
      },
    }),
...
**`before`** - Called before an API key is deleted.
- Return `false` to prevent the deletion
- Return `void` to proceed normally

**`after`** - Called after an API key is successfully deleted. Receives the original API key data.
