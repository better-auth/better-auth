    ...
    <Tab value="Server">
      ...
      const apiKey = await auth.api.createApiKey({
          body: {
            ...
            userId: user.id, // the user id. Helpful if you do not have the user headers to create this key. Otherwise, do not define this property.
          },
          headers: user_headers, // if you want to create an API key for a user, but do not have the headers, you can just pass `userId` in the body.
      });
      ```
    </Tab>
</Tabs>
...
All properties are optonal. However if you pass a `refillAmount`, you must also pass a `refillInterval`, and vice versa.
...
The result is the `ApiKey` object which includes the `key` value for you to use.
...
const { valid, error, key } = await auth.api.verifyApiKey({
  ...
  headers: user_headers, // the headers of the user who owns the API key
});
```
...
You'll recieve everything about the API key details, except for the `key` value itself.
...
const updatedApiKey = await auth.api.updateApiKey({
  body: {
    keyId: "your_api_key_id_here", // Required. Everything else is optional.
    userId: user.id,
  ...
  headers: user_headers, // if `userId` was provided, than this can be not defined
});
```
...
- `keyId`: The API key Id to update on.
  <DividerText>Optional values to update</DividerText>
...
Otherwise, you'll recieve the API Key details, except for the `key` value itself.

---

### Delete an API Key

<Endpoint method="DELETE" path="/api-key/delete" />
<Tabs items={['Client', 'Server']}>
    <Tab value="Client">
      ```ts
...
However, if the `refillInterval` is set, then the API key will be refilled automatically.  
This means that the `refillAmount` will be used to refill the API key.  
If the `refillAmount` is `null`, then the API key will be refilled with the `remaining` count.
