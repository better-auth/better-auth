...
The API Key plugin supports storing API keys in secondary storage (e.g., Redis) instead of the database adapter.
This can significantly improve performance for high-frequency API key lookups, especially in high-traffic applications.

### Benefits

- **Performance**: Faster lookups compared to database queries
- **Scalability**: Better suited for high-traffic scenarios
- **Automatic TTL**: Expired keys are automatically cleaned up when TTL is set
- **Reduced Database Load**: Offloads API key lookups from your primary database

### Configuration

To enable secondary storage for API keys, you need to:

1. Configure `secondaryStorage` in your Better Auth options (see [Secondary Storage](/docs/concepts/database#secondary-storage) for more details)
2. Enable `useSecondaryStorage` in the API Key plugin options
...
When `useSecondaryStorage` is enabled:

- API keys are stored in secondary storage using specific key patterns:
...
- **Automatic TTL**: If an API key has an expiration date (`expiresAt`), a TTL is automatically calculated and set in secondary storage. This ensures expired keys are automatically removed.

<Callout>
  When using secondary storage, ensure your secondary storage implementation supports TTL (time-to-live) for automatic expiration of keys. Most key-value stores like Redis, Memcached, and similar systems support this feature.
</Callout>

<Callout type="warn">
  If you enable `useSecondaryStorage` but don't configure `secondaryStorage` in your Better Auth options, API keys will fall back to using the database adapter.
</Callout>
...
`useSecondaryStorage` <span className="opacity-70">`boolean`</span>

Whether to store API keys in secondary storage instead of the database adapter. When enabled, API keys will be stored in the configured secondary storage (e.g., Redis). This can improve performance for high-frequency API key lookups. Default is `false`.

<Callout>
  When `useSecondaryStorage` is enabled, you must configure `secondaryStorage` in your Better Auth options. API keys will be stored using key-value patterns:
