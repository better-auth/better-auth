...
### Check permissions in API key

<Endpoint path="/api-key/has-permission" method="POST" isServerOnly />

```ts
const { success, missingPermissions } = await auth.api.hasPermissionApiKey({
  body: {
    key: "your_api_key_here",
    permissions: {
      projects: ["read", "read-write"]
...
type Result = {
  success: boolean;
  missingPermissions?: { [key: string]: string[] };
};
```
...
You can also check required permissions with `hasPermissionApiKey` function without the need of verifying an API key:

```ts
const result = await auth.api.hasPermissionApiKey({
  body: {
    key: "your_api_key_here",
    permissions: {
      files: ["write"]
...
if (result.success) {
  ...
  // API Key doesn't have the required permissions. You can access missingPermissions property in this instance
  const missingPerms = result.missingPermissions;
}
```
