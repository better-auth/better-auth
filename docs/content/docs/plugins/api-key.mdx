...
<Callout type="warn">
We recommend to utilize the [OAuth Provider Plugin](/docs/plugins/oauth-provider) with the `client_credentials` grant to prevent administering API Keys directly. For user-specific data, use the `client_credentials` grant with an optional `offline_access` scope to administer refresh tokens.
