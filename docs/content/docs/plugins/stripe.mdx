...
| `apiVersion` | `string`   | Optional API version override used to build the preview `Stripe-Version` request option. If omitted, the plugin falls back to request options `apiVersion`, then Stripe client API version. |
