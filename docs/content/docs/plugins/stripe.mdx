...
<Callout type="info">
When Managed Payments is enabled, the plugin does not force `customer_update` fields. If you use `getCheckoutSessionParams`, ensuring your custom Checkout parameters are compatible with Managed Payments is your responsibility.
</Callout>
...
| `apiVersion` | `string`   | Optional API version override used to build the preview `Stripe-Version` request option. |
