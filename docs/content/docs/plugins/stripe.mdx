...
This will create a Stripe billing portal session and return a URL that you can redirect users to. The billing portal allows customers to:
...
The plugin will automatically find the customer ID from either the user's `stripeCustomerId` field or from their active subscription records.

> **Note:** The user must have an existing Stripe customer ID (either directly on their user record or through a subscription) to create a billing portal session.
