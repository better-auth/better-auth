...
type createBillingPortal = {
    /**
    * The [IETF language tag of the locale](https://docs.stripe.com/js/appendix/supported_locales) customer portal is displayed in. If blank or auto, the customer's `preferred_locales` or browser's locale is used.
    */
    locale?: string
    /**
     * Reference id of the subscription to upgrade. 
