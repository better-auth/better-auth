...
<Callout type="info">
The plugin supports one active or trialing subscription per `referenceId + group` lane in plugin-managed flows. Plans without an explicit `group` use the implicit `"default"` group.

Subscriptions in different groups can coexist for the same reference ID. Inside the same group, an upgrade reuses the active subscription when there is a single match.

The implicit `"default"` group is only used when a specific group is being matched. Omitting `group` from list/cancel/restore does **not** mean `"default"`; it means the request is unscoped across all groups for that `referenceId`.

If multiple active or trialing subscriptions exist in the same group, you **must** provide `subscriptionId` when upgrading. Otherwise, the request is rejected as ambiguous.
</Callout>
...
type listActiveSubscriptions = {
    ...
    referenceId?: string = '123'
    /**
     * Optional subscription group to filter by.
     * Plans without a group use "default".
     */
...
When `group` is omitted, the endpoint returns active and trialing subscriptions across all groups for that `referenceId`.

To filter only the implicit default lane, pass `group: "default"`. This matches both subscriptions explicitly stored with `group = "default"` and legacy rows where `group` is missing.
...
type cancelSubscription = {
    ...
    /**
     * Optional subscription group to target when subscriptionId is not provided.
     * Plans without a group use "default".
     */
    group?: string = 'analytics'
    /**
...
If `subscriptionId` is omitted, the plugin selects the active or trialing subscription by `referenceId + group`. If `group` is omitted, the request is unscoped across all groups for that `referenceId`; if multiple groups are active, or if multiple subscriptions exist in the same group, the request is rejected and you must provide `subscriptionId`.

To target only the implicit default lane, pass `group: "default"`.
...
If `subscriptionId` is omitted, restore uses the same `referenceId + group` targeting rules as cancel.
...
The billing portal is customer-wide. It is not filtered to a specific subscription group.
<Callout type="info" >
...
<DatabaseTable
  fields={[
    ...
    },
    {
      name: "group",
      type: "string",
      description: "The subscription group. Plans without a configured group use the implicit 'default' group.",
      isOptional: true
    },
    { 
      name: "status", 
...
| `group`                   | `string`   | Defines the subscription lane for the plan. Plugin-managed flows allow one active or trialing subscription per `referenceId + group`. Omitted plans use `"default"`. |
