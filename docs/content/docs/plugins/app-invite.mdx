...
For app invites to work we first need to provide `sendInvitationEmail` to the `better-auth` instance. This function is responsible for sending the invitation email to the user.
...
- `resend`: A boolean value that determines whether to resend the invitation email, if the user is already invited
...
**allowUserToSendInvitation**: `boolean` | `((user: User) => Promise<boolean> | boolean)` - A function that determines whether a user can create invite others. By defaults it's `true`. You can set it to `false` to restrict users from creating invitations.
