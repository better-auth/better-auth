---
title: Openfort
description: Openfort plugin for Better Auth to manage crypto wallets and stablecoin interactions.
---

The Openfort plugin integrates [Openfort's](https://www.openfort.io/) wallet infrastructure with Better Auth. Since wallet creation and authentication are tightly coupled in applications, this plugin simplifies embedding blockchain wallets into your app — handling wallet provisioning, key management, transaction execution, and orchestration.

This plugin is maintained by the Openfort team. For bugs, issues, or feature requests, visit the [Openfort Adapters GitHub repo](https://github.com/openfort-xyz/openfort-adapters).
...
- Create embedded wallets automatically when users and AI agents sign up, with no private key exposure.
- Seamless Better Auth integration — wallets are tied directly to authenticated user and agent sessions.
- Multi-chain support across Ethereum (EVM-compatible networks) and Solana.
- Make gasless transactions via Openfort's policy engine.
- Multiple recovery methods — passkey, password, or automatic cloud backup.
- Sign messages and execute smart contracts directly from your application.
- Flexible wallet and permission assignment — associate wallets with users, organizations, or autonomous agents.
