...
Sign messages using wagmi hooks (requires wagmi setup):
...
Execute smart contract transactions using wagmi (requires wagmi setup):
...
Use wagmi's `useReadContract` hook to read data from smart contracts (requires wagmi setup):
