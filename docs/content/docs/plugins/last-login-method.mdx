    ...
    plugins: [
        lastLoginMethod({
            customResolveMethod: (ctx) => {
                ...
                if (ctx.path === "/auth/magic-link/verify") {
                    return "magic-link"
                }
                
                if (ctx.path === "/auth/phone/verify") {
                    return "phone"
                }
...
- ✅ Custom paths get your custom method names
- ✅ Standard paths (`/sign-in/email`, `/callback/google`, etc.) still work automatically  
- ✅ No need to reimplement built-in detection logic
