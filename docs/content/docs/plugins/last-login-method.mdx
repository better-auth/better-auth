...
**advanced.customGetMethod**: `() => string | null`
- Custom method to retrieve the last login method
- Returns the last login method

**advanced.customClearMethod**: `() => void`
- Custom method to clear the last login method

**advanced.customResolveMethod**: `(url: string | URL) => Awaitable<string | null>`
- Custom method for resolving the last login method (on client-side)
- Only applied when `advanced.onLastMethodRetrieved` is provided

**advanced.onLastMethodRetrieved**: `(method: string | null) => Awaitable<void>`
...
In non-browser environments like Expo, where there is no cookie storage, you can connect the client plugin to your own storage layer so the last login method can be persisted locally.
