...
```
<Callout type="info">
In Nuxt applications with SSR enabled, calling `getLastUsedLoginMethod()` during server rendering may cause hydration mismatches.  
To avoid layout shifts, read the value directly from the cookie using
`useCookie('better-auth.last_used_login_method')` (the default cookie name).
</Callout>
