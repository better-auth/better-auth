...
* Full literal cookie name. By default the cookie name composes `advanced.cookiePrefix` like other Better Auth cookies (e.g. `my-app.last_used_login_method`).
* To use a custom name, set this and the matching client-side `cookieName` to the same value.
* **Note**: By default this cookie is `httpOnly: false` so client-side JavaScript can read it for UI features. Overriding it to `true` via `advanced.cookies.last_used_login_method.attributes` breaks the client helper methods.
...
* Full literal cookie name to read. By default the cookie is resolved automatically regardless of `advanced.cookiePrefix`, so you only need to set this to match a custom server-side `cookieName`.
