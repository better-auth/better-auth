...
A custom function to generate email addresses for anonymous users. This allows you to define your own email format. The function can be synchronous or asynchronous, and receives the generated `name` of the anonymous user so you can derive the email from it.
...
export const auth = betterAuth({
    plugins: [
        anonymous({
            ...
            generateRandomEmail: ({ name }) => { // [!code highlight]
                const alias = name.replace(" ", "-").toLowerCase() // [!code highlight]
                const id = crypto.randomUUID() // [!code highlight]
                return `${alias}-${id}@example.com` // [!code highlight]
            } // [!code highlight]
        })
    ]
})
...
<Callout type="info">
  ...
  * The function receives `{ name }`, where `name` is the result of `generateName` (or `"Anonymous"` by default).
...
A callback function that is called to generate a name for the anonymous user. Useful if you want to have random names for anonymous users, or if `name` is unique in your database. The function can be synchronous or asynchronous and receives the request context. If not provided, the name defaults to `"Anonymous"`.
...
export const auth = betterAuth({
    plugins: [
        anonymous({
            generateName: () => { // [!code highlight]
                const id = crypto.randomUUID().slice(0, 8) // [!code highlight]
                return `Guest ${id}` // [!code highlight]
            } // [!code highlight]
        })
    ]
})
...
The generated name is also passed to [`generateRandomEmail`](#generaterandomemail), so you can derive the email address from it.
