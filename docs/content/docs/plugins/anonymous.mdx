...
- `emailDomainName`: The domain name to use when generating an email address for anonymous users. If not provided, a random domain will be generated.
...
export const auth = betterAuth({
    plugins: [
        anonymous({
            emailDomainName: "example.com" // [!code highlight]
        })
    ]
})
```

- `emailLocalPart`: The local part of the temporary email address. Defaults to "temp".
...
export const auth = betterAuth({
    plugins: [
        anonymous({
            emailLocalPart: "temp" // [!code highlight]
        })
    ]
})
```
...
<Callout type="info">
All temporary email settings are optional, but for cleaner addresses, it’s recommended to either use both or neither.
</Callout>
