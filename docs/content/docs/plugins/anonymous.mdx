...
If a user is already signed in anonymously and tries to `signIn` or `signUp` with another method, their anonymous activites can linked to the new account.
...
```ts title="example.ts"
const user = await client.signIn.email({
    email,
})
```
