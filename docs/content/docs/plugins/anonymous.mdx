...
To sign in a user anonymously, use the `signIn.anonymous()` method. This creates a new user with an auto-generated email and name, and establishes a session.
...
if (error) {
    // Handle error (e.g., already signed in as anonymous user)
    console.error(error.message)
} else {
    // User is now signed in anonymously
    console.log(data.user) // { id: "...", email: "temp-xxx@...", isAnonymous: true, ... }
}
```

<Callout type="info">
If you're already signed in as an anonymous user, calling `signIn.anonymous()` will return an error. You must first sign out or link to a real account.
</Callout>
