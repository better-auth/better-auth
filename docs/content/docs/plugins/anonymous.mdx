...
Controls what happens to the anonymous user when they sign in or sign up with another method.

- `"create"` (default): the credential creates its own user and the anonymous user is deleted afterwards. Use `onLinkAccount` to migrate data from the anonymous user to the new one.
- `"promote"`: when the incoming credential would create a brand-new user, the anonymous user is upgraded in place instead — their user id never changes. The anonymous row is updated with the credential's identity (email, verified status, name, image), `isAnonymous` is cleared, and the newly created account and session are re-pointed at the anonymous user, so existing sessions keep working and no data migration is needed.
...
<Callout type="info">
  ...
  already taken, or it's a returning social identity), in-place promotion is
  not possible. That case falls back to the classic flow: `onLinkAccount`
