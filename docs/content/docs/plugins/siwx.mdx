...
<Steps>
  ...
  <Step>
    ...
    import { siwx } from "@better-auth/siwx";
  ...
  <Step>
    ...
    import { siwxClient } from "@better-auth/siwx/client";
...
if (data) {
  ...
  console.log("Message:", data.message); // the canonical message to sign
  console.log("Statement:", data.statement);
  console.log("Expires:", data.expiresAt);
}
```

The server builds the canonical message (`data.message`) that the wallet must sign. Sign it verbatim, the client does not construct its own message.

### Verify and Sign In

Sign the server-issued `data.message` with the user's wallet, then submit that exact message with the signature. The server accepts only an exact match, so the issued nonce is authoritative:

```ts title="verify-siwx.ts"
// The `message` must be the exact string returned by `siwx.nonce` above.
// For EVM chains
const { data: nonce } = await authClient.siwx.nonce({
  address: "0x1234567890abcdef1234567890abcdef12345678",
  chainType: "evm",
});

const { data, error } = await authClient.siwx.verify({
  message: nonce.message, // the exact message returned by siwx.nonce
...
// For Solana
const { data: solanaNonce } = await authClient.siwx.nonce({
  address: "DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK",
  chainType: "solana",
});

const { data: solanaResult, error: solanaError } = await authClient.siwx.verify({
  message: solanaNonce.message, // the exact message returned by siwx.nonce
...
<Callout type="warning">
  The server builds the canonical message and its nonce, so the wallet must sign `data.message` verbatim and submit that exact string to `siwx.verify`. The server accepts only an exact match, which makes the issued nonce authoritative and prevents replaying a signature over any other message. Your `verifyMessage` only needs to confirm the signature is valid for the given `message` and `address`.
</Callout>
