...
<Callout type="warning">
  The signed `message` must contain the `nonce` returned by `siwx.nonce`. The server rejects any verify request whose message does not include the issued nonce, which prevents replaying a previously signed message. Sign the `nonce` directly (as in the examples above) or embed it in a structured message (for example a SIWE/CAIP-122 message).
</Callout>
