...
type sendVerificationOTP = {
    ...
    /**
     * Type of the OTP. `sign-in`, `email-verification`, or `request-password-reset`. 
     */
    type: "email-verification" | "sign-in" | "request-password-reset" = "sign-in"
}
```
</APIMethod>
...
To reset the user's password with OTP, use the `requestPasswordReset.emailOTP()` method to send a "request-password-reset" OTP to the user's email address.

<APIMethod path="/request-password-reset/email-otp" method="POST">
```ts
...
type checkVerificationOTP = {
     ...
     * Type of the OTP.
     */
    type: "request-password-reset" = "request-password-reset"
    /**
     * OTP sent to the email. 
     */
    otp: string = "123456"
...
- `sendVerificationOTP`: A function that sends the OTP to the user's email address. The function receives an object with the following properties:
  ...
  - `otp`: The OTP to send.
  - `type`: The type of OTP to send. Can be "sign-in", "email-verification", or "request-password-reset".
