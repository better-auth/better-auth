...
To override the default email verification, you can pass `overrideDefaultEmailVerification` as `true` in the options.
...
- `overrideDefaultEmailVerification`: A boolean value that determines whether to override the default email verification to use email otp instead. Defaults to `false`.
