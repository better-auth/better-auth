...
To reset the user's password with OTP, use the `requestPasswordReset.emailOtp()` method to send a "forget-password" OTP to the user's email address.
...
<Callout type="warn">
The `/forget-password/email-otp` endpoint and `forgetPassword.emailOtp()` client method are deprecated and will be removed in the next major release. Please migrate to `/request-password-reset/email-otp` and `requestPasswordReset.emailOtp()`.
</Callout>
