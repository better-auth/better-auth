...
If the address is already verified, a request over HTTP succeeds without sending a code unless it comes from the signed-in user who owns that address. Server-side calls made with neither a request nor headers are not subject to this check.
...
Before requesting the email change, use the `sendVerificationOtp()` method with type `email-verification` to send an OTP to the user's current email address. The user must be signed in for this request if their current address is already verified.
