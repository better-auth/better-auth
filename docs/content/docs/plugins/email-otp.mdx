...
If the address is already verified, the code is still consumed and the request returns the user with `token: null`. The account is not updated, `beforeEmailVerification` and `afterEmailVerification` do not run, and no session is created even when `autoSignInAfterVerification` is enabled, as with the core `/verify-email` endpoint.
