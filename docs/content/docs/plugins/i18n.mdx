...
The package ships with ready-to-use translations for 22 languages, so you don't need to write translations from scratch. Import them from `@better-auth/i18n/locales` to use them.
...
<Callout type="warn">
  Importing from the root locales entry point (`@better-auth/i18n/locales`) will bundle all 22 languages. To keep your bundle lean, import individual locales from their specific subpaths instead.
</Callout>

### Custom translations

Instead of using a built-in locale, you can provide your own translations directly as a plain object. Only the keys you provide will be overridden — everything else falls back to the default English messages:
