...
When an authenticated user verifies a wallet, the SIWE plugin automatically links the wallet to their existing account instead of creating a new user. This allows users who signed up via email/password or social login to add wallet authentication to their account.

### Configuration

Account linking is controlled by Better Auth's `accountLinking` configuration:
...
export const auth = betterAuth({
  account: {
    accountLinking: {
      ...
      trustedProviders: ["siwe"], // optional: explicitly trust SIWE
    },
  },
  plugins: [siwe({ /* ... */ })],
});
...
| User State        | `accountLinking` Config                              | Result                         |
| ----------------- | ---------------------------------------------------- | ------------------------------ |
| Not authenticated | Any                                                  | Creates new user               |
| Authenticated     | `enabled: true` (default)                            | Links wallet to existing user  |
| Authenticated     | `enabled: false`                                     | Creates new user               |
| Authenticated     | `trustedProviders: ["google"]` (siwe not included)   | Creates new user               |

### Security

- A wallet address cannot be linked to multiple users, even across different chains
- Attempting to link a wallet that belongs to another user returns a `WALLET_ALREADY_LINKED_TO_ANOTHER_USER` error
- Re-verifying an already-linked wallet is idempotent and returns success
