...
<Callout type="warning">
  A SIWE signature proves control of the wallet, not ownership of the `email` you pass. The plugin stores that email unverified and never treats it as proof of ownership. When `anonymous` is `false` and the supplied email already belongs to another account, verification is rejected with a 422 (`User already exists. Use another email.`) so a wallet sign-in cannot bind or collide with an email another account owns.
</Callout>
