...
<Steps>
    <Step>
        ...
        6.  **Generate the Client Secret (JWT):**
            ...
            or you can use our built in generator below
    ## Generate Apple Client Secret (JWT)

Provide your Apple Developer details below to generate the client secret JWT required for 'Sign in with Apple'.
...
*   **Client ID (Service ID):** The identifier (e.g., `com.yourdomain.app`) you configured for your App ID or Services ID.
*   **Key ID:** The 10-character ID generated when you created your `.p8` private key.
*   **Private Key Content:** The full text content of the `.p8` file you downloaded from Apple Developer. It must start with `-----BEGIN PRIVATE KEY-----` and end with `-----END PRIVATE KEY-----`.

The component uses the `jose` library to sign the JWT payload with your private key using the ES256 algorithm, following Apple's requirements. The generated JWT is valid for 180 days.
