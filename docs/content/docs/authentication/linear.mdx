...
<Steps>
    <Step> 
        ...
        <Callout type="info">
        OAuth2 applications created from October 1, 2025 onwards have refresh tokens enabled by default for user initiated OAuth with no option to disable them. Access tokens are valid for 24 hours. Older applications have access tokens that are valid for 10 years. This provider handles token rotation automatically.
        </Callout>
    </Step>
