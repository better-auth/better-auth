...
<Steps>
  ...
  <Step> 
    ...
    1. Search for `user:read:user` (View a user)
    1. Select `user:read:user` and click `Done`
  ...
  <Step>
    ...
    export const auth = betterAuth({
      socialProviders: {
        zoom: { // [!code highlight]
          clientId: process.env.ZOOM_CLIENT_ID, // [!code highlight]
          clientSecret: process.env.ZOOM_CLIENT_SECRET, // [!code highlight]
        }, // [!code highlight]
      },
    })
    ```
