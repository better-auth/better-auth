...
To check whether an account already holds a scope before triggering another flow, use the `hasGrantedScope` helper:

```ts title="auth.ts"
import { hasGrantedScope } from "better-auth/oauth2";

const accounts = await auth.api.listUserAccounts({ headers });
const google = accounts.find((account) => account.providerId === "google");

if (!hasGrantedScope(google?.grantedScopes, "https://www.googleapis.com/auth/drive.file")) {
