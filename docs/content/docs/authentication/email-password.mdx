...
<Callout type="info">
  `signUpEmail` accepts a password immediately and verifies the email afterward, as an opt-in step. If you'd rather verify the mailbox first and only then let the user set a password, use [enrollment](/docs/concepts/users-accounts#enrollment) instead.
</Callout>
