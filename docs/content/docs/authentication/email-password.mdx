...
- `password`: The password of the user. It should be at least 8 characters long and max 128 by default. You can configure additional complexity requirements like requiring uppercase letters, lowercase letters, numbers, and special characters.
...
**Password Complexity**: Better Auth allows you to enforce password complexity rules to ensure strong passwords. You can configure the following requirements:
...
Example configuration with password complexity rules:
...
export const auth = betterAuth({
  emailAndPassword: {
    ...
    specialCharacters: "!@#$%^&*()_+", // Custom set of special characters
  },
});
```

These validation rules are applied consistently across all authentication flows including sign-up, password reset, and password change.
...
<TypeTable
  type={{
    ...
    specialCharacters: {
      description: "The set of characters considered 'special' for password validation.",
      type: "string",
      default: "!@#$%^&*()_+-=[]{}|;:,.<>?",
    },
    sendResetPassword: {
