...
When you provide `getUserInfo`, you can return GitHub's raw [`GET /user` response](https://docs.github.com/en/rest/users/users#get-the-authenticated-user) as `data` without a type assertion. The numeric user ID and counters are numbers, nullable profile attributes can be `null`, and authenticated-only fields can be absent when GitHub returns the public profile shape.
