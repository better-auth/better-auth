...
- `openid` (required): Required for all requests
- `email` (default): Access user's email address
- `profile` (default): Access user's name and picture
...
export const auth = betterAuth({
    socialProviders: {
        railway: {
            ...
            scope: ["offline_access", "workspace:viewer"], // [!code highlight]
        },
    },
})
```
...
<Callout type="info">
    When requesting `offline_access` scope, Railway requires `prompt=consent` to be set. For more information about Railway's OAuth implementation, refer to the [official Railway documentation](https://docs.railway.com/reference/oauth/login-with-railway).
</Callout>
