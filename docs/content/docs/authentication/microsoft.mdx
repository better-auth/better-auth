...
* Update each row's account ID from the old `sub` value to the user's `oid` value.

When stored Microsoft ID tokens are available, decode each row's `idToken` and copy the token's `oid` claim into that row's account ID. If you override the account ID with `mapProfileToUser`, for example `${profile.tid}:${profile.oid}` for tenant-qualified identities, migrate existing rows to the exact same format.
