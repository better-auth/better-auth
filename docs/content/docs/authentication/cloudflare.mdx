...
Cloudflare's `/user` endpoint does not expose an email-verification status, so the account email is treated as unverified (`emailVerified: false`) by default. If you verify the email through another trusted mechanism, map it yourself with `mapProfileToUser`:
    ...
    socialProviders: {
        cloudflare: {
            ...
            mapProfileToUser: (profile) => ({
                emailVerified: true, // [!code highlight]
            }),
        },
    },
})
