...
Cloudflare OAuth clients are private by default, which limits authorization to members of the parent Cloudflare account. To let any Cloudflare user authorize your application, promote the client to public. Cloudflare requires a client name, logo, client URL, scopes, and DNS TXT verification for the client domain before promotion. Promotion is permanent, so review the client configuration first.

For a public client, configure `token_endpoint_auth_method` as `none`, omit `clientSecret`, and set `tokenEndpointAuthMethod` to `none`. Better Auth uses PKCE for the authorization code flow.
