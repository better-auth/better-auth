	...
	defaultSSO: [
		{
			...
			samlConfig: {
				...
				idpMetadata: {
					entityID:
						'https://trial-1076874.okta.com/app/exktofb0a62hqLAUL697/sso/saml/metadata',
					singleSignOnService: [
						{
...
<Callout type="info">
	**Explicit**: Pass providerId directly when signing in. **Domain fallback:**
	Matches based on the user’s email domain. e.g. user@company.com → matches
	`company-okta` provider.
</Callout>
