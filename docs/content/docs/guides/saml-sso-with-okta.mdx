...
<Callout type="info">
  **IdP-Initiated SSO**: If you want users to access your app from the Okta dashboard, point the **Single Sign-on URL** to `/api/auth/sso/saml2/sp/acs/{providerId}` and set `saml.allowIdpInitiated: true` in the SSO plugin. Better Auth rejects unsolicited SAML responses by default.
</Callout>
...
export const auth = betterAuth({
  plugins: [
    sso({
      saml: {
        allowIdpInitiated: true, // Omit unless users start sign-in from Okta
      },
      defaultSSO: [{
