...
In this guide, we'll walk through the steps to migrate a project from WorkOS to Better Auth — including email/password authentication, social/external accounts, phone number verification, two-factor authentication (2FA), and more.
...
Since WorkOS doesn't provide access to password hashes, Better Auth offers a secure password transition approach:

1. **Automated Password Reset Flow**
```typescript
// Configure password reset in your Better Auth setup
auth.init({
  emailAndPassword: {
    sendResetPassword: async ({ user, url }) => {
      await sendEmail({
        ...
        body: `Click here to set your new password: ${url}`
      });
    },
  }
});
...
The password reset flow is secure by default:
...
1. **Immediate Reset**: Send password reset emails to all users during migration
2. **Gradual Migration**: Trigger password reset on first login attempt
3. **OAuth-Only**: For users who only use OAuth, no password reset needed
``

**After Migration**
   - Monitor password reset completion rates
   - Provide support channels for users who need help
   - Consider implementing a grace period where both systems work
...
We've upgraded our authentication system to provide you with better security. To continue accessing your account, please:

1. Click the password reset link in the email you'll receive shortly
2. Set up your new password
3. Log in with your email and new password

If you typically use Google, GitHub, or other social login methods, you can continue using those without any changes.
...
- Password hashes cannot be directly migrated (requires password reset)
