...
<Steps>
  <Step>
    ...
    Enable [Email & Password authentication](/docs/authentication/email-password) as shown below. Since WorkOS verifies each user’s email by default, this setup is similar to the default behavior. You can adjust it if needed.
    ...
    You probably used metadata in WorkOS. To preserve that metadata and the user id from WorkOS (e.g., user\_01KBT4BMFF7ASGRDD0WZ6W63FF), [extend the `user` schema](/docs/concepts/database#extending-core-schema) as shown below. Better Auth provides a more flexible way to store user data.
    ```ts title="auth.ts"
    import { betterAuth } from "better-auth";
    import { Pool } from "pg";
    ...
    Refer to the [section](#before-we-begin) mapping WorkOS features to Better Auth. If a feature you used in WorkOS is available as a Better Auth plugin, add it to the plugin options. Better Auth provides a wider range of out-of-the-box features through [plugins](/docs/concepts/plugins).
  ...
  <Step>
    ...
    Better Auth allows you to control your own database, and you can easily generate the appropriate schema for your auth instance using the [CLI](/docs/concepts/cli).
  ...
  <Step>
    ...
    This [client instance](/docs/concepts/client) includes a set of functions for interacting with the Better Auth server instance.
