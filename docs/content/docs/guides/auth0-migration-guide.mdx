...
<Callout type="info">
Instead of using the Management API, you can use Auth0's bulk user export functionality and pass the exported JSON data directly to the `auth0Users` array. This is especially useful if you need to migrate password hashes, which are not available through the Management API. To get the password hashes and other password-related information, you must create an Auth0 support ticket.

> **Note:** Exporting password hashes is only available on Auth0's paid plans. On the free plan, password hashes cannot be exported and you need to ask for support ticket.  
> - [Auth0 Bulk User Export Documentation](https://auth0.com/docs/manage-users/user-migration/bulk-user-exports)
> - [Exporting Password Hashes](https://auth0.com/docs/troubleshoot/customer-support/manage-subscriptions/export-data#user-passwords)
...
const auth0Users = [
  {
    ...
    "name": "Hello world",
    // this will be available for enterprise users.
    ...
    try {
        ...
        while (true) {
            try {
                ...
                const response = await auth0Client.users.getAll(params);
...
### Custom Claims
If you have custom claims in Auth0, you'll need to modify the migration script to handle these claims and store them appropriately in Better Auth.
...
Congratulations! You've successfully migrated from Auth0 to Better Auth.
