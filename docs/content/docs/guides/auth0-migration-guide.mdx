...
<Steps>
  ...
  <Step>
    ...
    // Review every social provider in your Auth0 export. Use the exact trusted
    // issuer Better Auth uses, or local:oauth:<encoded providerId> when it has no issuer.
        ...
        for (const identity of auth0User.identities) {
            try {
                ...
                const issuer = providerId === "credential"
                    ? "local:credential"
                    : getIdentityIssuer(providerId);
                const providerAccountId = providerId === "credential"
                    ? userId
                    : identity.user_id;

                let linkedIdentity = await ctx.adapter.findOne<{
                    id: string;
                    userId: string;
                }>({
                    model: "identity",
                ...
                if (linkedIdentity && linkedIdentity.userId !== userId) {
                    throw new Error(`Identity ${issuer}/${providerAccountId} belongs to another user`);
                }

                linkedIdentity ??= await ctx.adapter.create<{
                    id: string;
                    userId: string;
                }>({
                    model: "identity",
                ...
                await ctx.adapter.create({
                    ...
                    data: {
                        ...
                        password: providerId === "credential"
                            ? await migratePassword(auth0User)
                            : null,
                        providerId,
