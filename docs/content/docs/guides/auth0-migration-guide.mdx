...
<Callout type="info">
Instead of using the Management API, you can use Auth0's bulk user export functionality and pass the exported JSON data directly to the auth0Users array. This is especially useful if you need to migrate password hashes, which are not available through the Management API. Example:
```ts
...
```bash
npm remove @auth0/auth0-react @auth0/auth0-spa-js
```
</Step>
</Steps>
