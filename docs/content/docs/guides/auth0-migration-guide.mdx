...
<Callout type="warn">
	This migration will invalidate all active sessions. This guide doesn't
	currently show you how to migrate Organizations but it should be possible with
	additional steps and the [Organization](/docs/plugins/organization) Plugin.
</Callout>
...
const auth0Users = [
	{
		...
		password_hash:
			'$2b$10$w4kfaZVjrcQ6ZOMiG.M8JeNvnVQkPKZV03pbDUHbxy9Ug0h/McDXi',
		// ... other user data
	},
];
```
...
async function migratePassword(auth0User: any) {
	if (auth0User.password_hash) {
		if (
			auth0User.password_hash.startsWith('$2a$') ||
			auth0User.password_hash.startsWith('$2b$')
		) {
...
async function migrateMFAFactors(
	auth0User: any,
	...
	if (
		!userId ||
		!auth0User.mfa_factors ||
		!Array.isArray(auth0User.mfa_factors)
	) {
		return;
	}
...
async function migrateOAuthAccounts(
	...
	if (
		...
		!Array.isArray(auth0User.identities)
	) {
		return;
	}
	...
	for (const identity of auth0User.identities) {
		try {
				...
				.catch((error: Error) => {
					...
					return ctx.adapter.create({
						...
						data: {
							...
							refreshTokenExpiresAt: identity.refresh_token_expires_in
								? new Date(
										Date.now() + identity.refresh_token_expires_in * 1000,
									)
								: undefined,
							scope: identity.scope,
							idToken: identity.id_token,
			...
			console.log(
				`Successfully migrated OAuth account for user ${userId} with provider ${providerId}`,
			);
		} catch (error) {
			console.error(
				`Failed to migrate OAuth account for user ${userId}:`,
				error,
			);
		}
	}
	...
	try {
		...
		for (const org of organizations.data || []) {
			try {
				...
				const members = await auth0Client.organizations.getMembers({
					id: org.id,
				});
				for (const member of members.data || []) {
					try {
						...
						const role = mapAuth0RoleToBetterAuthRole(
							userRoles.data?.map((r) => r.name) || [],
						);
						await ctx.adapter.create({
							model: 'member',
							data: {
				...
				console.log(
					`Successfully migrated organization: ${org.display_name || org.id}`,
				);
			} catch (error) {
				console.error(
					`Failed to migrate organization ${org.display_name || org.id}:`,
					error,
				);
			}
		}
...
async function migrateFromAuth0() {
	try {
		...
		const isOrganizationEnabled = ctx.options?.plugins?.find(
		...
		for (const auth0User of auth0Users) {
			try {
				...
				const isOAuthUser = auth0User.identities?.some(
...
<Callout type="warning">
	Important considerations: 1. Test the migration in a development environment
	first 2. Monitor the migration process for any errors 3. Verify the migrated
	data in Better Auth before proceeding 4. Keep Auth0 installed and configured
	until the migration is complete 5. The script handles bcrypt password hashes
	by default. For custom password hashing algorithms, you'll need to modify the
	`migratePassword` function
</Callout>
