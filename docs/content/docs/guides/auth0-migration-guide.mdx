...
<Steps>
  ...
  <Step>
    ...
    // Helper function to create recovery-code rows for method-based 2FA
    function createRecoveryCodes(methodId: string) {
        ...
        for (const factor of auth0User.mfa_factors) {
            try {
                if (factor.totp && factor.totp.secret) {
                    const totpMethodId = `${userId}:totp`;
                    ...
                    await ctx.adapter.create({
                        ...
                        data: {
                            ...
                            secret: await symmetricEncrypt({
                                data: factor.totp.secret,
                                key: ctx.secret,
                            }),
                        },
                    });

                    const recoveryMethodId = `${userId}:recovery`;
                    const { rows } = createRecoveryCodes(recoveryMethodId);
                    await ctx.adapter.create({
                        model: "twoFactorMethod",
                        data: {
                            id: recoveryMethodId,
            ...
            try {
                ...
                await ctx.adapter.create({
                    ...
                    data: {
                        ...
                        grantedScopes: identity.scope?.split(/[\s,]+/).filter(Boolean) ?? [],
