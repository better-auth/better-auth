        ...
        try {
            ...
            await ctx.adapter.create({
                ...
                data: {
                    ...
                    // Note: Full token sets (including refresh tokens) are only available for Auth0 Enterprise users
                    // Enterprise users can use: auth0Client.users.getAllTokensets() to get complete token information
