  ...
  <Step>
    ...
    <Tabs items={["Auth.js", "Better Auth"]}>
      ...
      <Tab value="Better Auth">
        ...
        const { data, error, refetch, isRefetching } = authClient.useSession()
        ```
      </Tab>
    </Tabs>
...
description: "Timestamp of when the verification request was updated",
    ...
    * Better Auth uses `id` for the local account record and the `providerId` plus `accountId` pair for the external identity
    ...
    * Better Auth includes `createdAt` and `updatedAt` timestamps
    ...
    <Callout type="warn">
      **Critical for Credentials Migration:**
      ...
      If you are migrating users with passwords (including for the Phone Number plugin), you **must** create a record in the `account` table for each user with `providerId: "credential"` and `accountId` set to the Better Auth user `id`. Without this record, password-based sign-ins will fail.
    </Callout>
