...
### Email Verified Column Migration (PostgreSQL/MySQL/SQLite/MSSQL)

If you're migrating from NextAuth v4 where `emailVerified` was a `timestamp` column and need to convert it to `boolean`, follow these steps:

Manually convert the column in your database before updating your schema. Connect to your PostgreSQL database and run:

```sql
-- Convert timestamp to boolean: true if emailVerified is not null, false otherwise
ALTER TABLE "user" 
  ALTER COLUMN "emailVerified" TYPE boolean 
  USING ("emailVerified" IS NOT NULL);
```

If you're using an ORM, you can map the `emailVerified` column to a boolean column.
