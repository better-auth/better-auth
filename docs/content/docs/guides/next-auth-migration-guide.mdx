...
description: "Timestamp of when the verification request was updated",
    ...
    * Better Auth includes `createdAt` and `updatedAt` timestamps
    ...
    Tables: <strong className="underline italic">Identity and Account</strong>

    * Better Auth uses camelCase naming (e.g. `refreshToken` vs `refresh_token`)
    * Better Auth stores `issuer`, `providerAccountId`, and `userId` on Identity, then links each provider Account through `identityId`
    * Identity is unique by `issuer` plus `providerAccountId`, while Account is unique by `identityId` plus `providerInstanceId`
    * Better Auth uses `providerId` as the public provider alias and `providerInstanceId` as the stable Account namespace
    ...
    * Better Auth includes `createdAt` and `updatedAt` timestamps
    ...
    <Callout type="warn">
      **Critical for Credentials Migration:**
      In Auth.js, passwords are often stored directly on the `User` model. In Better Auth, create an Identity with `issuer: "local:credential"`, `providerAccountId` set to the Better Auth user `id`, and `userId` set to the same user. Then create an Account with that Identity's `id`, `providerId: "credential"`, `providerInstanceId: "credential"`, and the password hash. Without both records, password-based sign-ins will fail.
    </Callout>
