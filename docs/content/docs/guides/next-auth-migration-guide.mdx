...
<Tabs items={["NextAuth.js", "Better Auth"]}>
<Tab value="NextAuth.js">
```ts 
...
<Callout type="info">
Now Better Auth supports stateless session management without any database. If you were using a Database adapter in NextAuth.js, you can refer to the [Database Adapters Section](#database-adapters) below to check the differences with Better Auth's core schema.
</Callout>
</Step>
...
In this section, we'll look at how to manage sessions in Better Auth compared to NextAuth.js. For more information, see [here](/docs/concepts/session-management).

### Client-side

#### Sign In

Here are the differences between NextAuth.js and Better Auth for signing in users. For example, with the GitHub OAuth provider:
...
Here are the differences between NextAuth.js and Better Auth when signing out users.
...
Here are the differences between NextAuth.js and Better Auth for getting the current active session.
...
<Tab value="Better Auth">
In Better Auth, sign-in is typically triggered using the Client Instance. If you need to perform any work on the server side before or after, you can use [hooks](/docs/concepts/hooks).
</Tab>
</Tabs>
...
<Tab value="Better Auth">
In Better Auth, sign-out is typically triggered using the Client Instance. If you need to perform any work on the server side before or after, you can use [hooks](/docs/concepts/hooks).
</Tab>
</Tabs>
...
NextAuth.js offers approaches using Proxy (Middleware), but we recommend handling auth checks on each page or route rather than in a Proxy or Layout. Here is a basic example of protecting resources with Better Auth.
...
This section compares the differences between NextAuth.js database adapters and Better Auth core schema. While this guide focuses on the core schema, you can customize the schema through the Better Auth instance. For more details, see [Concepts - Database](/docs/concepts/database).
