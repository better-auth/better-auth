...
<Step>
  ...
  <Step>
    ...
    An alternative to this step would be to implement a password reset flow to require users with credential logins to reset their password so the pasword is hashed using Better Auth's default `scrypt`
