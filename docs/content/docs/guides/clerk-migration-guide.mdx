  ...
  <Step>
    ...
    function createRecoveryCodes(methodId: string) {
      ...
      return {
          ...
          rows: recoveryCodes.map((code) => ({
              methodId,
              codeHash: createHash("sha256").update(code).digest("base64url"),
          })),
      };
    }
    ...
    async function migrateFromClerk() {
      ...
      for (const user of jsonData) {
          ...
          // two factor
          if (isTwoFactorEnabled && createdUser?.id && totp_secret) {
              const totpMethodId = `${createdUser.id}:totp`;
              await ctx.adapter.create({
              ...
              const recoveryMethodId = `${createdUser.id}:recovery`;
