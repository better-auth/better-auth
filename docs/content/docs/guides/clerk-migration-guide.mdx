...
<Callout type="warn">
	...
	currently show you how to migrate Organization but it should be possible with
					...
					...(isAdminEnabled
						? {
								...
								banExpires: clerkUser?.lockout_expires_in_seconds
									? new Date(
											Date.now() + clerkUser.lockout_expires_in_seconds * 1000,
										)
									: undefined,
								role: 'user',
							}
...
<Callout type="warning">
	Make sure to: 1. Test the migration in a development environment first 2.
	Monitor the migration process for any errors 3. Verify the migrated data in
	Better Auth before proceeding 4. Keep Clerk installed and configured until the
