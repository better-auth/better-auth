...
<Steps>
  ...
  <Step>
    ...
    // Better Auth uses, or local:oauth:<encoded providerId> when it has no issuer.
    const identityIssuers: Record<string, string> = {
      github: "local:oauth:github",
      google: "https://accounts.google.com",
    };

    function getIdentityIssuer(providerId: string) {
      const issuer = identityIssuers[providerId];
      if (!issuer) {
    ...
    async function migrateFromClerk() {
      ...
      async function findOrCreateIdentity(data: {
          userId: string;
          issuer: string;
          ...
          const existingIdentity = await ctx.adapter.findOne<{
              id: string;
              userId: string;
          }>({
              model: "identity",
              where: [
                  { field: "issuer", value: data.issuer },
                  { field: "providerAccountId", value: data.providerAccountId },
              ],
          });

          if (existingIdentity && existingIdentity.userId !== data.userId) {
              throw new Error(`Identity ${data.issuer}/${data.providerAccountId} belongs to another user`);
          }

          return existingIdentity ?? ctx.adapter.create<{
              id: string;
              userId: string;
          }>({
              model: "identity",
      ...
      for (const user of jsonData) {
          ...
          // Password hashes come from the CSV export, not external accounts.
          if (password_digest) {
              if (password_hasher !== "bcrypt") {
                  throw new Error(`Configure password verification for ${password_hasher} before migrating ${id}`);
              }

              const credentialIdentity = await findOrCreateIdentity({
              ...
              const existingCredentialAccount = await ctx.adapter.findOne({
                  model: "account",
                  where: [
                      { field: "identityId", value: credentialIdentity.id },
                      { field: "providerInstanceId", value: "credential" },
                  ],
              });
          ...
          // Create Accounts for OAuth connections returned by Clerk.
          const externalAccounts = clerkUser?.external_accounts;
          if (externalAccounts) {
                  for (const externalAccount of externalAccounts) {
                      const { id, provider, provider_user_id, approved_scopes, created_at, updated_at } = externalAccount;
                      if (provider === "credential") continue;
                      const providerId = provider.replace(/^oauth_/, "");
                  const identity = await findOrCreateIdentity({
                      userId: createdUser.id,
                      issuer: getIdentityIssuer(providerId),
                  ...
                  const existingAccount = await ctx.adapter.findOne({
                      ...
                      where: [
                          ...
                          { field: "providerInstanceId", value: providerId },
                      ],
                  });
