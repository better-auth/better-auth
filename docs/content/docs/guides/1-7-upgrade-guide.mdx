...
1. Complete the manual preparation in [OAuth client records](#migrate-oauth-client-records), [SCIM](#scim-requires-full-reprovisioning), and [Device Authorization](#device-authorization) when those sections apply.
...
**Upgrading from 1.6 changes nothing.** The account schema is the same one 1.6 shipped, so there is no column to add and no data to migrate.

<Callout type="warn">
  Only databases that ran 1.7.0 through 1.7.2 need the steps below. Better Auth keeps working on them: the first account write detects the leftover `NOT NULL` `issuer` column, keeps filling it, and logs one warning naming this section. That fallback is a temporary bridge and will be removed in a later release, so relax the column now.
</Callout>
...
<Steps>
  <Step>
    ...
    If the result has no `issuer` column, you are done. A 1.6 database whose `account` table already had rows could not take the column, because a `NOT NULL` column with no default cannot be added to a populated table. If the column is there, your logs already carry the warning from the first account write.
  </Step>
  ...
  <Step>
    ...
    The generated index name is `account_issuer_accountId_uidx`. Adjust it if your own migration used another name, and use your column name if you renamed the column through `account.fields.issuer` on 1.7; the fallback follows that setting as well.
  </Step>

  <Step>
    #### Confirm

    Restart the application and sign up a new user: the warning no longer appears. Existing sessions and sign-ins were never affected, and neither were sign-ups before this step. Accounts are found by `providerId` and `accountId`, which every row already carries.
  </Step>
</Steps>
