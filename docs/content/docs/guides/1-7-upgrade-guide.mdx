...
| Account identity           | A required `issuer` column and unique compound index on `issuer` and `accountId` in every 1.7 schema                    | **Yes for populated 1.6 account tables**                  |
...
<Callout type="warn">
  Never apply the 1.7 account schema directly over a populated 1.6 `account` table. A plain schema migration cannot choose identity namespaces or resolve collisions. The OAuth provider and SCIM changes also require their guided or manual data steps.
</Callout>

### Migrate from 1.6 to 1.7

`auth migrate` recognizes 1.6 data and switches to a guided migration that changes data and schema together. It covers the built-in PostgreSQL, MySQL, SQLite, and SQL Server configurations, plus SQL-backed Drizzle and Prisma adapters. Every 1.7 account schema requires `issuer` and `accountId` and enforces a unique compound index across them. A populated 1.6 database must therefore set `account.identityStrategy` explicitly before migration: choose `"provider-id"` to preserve the 1.6 logical identity or `"issuer"` to adopt authority-scoped identity. An already-migrated v1.7 database keeps issuer behavior when the option is omitted.

Back up the database, stop authentication writes, and run:
...
`migrate plan` only reads the database. It prints the selected and detected account identity strategies, account counts, projected collisions, automatic namespace resolution, providers that need manual review, schema changes, irreversible release actions, and blockers. Each blocker has a stable code and links to the section of this guide that resolves it. On a populated 1.6 account table, an omitted strategy produces `account-identity-strategy-required`; set the option and run the plan again. `migrate apply` derives what it can from your configuration, asks about the decisions it cannot make, and saves the reviewed answers to `better-auth-migration.json`. The file identifies the `1.6-to-1.7` transition and records the effective persisted namespaces from your configuration, so applying it under a changed configuration produces a blocker instead of silently changing account identity. A blocked run leaves the database unchanged.
...
#### Populated 1.6: adopt issuer-scoped account identity

Set `account.identityStrategy` to `"issuer"` before planning when you deliberately want issuer-scoped identity:
...
`auth migrate plan` projects identities using each provider's trusted authority, reports collisions, and lists providers whose issuer requires manual review. `auth migrate apply` backfills those reviewed authorities, makes `issuer` required, and creates the same `(issuer, accountId)` index. This strategy can merge provider aliases that share an issuer and can change the namespace observed by systems that use the 1.6 `providerId` and `accountId` pair. Follow the manual steps below when you use another adapter, or when the plan reports a collision that only you can resolve.

#### Already migrated v1.7: keep issuer identity

Leave `account.identityStrategy` omitted, or set it explicitly to `"issuer"`. `auth migrate plan` detects the complete issuer data and index, selects issuer behavior, and reports no account identity schema or data actions. `auth migrate apply` is a no-op for account identity, and runtime account recognition remains issuer-scoped.
