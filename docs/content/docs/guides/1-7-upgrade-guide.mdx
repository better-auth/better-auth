...
The upgrade command handles package updates. The database needs more care because 1.7 changes account identity and replaces the legacy OAuth provider and SCIM models.

### Migrate from 1.6 to 1.7

The CLI can migrate built-in PostgreSQL, MySQL, SQLite, and SQL Server configurations, plus SQL-backed Drizzle or Prisma adapters. Back up the database, stop authentication writes, and run:

```bash title="Terminal"
npx auth@rc migrate
```

The command detects a 1.6 database, derives what it can from your configuration, and asks only about unresolved issuers, legacy OAuth client-secret storage and consent handling, customized legacy table names, and SCIM account retirement. Providers that determine their issuer during sign-in are reviewed per account. Unsupported client-secret storage transitions require rotating or re-registering those clients. The command validates every answer before writing, saves the answers to `better-auth-migration.json`, and shows the irreversible actions before the final confirmation. If the migration is blocked, it leaves the database unchanged and explains what to fix.

For Drizzle or Prisma, run `npx auth@rc generate` first and update the schema used by your Better Auth adapter, but do not apply it to the database. The CLI must migrate the 1.6 data and schema together. After it succeeds, record the cutover in your ORM migration history so a later deployment does not apply the same schema changes again.

Use `--dry-run` for a readable preview or `--json` for a machine-readable plan. Run the migration interactively once to create the decisions file, then replay it in a non-interactive environment:
...
<Callout type="warn">
  The migration cannot be undone. Keep authentication writes stopped until it finishes, verify sign-in before deploying 1.7, and complete a full reprovisioning cycle if you use SCIM.
</Callout>

Adapters without a SQL migration connection must apply the relevant account, OAuth provider, and SCIM steps from the feature sections below through their own migration tooling.

### Schema changes

The `auth` CLI now requires Node.js 22.12 or newer. These features change the schema:
...
| Account identity           | An issuer column, a renamed provider-account column, and a compound index  | Guided by `auth migrate`             |
| SCIM                       | Seven provisioning models with layouts that replace the legacy SCIM models | Retire legacy data, then reprovision |
| Organization team counters | `team.memberCount` and `teamMember.membershipKey` columns                  | Not needed                           |
| Provider client store      | `oauthApplication` becomes `oauthClient`, plus new token tables            | Guided by `auth migrate`             |

<Callout type="warn">
  Do not run a generated schema migration over populated 1.6 account, OAuth provider, or SCIM tables before running `auth migrate`. If you adopted prerelease SCIM Group tables, follow the [SCIM cutover](#scim-requires-full-reprovisioning) before creating the final schema.
</Callout>
...
`auth migrate` handles this backfill for supported SQL adapters. If you use another adapter or the CLI reports an identity conflict, apply the following steps with your migration tooling.

<Callout type="warn">
  Use a maintenance window and stop authentication writes before changing the account schema. Never infer account ownership from email or another mutable profile field.
</Callout>
...
4. Find collisions before creating the unique index. Adapt this query to your physical table and field names:
   ...
   If duplicate rows belong to one user, choose the account record to keep and reconcile its provider configuration, tokens, scopes, and timestamps before deleting the others. If a key belongs to multiple users, stop the migration and establish the owner from trusted provider data.
...
<Callout type="warn">
  **Using SQL Server?** Align the legacy OAuth client ID column widths before migrating. Use `NO ACTION` for token and consent foreign keys that would otherwise create multiple cascade paths.
</Callout>
...
The OAuth endpoints move from `/mcp/*` to `/oauth2/*`. Discovery-based MCP clients find the new locations on their own. Existing registered clients move to the shared `oauthClient` store, which the [CLI migration](#migrate-from-16-to-17) handles as part of the cutover. MCP also defaults `refreshTokenReuseInterval` to 30 seconds for native and public clients; set it to `0` if you want strict refresh-token replay handling.
...
3. Retire whichever legacy plugin models exist. For stable 1.6, the CLI backs up `scimProvider`. If you adopted prerelease Group provisioning, first back up and drop `scimGroupRoleGrant`, `scimGroupRole`, `scimGroupMember`, and `scimGroup` in that child-first order, using their physical table names if you customized model names.
...
5. Run `npx auth@rc migrate` and verify that its SCIM account inventory matches yours before confirming. For an unsupported adapter, remove the inventoried legacy accounts and create the new SCIM models with your own migration tooling.
...
After applying the changes that affect your project:

* Run `npx auth@rc migrate`. Use `--dry-run` first if you want to review the plan.
* Verify sign-in and account linking against the migrated account identities.
* Verify migrated OAuth or MCP clients if you previously used `oidcProvider` or the in-core MCP plugin.
* Complete a full User and Group provisioning cycle if you use SCIM.
