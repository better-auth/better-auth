...
Set `account.identityScope` to `"issuer"`. If omitted, the runtime keeps the same identity behavior. In both cases, `auth migrate plan` detects the existing issuer data and `auth migrate apply` makes no account identity changes.
