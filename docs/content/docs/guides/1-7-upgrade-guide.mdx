...
| Device Authorization       | Indexes on `deviceCode` and `userCode`                                                                                  | MySQL and SQL Server only |
...
The 1.7 schema indexes `deviceCode` and `userCode` and limits both values to 191 characters. Before applying the schema on MySQL or SQL Server, change both columns to bounded strings and resolve any existing values longer than 191 characters. Custom `generateDeviceCode` and `generateUserCode` functions must also stay within that limit.
