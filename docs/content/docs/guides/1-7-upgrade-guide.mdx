...
Better Auth 1.6 recognized an external account by `providerId` and `accountId`. If another system depends on that namespace, configure the compatibility strategy before running `auth migrate plan` or `auth migrate apply`, then deploy the same option with 1.7:
...
Do not enable this option after completing an issuer-scoped 1.7 migration. The guided migration reports a blocker if an account already stores a different issuer rather than silently re-keying it. Leave the default `"issuer"` strategy in place for an already migrated database; changing its identity strategy requires a separate, reviewed account re-key migration.
