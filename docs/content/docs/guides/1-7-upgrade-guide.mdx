...
| Account identity           | An issuer column and a compound index with the existing `accountId` column                                              | **Yes, guided or manual backfill**                        |
| SCIM                       | Seven provisioning models, plus three optional managed-catalog models, with layouts that replace the legacy SCIM models | **Yes, retire, then reprovision**                         |
| Organization team counters | `team.memberCount` and `teamMember.membershipKey` columns                                                               | No                                                        |
| Provider client store      | `oauthApplication` becomes `oauthClient`, plus new token tables                                                         | **Yes, guided or manual client move**                     |
...
<Callout type="warn">
  Never apply a generated 1.7 schema over populated 1.6 account, OAuth provider, or SCIM tables. A plain schema migration adds tables, columns, and indexes; it cannot choose issuers, copy OAuth clients, or convert legacy SCIM data.
</Callout>

### Migrate from 1.6 to 1.7

`auth migrate` recognizes 1.6 data and switches to a guided migration that changes data and schema together. It covers the built-in PostgreSQL, MySQL, SQLite, and SQL Server configurations, plus SQL-backed Drizzle and Prisma adapters. Back up the database, stop authentication writes, and run:

If the 1.6 database contains external accounts, first set `account.identityStrategy` explicitly. Use `"provider-id"` to preserve 1.6 account identity, which is the recommended migration path. Set `"issuer"` only when you intend to adopt issuer-scoped identity and have audited every integration that depends on account identity. The migration blocks before writing when external accounts need a backfill and this option is omitted.
...
`migrate plan` only reads the database. It prints the adapter and dialect it detected, the schema changes it would make, every irreversible release action, and every blocker it found. Each blocker has a stable code and links to the section of this guide that resolves it. `migrate apply` derives what it can from your configuration, asks about the decisions it cannot make, and saves the reviewed answers to `better-auth-migration.json`. The file identifies the `1.6-to-1.7` transition and records the effective issuers from your configuration, so applying it under a changed configuration produces a blocker instead of silently changing account identity. A blocked run leaves the database unchanged.

Pass the saved file positionally to review or apply the same decisions in another environment:
...
With Drizzle or Prisma, run `npx auth generate` first and point the adapter at the generated 1.7 schema, but do not apply that schema to the database: the CLI migrates the 1.6 data and schema together. For a Drizzle SQLite or PostgreSQL migration, set the adapter's `transaction` option to `true` so the release migration can run atomically. MySQL databases with populated legacy SCIM accounts also require a transaction-capable migration connection so account retirement can roll back independently of MySQL's non-transactional schema changes. Record the cutover in your ORM migration history afterwards, so a later deployment does not apply the same changes twice.
...
`auth migrate apply` performs this backfill for the adapters it supports, deriving each provider's issuer from your configuration and asking for the ones it cannot derive. Follow the steps below when you use another adapter, or when the CLI reports a collision only you can resolve.

#### Preserve 1.6 provider-scoped account identity

Better Auth 1.6 recognized an external account by `providerId` and `accountId`, so the compatibility-preserving migration path uses `"provider-id"`. The guided migration recommends this strategy and requires an explicit choice when populated external accounts still need an issuer backfill. Configure it before running `auth migrate plan` or `auth migrate apply`, then deploy the same option with 1.7:
...
This option keeps the required 1.7 `issuer` column and compound index, but stores `local:oauth:<encoded providerId>` as the issuer for external accounts. Credential and SIWE accounts keep their existing local namespaces. Better Auth still performs each provider's normal token and profile verification before accepting the account subject; the option changes only the namespace used to recognize and persist that account.

Keep this option identical on every instance that reads or writes the database. Under this strategy, renaming a provider changes its account namespace, and two provider aliases for the same OpenID Connect issuer remain separate identities, matching the provider-scoped behavior of 1.6.

Set `account.identityStrategy` to `"issuer"` before migration only when you deliberately want the 1.7 identity behavior. This can merge provider aliases that share an issuer and change the namespace observed by systems that use the 1.6 `providerId` and `accountId` pair.

Fresh 1.7 applications still default to `"issuer"`. Do not enable `"provider-id"` after completing an issuer-scoped 1.7 migration. The guided migration reports a blocker if an account already stores a different issuer rather than silently re-keying it. Leave the `"issuer"` strategy in place for an already migrated database; changing its identity strategy requires a separate, reviewed account re-key migration.
...
3. Populate both fields according to the account type:
   ...
   | Account type                          | Identity strategy | `issuer`                                  | `accountId`                            |
   | ------------------------------------- | ----------------- | ----------------------------------------- | -------------------------------------- |
   | Email/password credential             | Either            | `local:credential`                        | Stable `id` from the linked `user` row |
   | SIWE wallet, `providerId` `siwe`      | Either            | `local:siwe`                              | Existing `<address>:<chainId>` value   |
   | Any external provider                 | `provider-id`     | `local:oauth:<encoded providerId>`        | Existing provider account identifier   |
   | Google One Tap, `providerId` `google` | `issuer`          | `https://accounts.google.com`             | Existing Google `sub`                  |
   | Provider with an issuer               | `issuer`          | Exact trusted issuer used by the provider | Existing provider account identifier   |
   | OAuth provider without an issuer      | `issuer`          | `local:oauth:<encoded providerId>`        | Existing provider account identifier   |

   The synthetic issuer percent-encodes its provider ID segment exactly as `encodeURIComponent(providerId)`; for example, `local:oauth:github` and `local:oauth:team%2Fgithub`. Build an explicit `providerId`-to-issuer map for your deployment. Under the `"issuer"` strategy, multiple provider configurations that represent the same OpenID Connect authority must use the same issuer. Do not derive an issuer from email, display name, an unverified request value, or a mutable authorization endpoint.
