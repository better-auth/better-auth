...
Security is the reason for some changes, not a separate type. If a safer check changes runtime behavior, the section is marked **Behavior**. If it needs code, config, data, or schema changes, it is marked **Breaking**.
...
### Discovery providers verify their identity tokens - Behavior, Medium
...
### Client authentication is tied to the grant - Behavior, Medium
...
### Server-side OAuth requests refuse redirects - Behavior, Low
...
### SSO domain verification checks every listed domain - Behavior, Medium
...
### SAML Single Logout accepts only http(s) URLs - Behavior, Low
