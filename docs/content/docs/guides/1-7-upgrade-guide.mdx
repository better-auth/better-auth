...
| Uses OAuth client CRUD, DCR, or CIMD                                   | [As an identity provider](#as-an-identity-provider)                             |
| Runs MCP                                                               | [MCP](#mcp)                                                                     |
...
| Provider client store      | `oauthApplication` becomes `oauthClient`, plus new token tables            | **No, move client data** |
| OAuth client registration  | `applicationType`, discovery provenance, and a unique resource-link index  | **No, backfill clients** |

Account identity, OAuth clients, SCIM, and the provider client store require manual data steps. Plan each applicable cutover before you deploy.
...
<Callout type="warn">
  **Migrating from `oidcProvider` or the in-core MCP plugin moves client data.** `@better-auth/oauth-provider` stores registered clients in `oauthClient`, not the old `oauthApplication`, and renames the `oauthAccessToken.accessToken` column to `token`. `auth migrate` only adds tables and columns; it never renames or copies data. Copy clients with an explicit field mapping or re-register them before cutover. The old and new client tables are not column-compatible: `redirectUrls` becomes the `redirectUris` array, `metadata` becomes JSON, and fields such as `grantTypes` and `tokenEndpointAuthMethod` are new. Dynamically registered clients can re-register. Drop or rename the legacy `oauthAccessToken` table before migrating because its rows are ephemeral access tokens and the new required `token` column cannot be added to populated rows automatically.
</Callout>

<Callout type="warn">
  **Existing `@better-auth/oauth-provider` clients need a separate backfill.** The client schema replaces `type` and `public` with `applicationType`, adds nullable `clientDiscoveryId`, and the resource-link table gains a compound unique index. Follow [the OAuth client-record migration](#oauth-clients-separate-application-type-from-authentication) before dropping either legacy column or creating the unique index.
</Callout>
...
### OAuth clients separate application type from authentication

OAuth clients no longer store `type` or `public`. The new `applicationType` field controls redirect URI policy, while `tokenEndpointAuthMethod` alone determines whether the client authenticates. OAuth requests and responses use the wire names `application_type` and `token_endpoint_auth_method`.

Use a maintenance window for this migration, and stop client registration and authorization writes before changing the tables:

1. Back up `oauthClient` and `oauthClientResource`.
2. Add `applicationType`, `clientDiscoveryId`, and `clientCredentialsScopes` as nullable. Copy `type: "web"` to `applicationType: "web"` and `type: "native"` to `applicationType: "native"`. Leave `user-agent-based` as `NULL`, then classify each affected client as `web` or `native` from its deployment and redirect behavior. Classify any other missing value explicitly instead of relying on a runtime default.
3. Set `clientDiscoveryId` to `"cimd"` only for records that were actually discovered by CIMD. Leave DCR, administrative, user-managed, and migrated legacy clients as `NULL`. Do not infer discovery ownership from an `https://` client ID.
4. Audit `tokenEndpointAuthMethod` independently. Store `none` for a client that does not authenticate, or its configured authentication method for a confidential client. Do not derive this value from the old `public` field.
5. Find duplicate `(clientId, resourceId)` rows in `oauthClientResource`, decide which row to keep for each pair, and remove the duplicates before adding the compound unique index.
6. Backfill `clientCredentialsScopes` to `[]` for every existing client and configure `[]` as the database default for new rows. Missing, `NULL`, and empty values all deny machine tokens at runtime, but the migration must write the exact empty-array value so stored state is deterministic. Audit every client that declares `client_credentials`, then let an administrator explicitly replace `[]` with its approved non-OIDC scopes. Never copy user-delegated `scopes` into this field.
7. Drop `type` and `public` only after the backfill is complete. There is no compatibility reader for either field.

Client creation and registration also use the new model:

* Dynamic, administrative, and user-managed registration default a missing `application_type` to `web`. Client ID Metadata Documents preserve an omitted application type as `NULL`.
* Web clients must use HTTPS redirects on non-loopback hosts.
* Native clients may use a claimed HTTPS redirect, exact HTTP loopback hosts (`localhost`, `127.0.0.1`, or `[::1]`) with any port, or a reverse-domain private-use scheme such as `com.example.app:/callback`. Alternative numeric spellings such as `127.1` are rejected.
* `token_endpoint_auth_method: "none"` creates a public client. Every authenticated method creates a confidential client, regardless of `application_type`.
* Inline client keys must use the RFC 7517 JWK Set shape. Replace `jwks: [key]` with `jwks: { keys: [key] }` in DCR payloads, administrative and user client creation, Client ID Metadata Documents, test fixtures, and generated client code. EC keys must use P-256, P-384, or P-521; OKP keys must use Ed25519. If a key includes `alg`, use a supported `private_key_jwt` algorithm that matches its key type and curve, or omit `alg` and let the assertion header select it. OAuth client rows previously written through `oauthToSchema` already store the object shape, so this is a request, configuration, and type migration rather than another database rewrite. Audit custom-written rows separately if they bypassed Better Auth persistence.
* Every `backchannel_logout_uri` must be a public HTTPS URL. Loopback HTTP callbacks are no longer accepted.

**What to do:** update custom schemas, adapters, client-management code, and registration payloads to use the two independent fields.

`OAuthClient` no longer has a catch-all string index, so removed fields such as `type` and `public` no longer type-check as unknown metadata. Model custom wire extensions explicitly:

```ts
interface YourExtensionMetadata {
  your_extension: string;
}

type ExtendedOAuthClient = OAuthClient & YourExtensionMetadata;
```

Remove `clientCredentialGrantDefaultScopes`. Administrative client creation and updates now use `client_credentials_scopes`; assigning a non-empty value requires `clientPrivileges` to approve `configure-client-credentials-scopes`. DCR, CIMD, and user-managed routes cannot assign it, and removing `client_credentials` from a client clears the stored ceiling.
...
### Registration resources and scopes are server-owned policies

The provider now distinguishes the capabilities an operator permits from the values a client requests during registration. `clientRegistrationDefaultResources` links every new client to server-owned resources. `clientRegistrationAllowedResources` controls which additional resources the optional DCR `resources` extension may request. A requested resource outside those lists returns `invalid_target`.

Registration scopes follow the same boundary. Better Auth validates a client's requested `scope` as a subset of the configured registration scopes, but persists the full operator-approved capability set. Registration does not grant those scopes to a user.

**What to do:** configure default and allowed resources before enabling DCR or client discovery. If existing DCR clients send `resources`, add each supported identifier to one of those options. Review `clientRegistrationDefaultScopes` and `clientRegistrationAllowedScopes` as capability policy rather than a default user grant.
...
### Protected-resource scope challenges are explicit

Protected operations now separate scope enforcement from generic authorization failures. Configure required scopes with `requiredScopes`, and provide `isScopeSatisfied` only when your scope model is hierarchical. Use `createInsufficientScopeError` for a denial that the client can solve by authorizing more scopes.

`createResourceServerChallenge` builds the response for a protected-resource challenge. It returns an `APIError` for recognized token or scope failures and `undefined` for unrelated errors. Ordinary handler responses and unrelated thrown values keep their identity instead of turning into OAuth challenges.

**What to do:** rename protected-operation `scopes` to `requiredScopes`, and use `challengeScopes` for the challenge hint.
...
**What to do:** set `refreshTokenReuseInterval` only when a client can retry a refresh request with an old token after another local session already rotated it. OAuth Provider keeps strict handling at `0`; `mcp()` defaults the interval to 30 seconds for every client configured through that plugin. Set `refreshTokenReuseInterval: 0` on `mcp()` to disable the overlap window.
...
1. Install `@better-auth/mcp`, `@better-auth/cimd`, and the official version 2 MCP client or server package needed by your application. Authorization plugins and protected-request helpers come from the `@better-auth/mcp` package root. The in-core MCP client (`createMcpAuthClient` and its adapters under `better-auth/plugins/mcp/client`) is removed; MCP protocol and transport code comes from `@modelcontextprotocol/client` and `@modelcontextprotocol/server`.
2. Add the `jwt()` plugin, which is now required.
3. Compose `mcp({ ... })` with `cimd({ fetchClientMetadataResource, metadataProfile: "mcp-2026-07-28" })`. Client ID Metadata Documents are the recommended registration mechanism for MCP 2026-07-28, which pins CIMD draft-00, and `mcp()` no longer enables Dynamic Client Registration.
4. Move options that were nested under `oidcConfig` up to the top level of `mcp({ ... })`. Replace `resourceMetadataMappings` with one canonical HTTPS `resource`, for example `resource: "https://api.example.com/mcp"`. `resource` was optional and defaulted to the server origin; it is now required and must be a query-free HTTPS URL without credentials or a fragment. HTTP is accepted only on loopback hosts for development.
5. Rename `withMcpAuth` to `requireMcpAuth` and `mcpHandler` to `createMcpProtectedRequestHandler`. The standalone factory accepts one flat `McpProtectedRequestHandlerOptions` object with `issuer`, a single `audience`, optional `jwtVerifyOptions`, verification fields, and challenge fields. Its callback receives `accessTokenClaims`. In protected-route options, rename `scopes` to `requiredScopes`; use the new `challengeScopes` option to advertise scopes in `WWW-Authenticate` challenges.
6. If application code raised or caught scope challenges directly, replace the exception-driven API with `createInsufficientScopeError`, `isInsufficientScopeError`, and `createResourceServerChallenge`.
7. Replace the session-oriented MCP route with the SDK v2 `createMcpHandler`, set `legacy: "reject"`, wrap its handler with `requireMcpAuth`, and export only `POST`.
8. Regenerate or migrate your schema.

The OAuth endpoints move from `/mcp/*` to `/oauth2/*`. Discovery-based MCP clients find the new locations on their own. Existing registered clients move to the shared `oauthClient` store and are not copied automatically; complete the provider-client data step in [Before you upgrade](#before-you-upgrade) before cutover.

MCP 2026-07-28 does not maintain protocol-level sessions between requests. Remove MCP-route `GET` and `DELETE` exports and session-store options such as `redisUrl`; each client JSON-RPC request or notification now travels through an independent `POST`. This does not remove the OAuth database tables, which still store clients, consent, authorization codes, refresh tokens, and security records. Deployments that use `subscriptions/listen` across multiple server instances need a shared SDK event bus, not an MCP session store. See [Protecting an MCP Route](/docs/plugins/mcp#protecting-an-mcp-route) for the complete SDK v2 example.

If you deliberately support an older client that still requires Dynamic Client Registration, set both `allowDynamicClientRegistration` and `allowUnauthenticatedClientRegistration` on `mcp()`. Discovery advertises a registration endpoint only in that configuration. `mcp()` links its protected resource to every newly registered client, so a DCR request no longer needs Better Auth's `resources` extension.

MCP defaults `refreshTokenReuseInterval` to 30 seconds for every client so a retry can recover a recently rotated refresh response. Set it to `0` if you require strict refresh-token replay handling.

An insufficient-scope challenge names only the scopes the token is missing. It does not expose `offline_access` or repeat scopes the token already has.
...
### Dynamic base URLs now fail closed

When `baseURL` is resolved from each request, Better Auth now rejects a direct server API call that has no usable URL or host, or whose host is outside `allowedHosts`. Trusted proxy headers are considered only when `advanced.trustedProxyHeaders` enables them. OAuth issuer, discovery, protected-resource, and JWKS URLs then use the accepted request host.

**What to do:** pass request URL and header data to direct server API calls that depend on a dynamic base URL. Configure `allowedHosts` and proxy trust before relying on forwarded values. For an MCP resource server whose host cannot be resolved from the Better Auth request, use `createMcpProtectedRequestHandler` with explicit issuer, audience, and JWKS values.
...
Some data changes must happen before the generated schema migration. Use a maintenance window, stop authentication and provisioning writes, and complete the applicable preparation:

* **Backfill account identities:** assign every account a reviewed issuer and provider account identifier, resolve collisions, and prepare the compound unique index.
* **Move provider clients:** copy or re-register clients from `oidcProvider` or the in-core MCP plugin before switching to the shared OAuth provider tables.
* **Backfill OAuth clients:** populate `applicationType`, assign `clientDiscoveryId` only from known provenance, audit `tokenEndpointAuthMethod`, and deduplicate client-resource links before dropping the legacy fields or adding the unique index.
* **Prepare the SCIM cutover:** inventory retained identities and drop incompatible legacy models in the order described in the [full SCIM reprovisioning guide](#scim-requires-full-reprovisioning).

After completing those steps, generate and apply the schema:
...
Then finish any post-migration work. Make the new account identity fields required, deploy the new OAuth client schema and configuration, and run a complete SCIM User and Group provisioning cycle when SCIM applies.
...
An earlier 1.7 prerelease accepted looser Client ID Metadata Documents, permitted loopback Client Identifier URLs, and refreshed documents on a fixed timer. The final generic plugin follows CIMD draft-02; `client_name` and `redirect_uris` are optional unless `metadataProfile: "mcp-2026-07-28"` selects MCP's draft-00 requirements. Revalidate every document: shared secrets, resource links, server-controlled fields, private JWK material, and back-channel logout metadata are forbidden, while the authentication method must be `none` or `private_key_jwt`.

Update prerelease CIMD integrations as follows:

* Replace `createCimdResolver` with `createCimdClientDiscovery`.
* Replace `cimdClientDiscovery` with `createCimdClientDiscovery`.
* Replace `ClientIdMetadataDocumentResult` with `CimdMetadataValidationResult` and `ValidateCimdMetadataOptions` with `CimdMetadataValidationOptions`.
* Replace `isUrlClientId` with `isCimdClientIdUrlCandidate`.
* Replace the `MetadataDocumentFetch` type with `ClientMetadataResourceFetch`.
* Replace `refreshRate` with `metadataRevalidationInterval`. Numeric values are seconds, as are numeric `minimumFetchInterval` values.
* Update lifecycle callback parameters: use the exported `CimdClientCreatedEvent` and `CimdClientRefreshedEvent` contracts, read `clientMetadataDocument` instead of `metadata`, and read `context` instead of `ctx`.
* Pass the now-required `fetchClientMetadataResource` option. `CimdOptions` is no longer optional because every deployment must provide this network transport.
* Remove `allowFetch`, `fetchMetadataDocument`, and `allowLoopback`.

The transport must resolve once, reject RFC 6890 addresses, pin the approved address, preserve TLS hostname verification, and refuse redirects. Calling `globalThis.fetch` after a separate DNS check is not safe because Fetch can resolve the hostname again.

The default `originBoundFields` no longer includes `redirect_uris`; requested redirects still require an exact match, and the consent page should show both the client-ID and redirect hosts. Cache freshness follows HTTP cache headers, with `private` and `Vary: *` treated as noncacheable. A stale refresh failure preserves stored state but fails the current request closed. Lifecycle hooks remain best-effort notifications, and `onClientRefreshed` now receives `previousClient`. Add nullable `oauthClient.clientDiscoveryId`, set it to `"cimd"` only for known CIMD-owned clients, and leave every managed or DCR client `NULL`. See the [CIMD requirements](/docs/plugins/cimd#draft-02-validation) for the complete contract.

An earlier 1.7 prerelease shipped a different OAuth-provider and MCP surface. If you adopted it, update these names before finishing the upgrade:

* Replace `raiseResourceServerChallenge` with `createResourceServerChallenge`.
* The package root no longer exports `checkOAuthClient` or `oauthToSchema`.
* The `@better-auth/mcp/client` and `@better-auth/mcp/client/adapters` subpaths are removed; use the official version 2 MCP packages.
* The challenge `scope` option is renamed `challengeScopes`.
* `mcp()` rejects a `resource` containing a query string; move discriminators into the path.
* The client-privileges action `configure-client-credentials` is renamed `configure-client-credentials-scopes`.
