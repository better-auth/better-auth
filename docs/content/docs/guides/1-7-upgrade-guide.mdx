...
The changes that need more care are mostly in OAuth, OpenID Connect, SAML, SCIM, two-factor authentication, MCP, custom storage, and proxy setups. Use this table to choose the sections that apply to your project.
...
| **Security**    | A stricter check or safer default may reject unsafe input it allowed.  |
...
The Electron login flow now requires S256 PKCE, no longer trusts a custom origin header, and matches custom URL schemes more strictly.
...
### SCIM write paths are stricter - Behavior, Medium
