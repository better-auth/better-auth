...
The supported path for a populated 1.6 database keeps existing accounts linked to the same users by preserving provider-scoped account identity. Prepare the v1.7 configuration with the provider scope explicitly selected:
...
1. Complete the manual preparation in [Account identity](#choose-account-identity-scope), [OAuth client records](#migrate-oauth-client-records), [SCIM](#scim-requires-full-reprovisioning), and [Device Authorization](#device-authorization) when those sections apply.
...
### Choose account identity scope

Every 1.7 account uses the unique key `(issuer, accountId)`. `account.identityScope` controls the value stored in `issuer`. The `"issuer"` scope stores the verified authority, while `"provider"` stores a deterministic namespace for each configured provider. Configurations generated by `auth init` use `"provider"`. Omitting the option continues to use issuer scope and logs a warning during auth initialization.

#### Populated 1.6: preserve provider-scoped account identity

`"provider"` preserves the 1.6 `(providerId, accountId)` identity. `auth migrate plan` checks for collisions and plans the issuer backfill. `auth migrate apply` stores deterministic namespaces such as `local:oauth:google` and creates the `(issuer, accountId)` index.

This changes stored identity, not protocol verification. Keep the scope identical on every instance that uses the database. Renaming a provider changes its namespace, and aliases for the same authority remain separate accounts.

#### Populated 1.6: issuer identity requires a separate re-key

Do not set `account.identityScope` to `"issuer"` and expect the guided 1.6 migration to infer authorities:
...
`auth migrate plan` blocks this choice because a 1.6 row does not reliably record the verified authority. Adopting issuer scope can also merge provider aliases. Use provider scope for the supported upgrade, or prepare a reviewed re-key migration that establishes every authority and resolves collisions.

#### Already migrated v1.7: keep issuer identity

Set `account.identityScope` to `"issuer"`. If omitted, the runtime keeps the same identity behavior but logs a warning during auth initialization. In both cases, `auth migrate plan` detects the existing issuer data and `auth migrate apply` makes no account identity changes.
...
#### Already migrated provider-scoped data: keep provider identity

Set `account.identityScope` explicitly to `"provider"`. The plan detects deterministic namespaces such as `local:oauth:google`, and apply is a no-op. Omitting the scope or selecting `"issuer"` is blocked and instructs you to restore `account: { identityScope: "provider" }`.

<Callout type="warn">
  Changing `account.identityScope` on populated v1.7 data requires a re-key migration. Setting `"provider"` against an issuer-keyed database is blocked. Keep `"issuer"` until a reviewed migration rewrites every namespace and resolves collisions. Runtime behavior never selects a scope by inspecting the database.
</Callout>
...
3. Populate both fields according to the selected scope and account type:

   | Account type                          | `issuer` scope                            | `provider` scope                   | `accountId`                            |
   | ------------------------------------- | ----------------------------------------- | ---------------------------------- | -------------------------------------- |
   ...
   The synthetic issuer percent-encodes its provider ID segment exactly as `encodeURIComponent(providerId)`. For example, it stores `local:oauth:github` and `local:oauth:team%2Fgithub`. Build an explicit `providerId`-to-namespace map for the selected scope. Under the `"issuer"` scope, multiple provider configurations that represent the same OpenID Connect authority must use the same issuer. Under the `"provider"` scope, every external connection keeps a distinct synthetic namespace. Do not derive an issuer from email, display name, an unverified request value, or a mutable authorization endpoint.
...
Microsoft accounts now use the stable directory `oid` claim instead of the pairwise, app-specific `sub` claim. Under issuer-scoped identity, complete this during step 3 of the account identity backfill, before checking for collisions or adding the compound account index. Under the explicit provider scope, update the same account identifiers as a separate data migration before accepting production traffic on 1.7:
...
2. Set each row's namespace according to the selected scope: use the trusted Microsoft authority for `"issuer"`, or `local:oauth:<encoded providerId>` for `"provider"`. Replace the old `sub`-based `accountId` with the verified `oid` for that directory user.
...
**What to do:** remove `id` from every OIDC and SAML mapping, then confirm that each OIDC provider returns a stable `sub` and each SAML provider returns a stable, signed `NameID`. During the account-identity backfill, keep that protocol-defined subject and store either the exact OIDC issuer or SAML IdP entity ID under the `"issuer"` scope, or the deterministic provider namespace under `"provider"`. Provider aliases with the same authority and subject deduplicate one external identity only under issuer scope. This does not create independent grant or provider lifecycle records for those aliases.

If a previous mapping used `mapping.id`, prepare its replacement before the maintenance window. Build a trusted mapping from every old account subject to the protocol-defined identity, then rewrite those account rows before adding the compound index. Under issuer scope, use the exact OIDC issuer and verified `sub`, or the actual SAML IdP metadata entity ID and signed `NameID`. Under provider scope, keep the verified subject or signed `NameID` but store the deterministic provider namespace. Obtain the subject mapping from the identity provider, not email or a mutable profile attribute. Do not deploy a runtime fallback for the old mapping.
