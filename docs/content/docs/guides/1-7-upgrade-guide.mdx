...
| OAuth client registration  | `applicationType` and a unique client-resource link index                  | **No, backfill clients** |
...
<Callout type="warn">
  **Existing `@better-auth/oauth-provider` clients need a separate backfill.** The client schema replaces `type` and `public` with `applicationType`, and the resource-link table gains a compound unique index. Follow [the OAuth client-record migration](#oauth-clients-separate-application-type-from-authentication) before dropping either legacy column or creating the unique index.
</Callout>
...
2. Add `applicationType` as nullable. Copy `type: "web"` to `applicationType: "web"` and `type: "native"` to `applicationType: "native"`. Leave `user-agent-based` as `NULL`, then classify each affected client as `web` or `native` from its deployment and redirect behavior. Classify any other missing value explicitly instead of relying on a runtime default.
3. Audit `tokenEndpointAuthMethod` independently. Store `none` for a client that does not authenticate, or its configured authentication method for a confidential client. Do not derive this value from the old `public` field.
4. Find duplicate `(clientId, resourceId)` rows in `oauthClientResource`, decide which row to keep for each pair, and remove the duplicates before adding the compound unique index.
5. Drop `type` and `public` only after the backfill is complete. There is no compatibility reader for either field.
...
* Native clients may use a claimed HTTPS redirect, exact HTTP loopback hosts (`localhost`, `127.0.0.1`, or `[::1]`) with any port, or a reverse-domain private-use scheme such as `com.example.app:/callback`.
...
3. Compose `mcp({ ... })` with `cimd()`. Client ID Metadata Documents are the recommended registration mechanism for MCP 2026-07-28, and `mcp()` no longer enables Dynamic Client Registration.
...
* **Backfill OAuth clients:** populate `applicationType`, audit `tokenEndpointAuthMethod`, and deduplicate client-resource links before dropping the legacy fields or adding the unique index.
...
An earlier 1.7 prerelease accepted looser Client ID Metadata Documents and refreshed them on a fixed timer. Revalidate every CIMD document before upgrading: `client_name` must be nonblank; shared secrets, resource links, and server-controlled fields are forbidden; and the authentication method must be `none` or `private_key_jwt`. The default `originBoundFields` no longer includes `redirect_uris`; requested redirects still require an exact match, and the consent page should show both the client-ID and redirect hosts. The final plugin follows HTTP cache headers, fails the current authorization request when a stale refresh fails, and treats lifecycle hooks as best-effort notifications. See the [CIMD requirements](/docs/plugins/cimd#metadata-document-requirements) for the complete contract.
