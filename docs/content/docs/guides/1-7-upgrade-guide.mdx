...
<Callout type="warn">
  While 1.7 is a release candidate, use the `rc` tag for the CLI and packages. Once 1.7 is stable, use `latest` or omit the tag.
</Callout>
...
| Device Authorization       | Indexes on `deviceCode` and `userCode`; the optional OAuth grant also adds `oauthClientId` and `resources`              | **Yes, MySQL/SQL Server** |
...
2. Apply the 1.7 schema. With the built-in Kysely adapter, run `npx auth@rc migrate`. With Drizzle, Prisma, or a custom schema workflow, run `npx auth@rc generate`, review the output, and apply it with your migration tooling.
...
### Device codes are indexed and bounded

The 1.7 schema indexes `deviceCode` and `userCode` and limits both values to 191 characters. Before applying it on MySQL or SQL Server, change both columns to bounded strings and resolve any existing values longer than 191 characters. Custom `generateDeviceCode` and `generateUserCode` functions must also stay within that limit.
...
To let a registered CLI, TV app, or other limited-input client obtain OAuth tokens, create one shared grant and pass it explicitly to both plugins:
...
This composition adds nullable `oauthClientId` and `resources` fields to `deviceCode`, validates the OAuth client, scopes, and resources when the code is created, advertises the device authorization endpoint in discovery, and exchanges approved codes at `/oauth2/token`. Regenerate and apply the schema when you enable the grant. Existing 1.6 rows need no backfill and remain on the session-token path, even if their `clientId` later matches a registered OAuth client. Allow pending 1.6 clients to finish polling `/device/token` or let their codes expire before moving those clients to the OAuth flow.

The client type only exposes the RFC 8707 `resource` request field when the server uses the OAuth grant:

```ts title="auth-client.ts"
import type { DeviceCodeGrant } from "@better-auth/oauth-provider";
