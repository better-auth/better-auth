...
| Account identity           | An issuer column and a compound index with the existing provider-account column                                         | **No, backfill**         |
...
<Callout type="warn">
  The `1.7.0-rc.2` and `1.7.0-rc.3` prereleases temporarily used `providerAccountId`. If you deployed either prerelease, rename that physical column and any custom field mapping back to `accountId` as part of this migration. Also rename or recreate the generated compound index so its physical name uses `accountId`; leaving the prerelease index name can make schema tools attempt to create a duplicate index. Applications upgrading from Better Auth 1.6 should keep their existing `accountId` column.
</Callout>
...
2. Add `issuer` as nullable during the backfill. Keep the existing physical `accountId` column; only prerelease deployments that adopted `providerAccountId` need to rename it back.
