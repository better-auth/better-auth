...
6. Drop `type` and `public` only after the backfill is complete. There is no compatibility reader for either field.
...
**What to do:** update custom schemas, adapters, client-management code, and registration payloads to use the two independent fields. If you imported `checkOAuthClient` or `oauthToSchema` from the package root, remove that dependency and use the provider endpoints or supported plugin APIs.
...
1. Install `@better-auth/mcp` and `@better-auth/cimd`. Update imports: the server plugin and helpers come from `@better-auth/mcp`, while the client and adapters come from `@better-auth/mcp/client` and `@better-auth/mcp/client/adapters`.
...
5. Rename `withMcpAuth` to `requireMcpAuth`, and `createMcpAuthClient` to `createMcpResourceClient`. In protected-route options, rename `scopes` to `requiredScopes` and challenge `scope` to `challengeScopes`.
...
7. Regenerate or migrate your schema.
...
* Replace `createCimdResolver` with `cimdClientDiscovery`.
