...
Before removing `issuer`, check for duplicate `(providerId, accountId)` pairs. If different issuers share a provider ID and use the same account IDs, give each issuer its own provider ID and update the matching account rows before upgrading. Keep different users separate. Better Auth rejects account lookups when more than one row matches the key.
