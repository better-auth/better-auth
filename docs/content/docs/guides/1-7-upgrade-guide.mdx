...
With the built-in Kysely adapter, inspect the database without changing it after updating the configuration for 1.7:
...
A blocked plan exits with status `1` and does not change the database. Resolve each blocker using the applicable section below, then run the inspection again.

With the built-in Kysely adapter on PostgreSQL, MySQL, or SQL Server, use `migrate --from 1.6` with the explicit decisions described below. With Drizzle or Prisma, inventory the applicable legacy data using your ORM, complete the manual data step first, then run `generate`, review the result, and apply it through your ORM's migration tooling.
...
With the built-in Kysely adapter on PostgreSQL, MySQL, or SQL Server, pass the reviewed mapping to the release migration:
...
With another adapter or migration tool, backfill the account identity manually:
...
5. For the built-in Kysely adapter, run the release migration with `--reprovision-scim` and one `--retire-scim-account` value for every exact account ID in the reviewed inventory. The command retires those account rows, backs up the stable provider table, and creates `scimConnectionBinding`, `scimIdentityTombstone`, `scimSubject`, `scimUser`, `scimGroup`, `scimGroupMember`, and `scimProjectionGrant`. For other adapters, apply the equivalent operations through your ORM.
...
Update the Better Auth packages and configuration. With the built-in Kysely adapter, inspect without writing:
...
For Drizzle or Prisma, complete the equivalent data inventory in your ORM, use `generate`, and apply the reviewed output through your ORM's migration tooling. Deploy 1.7 only after the post-migration row counts, collision checks, sign-in checks, inactive legacy-token check, and feature-specific verification pass.
