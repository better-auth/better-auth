...
2. Add nullable staging columns named `issuer`, `providerAccountId`, and `providerInstanceId` to the existing `account` table. Copy the old provider-side `accountId` value into `providerAccountId`, then populate the issuer and provider instance according to the account type:
   ...
   | TikTok                      | `local:oauth:tiktok`                      | TikTok `union_id`                      | `tiktok`                        |
   | Provider without an issuer  | `local:oauth:<encoded providerId>`              | Existing provider account identifier   | Existing `providerId`           |
...
An SSO `providerId` must be 1 to 128 characters and contain only letters, numbers, `.`, `_`, `~`, or `-`. The path segments `.` and `..` are invalid. Provider domains are normalized into a canonical, comma-separated list of hostnames, and every entry in that list must resolve to a hostname. A single unusable entry rejects the whole provider, even when the other entries are valid. The same validation runs against `defaultSSO` during startup.

**What to do:** during the paused-write migration, map each persisted ID that contains spaces, slashes, percent escapes, or other unsupported characters to a valid ID. Update the `ssoProvider.providerId` value, every Account that you can prove belongs to that provider, and the IdP callback URL together. `providerId` is immutable through the management endpoint, so do not try to rename a live provider through the API. If an old Account cannot be assigned unambiguously, remove it and require the user to authenticate through the replacement provider. Review every entry of each stored domain list, not just the first: a multi-domain provider is rejected outright when any one of its entries does not resolve to a hostname. A new registration or update with invalid data is rejected, while an invalid `defaultSSO` entry prevents Better Auth from starting.
...
An earlier 1.7 prerelease stored `issuer` and `providerAccountId` directly on `account` and exposed Generic OAuth options as `accountIssuer` and `accountSubject`. Those columns already hold the final issuer values, so reuse them and start the 2-table backfill at the collision checks, then rename the options to `identityIssuer` and `identitySubject`. Replace `OAuthAccountKeyContext` and `OAuthAccountSubject` imports with `OAuthIdentityKeyContext` and `OAuthIdentitySubject`.

One subject did change in the same window. That prerelease keyed TikTok on the app-scoped `open_id`, and the final release keys it on `union_id`, so convert those rows as described in the issuer table above before you run the collision checks.
