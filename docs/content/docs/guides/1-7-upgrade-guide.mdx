...
<Callout type="warn">
  ...
  A nonzero count means the database needs repair, not just backfill: drop the compound unique index before re-backfilling, since it was built over the corrupted empty-string values and throws duplicate-key errors as rows resolve to the same real issuer.

  ```sql title="Drop the compound index (MySQL)"
