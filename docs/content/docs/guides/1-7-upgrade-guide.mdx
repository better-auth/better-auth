...
`migrate plan` only reads the database. It prints the adapter and dialect it detected, the changes it would make, and every blocker it found, each with a stable code and the section of this guide that resolves it. `migrate apply` derives what it can from your configuration, asks about the decisions it cannot make, saves the reviewed answers to `better-auth-migration.json`, and lists every irreversible action before it writes anything. A blocked run leaves the database unchanged.
