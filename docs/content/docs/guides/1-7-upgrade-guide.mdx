...
| Account identity           | No schema change by default; an issuer column and compound index when issuer-scoped identity is enabled                 | **Only when opting into issuer-scoped identity**          |
...
<Callout type="warn">
  Do not apply an issuer-scoped schema over a populated 1.6 `account` table before backfilling trusted issuers and resolving collisions. OAuth client and SCIM changes also require their manual data steps.
</Callout>
...
Better Auth 1.7 preserves the 1.6 account identity by default. When `account.identityStrategy` is omitted, an external account remains unique by `providerId` and `accountId`, and the account schema does not add or persist an issuer. You can also make this compatibility policy explicit:
...
Use the same strategy on every application instance that reads or writes the database. Renaming a provider changes its account namespace, and 2 provider aliases for the same OpenID Connect issuer remain separate identities, matching the provider-scoped behavior of 1.6.

#### Adopt issuer-scoped account identity

Set `account.identityStrategy` to `"issuer"` when you deliberately want an external account to be unique by `issuer` and `accountId`:
...
The issuer-scoped account schema adds the required `issuer` field and creates a unique compound index across `issuer` and `accountId`. The `providerId` remains the local provider configuration, while `account.id` identifies the Better Auth account row and `account.accountId` remains the stable identifier assigned by the provider.
