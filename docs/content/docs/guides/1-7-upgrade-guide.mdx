...
<Callout type="warn">
  Inspect and complete the data migration before applying the 1.7 schema or deploying 1.7. The CLI blocks schema writes when it finds populated models that need a reviewed backfill, data move, or reprovision.
</Callout>

The CLI can inspect the built-in SQL database configuration and SQL-backed Drizzle or Prisma adapters. Prepare the 1.7 configuration without changing the 1.6 database:

* **Drizzle:** run `npx auth@rc generate`, review the generated 1.7 schema, and make the adapter import it. Keep existing `camelCase`, `schemaName`, and custom model or field mappings unchanged. Do not run `drizzle-kit migrate` or `drizzle-kit push` yet.
* **Prisma:** run `npx auth@rc generate`, review `schema.prisma`, and run `npx prisma generate` so the configured Prisma Client contains the 1.7 models. Keep existing `@map` and `@@map` mappings unchanged. Do not run `prisma migrate`, `prisma db push`, or `prisma migrate deploy` yet.
* **Built-in SQL database:** update the Better Auth packages, plugins, and configuration; no ORM schema preparation is needed.

After preparing the configuration, inspect the database without changing it:

```bash title="Terminal"
npx auth@rc migrate --dry-run
```

Use the JSON form in CI or when a coding agent is helping with the migration:

```bash title="Terminal"
npx auth@rc migrate --json > better-auth-migration-plan.json
```

A blocked plan exits with status `1` and does not change the database. Its `target` identifies the detected adapter and SQL dialect, which lets a coding agent verify that it is planning against the intended database. The human-readable dry run prints the same target and blocker codes. Resolve each blocker using the applicable section below, then run the inspection again.

Use `migrate --from 1.6` with the explicit decisions described below. The command uses the configured SQL connection for Kysely, Drizzle, or Prisma. When SQLite cannot add the required constraints in place, the migration rebuilds the affected table, preserves its explicit indexes and triggers, and validates foreign keys before committing.

Add `--dry-run` or `--json` to the complete `--from 1.6` command to validate the issuer mappings and OAuth or SCIM decisions without writing. Missing or inconsistent release decisions appear as a `release-migration-preflight` blocker.

PostgreSQL, SQLite, and SQL Server apply the relational data and schema cutover through one transaction-scoped connection. MySQL validates deterministic failures before writing and uses resumable operations because its DDL can commit implicitly. Keep authentication writes stopped for the whole cutover. If an infrastructure or permission error interrupts a MySQL migration, correct the error and rerun the same command with the same reviewed decisions.

<Callout type="warn">
  The release command applies data and schema changes directly. If Prisma Migrate or Drizzle Kit owns your versioned schema history, prepare a matching migration record for the cutover and record it as applied after the CLI succeeds. Do not execute equivalent 1.7 data definition language (DDL) twice.
</Callout>
...
| Account identity           | An issuer column, a renamed provider-account column, and a compound index  | Requires explicit issuers                      |
...
| Provider client store      | `oauthApplication` becomes `oauthClient`, plus new token tables            | Requires explicit migration strategies         |

Account identity, SCIM, and the provider client store require reviewed data decisions. Plan each applicable cutover before you deploy.
...
<Callout type="warn">
  **Migrating from `oidcProvider` or the in-core MCP plugin moves client data.** `@better-auth/oauth-provider` stores registered clients in `oauthClient`, not the old `oauthApplication`, and replaces the combined access/refresh-token layout in `oauthAccessToken` with separate token models. The old and new client tables are not column-compatible: `redirectUrls` becomes the `redirectUris` array, `metadata` becomes JSON, and fields such as `grantTypes` and `tokenEndpointAuthMethod` are new. `auth migrate --from 1.6 --migrate-oauth-clients` performs this mapping and hashes each legacy plaintext client secret. You must also choose `--migrate-oauth-consents` or `--reauthorize-oauth-consents`, and acknowledge token retirement with `--revoke-oauth-tokens`. Dynamically registered clients can re-register instead.

  The old `oauthConsent.scopes` value is a space-delimited string; the new provider stores a string array and ties consent to the new client store. The release migration can convert reviewed consents when client IDs are preserved, or retire them and require users to consent again. The legacy tables are retained with a `__better_auth_1_6` suffix for inspection and manual recovery planning. Pass customized 1.6 physical names with `--legacy-oauth-application-table`, `--legacy-oauth-access-token-table`, and `--legacy-oauth-consent-table`.
</Callout>
...
With the built-in SQL database configuration or a SQL-backed Drizzle or Prisma adapter, pass the reviewed mapping to the release migration:

```bash title="Terminal"
npx auth@rc migrate --from 1.6 \
  --account-issuer credential=local:credential \
  --account-issuer github=https://github.com
```

The command stops before changing data if any populated provider lacks a mapping or if the resulting `(issuer, providerAccountId)` key is not unique.

If your adapter does not expose a SQL migration connection, backfill the account identity manually:
...
The OAuth endpoints move from `/mcp/*` to `/oauth2/*`. Discovery-based MCP clients find the new locations on their own. Existing registered clients move to the shared `oauthClient` store and are not copied automatically; complete the provider-client data step in [Before you upgrade](#before-you-upgrade) before cutover. MCP also defaults `refreshTokenReuseInterval` to 30 seconds for native and public clients; set it to `0` if you want strict refresh-token replay handling.
...
3. Retire whichever legacy plugin models exist. Stable 1.6 uses `scimProvider`. `auth migrate --from 1.6 --reprovision-scim` renames the stable table with a `__better_auth_1_6` suffix. If you adopted prerelease Group provisioning, first back up and drop `scimGroupRoleGrant`, `scimGroupRole`, `scimGroupMember`, and `scimGroup` in that child-first order. Use their physical table names if you customized model names, and pass a customized stable table with `--legacy-scim-provider-table`.
...
5. For the built-in SQL database configuration or a SQL-backed Drizzle or Prisma adapter, run the release migration with `--reprovision-scim` and one `--retire-scim-account` value for every exact account ID in the reviewed inventory. The command retires those account rows, backs up the stable provider table, and creates `scimConnectionBinding`, `scimIdentityTombstone`, `scimSubject`, `scimUser`, `scimGroup`, `scimGroupMember`, and `scimProjectionGrant`. For other adapters, apply the equivalent operations through your database tooling.
...
Update the Better Auth packages and configuration. For Drizzle, update the generated schema used by the adapter without applying it. For Prisma, update `schema.prisma` and regenerate the client without applying the schema. Then inspect without writing:

```bash title="Terminal"
npx auth@rc migrate --json > better-auth-migration-plan.json
```

Review every reported data step and prepare the corresponding release-migration decisions:

* **Backfill account identities:** assign every populated provider a reviewed issuer and resolve collisions before deploying.
* **Move provider state:** choose whether to convert or retire consents. The release migration copies clients, hashes their plaintext secrets, and retires old tokens.
* **Reprovision SCIM:** inventory exact legacy SCIM account IDs and follow the [full cutover](#scim-requires-full-reprovisioning), including reviewed handling of legacy core identities.

Apply the data and schema migration together with the decisions that match your plan. This example migrates OAuth consents and does not include SCIM:
...
If Prisma Migrate or Drizzle Kit tracks your schema history, record the direct release migration before the next deployment and do not apply duplicate DDL. Deploy 1.7 only after the post-migration row counts, collision checks, sign-in checks, inactive legacy-token check, and feature-specific verification pass.
