...
Remove `clientCredentialGrantDefaultScopes`. Administrative client creation and updates now use `client_credentials_scopes`; assigning a non-empty value requires `clientPrivileges` to approve `configure-client-credentials-scopes`. If you adopted the prerelease action name `configure-client-credentials`, rename it to `configure-client-credentials-scopes`; there is no compatibility fallback. DCR, CIMD, and user-managed routes cannot assign it, and removing `client_credentials` from a client clears the stored ceiling.
...
The resource-server challenge API is no longer exception-driven. Replace `raiseResourceServerChallenge` with `createResourceServerChallenge`, which returns an `APIError` for recognized token or scope failures and `undefined` for unrelated errors. Preserve ordinary handler responses and unrelated thrown values instead of converting them into OAuth challenges.

**What to do:** rename protected-operation `scopes` to `requiredScopes` and challenge `scope` to `challengeScopes`. Update any wrapper around `raiseResourceServerChallenge` to handle the new return value.
...
1. Install `@better-auth/mcp`, `@better-auth/cimd`, and the official version 2 MCP client or server package needed by your application. Authorization plugins and protected-request helpers come from the `@better-auth/mcp` package root. MCP protocol and transport code comes from `@modelcontextprotocol/client` and `@modelcontextprotocol/server`; remove imports from the deleted `@better-auth/mcp/client` and `@better-auth/mcp/client/adapters` subpaths.
...
4. Move options that were nested under `oidcConfig` up to the top level of `mcp({ ... })`. Replace `resourceMetadataMappings` with one canonical HTTPS `resource`, for example `resource: "https://api.example.com/mcp"`.
5. Rename `withMcpAuth` to `requireMcpAuth` and `mcpHandler` to `createMcpProtectedRequestHandler`. The standalone factory accepts one flat `McpProtectedRequestHandlerOptions` object with `issuer`, a single `audience`, optional `jwtVerifyOptions`, verification fields, and challenge fields. Its callback receives `accessTokenClaims`. In protected-route options, rename `scopes` to `requiredScopes` and challenge `scope` to `challengeScopes`.
