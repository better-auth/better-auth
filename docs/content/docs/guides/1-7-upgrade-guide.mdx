...
`auth migrate` recognizes 1.6 data and switches to a guided migration that changes data and schema together. It covers the built-in PostgreSQL, MySQL, SQLite, and SQL Server configurations, plus SQL-backed Drizzle and Prisma adapters. Every 1.7 account schema requires `issuer` and `accountId` and enforces a unique compound index across them. A populated 1.6 database must set `account: { identityStrategy: "provider-id" }` before migration so the guided path preserves its logical `(providerId, accountId)` identities. Selecting `"issuer"` for populated 1.6 data is blocked because adopting authority-scoped identity is a re-key migration that needs separate review. An already-migrated v1.7 issuer database remains unchanged when the option is omitted.
...
`migrate plan` only reads the database. It prints the selected and detected account identity strategies, account counts, automatic namespace resolution, projected collisions, schema changes, irreversible release actions, and blockers. Each blocker has a stable code and links to the section of this guide that resolves it. On a populated 1.6 account table, an omitted strategy produces `account-identity-strategy-required`; add the exact provider-id configuration above and run the plan again. Explicit issuer produces `account-identity-strategy-unsupported`. `migrate apply` reruns the same preflight before changing data, asks about other release decisions it cannot make, and saves the reviewed answers to `better-auth-migration.json`. The file identifies the `1.6-to-1.7` transition, so the same choices can be inspected and replayed. A blocked run leaves the database unchanged.
