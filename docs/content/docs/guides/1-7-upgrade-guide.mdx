...
Set `account.identityStrategy` to `"issuer"` before migration only when you deliberately want the 1.7 identity behavior. This can merge provider aliases that share an issuer and changes the namespace observed by systems that use the 1.6 `providerId` and `accountId` pair.
