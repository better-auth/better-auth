...
Organization SCIM connections now require `organizationId`, add a `providerKey` column, and drop the old `userId` column. The `defaultSCIM` option becomes `staticProviders`, `trustedDomains` is removed, and provider IDs are namespaced per organization. This is separate from the personal-connection ownership change above.

**What to do:** run the migration, set `organizationId` on organization connections, rename the `defaultSCIM` config to `staticProviders`, and remove `trustedDomains`.
...
### Organization subscriptions require the organization plugin

`referenceMiddleware` now rejects organization-scoped subscriptions when the organization plugin is absent.

**What to do:** add the organization plugin if you use organization-scoped Stripe subscriptions.
