...
A blocked plan exits with status `1` and does not change the database. Its `target` identifies the detected adapter and SQL dialect, which lets a coding agent verify that it is planning against the intended database. Resolve each blocker using the applicable section below, then run the inspection again.
