...
Better Auth 1.7.0 through 1.7.2 added a required `issuer` column to the `account` table and a unique index on `issuer` and `accountId`. Better Auth 1.7.3 removes that requirement. Accounts are recognized by `providerId` and `accountId`, as in 1.6, and upgrading from 1.6 does not change the account schema.

If you added the `issuer` column while on 1.7.0 through 1.7.2, drop the unique index and the column before deploying 1.7.3. New rows no longer write `issuer`, so a `NOT NULL` column rejects every sign-up and account link. The generated index name is `account_issuer_accountId_uidx`; adjust it if your migration used another name.

```sql title="Postgres and SQLite"
DROP INDEX account_issuer_accountId_uidx;
ALTER TABLE account DROP COLUMN issuer;
```

```sql title="MySQL and SQL Server"
DROP INDEX account_issuer_accountId_uidx ON account;
ALTER TABLE account DROP COLUMN issuer;
```

With Drizzle or Prisma, remove the `issuer` column and the compound unique index from your schema and generate the migration with your usual tooling. If you would rather keep the column, make it nullable and drop the unique index; Better Auth ignores it.
