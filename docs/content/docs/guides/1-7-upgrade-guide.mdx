...
```bash title="Terminal"
npx auth@rc migrate plan
npx auth@rc migrate apply
```

`migrate plan` is read-only. `migrate apply` derives what it can from your configuration, asks about unresolved migration decisions, saves the reviewed answers to `better-auth-migration.json`, and confirms irreversible actions before writing. If blocked, it leaves the database unchanged and explains what to fix.
...
Pass the saved decisions file positionally to review or apply the same migration in another environment:

```bash title="Terminal"
npx auth@rc migrate plan better-auth-migration.json
npx auth@rc migrate apply better-auth-migration.json --yes
```
...
<Callout type="warn">
  Do not run a generated schema migration over populated 1.6 account, OAuth provider, or SCIM tables before running `auth migrate apply`. If you adopted prerelease SCIM Group tables, follow the [SCIM cutover](#scim-requires-full-reprovisioning) before creating the final schema.
</Callout>
...
`auth migrate apply` handles this backfill for supported SQL adapters. If you use another adapter or the CLI reports an identity conflict, apply the following steps with your migration tooling.
...
* Run `npx auth@rc migrate plan`, then `npx auth@rc migrate apply`.
