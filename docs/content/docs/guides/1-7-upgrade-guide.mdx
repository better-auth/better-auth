...
The 1.7 schema adds a nullable `resource` field to `deviceCode` for resource-bound device grants. Apply the generated schema change; existing rows need no backfill.

The schema also indexes `deviceCode` and `userCode` and limits both values to 191 characters. Before applying it on MySQL or SQL Server, change both columns to bounded strings and resolve any existing values longer than 191 characters. Custom `generateDeviceCode` and `generateUserCode` functions must also stay within that limit.
