...
export const myAdapter = (config: CustomAdapterConfig = {}) =>
  createAdapter({
    config: {
      ...
      supportsArrays: true, // Whether the database supports arrays. (Default: true)
      supportsNumbers: true, // Whether the database supports numbers. (Default: true)
...
Note: If `supportsBooleans` is false, and `supportsNumbers` is false, then we will use a string of `"0"` or `"1"` to save the boolean value.

### `supportsArrays`

Whether the database supports arrays. If the database doesn't support arrays, we will use a JSON.strigify `string` to save the array. When we retrieve the data, we will safely parse the `string` back into an array.

### `supportsNumbers`

Whether the database supports numbers. If the database doesn't support numbers, we will use a `string` to save the number. When we retrieve the data, we will safely parse the `string` back into a number.
...
<Callout type="warn">
  If you're using `supportsJSON`, `supportsDates`, `supportsBooleans`, or any others, then
