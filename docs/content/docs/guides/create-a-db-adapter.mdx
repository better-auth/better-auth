...
<Accordion title="schema">
  The `schema` object is the schema that is being used.
</Accordion>
...
<Accordion title="getField">
  The `getField` function is used to get the field name from the schema.
  ...
  Just a note: By default we already transform the field names to the correct field names for the database.
  So it's unlikely you'll need to use this function.
</Accordion>
