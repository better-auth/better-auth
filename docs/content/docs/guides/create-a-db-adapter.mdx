...
<Callout type="info">
  When using your custom adapter, import `betterAuth` from `better-auth/minimal` instead of `better-auth`. The minimal export excludes Kysely and reduces bundle size. See [Bundle Size Optimization](/docs/guides/optimizing-for-performance#bundle-size-optimization) for details.
</Callout>
