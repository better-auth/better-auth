...
export const myAdapter = (config: CustomAdapterConfig = {}) =>
  createAdapter({
    ...
    // Define only when transactions are supported.
    context: {
      // ...
    },
    adapter: ({}) => {
      return {
        create: async ({ data, model, select }, ctx) => {
          // ...
        },
        update: async ({ data, model, select }, ctx) => {
          // ...
        },
        updateMany: async ({ data, model, select }, ctx) => {
          // ...
        },
        delete: async ({ data, model, select }, ctx) => {
          // ...
        },
        // Define only when context is defined.
...
- All methods receive an `ctx` parameter that contains the adapter context. When called within a transaction, this context should include transaction-specific information.
...
```ts title="Example"
create: async ({ model, data, select }, ctx) => {
  // Example of inserting data into the database.
  return await ctx.db.insert(model).values(data);
};
```
...
```ts title="Example"
update: async ({ model, where, update }, ctx) => {
  // Example of updating data in the database.
  return await ctx.db.update(model).set(update).where(where);
};
```
...
```ts title="Example"
updateMany: async ({ model, where, update }, ctx) => {
...
```ts title="Example"
delete: async ({ model, where }, ctx) => {
  // Example of deleting a record from the database.
  await ctx.db.delete(model).where(where);
}
```
...
```ts title="Example"
deleteMany: async ({ model, where }, ctx) => {
  // Example of deleting multiple records from the database.
  return await ctx.db.delete(model).where(where);
};
```
...
```ts title="Example"
findOne: async ({ model, where, select }, ctx) => {
  // Example of finding a single record in the database.
  return await ctx.db.select().from(model).where(where).limit(1);
};
```
...
```ts title="Example"
findMany: async ({ model, where, limit, sortBy, offset }, ctx) => {
...
```ts title="Example"
count: async ({ model, where }, ctx) => {
  // Example of counting the number of records in the database.
  return await ctx.db.select().from(model).where(where).count();
};
```

### `transaction` method (optional)

The `transaction` method allows you to execute multiple database operations within a single transaction. This ensures data consistency by rolling back all changes if any operation fails.

<Callout>
If your database doesn't support transactions or you want to opt out of using them, you can set `bypassTransactions: true` in your adapter config. When transactions are bypassed, the callback will be executed with the current context without any transaction wrapping.
</Callout>

parameters:

- `callback`: A function that receives a transaction context and performs database operations.
- `ctx`: The current adapter context.

<Callout>
The transaction context is the same type as your adapter context but may contain transaction-specific information.
</Callout>

```ts title="Example"
transaction: async (callback, ctx) => {
  // Example of implementing database transactions
...
<Callout type="warn">
If your adapter doesn't implement the `transaction` method, Better-Auth will automatically bypass transactions and execute the callback with the current context.
</Callout>

#### Using Transactions in Practice

Transactions are automatically used by Better-Auth for operations that require data consistency.

When implementing an adapter you should ensure that your adapter methods properly use the transaction context when provided.

```ts title="Example"
create: async ({ model, data, select }, ctx) => {
  // Example of inserting data into the database with transaction support
  return await ctx.db.insert(model).values(data);
};
```

<Callout>
For databases that don't support transactions (like Cloudflare D1), set `bypassTransactions: true` in your adapter config to avoid transaction-related errors.
</Callout>
...
Opt out of using database transactions. When set to `true`, any transaction calls will execute the callback with the current context without any transaction wrapping.

<Callout>
This is particularly useful for databases that don't support transactions or when you want to disable transaction functionality for performance reasons.
</Callout>

```ts title="Example"
// Disable transactions for databases that don't support them
const adapter = myAdapter({
  // This value then gets passed into the `bypassTransactions`
  // option in the createAdapter `config` object.
