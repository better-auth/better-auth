...
export const auth = betterAuth({
	hooks: {
		before: createAuthMiddleware(async (ctx) => {
			// Check that the request is for the sign-up/email endpoint
			if (ctx.path !== "/sign-up/email") return;

			// Check that the image field is present
			if (!ctx.body.image) return;
			const img = ctx.body.image;

			// Validate that the image URL is a valid URL
			const url_validator = z.string().url();
			if (!url_validator.safeParse(img).success) {
...
Proper testing ensures not only a secure application, but also a smooth user experience that fails safely under edge conditions.
