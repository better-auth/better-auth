...
Better Auth loads Kysely lazily, only when you pass a raw database connection. If you configure `database` with an ORM adapter (Drizzle, Prisma, or MongoDB), Kysely is never loaded or bundled, so there is nothing extra to do for a smaller bundle.

Apps that pass a raw connection (the built-in adapter) should install Kysely with `npm install kysely`.
