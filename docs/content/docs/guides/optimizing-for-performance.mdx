...
If you're using custom adapters (like [Prisma](/docs/adapters/prisma), [Drizzle](/docs/adapters/drizzle), or [MongoDB](/docs/adapters/mongo)), you can reduce your bundle size by using `better-auth/minimal` instead of `better-auth`. This version excludes Kysely, which is only needed when using direct database connections.
