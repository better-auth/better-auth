...
<Callout type="warning">
	**Limitations:** - Direct database connections are not supported (you must use
	an adapter) - Built-in migrations are not supported. Use external migration
	tools (or use `better-auth` if you need built-in migration support)
</Callout>
