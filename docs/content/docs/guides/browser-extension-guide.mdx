...
Instead, call authentication functions from your background script, and forward responses to your content script.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "getSession") {
    authClient.getSession().then(session => { // [!code highlight]
      sendResponse(session) // [!code highlight]
    }) // [!code highlight]
...
```typescript title="content-script.ts"
