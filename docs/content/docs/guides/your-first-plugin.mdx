...
<Steps>
  ...
  <Step>
    ...
    <Callout type="info">
      Publishing a reusable plugin? <Link href="/docs/concepts/plugins#register-a-plugin-type">Register its server type</Link> for type-safe access through `getPlugin()` and `hasPlugin()`.
    </Callout>
  </Step>
