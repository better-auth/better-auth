...
<Steps>
  ...
  <Step>
    ...
    <Callout type="info">
      Publishing a reusable plugin? <Link href="/docs/concepts/plugins#register-a-plugin-type">Register its server type</Link> so `getPlugin()` can infer its exact return type.
    </Callout>
  </Step>
