...
<Callout type="warn">
	...
	<Link href="/docs/installation">setup the basics</Link> of Better Auth and are
...
In the `index.ts` file, we’ll export a function that represents our server
plugin. This will be what we will later add to our plugin list in the `auth.ts`
file.
...
<Callout type="info">
	...
	<Link href="/docs/concepts/plugins#schema">plugin schemas here</Link>.
</Callout>
...
Then, add the following code: ```ts title="client.ts" import
{BetterAuthClientPlugin} from "better-auth"; import type {birthdayPlugin} from
"./index"; // make sure to import the server plugin as a type // [!code
highlight]
