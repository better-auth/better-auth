...
<Steps>
  ...
  <Step>
    ...
    const CONFIG = {
       ...
       * trusted issuer Better Auth uses, or local:<providerId> when it has none.
       */
      accountIssuers: {
        github: 'local:github',
      ...
      try {
          ...
          for (const identity of user.identities ?? []) {
            ...
            if (supportedProviders.includes(identity.provider)) {
              accountsData.push({
                ...
                providerAccountId: identity.identity_data?.sub || identity.provider_id,
                password: null,
        ...
        if (accountsData.length > 0) {
          ...
          for (let i = 0; i < accountsData.length; i += accountsPerChunk) {
            ...
            await toDB.query(
              `
              INSERT INTO "account" ("id", "userId", "providerId", "issuer", "providerAccountId", "password", "createdAt", "updatedAt")
    ...
    for (const identity of user.identities ?? []) {
      ...
      if (identity.provider.startsWith('sso:')) { // [!code highlight]
        ...
        accountsData.push({ // [!code highlight]
          ...
          issuer: `local:sso-${supabaseProviderId}`, // [!code highlight]
          providerAccountId: identity.identity_data?.sub || identity.provider_id, // [!code highlight]
