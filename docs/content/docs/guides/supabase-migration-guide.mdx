...
<Steps>
  ...
  <Step>
    ...
    const CONFIG = {
      ...
      /**
       * Review every social provider in your Supabase export. Use the exact
       * trusted issuer Better Auth uses, or local:oauth:<encoded providerId> when it has none.
       */
      identityIssuers: {
    ...
    function getIdentityIssuer(providerId: string) {
      const issuer = CONFIG.identityIssuers[providerId];
      if (!issuer) {
    ...
    async function processBatch(
      ...
      try {
        ...
        const identitiesData: IdentityInsertData[] = [];
        const accountsData: AccountInsertData[] = [];
        const identitiesByKey = new Map<string, { id: string; userId: string }>();

        function registerIdentity(
          data: Omit<IdentityInsertData, 'id'>,
        ): string {
          const identityKey = `${data.issuer}\u0000${data.providerAccountId}`;
          const existingIdentity = identitiesByKey.get(identityKey);
          if (existingIdentity) {
            if (existingIdentity.userId !== data.userId) {
              throw new Error(`Identity ${data.issuer}/${data.providerAccountId} belongs to several users`);
            }
            return existingIdentity.id;
          }

          const id = generateId();
          identitiesByKey.set(identityKey, { id, userId: data.userId });
          identitiesData.push({ id, ...data });
          return id;
        }
        ...
        for (const { user } of validUsersData) {
          for (const identity of user.identities ?? []) {
            if (identity.provider === 'email') {
              const identityId = registerIdentity({
                userId: user.id,
            ...
            if (supportedProviders.includes(identity.provider)) {
              const issuer = getIdentityIssuer(identity.provider);
              const providerAccountId = identity.identity_data?.sub || identity.provider_id;
              const identityId = registerIdentity({
                userId: user.id,
                issuer,
                providerAccountId,
              ...
              accountsData.push({
                ...
                providerInstanceId: identity.provider,
                password: null,
                createdAt: identity.created_at ?? user.created_at,
                updatedAt: identity.updated_at ?? user.updated_at,
              });
        ...
        if (identitiesData.length > 0) {
          const maxParamsPerQuery = 65000;
          const fieldsPerIdentity = 6;
          const identitiesPerChunk = Math.floor(maxParamsPerQuery / fieldsPerIdentity);

          for (let i = 0; i < identitiesData.length; i += identitiesPerChunk) {
            const chunk = identitiesData.slice(i, i + identitiesPerChunk);
            const placeholders: string[] = [];
            ...
            await toDB.query(
              `
              INSERT INTO "identity" ("id", "userId", "issuer", "providerAccountId", "createdAt", "updatedAt")
              VALUES ${placeholders.join(', ')}
              ON CONFLICT ("id") DO NOTHING
            `,
              values,
        ...
        if (accountsData.length > 0) {
          ...
          for (let i = 0; i < accountsData.length; i += accountsPerChunk) {
            ...
            const accountValues: Array<string | null> = [];
            let paramIndex = 1;
            ...
            await toDB.query(
              `
              INSERT INTO "account" ("id", "identityId", "providerId", "providerInstanceId", "password", "createdAt", "updatedAt")
...
<Steps>
  ...
  <Step>
     ...
     * @param ownerUserId - Existing Better Auth user that owns the provider records
     */
    async function migrateSSOProviders(
      ...
      for (const provider of providers) {
        ...
        const providerRecordId = provider.id;
        const serviceProviderIssuer = `${betterAuthUrl}/api/auth/sso/saml2/sp/metadata?providerId=${providerId}`;
        ...
        try {
          await toDB.query(`
            INSERT INTO "ssoProvider" (
              "id", "userId", "providerId", "issuer", "domain",
          ...
          console.log(`      Service provider issuer: ${serviceProviderIssuer}\n`);
        } catch (error: any) {
  ...
  <Step>
    ...
    for (const identity of user.identities ?? []) {
      ...
      if (identity.provider.startsWith('sso:')) { // [!code highlight]
        ...
        const providerId = `sso-${supabaseProviderId}`; // [!code highlight]
        const providerInstanceId = `sso:provider:${supabaseProviderId}`; // [!code highlight]
        const providerAccountId = identity.identity_data?.sub || identity.provider_id; // [!code highlight]
        const identityId = registerIdentity({ // [!code highlight]
          userId: user.id, // [!code highlight]
          issuer: `${providerInstanceId}:saml`, // [!code highlight]
          providerAccountId, // [!code highlight]
        ...
        accountsData.push({ // [!code highlight]
          ...
          identityId, // [!code highlight]
          providerId, // [!code highlight]
          providerInstanceId, // [!code highlight]
  ...
  <Step>
    ...
    // Use a user from the Supabase export who should own the provider records.
    const SSO_OWNER_USER_ID = process.env.BETTER_AUTH_SSO_OWNER_USER_ID;
    if (!SSO_OWNER_USER_ID) {
      throw new Error('BETTER_AUTH_SSO_OWNER_USER_ID is required');
    }

    async function main() {
      // Migrate users first so the selected SSO provider owner exists.
      await migrateFromSupabase();

      // The deterministic provider record IDs already match the migrated Accounts.
      await migrateSSOProviders(providers, BETTER_AUTH_URL, SSO_OWNER_USER_ID);
    }
