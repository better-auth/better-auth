...
<Callout type="info">
  `cookiePrefix` is deprecated across all Electron plugins and will be removed in a future minor release. Do not set both options.
</Callout>
...
Cookie namespace. Defaults to `better-auth`.
...
Security policy used to read the redirect cookie. Match the server's explicit `advanced.cookieSecurity` value.
...
Namespace(s) used to identify Better Auth cookies. Defaults to `better-auth`.
...
```ts title="electron/auth-client.ts"
electronClient({
  cookieNamespace: ["better-auth", "my-app"],
});
```
