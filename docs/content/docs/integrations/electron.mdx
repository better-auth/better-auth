...
<Callout type="warn">
  The Electron browser-auth handoff currently requires sign-in to be fully completed before the authorization code is minted. If your app uses the [2FA plugin](/docs/plugins/2fa), the native Electron handoff does not yet continue `/two-factor/verify-*` back into Electron.

  Complete the 2FA challenge in the browser first, then use `electron.transferUser()` from an already authenticated web session to hand the user off to Electron.
</Callout>
...
<Callout type="warn">
  `electron_authorization_code` is only available after the browser flow has an authenticated session. If the browser flow stops on a 2FA challenge, no authorization code is generated yet.
</Callout>
