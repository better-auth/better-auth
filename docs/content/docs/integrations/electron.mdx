...
<Steps>
    ...
    set to `true` to ensure NodeJS APIs aren't exposed to any JavaScript running in the renderer.
    ...
    This will setup:
    - **IPC bridges** for authentication requests from the renderer process.
...
ipcMain.handle("customBridge", async (_event, data) => {
...
- **customClientID**: The client ID to use for identifying the Electron client during authorization. Defaults to `electron`.
...
- **cookiePrefix**: The prefix to use for cookies set by the plugin. Note that this also should include the `__Secure` or `__Host` prefix depending on your configuration.
