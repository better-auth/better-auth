...
<Callout type="warn">
	Make sure to export the auth instance with the variable name `auth` or as a
	`default` export.
</Callout>
...
<Callout type="info">
	...
	to keep it as `src/pages/api/auth/[...route].ts`
</Callout>
...
<Callout type="warn">
	...
	[cookie cache](/docs/concepts/session-management#cookie-cache) will not be
	refreshed until the server is interacted with from the client via Server
