...
The middleware will add a boolean flag to subsequent routes. When enabled, the above function will run and the `session` and `user` information will be available in your context. For custom error handling feel free to update the logic inside the session statement.  
