...
<Callout type="info">
  Cookie helpers do not read your auth configuration. Pass matching options when you customize them.
  ...
  getSessionCookie(request, cookieConfig);
  getCookieCache(request, cookieConfig);
  ```
</Callout>
