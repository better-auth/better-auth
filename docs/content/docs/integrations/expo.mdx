...
By default, the cookie is stored in `SecureStore` with the key `better-auth_cookies`. You can change this key by setting a custom `storagePrefix` in the client configuration. If you do, ensure you retrieve the cookie using the new prefix (e.g., `myapp_cookies`).

```tsx
import * as SecureStore from 'expo-secure-store';

const getCookies = async () => {
  const cookieName = 'better-auth_cookies'; // Update if using a custom prefix
      ...
      links: [
        httpBatchLink({
          ...
          headers() {
            ...
            const cookies = SecureStore.getItemAsync ("better-auth_cookies"); // [!code highlight]
