...
**cookieNamespace**: Namespace(s) used to identify Better Auth cookies. Accepts a string or an array and defaults to `"better-auth"`.

`cookiePrefix` is deprecated and will be removed in a future minor release. Do not set both options.

**cookieSecurity**: Security policy used to read OAuth state cookies. Match the server's explicit `advanced.cookieSecurity` value.
...
const authClient = createAuthClient({
    ...
    plugins: [
        expoClient({
            storage: SecureStore,
            // Single namespace
            cookieNamespace: "better-auth"
        })
    ]
});
```

You can also provide multiple namespaces to match cookies from different authentication systems:
...
const authClient = createAuthClient({
    ...
    plugins: [
        expoClient({
            storage: SecureStore,
            // Multiple namespaces
            cookieNamespace: ["better-auth", "my-app", "custom-auth"]
        })
    ]
});
```
...
<Callout type="info">
  Custom plugin cookies must match a configured namespace. See the [Passkey integration](/docs/plugins/passkey#expo-integration).
</Callout>
