...
**CORS `exposeHeaders` Configuration**: Better Auth automatically manages the `access-control-expose-headers` header to expose authentication-related headers. If you need to use the `exposeHeaders` option in your CORS configuration, you **must** include the following headers that Better Auth plugins use:

- `set-auth-token` (if using the [Bearer plugin](/docs/plugins/bearer))
- `set-auth-jwt` (if using the [JWT plugin](/docs/plugins/jwt))
- `set-ott` (if using the One-time-token plugin)

For example, if you're using the Bearer plugin:
...
cors({
	...
	allowHeaders: ["Content-Type", "Authorization"],
	allowMethods: ["POST", "GET", "OPTIONS"],
	exposeHeaders: ["set-auth-token"], // Include Better Auth headers
	maxAge: 600,
	credentials: true,
})
```

If you omit `exposeHeaders` from your CORS configuration, Better Auth will automatically handle exposing the necessary headers.
</Callout>
