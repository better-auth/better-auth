...
```vue title="app/pages/login.vue"
<script setup lang="ts">
import { authClient } from "~~/lib/auth-client";
import { Auth } from "better-auth/vue";

const ui = authClient.ui.signIn();

function handleSuccess(data: { redirectTo?: string }) {
  navigateTo(data.redirectTo || "/dashboard");
}
</script>

<template>
  <div class="flex items-center justify-center min-h-screen">
    <Auth :ui="ui" @success="handleSuccess" />
  </div>
</template>
```
