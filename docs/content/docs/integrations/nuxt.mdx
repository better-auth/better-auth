...
This SSR overload uses the read-only `GET /get-session` endpoint so rendering never mutates the session or attempts to write cookies. Trigger `authClient.refreshSession()` from the browser when you need an explicit sliding-session refresh.
