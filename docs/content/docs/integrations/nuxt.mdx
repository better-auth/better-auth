...
sidebarTitle: "Nuxt"
title: Nuxt
description: Add authentication with Better Auth.
---

## Install the module

[`@nuxtjs/better-auth`](https://better-auth.nuxt.dev) is the recommended integration. It mounts the auth handler and provides session composables for SSR and the browser.

<Callout type="info">
  The Nuxt Modules team maintains this integration outside the Better Auth repository. Report module-specific issues in [`nuxt-modules/better-auth`](https://github.com/nuxt-modules/better-auth/issues).
</Callout>
...
The command adds the module to `nuxt.config.ts` and creates `server/auth.config.ts` and `<srcDir>/auth.config.ts`, usually `app/auth.config.ts`. Continue with the [configuration guide](https://better-auth.nuxt.dev/getting-started/configuration) to set up your secret, authentication methods, and database.

## Manual setup

<Accordions>
  <Accordion id="manual-setup-details" title="View manual setup">
