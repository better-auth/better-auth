---
title: MongoDB Adapter
description: Integrate Better Auth with MongoDB.
---

MongoDB is a popular NoSQL database that is widely used for building scalable and flexible applications. It provides a flexible schema that allows for easy data modeling and querying.

Before getting started, make sure you have MongoDB installed and configured. For more information, see [MongoDB Documentation](https://www.mongodb.com/docs/)

## Installation

To use the MongoDB adapter, you need to install the `@better-auth/mongo-adapter` package:

```package-install
@better-auth/mongo-adapter
```

## Example Usage

You can use the MongoDB adapter to connect to your database as follows.

```ts title="auth.ts"
import { betterAuth } from "better-auth/minimal"; // [!code highlight]
import { MongoClient } from "mongodb";
import { mongodbAdapter } from "better-auth/adapters/mongodb";

const client = new MongoClient("mongodb://localhost:27017/database");
const db = client.db();

export const auth = betterAuth({
  database: mongodbAdapter(db, {
    // Optional: if you don't provide a client, database transactions won't be enabled.
    client
  }),
});
```

## Schema generation & migration

For MongoDB, we don't need to generate or migrate the schema.

## Joins (Experimental)

Database joins is useful when Better-Auth needs to fetch related data from multiple tables in a single query.
Endpoints like `/get-session`, `/get-full-organization` and many others benefit greatly from this feature,
seeing upwards of 2x to 3x performance improvements depending on database latency.

The MongoDB adapter supports joins out of the box since version `1.4.0`.
To enable this feature, you need to set the `experimental.joins` option to `true` in your auth configuration.

```ts title="auth.ts"
import { betterAuth } from "better-auth";

export const auth = betterAuth({
  experimental: { joins: true }
});
```

## Bundle Size Optimization

When using the MongoDB adapter, import `betterAuth` from `better-auth/minimal` instead of `better-auth`. The minimal export excludes Kysely, which is only needed for direct database connections. This can significantly reduce your bundle size in serverless and edge environments.

<Callout type="info">
  Use `better-auth` (full) instead if you need built-in Kysely migrations via `runMigrations()` or `npx auth@latest migrate`. See the [bundle size optimization guide](/docs/guides/optimizing-for-performance#bundle-size-optimization) for more details.
</Callout>

## Additional Information

* We recommend using `better-auth/minimal` with the MongoDB adapter to reduce bundle size. See [Bundle Size Optimization](#bundle-size-optimization) above.
* If you're looking for performance improvements or tips, take a look at our guide to <Link href="/docs/guides/optimizing-for-performance">performance optimizations</Link>.
