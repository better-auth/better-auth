...
The Kysely adapter enables interactive transactions by default. Better Auth requires an interactive transaction or a database-native atomic batch for lifecycle operations that change related User, Identity, and Account records.

Set `transaction: false` only when your dialect cannot run interactive transactions. Without another atomic capability, Better Auth rejects these multi-record mutations before writing.
