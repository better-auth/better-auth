...
When using the Drizzle adapter, import `betterAuth` from `better-auth/minimal` instead of `better-auth`. The minimal export excludes Kysely, which is only needed for direct database connections. This can significantly reduce your bundle size in serverless and edge environments.
...
* We recommend using `better-auth/minimal` with the Drizzle adapter to reduce bundle size. See [Bundle Size Optimization](#bundle-size-optimization) above.
