...
export const auth = betterAuth({
  database: drizzleAdapter(db, { // [!code highlight]
    provider: "pg", // or "mysql" or "sqlite" // [!code highlight]
  }), // [!code highlight]
...
The adapter defaults to Drizzle's asynchronous transaction API. Set `transaction` to match the database driver:

* Use `"async"` for PostgreSQL, MySQL, and asynchronous SQLite drivers.
* Use `"sync"` for synchronous SQLite drivers such as `node:sqlite` and `better-sqlite3`.
* Use `false` when interactive transactions are unavailable. Cloudflare D1 still uses its atomic `batch()` capability.

This setting is a capability declaration. Choosing `"async"` for a synchronous driver can commit before an asynchronous authentication callback finishes.
...
Model identifiers ending in a consonant followed by `y` use `ies`, so the `identity` model maps to `identities`. Generated Drizzle relation keys use the same pluralization.
