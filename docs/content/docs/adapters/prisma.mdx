...
<Callout type="info">
  ...
  unchanged. Prisma 8 (Prisma Next) is not supported yet.
</Callout>
...
If you are starting a new Prisma project, install the Prisma 7 CLI:

```package-install title="Install Prisma CLI"
-D prisma@7
```

Then initialize it with PostgreSQL and an explicit Prisma Client output path:

```package-install title="Initialize Prisma"
npx prisma init --datasource-provider postgresql --output ../src/generated/prisma
```

This creates `prisma/schema.prisma`, `prisma7.config.ts`, and `.env`. Prisma
versions before 7.10 name the config file `prisma.config.ts`. Set
