...
The adapter uses Prisma interactive transactions by default. Better Auth requires them for lifecycle operations that create or remove related User, Identity, and Account records.

Set `transaction: false` only when the database driver cannot run interactive transactions. Without transaction support or a database-native atomic batch capability, Better Auth rejects multi-record identity mutations before writing.
