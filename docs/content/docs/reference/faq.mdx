...
<Accordion title='Google One Tap: "Server did not send the correct CORS headers"'>
If you're encountering FedCM-related errors with Google One Tap in Chrome 108+:
...
This happens because Chrome is trying to use the new FedCM (Federated Credential Management) API, which requires additional server endpoints.

**Quick Fix** - Disable FedCM (recommended for most users):

```ts title="auth-client.ts"
oneTapClient({
  clientId: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID!,
  additionalOptions: {
    use_fedcm_for_prompt: false  // Force traditional mode
  }
})
```

**Alternative** - Enable FedCM support on your server (advanced):
...
See the <Link href="/docs/plugins/one-tap#fedcm-support">One Tap plugin documentation</Link> for more details.
</Accordion>
