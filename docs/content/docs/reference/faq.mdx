...
<Accordion title="Field renaming (fields config) not working as expected">

The `fields` configuration in your auth config **only affects database column names**, not the field names you use in your JavaScript/TypeScript code.
...
**In your database:** columns will be named `email_verified` and `created_at`

**In your code:** you must still use `emailVerified` and `createdAt`
...
This design keeps your TypeScript types consistent and type-safe while allowing you to use any naming convention in your database (useful for migrations or matching existing schemas).

For detailed information and more examples, see the <Link href="/docs/concepts/database#understanding-field-mapping">Understanding Field Mapping</Link> section in the database documentation.
