...
* `scope`: Extra OAuth scopes to request, added on top of the provider's default scopes (optional). The scopes a user actually grants are accumulated on the account's `grantedScopes` array. For union-only providers (generic OAuth, most social providers, and Google with `includeGrantedScopes: false`), the stored grant only grows: a narrower sign-in or refresh response never shrinks it. The one exception is Google with the default `includeGrantedScopes: true`, where each sign-in resyncs `grantedScopes` to Google's authoritative combined grant, so a scope the user revoked at Google drops on the next sign-in. Token refresh never changes the grant for any provider. Use `signIn.social({ scopes })` or `linkSocial({ scopes })` to request additional scopes per request, which are also additive to the defaults.
...
* `includeGrantedScopes`: (Google only) Controls Google's server-side incremental authorization via `include_granted_scopes` (default: `true`). When enabled, Google's token response is the authoritative combined grant for the project, so on each sign-in the account's `grantedScopes` is resynced (replaced) to it: scopes the user revoked at Google are reflected and the stored grant can shrink. Set to `false` to authorize only the scopes requested in the current flow; the stored grant then becomes union-only and only ever grows.
...
* `logoUrl`: A single image URL or `{ light: string; dark: string }` object rendered next to the app name and used as the UI favicon. When an object is provided, Better Auth renders the light logo by default, uses the dark logo for dark UI, and emits `prefers-color-scheme` favicon links so the browser can switch the favicon for light and dark CSS theme preferences.
...
* `darkMode`: `"auto"` follows the user's system preference. `true` renders dark by default, and `false` renders light only.
* `dark`: Optional dark-mode overrides for the same color tokens (`background`, `surface`, `text`, `textSecondary`, `border`, `primary`, `error`, `success`).
* `borderRadius`: Controls shared component radius tokens, including Auth UI inputs and buttons.
