...
export const auth = betterAuth({
	advanced: {
		...
		database: {
			...
			// or use "serial" to use your database's auto-incrementing ID,
			// or "uuid" to use a random UUID.
			// or "uuidv7" to use a time-ordered UUID (requires PostgreSQL 18+ for database generation).
			generateId: (((options: {
				model: LiteralUnion<Models, string>;
				size?: number;
			}) => {
				return "my-super-unique-id";
			})) | false | "serial" | "uuid" | "uuidv7",
