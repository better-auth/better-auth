...
* `emailValidator`: A synchronous or asynchronous function that replaces the default email syntax validator and receives the email normalized to lowercase
...
export const auth = betterAuth({
	account: {
		modelName: "accounts",
		identityStrategy: "provider-id", // Preserve Better Auth 1.6 provider-scoped identity
		fields: {
			userId: "user_id"
		},
...
Controls the namespace Better Auth persists in the required `issuer` field when recognizing an external account. providerId identifies the configured connection; accountId is the provider subject; issuer stores the identity namespace—verified authority under issuer strategy, deterministic provider namespace under provider-id strategy.

The `auth init` command generates `account: { identityStrategy: "provider-id" }` so a new project has an explicit identity policy. In v1.7.x, omitting `identityStrategy` remains a compatibility mode that uses `"issuer"`; Better Auth emits this warning once per auth instance:
...
Set it to `"provider-id"` when migrating populated 1.6 data and you need to preserve logical `(providerId, accountId)` recognition. Better Auth then persists `local:oauth:<encoded providerId>` as the issuer namespace after the provider response has been verified. This option does not change the Account schema: `issuer` and `accountId` remain required and uniquely indexed together under both strategies.

Changing this option on populated v1.7 data requires a separate, reviewed account re-key migration. It is not a configuration-only toggle.
