...
<Callout type="warn">
	...
	always set `baseURL` explicitly in your config or via the `BETTER_AUTH_URL`
...
<Callout type="info">
	The `request` parameter is `undefined` during initialization and when calling
	`auth.api` directly. Make sure to handle this case by returning default
	trusted origins.
</Callout>
