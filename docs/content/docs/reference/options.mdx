...
Controls the database key Better Auth uses to recognize a provider account. The default `"provider-id"` strategy uses `(providerId, accountId)` and does not add or persist an `issuer` field. Set `"issuer"` to use `(issuer, accountId)` and include the required issuer field and compound index in generated schemas. Keep the value consistent across every instance connected to the database, and follow the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#choose-account-identity-strategy) before changing it on existing data.
