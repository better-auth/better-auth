...
export const auth = betterAuth({
	session: {
		...
		deleteSessionOnSignOut: {
			enabled: false, // Controls whether the session is deleted on sign-out. If false, the session remains in the database but is marked inactive via `isActive`. (default: `true`)
			timestamp: true // Store the field to determine whether a session is active or not using a timestamp instead of boolean (default: `false`)
		},
	},
})
```
...
- `deleteSessionOnSignOut`: Configure whether sessions are deleted on sign-out or preserved (set `enabled` to `false` to keep history, default: `true`). Requires database migration to add `isActive` or `invalidatedAt` column when preserving sessions.
