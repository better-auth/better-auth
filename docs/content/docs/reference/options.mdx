    ...
    oauth: {
        stateManagement: {
            generateState: async (context, payload: OAuthStatePayload) => {
                ...
                // Return undefined to use default database approach
            },
...
Configuration for custom [OAuth state management](/docs/concepts/oauth#oauth-state-management), particularly when used in conjunction with the [OAuth Proxy Plugin](/docs/plugins/oauth-proxy).

- `generateState`: Function to generate custom OAuth state. Receives the endpoint context and OAuth payload. Return `undefined` to use the default database approach.
- `parseState`: Function to parse custom OAuth state. Receives the endpoint context and raw state string. Return `undefined` to use the default database approach.

If the `stateManagement` configuration is missing, or if either function returns `undefined`, Better Auth automatically falls back to the default database-based state management using the `verification` table.
