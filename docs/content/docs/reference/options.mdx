...
<Callout type="info">
  Passing a raw database connection uses Better Auth's built-in Kysely adapter.
  Install Kysely alongside Better Auth with `npm install kysely`. ORM adapters
  (Drizzle, Prisma, MongoDB) don't need it.
</Callout>
