...
export const auth = betterAuth({
	advanced: {
		...
		cookiePrefix: "myapp",
		database: {
			// Use your own custom ID generator, disable generating IDS so your database will generate them, or use "serial" to use your database's auto-incrementing ID, or "uuid" to use a random UUID.
			generateId: (((options: {
