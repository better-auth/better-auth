...
Plugins can expose `ui.pages` for full pages and `ui.capabilities` for data-oriented integration with first-party Auth UI pages. Capabilities expose route descriptors and metadata that Auth UI reads explicitly with `ctx.capability("plugin-id")`; they are not arbitrary UI injection points.
