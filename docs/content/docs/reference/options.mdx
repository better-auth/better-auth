...
export const auth = betterAuth({
	session: {
		...
		onSessionExpired: async ({ session, user }, ctx) => {
			// Called when Better Auth detects an expired session
		},
	},
})
```
...
* `onSessionExpired`: Callback invoked when Better Auth detects an expired session during validation (including `getSession`, `findSession`, and OAuth Provider checks). Receives `{ session, user }` and the endpoint `ctx` (`GenericEndpointContext`). Not called for explicit revocation (sign-out, revoke).
