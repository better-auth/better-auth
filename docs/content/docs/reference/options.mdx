...
export const auth = betterAuth({
	onAPIError: {
		...
		errorURL: "/auth/error",
		errorUrlBuilder: ({ error, baseURL }) => `${baseURL}?code=${error}`,
...
* `errorURL`: URL to redirect to on error (default: `/api/auth/error`). Existing query parameters are preserved; `error` is merged in rather than concatenated with a second `?`.
* `errorUrlBuilder`: Optional function that receives `{ error, error_description, baseURL }` and returns the **final** redirect URL. When set, Better Auth does not append `error` afterwards.
