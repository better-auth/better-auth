...
export const auth = betterAuth({
	account: {
		modelName: "accounts",
		identityStrategy: "provider-id", // Explicit Better Auth 1.6 compatibility path
		fields: {
			userId: "user_id"
		},
...
Controls the logical namespace Better Auth uses to recognize a provider account. Better Auth 1.7.x defaults an omitted `identityStrategy` to `"issuer"`, which stores the verified authority in `issuer`. Set `"provider-id"` as an explicit Better Auth 1.6 compatibility path; it stores a deterministic `local:oauth:<encoded providerId>` namespace in `issuer` and preserves logical `(providerId, accountId)` recognition.

Both strategies use the same Account schema: required `issuer`, required `accountId`, and a unique compound index on `(issuer, accountId)`. The option changes the logical identity namespace, not the schema. providerId identifies the configured connection; accountId is the provider subject; issuer stores the identity namespace—verified authority under issuer strategy, deterministic provider namespace under provider-id strategy.

Keep the value consistent across every instance connected to the database. Changing the strategy on populated 1.7 data is a re-key migration, not a configuration-only toggle. Follow the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#choose-account-identity-strategy) before migrating existing data.
