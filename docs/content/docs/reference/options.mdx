...
export const auth = betterAuth({
	user: {
		...
		enrollment: {
			enabled: true,
			sendEnrollmentVerification: async ({ user, url, token, invitation }) => {
				// Send enrollment verification
			}
		}
	},
})
...
* `enrollment`: Configuration for passwordless enrollment (see [Enrollment](/docs/concepts/users-accounts#enrollment))
