...
* `validateSchema`: Check the database schema against the configuration and report every required change at once instead of failing on the first insert. Core table problems throw, plugin table problems warn. Drizzle and Prisma check their schema definition when the adapter is created; the built-in database runs one introspection query before the first database call. See the [Database documentation](/docs/concepts/database#schema-validation) for more info. (default: `true`)
