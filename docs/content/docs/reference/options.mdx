...
The canonical, stable URL where this Better Auth server is hosted, as a string. It anchors identity-bearing values that must not change between requests: the OAuth/OIDC issuer, JWT `iss`/`aud`, the social-login `redirect_uri`, and the Passkey relying-party id.

If you include a path in the baseURL string, it takes precedence over the default path.
...
If not explicitly set, the system checks the environment variable `BETTER_AUTH_URL`. If that's also not set, the origin is inferred from the incoming request.
...
### Multiple domains, previews, and white-label

Keep `baseURL` fixed and list the additional hosts in [`trustedOrigins`](#trustedorigins) (a static array or a per-request function). Cookies and self-referential links (email verification, magic links, password reset) then follow the host the request arrived on, while identity stays anchored to `baseURL`.
...
For deployment patterns and end-to-end examples, see the [Multiple Domains guide](/docs/guides/dynamic-base-url).
...
export const auth = betterAuth({
	trustedOrigins: async (request) => {
		// request is undefined during initialization and during auth.api calls
		// that pass no headers; it is present (synthesized from the headers) when
		// you pass `headers`/`request` to the call
		if (!request) {
...
<Callout type="info">
  The `request` parameter is `undefined` during initialization and during direct `auth.api` calls that pass no headers. When you pass `headers` or `request` to an `auth.api` call, the callback receives a `Request` carrying them. Handle the `undefined` case by returning default trusted origins.
</Callout>
