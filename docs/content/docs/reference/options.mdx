...
* `cookieSecurity`: Omit for the URL/environment-based default. `"none"` omits the name prefix and defaults Secure to false; `"secure"` uses `__Secure-` and Secure; `"host"` uses `__Host-`, Secure, `Path=/`, and no Domain. Host mode cannot be combined with `crossSubDomainCookies`. Changing modes renames cookies and ends existing sessions.
* `useSecureCookies`: Deprecated alias for `cookieSecurity`; it will be removed in a future minor release.
...
* `cookieNamespace`: Application namespace in default cookie names. Custom cookie names override it.
* `cookiePrefix`: Deprecated alias for `cookieNamespace`; it will be removed in a future minor release.
...
* `validateSchema`: Compare the schema with the tables Better Auth writes before the first request and report every difference with its fix. See the [Database documentation](/docs/concepts/database#schema-validation) for more info. (default: `true` outside production)
