...
Kysely-backed configurations use interactive transactions by default. Set `transaction: false` only when the dialect cannot run them. Multi-record User, Identity, and Account lifecycle operations still require an interactive transaction or a database-native atomic batch and fail before writing when neither capability is available.

When a community Kysely dialect uses Cloudflare D1, pass the raw binding as `d1Database`. Better Auth disables interactive transactions for that configuration and uses `D1Database.batch()` for atomic writes.
...
Identity configuration controls the table that links a Better Auth user to a stable provider-side key.
...
* `modelName`: The model name for the Identity
* `fields`: Map Identity fields to different column names
* `additionalFields`: Add application-defined fields to the Identity schema
...
export const auth = betterAuth({
	account: {
		...
		skipStateCookieCheck: false, // Keep OAuth state bound to the initiating browser
...
Skip the signed cookie check that binds database-backed OAuth state to the browser that started the flow. Default: `false`. Enabling this weakens OAuth callback correlation and can allow account takeover, so use it only when the integration cannot preserve the state cookie and you have evaluated the security impact.
...
The account cookie and its session binding use `session.cookieCache.maxAge`. The default is five minutes when a server-side account store is available. In a fully stateless deployment, Better Auth defaults the cookie cache to the session lifetime, so the provider account remains available for that session. The account cookie is refreshed when Better Auth writes updated provider account data, for example after `getAccessToken` or `/refresh-token` refreshes provider tokens. When this happens in server-side code, forward the returned `Set-Cookie` header to the browser so the refreshed account cookie is retained.
...
With a database, the cookie selects the Account by its Better Auth `id`, and Better Auth reloads the current Account and Identity before using provider credentials. Unlinking an Account or rotating its tokens therefore takes effect immediately. Without a database, the encrypted cookie remains the source of the provider account data. Better Auth binds each account-cookie issuance to the exact session, Better Auth Account, external Identity, and provider instance that authenticated it. Copying an account cookie from another session cannot authorize account operations, even when both sessions use the same upstream Identity. A configured `secondaryStorage` remains authoritative for session validity; provider account binding does not enable session caching.
...
const hook = createAuthMiddleware(async (ctx) => {
	const accountWithIdentity = await getAccountCookie(ctx);

	console.log(accountWithIdentity?.account.providerId);
	console.log(accountWithIdentity?.identity.issuer);
});
```
