...
export const auth = betterAuth({
	account: {
		modelName: "accounts",
		identityScope: "provider", // Preserve Better Auth 1.6 provider-scoped identity
		fields: {
			userId: "user_id"
		},
...
Controls the namespace stored in `issuer` and paired with `accountId` to identify an external account.

The `"issuer"` scope stores the verified authority. When a provider has no trusted issuer, Better Auth uses a deterministic provider namespace. The `"provider"` scope always stores a deterministic namespace such as `local:oauth:google`.

The `auth init` command generates `account: { identityScope: "provider" }`. Omitting this option continues to use issuer scope. Auth initialization remains silent, while `auth migrate plan` reports this compatibility choice.

Use `"provider"` when migrating populated 1.6 data to preserve `(providerId, accountId)` identity. Both scopes use the same Account schema and unique `(issuer, accountId)` index.

Switching scopes on a populated v1.7 database requires a reviewed account re-key migration.
