...
- **Anonymous identifier**: A non-reversible hash derived from your project (first Git commit hash or `package.json` name and optionally `baseURL`). This lets us de‑duplicate events per project without knowing who you are.
...
Example payload shape (values vary):

```ts
{
  type: "init" | "cli_generate" | "cli_migrate",
  anonymousId: "<anonymous-hash>",
  payload: {
    runtime: { name: "node", version: "20.x" },
    environment: "development",
    framework: { name: "next", version: "14.x" },
    database: { name: "postgresql", version: "8.x" },
    systemInfo: { platform: "darwin", cpuCount: 8, /* … */ },
    packageManager: { name: "pnpm", version: "9.x" },
    config: getTelemetryAuthConfig(/* your options */)
  }
}
```
...
- **Redaction by design**: See `getTelemetryAuthConfig` in the Better Auth source for the exact shape of what is included. It purposely converts sensitive values to booleans, counts, or generic identifiers.

Specifically, `getTelemetryAuthConfig` includes non‑sensitive aspects such as:

- **Feature toggles**: e.g., whether email/password is enabled, whether verification emails are sent, rate limit enabled.
- **Numeric settings**: e.g., expiration durations, update windows, limits.
- **Provider metadata**: provider ids and non‑secret options like `scope`, `prompt`, `disableImplicitSignUp` (never client IDs or secrets).
- **Model/config structure hints**: model names and fields lists; cookie and session strategy flags; logger level; adapter/database kind. Values like cookie domains are only reported as booleans indicating presence, not actual domain strings.

Review the implementation here in the package source:

- `packages/better-auth/src/telemetry/detectors/detect-auth-config.ts` (function `getTelemetryAuthConfig`)
