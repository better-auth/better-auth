...
This error occurs during the OAuth flow when attempting to link an OAuth provider Identity
to the currently authenticated user, but that exact Identity is already linked to
...
* You rely on email matching to decide linking, but an Identity is uniquely keyed by `issuer` and
  `providerAccountId`. If that Identity belongs to another user, linking will be blocked.
...
* Inspect the `identity` row selected by the Account. Each Identity points to a `userId` and is
  uniquely keyed by `issuer` and `providerAccountId`. For OpenID Connect providers, these values
  correspond to the verified issuer and stable subject.
* Inspect the related `account` rows by `identityId`. Provider aliases can share the Identity, but
  each provider configuration has its own Account and token state.
...
* Ensure you request stable user identifiers from the provider, such as the OpenID Connect `openid`
  scope, so `providerAccountId` remains consistent across sessions.
