...
* You rely on email matching to decide linking, but the actual unique key is `(providerId, accountId)` by default or `(issuer, accountId)` under issuer-scoped identity. If that identity belongs to another user, linking will be blocked.
...
* Inspect your `account` database table. Each row points to a `userId` and is uniquely keyed by `providerId` and `accountId` by default. Under issuer-scoped identity, the key uses the verified issuer and stable subject instead.
