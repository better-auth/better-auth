...
* You rely on email matching to decide linking, but the unique account key is `(issuer, accountId)`. The issuer is either the verified authority or the provider namespace selected by `account.identityScope`. If that identity belongs to another user, linking is blocked.
...
* Inspect your `account` database table. Each row points to a `userId` and is uniquely keyed by `issuer` and `accountId`. The issuer is either the verified authority or the deterministic provider namespace selected by `account.identityScope`.
