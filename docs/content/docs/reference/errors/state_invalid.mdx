...
This error only occurs with the cookie strategy. If you are using the default database strategy, state-related
failures may surface as other errors instead, such as `state_mismatch` or `state_security_mismatch`, depending on
what failed.
