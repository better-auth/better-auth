...
1. **Restricted Request Types**
   POST requests must either have a `Content-Type` header of `application/json` or include the custom header `x-auth-request: true`. This blocks simple form or cross-site requests that browsers can make automatically.
