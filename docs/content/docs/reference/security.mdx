...
Better Auth refuses redirects for server-side OAuth and OIDC fetches, including discovery, token, refresh, introspection, userinfo, and JWKS requests. Provider endpoints should answer directly.

When a provider, OAuth client, or registration payload controls the host, Better Auth also blocks private, link-local, and cloud-metadata targets. Node.js-compatible runtimes check DNS before fetching; edge runtimes without `node:dns` still reject non-public literal hosts. Add private IdPs to [`trustedOrigins`](#trusted-origins).

Directly configured social-provider hosts skip the DNS host gate, but still refuse redirects. The Electron user-image proxy re-checks each avatar redirect hop.
