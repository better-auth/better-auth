...
The `/enroll` endpoint applies the same posture unconditionally: it always returns a generic `200` response regardless of whether the email is already registered and verified. See [Enrollment](/docs/concepts/users-accounts#enrollment).
