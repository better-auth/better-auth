...
`Secure` restricts cookie transport to HTTPS; it does not encrypt the cookie value. Core auth cookies default to `HttpOnly` and `SameSite=Lax`. See [Cookie Security](/docs/concepts/cookies#cookie-security) to configure the policy.
