...
To secure OAuth flows, Better Auth stores the OAuth state and PKCE (Proof Key for Code Exchange) in the database by default. The state helps prevent CSRF attacks, while PKCE protects against code injection threats. Once the OAuth process completes, these values are removed from the database.

### Configurable State Management

Better Auth supports configurable [OAuth state management](/docs/concepts/oauth#oauth-state-management) strategies to accommodate different deployment scenarios:

- **Storage-based (default)**: State is stored securely in the database with automatic cleanup
- **Custom state management**: Implement your own state generation and parsing logic
- **Stateless approaches**: Encrypt state data directly in the OAuth state parameter

This flexibility is particularly useful for [OAuth proxy](/docs/plugins/oauth-proxy) scenarios where the proxy server and application server have separate datastores.
