...
Better-Auth uses Vitest as our testing framework. Currently, we focus on unit tests, with plans to expand to integration testing before our v1 release. This guide will help you understand our testing philosophy and how to write effective tests.
...
describe("Async Operations", () => {
  it("should handle async operations correctly", async () => {
    ...
    await expect(auth.someAsyncOperation()).resolves.toBeDefined();
    await expect(auth.someFailingOperation()).rejects.toThrow();
  });
});
```
