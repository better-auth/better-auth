...
See [Get the current user's audit logs](/docs/infrastructure/plugins/audit-logs#get-the-current-users-audit-logs) for more information.
...
| `limit`          | `number` | Results per page (max 100, default 50)         |
| `offset`         | `number` | Pagination offset (default 0)                  |
| `organizationId` | `string` | One organization (must be owner or admin)      |
| `userId`         | `string` | One user’s activity across orgs you administer |
| `eventType`      | `string` | Filter by event type                           |
| `identifier`     | `string` | Match `eventData.identifier`                   |
| `session`        | `object` | Session object with user                       |
...
const invites = await authClient.dash.getAllAuditLogs({
  session,
