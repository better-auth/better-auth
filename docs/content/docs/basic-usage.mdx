...
Use `signOut` to end the current session.
...
<Callout>
  `signOut` always clears the local session cookies. It is idempotent, so
  requests without an active session still succeed. If the server-side session
  cannot be deleted, the request returns a `FAILED_TO_DELETE_SESSION` error.
</Callout>
