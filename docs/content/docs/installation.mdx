...
Our `auth init` CLI allows you to quickly setup better-auth in your project.
It does everything for you that you would normally do manually.

```package-install
npx auth init
```

Once completed, if you've selected to use a database, you'll need to configure the database ENVs in your project.

That's it! 🎉 You're now ready to use better-auth in your application.
Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
