...
<Steps>
  ...
  <Step>
    ...
    <Callout>
      When using an ORM adapter, import from `better-auth/minimal` to exclude Kysely and reduce bundle size. Use the full `better-auth` import only if you need direct database connections or built-in Kysely migrations. See [Bundle Size Optimization](/docs/guides/optimizing-for-performance#bundle-size-optimization).
    </Callout>
