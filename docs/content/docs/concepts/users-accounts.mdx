...
The `existingUserMode` option controls when OAuth/SSO providers can auto-link to an existing user with the same email address. This provides fine-grained control over account linking security.
...
export const auth = betterAuth({
    account: {
        accountLinking: {
            enabled: true,
            existingUserMode: "trusted_providers_only", // recommended
        }
    },
});
```
...
- **`"trusted_providers_only"`** (recommended): Only auto-link if the provider passes a trust check. This means the provider must be listed in `trustedProviders`. For SSO providers, domain verification also counts as a trust signal.

- **`"email_match_any"`** (default): Legacy behavior for backward compatibility. Auto-link when either the provider is trusted OR the provider returns `email_verified: true`.

<Callout type="info">
**Security Recommendation**: For production applications, we recommend setting `existingUserMode: "trusted_providers_only"` and explicitly listing your trusted providers. The `"email_match_any"` default is maintained for backward compatibility but relies on trusting the `email_verified` claim from any provider.
</Callout>

<Callout type="warn">
**SSO Plugin Behavior**: When using the SSO plugin (`@better-auth/sso`), the linking policy is stricter. Even in `"email_match_any"` mode, SSO requires a trust signal (trusted provider or domain-verified) before auto-linking. This is because enterprise SSO providers should prove domain ownership before linking to existing accounts.
</Callout>
