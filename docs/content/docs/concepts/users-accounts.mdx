...
When skipping old email verification, you can still notify the old email address about the change. Unlike `sendChangeEmailConfirmation`, this is non-blocking — no action is required from the user.
...
export const auth = betterAuth({
    user: {
        changeEmail: {
            enabled: true,
            sendChangeEmailNotification: async ({ user, oldEmail, newEmail }) => {
                void sendEmail({
                    to: oldEmail,
...
You can run custom logic after an email change is verified and applied using `afterEmailChange`. This is useful for syncing with external systems, audit logging, or sending confirmation notifications.
