...
In `"explicit"` mode, the `url` passed to `sendChangeEmailConfirmation` (or to
`emailVerification.sendVerificationEmail`, when no confirmation step is configured) is your
app's own URL with `token` attached, instead of better-auth's `/verify-email` endpoint. Your
app renders a confirmation page from it, then:
