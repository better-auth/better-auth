...
To check whether an account holds a particular scope, use the `hasGrantedScope` helper instead of inspecting the array yourself:

```ts
import { hasGrantedScope } from "better-auth/oauth2";

const [google] = accounts.filter((account) => account.providerId === "google");
const canReadDrive = hasGrantedScope(
