...
By default (`confirmationMode: "instant"`), the emailed verification link changes the email
address the moment it's visited — a plain `GET`. Some email clients and security scanners open
links automatically to scan them (Outlook Safe Links, link-preview proxies), which can trigger
the change without the user's intent. Set `confirmationMode: "explicit"` to require a separate
...
export const auth = betterAuth({
    user: {
        changeEmail: {
            ...
            sendChangeEmailConfirmation: async ({ url, token }) => { /* ... */ },
        }
    }
})
```

In `"explicit"` mode, the `url` passed to `emailVerification.sendVerificationEmail` is your
app's own URL with `token` attached, instead of better-auth's `/verify-email` endpoint — this
holds whether that's the only email sent, or the follow-up sent after the user approves
`sendChangeEmailConfirmation`'s own link (which is always better-auth's instant callback,
regardless of `confirmationMode`). Your app renders a confirmation page from the app-owned URL,
then:

* `GET /change-email/preview` — validates the token and returns the pending email change
  without applying it.
* `POST /change-email/confirm` — applies the change, the same way the instant link does.

Unlike the instant link — a direct browser navigation that may legitimately arrive with no
active session — both `preview` and `confirm` require the caller to already be signed in as
the user the token belongs to, and reject otherwise. Forward the user's session (cookies or an
equivalent auth header) when your confirmation page calls them.

<Callout type="warn">
  The change-email token is a signed JWT, not a single-use database row, so it is not
  explicitly invalidated after use the way the delete/transfer tokens are. In practice a
  sequential replay of `/change-email/confirm` (or the instant `/verify-email` link) fails once
  the change has applied, because the token's lookup key is the *old* email address and that no
  longer belongs to any user — but two sufficiently concurrent replays are not guarded against
  the way single-use verification rows are. This is pre-existing behavior of the instant-mode
  link too; it is not specific to explicit mode.
</Callout>
...
By default (`confirmationMode: "instant"`), the pre-generated `url` above deletes the account
the moment it's visited — a plain `GET`. Some email clients and security scanners open links
automatically to scan them (Outlook Safe Links, link-preview proxies), which can trigger the
deletion without the user's intent. Set `confirmationMode: "explicit"` to require a separate
confirmation step instead:
...
export const auth = betterAuth({
    user: {
        deleteUser: {
            enabled: true,
            confirmationMode: "explicit", // default: "instant"
            sendDeleteAccountVerification: async ({ url, token }) => { /* ... */ },
        },
    },
});
```

In `"explicit"` mode, `url` is your app's own URL (built from `callbackURL`) with `token`
attached, instead of better-auth's `/delete-user/callback` endpoint. Your app renders a
confirmation page from it, then:

* `GET /delete-user/preview` — validates the token and returns the pending user without
  deleting it, so you can render "you're about to delete your account".
* `POST /delete-user/confirm` — body `{ token, password? }`; consumes the token and deletes
  the account, the same way the instant callback does. Passing `password` here re-verifies it
  first, letting you require a second factor at confirmation time — something the instant
  `GET` link can't do at all, since a link click carries no request body.
