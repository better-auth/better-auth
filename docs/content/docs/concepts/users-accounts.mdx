...
#### Skipping Current Email Verification

If your users may no longer have access to their old email address, you can skip the confirmation step on the current email by setting `sendOldEmailVerification` to `false`. The verification email will be sent directly to the new email address instead.

When this option is disabled, a password (or fresh session) is required to compensate for the reduced security.
...
<Callout type="warn">
  Disabling old email verification reduces security. An attacker with access to an active session could change the account email. The password or fresh session requirement mitigates this risk, but consider your threat model before disabling.
</Callout>

#### After Email Change Hook

You can run custom logic after an email change is verified and applied using `afterEmailChange`. This is useful for syncing with external systems, audit logging, or notifying the old email address about the change.
...
export const auth = betterAuth({
    user: {
        changeEmail: {
            enabled: true,
            afterEmailChange: async ({ user, oldEmail, newEmail }) => {
                await updateExternalSystem(user.id, newEmail);
                void sendEmail({
                    to: oldEmail,
                    subject: 'Your email address was changed',
                    text: `Your email was changed to ${newEmail}. If this wasn't you, contact support.`
                })
            }
        }
    },
...
When `sendOldEmailVerification` is `false`, you must provide the user's password (unless the session is fresh):
...
Each account exposes a `grantedScopes` field: a `string[]` of the OAuth scopes the user has **accumulated** for that provider over time. It is not the latest token's `scope` claim. Every API that surfaces scopes returns this same array shape, including `listAccounts`, `getAccessToken`, and `refreshToken`.

To check whether an account holds a particular scope, use the `includesGrantedScope` helper instead of inspecting the array yourself:

```ts
import { includesGrantedScope } from "better-auth/oauth2";

const google = accounts.find((account) => account.providerId === "google");
const canReadDrive = includesGrantedScope(
    google?.grantedScopes,
...
* **Linking Social Accounts:** Use the `linkSocial` method on the client to link a social provider to the user's account.
  ...
  You can also request specific scopes when linking a social account. The `scopes` you pass are **added** to the provider's default scopes; they don't replace them. This is how you perform incremental authorization: each `linkSocial` call asks for more access, and the newly granted scopes **accumulate** into the account's stored `grantedScopes` rather than overwriting it.
  ...
  await authClient.linkSocial({
      ...
      scopes: ["https://www.googleapis.com/auth/drive.readonly"] // Added on top of the provider defaults
  });
  ```

  <Callout type="info">
    `signIn.social({ scopes })` works the same way: the scopes are additive to the provider defaults. Token refresh never changes the stored grant, and for most providers a narrower sign-in never shrinks it either. Google with the default `includeGrantedScopes: true` is the exception: it resyncs `grantedScopes` to Google's authoritative combined grant on each sign-in, so a scope revoked at Google drops on the next sign-in (see [OAuth scopes](/docs/concepts/oauth#how-granted-scopes-accumulate)).
  </Callout>
