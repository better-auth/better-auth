...
<Callout type="info">
**Security Recommendation**: For production applications, we recommend setting `linkingPolicy: "trusted_providers_only"` and explicitly listing your trusted providers. The `"email_match_any"` default is maintained for backward compatibility but relies on trusting the `email_verified` claim from any provider.
</Callout>
