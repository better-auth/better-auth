---
title: Users, Identities & Accounts
description: Learn how to manage users, provider identities, and accounts, including profile updates, passwords, deletion, token storage, linking, and unlinking.
---
...
**validateUserInfo**: A gate that decides which identities Better Auth admits. It fires just before a User is created (`create-user`) or a provider Account is linked (`link-account`) across OAuth, OIDC SSO, SAML SSO, email and password, magic link, email OTP, anonymous, SIWE, phone number, and admin-created User flows. SCIM invokes the same gate when provisioning creates a Better Auth User. Authentication flows that support stateless deployments still invoke it without a persistent database, while SCIM requires a database adapter with native interactive transactions.
...
`source.action` is `"create-user"`, `"link-account"`, or `"sign-in"`, and `source.method` is the authentication method. For OAuth, `source.oauth` carries the provider id and raw provider profile. For OIDC and SAML SSO, `source.sso` carries the SSO provider id and raw provider claims or assertion attributes. For SCIM, `source.connectionId` identifies the provisioning connection.
...
Better Auth supports multiple authentication methods through providers such as email and password, Google, or an enterprise identity provider. It stores the durable provider identity separately from the provider configuration used to authenticate it:

* An **Identity** links a user to one stable provider-side key. Its `issuer` names the trusted authority, and its `providerAccountId` is the stable identifier assigned by that authority.
* An **Account** links that Identity to one provider configuration. Its public `providerId` selects the configuration used for protocol operations. Its internal `providerInstanceId` supplies a stable instance namespace, while tokens, password, scope, and expiry fields belong only to that instance.

The pair of `issuer` and `providerAccountId` is unique across Identities, while the pair of `identityId` and `providerInstanceId` is unique across Accounts. Provider configurations can recognize the same Identity without sharing tokens or lifecycle records when their trust model permits a shared issuer, as with Generic OAuth aliases or server-configured `defaultSSO` OIDC providers. Persisted SSO providers use instance-scoped Identities, and equal provider subjects from different issuers remain separate.

An Account's `id` identifies its local record and is the value to pass as `accountId` to account-management APIs. See the [Identity and Account schemas](/docs/concepts/database#identity) for the complete field lists.
...
The response includes `providerId`, so callers can match an Account to its configured provider alias. It also includes public additional fields from the Account and its nested Identity. Fields configured with `returned: false`, provider tokens, passwords, `identityId`, `providerInstanceId`, and the Identity's `userId` remain server-only. Add `inferAdditionalFields` to the auth client to infer configured Account and Identity fields.
...
console.log(googleAccount?.identity.issuer);
console.log(googleAccount?.identity.providerAccountId);
```
...
Account linking is [enabled by default](https://www.better-auth.com/docs/reference/options#accountlinking) and lets a User attach multiple authentication methods. Users can connect additional social sign-ons or OAuth providers when the provider confirms their email as verified.
...
You can specify a list of "trusted providers." When a User signs in through a trusted provider, Better Auth can attach the provider Identity and Account to a matching User even if the provider does not confirm the email verification status. Use this with caution because it may increase the risk of account takeover.
...
Users who are already signed in can manually add social providers or credentials as authentication methods.
