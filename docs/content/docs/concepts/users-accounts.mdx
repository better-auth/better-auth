...
- **`"trusted_providers_only"`** (default): Only auto-link if the provider passes a trust check. This means the provider must be listed in `trustedProviders`. For SSO providers, domain verification also counts as a trust signal.

- **`"email_match_any"`**: Legacy/relaxed behavior. Auto-link when either the provider is trusted OR the provider's `email_verified` claim is accepted. This mode is less secure and not recommended for production—it's provided as an escape hatch for applications that relied on the previous email-based auto-linking behavior.

<Callout type="warn">
The default `existingUserMode` is `"trusted_providers_only"`, which is the most secure option. If you're upgrading from a previous version and relied on email-based auto-linking, you may need to explicitly set `existingUserMode: "email_match_any"` to preserve the old behavior.
</Callout>
