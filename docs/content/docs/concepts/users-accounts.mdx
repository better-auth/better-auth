...
| ------------ | ---------------------------------------------------------------------- |
| `providerId` | Identifies the configured connection used to authenticate the account. |
| `accountId`  | Contains the stable subject assigned by the provider.                  |
| `issuer`     | Stores the persisted identity namespace used with `accountId`.         |
...
An account also has a local record `id`, which is the value to pass as `accountId` to account-management APIs. The provider identity fields have these concrete forms:

| Strategy    | `providerId` | `accountId` | persisted `issuer`        |
| ----------- | ------------ | ----------- | ------------------------- |
| issuer      | `workforce`  | `user-42`   | `https://idp.example.com` |
| provider-id | `workforce`  | `user-42`   | `local:oauth:workforce`   |

Every 1.7 account schema requires `issuer` and `accountId` and enforces a unique compound index across them. Better Auth 1.7.x defaults an omitted `account.identityStrategy` to `"issuer"`. Set it to `"provider-id"` to retain the logical Better Auth 1.6 `(providerId, accountId)` identity model; Better Auth percent-encodes the provider ID in a deterministic `local:oauth:<encoded providerId>` issuer namespace. Credential accounts use `local:credential`.

Issuer-scoped identity allows multiple provider configurations for the same issuer to deduplicate the same external identity without treating an identifier from another issuer as the same person. Provider aliases then share one account row and token set; they do not have independent grants or provider lifecycle records. See the [account schema](/docs/concepts/database#account) for the complete set of fields.

Keep the selected strategy consistent across every application instance connected to the database. Changing the strategy on populated 1.7 data is a re-key migration, not a configuration-only toggle. See the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#choose-account-identity-strategy) before migrating existing data.
