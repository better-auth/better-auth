...
The `verifyPassword` function allows you to verify a user's current password. This is useful for confirming user identity before performing sensitive operations like changing email, deleting an account, or updating security settings. This function can only be called from the server.
