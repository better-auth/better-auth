...
`account.identityScope` controls the namespace stored in `issuer`. Both scopes use the same required fields and unique `(issuer, accountId)` index.

| Scope        | `providerId`       | `accountId` | `issuer`                       |
| ------------ | ------------------ | ----------- | ------------------------------ |
| `"issuer"`   | `workforce-google` | `104925...` | `https://accounts.google.com`  |
| `"provider"` | `workforce-google` | `104925...` | `local:oauth:workforce-google` |
...
The `"issuer"` scope stores the verified authority. Provider configurations that return the same issuer and subject resolve to the same external account. Providers without a trusted issuer use a deterministic `local:oauth:<encoded providerId>` fallback.

The `"provider"` scope stores a deterministic namespace for each configured provider. Provider aliases therefore remain separate accounts. Protocol verification is unchanged by this storage choice.

The `auth init` command selects `"provider"` for new projects. Existing configurations that omit `account.identityScope` continue to use issuer scope. `auth migrate plan` reports this compatibility choice. Keep the scope consistent across every application instance. Switching scopes on a populated v1.7 database requires an account re-key migration. See the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#choose-account-identity-scope) before changing it.
