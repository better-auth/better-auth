...
An account has a local record ID and a provider identity. `id` identifies the Better Auth account record and is the value to pass as `accountId` to account-management APIs. By default, `(providerId, accountId)` identifies the provider account, matching Better Auth 1.6. `providerId` identifies the configured provider, and `accountId` is the stable identifier that provider assigned.

Set `account.identityStrategy` to `"issuer"` to identify accounts by `(issuer, accountId)` instead. In that mode, `issuer` names the trusted authority. OAuth providers without a trusted issuer use `local:oauth:<encoded providerId>`, with the provider ID segment percent-encoded, while credential accounts use `local:credential`.
...
The 1.7 default is `"provider-id"`, and this path does not add or persist an `issuer` field. Keep the selected strategy consistent across every application instance and database migration. If a database already contains issuer-scoped account rows, configure `"issuer"` explicitly before starting 1.7. See the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#preserve-16-provider-scoped-account-identity) before changing the strategy on existing data.
