...
OAuth providers without a trusted issuer use `local:oauth:<providerId>` as their account namespace. Credential accounts use `local:credential`; do not use that credential namespace for an OAuth provider.
