...
* `GET /change-email/preview` — validates the token and returns the pending email change
  without applying it.
* `POST /change-email/confirm` — consumes the token and applies the change, the same way the
  instant link does.

<Callout type="warn">
  The change-email token is a signed JWT, not a single-use database row, so it is not
  invalidated on first use — a `/change-email/confirm` replay before it expires just re-applies
  the same (idempotent) change. This matches the existing instant-mode behavior; it is not
  specific to explicit mode.
</Callout>
