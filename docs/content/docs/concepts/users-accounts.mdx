...
The runtime default `account.identityStrategy` is `"issuer"` for fresh 1.7 applications. A guided migration from Better Auth 1.6 requires an explicit strategy when populated external accounts still need an issuer backfill, and recommends `"provider-id"` to preserve the previous account identity. In that compatibility mode, external accounts use `local:oauth:<encoded providerId>` as their stored issuer, so provider aliases remain separate identities. Keep the selected strategy consistent across every application instance and database migration. See the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#preserve-16-provider-scoped-account-identity) before enabling it on existing data.
