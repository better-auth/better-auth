...
An account has a local record ID and a provider identity. `id` identifies the Better Auth account record and is the value to pass as `accountId` to account-management APIs. The pair of `providerId` and `accountId` identifies the external account: `providerId` names the provider configuration, and `accountId` is the stable identifier that provider assigned. Credential accounts use `providerId: "credential"` and the linked user's `id` as `accountId`. See the [account schema](/docs/concepts/database#account) for the complete set of fields.
