...
To allow users to change their email, enable the `changeEmail` feature (disabled by default) and provide a `sendVerificationEmail` function to send the verification link to the new email address:
...
1. The new email is stored in a `pendingEmail` field on the user record
2. A one-time verification token is created in the `Verification` table
3. A verification email is sent to the **new** email address via `changeEmail.sendVerificationEmail`
4. When the user clicks the verification link, the email is updated, `pendingEmail` is cleared, and `emailVerified` is set to `true`
5. The verification token is deleted (one-time use) and a new session is created

The user can cancel the pending change at any time before verification, which clears `pendingEmail` and deletes the verification token.

<Callout>
When `changeEmail.enabled` is `true`, a `pendingEmail` field is required on the `user` table. Run the [CLI](/docs/concepts/cli) to generate the migration, then apply it to your database.
</Callout>

#### Revoking Other Sessions

To automatically revoke all other sessions when an email change is completed:
...
You can hook into the email change lifecycle for logging, notifications, or analytics:
...
export const auth = betterAuth({
    user: {
        changeEmail: {
            enabled: true,
            onChangeEmailRequested: async ({ user, newEmail }, request) => {
                // Notify the current email that a change was requested
            },
            onChangeEmailCompleted: async ({ user, oldEmail, newEmail }, request) => {
                // Notify the old email that the change is complete
            },
            onChangeEmailCancelled: async ({ user }, request) => {
                // Notify the current email that the change has been canceled
            },
        }
    }
})
...
You can initiate and cancel email changes from both server-side and client-side code.

##### Server-side

Request an email change (requires an active session):
...
```ts
await auth.api.cancelEmailChange({
    headers: await headers(),
});
```
