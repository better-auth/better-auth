...
The `createAuthClient` function accepts the following configuration options:

- `baseURL`: The base URL of your auth server. Required if your auth server is on a different domain.
- `basePath`: The base path of the auth API (default: `/api/auth`).
- `baseCallbackURL`: The base URL for callback redirects. Useful when your frontend and backend are on different origins.
- `fetchOptions`: Custom fetch options passed to all requests.
- `plugins`: Array of client plugins to extend functionality.
