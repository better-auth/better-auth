...
<Tabs items={["React", "Vue","Svelte", "Solid"]} defaultValue="react">
  <Tab value="React">
    ...
    export function UserWithFallback() {
        return (
...
<Callout type="info">
  Suspense-enabled React hooks require React 19 or newer. In a Server Component,
  await `auth.api.getSession` instead of using the React client hook.
</Callout>
...
React hooks like `useSession` suspend while their initial request is pending. Wrap the component that calls the hook in a [`Suspense`](https://react.dev/reference/react/Suspense) boundary to provide its loading UI. Other framework clients continue to expose `isPending`.

React query hooks render only in the browser. During server rendering, the nearest Suspense boundary renders its fallback without reading the client query or starting a request. In a Server Component, await `auth.api.getSession({ headers })` with the incoming request's headers instead.

When React DOM provides [`browser`](https://react.dev/reference/react-dom/browser), Better Auth calls `use(browser())` before reading the query. This API is currently available in React Canary and Experimental. On stable React 19, Better Auth uses a server error to leave the Suspense fallback in place and retry in the browser. This compatibility path is reported to React's `onError` and `onRecoverableError` callbacks; the native `browser()` path uses `onBrowserBailout` instead. Ordinary plugin atoms that do not fetch data remain server-renderable.
...
With SSR frameworks, `useSession` fetches the session on the client. In React, pass the server-fetched session to `hydrateSession` and render it in the Suspense fallback so the initial HTML has session content while the browser takes over:
...
export function SessionCard({ initialSession }: Props) {
    return (
        <Suspense fallback={<SessionContent session={initialSession} />}>
            <ClientSessionCard initialSession={initialSession} />
        </Suspense>
    )
}

function ClientSessionCard({ initialSession }: Props) {
    authClient.hydrateSession(initialSession)
    const { data } = authClient.useSession()
    return <SessionContent session={data} />
}

function SessionContent({ session }: { session: Props["initialSession"] }) {
...
<Callout>
  ...
  * `hydrateSession` does not write to the shared client atom on the server. The
    Suspense fallback renders the server-provided session directly.
</Callout>
