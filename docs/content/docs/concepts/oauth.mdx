...
### Validate OAuth Users

Use `validateUser` when you need to reject an OAuth sign-in or account link before Better Auth creates a user, links an account, or creates a session. The callback receives the mapped `user` and the raw provider `profile`, so you can enforce rules like domain allowlists, disposable email blocking, or organization membership checks.

Return nothing to allow the flow. Return an object with `error` or `errorDescription` to reject it. Redirect-based OAuth flows send the rejection to the configured OAuth error URL, while API-based flows such as ID-token sign-in return a 401 API error. Avoid putting sensitive details in `errorDescription` because it is returned to the client.
