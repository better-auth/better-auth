...
Better Auth verifies the OAuth or OpenID Connect protocol issuer independently from the namespace it persists for account recognition. Better Auth 1.7.x uses the verified authority and provider subject by default. With `account.identityStrategy: "provider-id"`, it still performs the same protocol verification but persists a deterministic `local:oauth:<encoded providerId>` issuer namespace with the provider subject. The linked Better Auth user still requires an email address. Support for users without an email is tracked in [#9124](https://github.com/better-auth/better-auth/issues/9124).
