...
| Cloudflare         | The default provider reads `email` from Cloudflare's API `/user` endpoint and requires the `user-details.read` scope                                                     | `profile.id`                                        | Treated as verified by default; Cloudflare's `/user` endpoint does not expose an email-verification field                                                           |
