...
Better Auth verifies the OAuth or OpenID Connect issuer before resolving an account. `account.identityScope` only controls the namespace stored after verification. The `"issuer"` scope stores the verified authority, while `"provider"` stores a deterministic namespace such as `local:oauth:google`. See [Accounts](/docs/concepts/users-accounts#accounts) for details. The linked Better Auth user still requires an email address. Support for users without an email is tracked in [#9124](https://github.com/better-auth/better-auth/issues/9124).
