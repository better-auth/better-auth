...
You can provide a login hint (such as an email address) to pre-fill or suggest an account to the OAuth provider. This is particularly useful when linking accounts to ensure the user selects the correct account:
...
await authClient.linkSocial({
  provider: "google",
  loginHint: user.email, // filter the accounts to only show the one with the given email
})
```
    </Tab>
