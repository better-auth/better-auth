...
Use [`user.validateUserInfo`](/docs/concepts/users-accounts#callbacks) when you need to reject an OAuth sign-in or account link before Better Auth creates a user, links an account, or creates a session. The callback receives the mapped `user` and OAuth metadata in `source`, including the provider ID and raw provider profile.

This runs during regular OAuth callbacks, ID-token sign-in, and account linking. It also works in stateless setups because it runs before any database-backed user creation or account linking step.
...
export const auth = betterAuth({
  user: {
    validateUserInfo: ({ user, source }) => {
      if (source.type !== "oauth" || source.providerId !== "google") return;
...
```ts
const { accessToken, grantedScopes } = await authClient.getAccessToken({
...
The response includes `grantedScopes` (a `string[]`) holding the account's accumulated granted scopes. The `/list-accounts`, `/get-access-token`, and `/refresh-token` endpoints all return this field.
...
The `scopes` you pass to `signIn.social` or `linkSocial` are **additive**: they are appended to the provider's default scopes rather than replacing them. (To start from an empty set instead, use [`disableDefaultScope`](#disabledefaultscope) on the provider.)

#### How granted scopes accumulate

Each account stores the user's accumulated granted scopes in `account.grantedScopes` (a `string[]`). Incremental authorization through `linkSocial` unions the newly granted scopes into this set. Whether a later sign-in can shrink the stored grant depends on the provider:

* **Union-only providers** (generic OAuth, most social providers, and Google with [`includeGrantedScopes: false`](#includegrantedscopes)) only ever grow the set. A sign-in or refresh that returns a narrower scope list leaves the recorded scopes intact, so once a scope is granted it stays recorded until the account is unlinked.
* **Google with the default [`includeGrantedScopes: true`](#includegrantedscopes)** treats its token response as the authoritative combined grant for the project, so each sign-in resyncs `account.grantedScopes` to match it. A scope the user revoked at Google therefore drops from the stored grant on the next sign-in.

Token refresh never changes the grant for any provider ([RFC 6749 §6](https://datatracker.ietf.org/doc/html/rfc6749#section-6)).

To check whether an account already holds a scope, use the `includesGrantedScope` helper instead of inspecting the raw array:
...
if (google && !includesGrantedScope(google.grantedScopes, "https://www.googleapis.com/auth/drive.file")) {
...
The default scopes requested for this provider, for example `email` or `profile`. Scopes passed per request through `signIn.social({ scopes })` or `linkSocial({ scopes })` are added to these defaults rather than replacing them. To start from an empty set, use [`disableDefaultScope`](#disabledefaultscope).
...
Google-specific. Controls Google's server-side [incremental authorization](https://developers.google.com/identity/protocols/oauth2/web-server#incrementalAuth) via the `include_granted_scopes` parameter. Defaults to `true`, so an authorization returns a token covering the user's accumulated grant for the project. Set it to `false` to authorize only the scopes requested by the current flow.
