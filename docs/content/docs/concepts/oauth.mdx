...
**overrideUserInfoOnSignIn**: by default, all extra fields other than the core ones (name, email, emailVerified, image) are not updated on sign in. If you want to update them, set this to `true`. You can also pass an array of fields to override.
