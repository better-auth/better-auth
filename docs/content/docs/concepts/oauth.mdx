...
Optionally, this can also be a custom function if you want to choose what gets updated. The callback receives the user's information and an object containing the new `email` and `emailVerified` values. These can be passed back through, or excluded in case you don't want email addresses to be updated on sign-in.
...
export const auth = betterAuth({
  ...
  socialProviders: {
    discord: {
      ...
      overrideUserInfoOnSignIn: (profile, data) => {
        return {
          name: profile.name,
