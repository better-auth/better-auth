...
Better Auth recognizes a returning OAuth account by its stable `(issuer, accountId)` key, but the linked Better Auth user still requires an email address. providerId identifies the configured connection; accountId is the provider subject; issuer stores the identity namespace—verified authority under issuer strategy, deterministic provider namespace under provider-id strategy. With `account.identityStrategy: "provider-id"`, Better Auth still verifies the provider response, token signature, protocol issuer, audience, and subject before persisting the synthetic `local:oauth:<encoded providerId>` namespace. The synthetic storage value never replaces protocol issuer verification. Support for users without an email is tracked in [#9124](https://github.com/better-auth/better-auth/issues/9124).
