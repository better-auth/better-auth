...
**overrideUserInfoOnSignIn**: `boolean | string[]` If you want to update the user information every time they sign in, set this to `true`. You can also pass an array of fields to override.
...
export const auth = betterAuth({
  ...
  socialProviders: {
    github: {
      overrideUserInfoOnSignIn: ["githubName", "githubAvatar"], //or true to override all fields
    },
  },
});
```
