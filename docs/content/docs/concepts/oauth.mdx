...
Sometimes you need to pass custom data through the OAuth flow and access it in your hooks. For example, you might want to pass:
- User preferences or settings
- Referral codes or tracking information
- Custom metadata that should be processed during authentication

Better Auth allows you to pass additional data through the OAuth flow without storing it in the database. This data is made available to your hooks via request state.

<Callout type="warn">
**Security Warning**: The additional data comes from the client side and should **not be trusted** for security-critical operations. Always validate and sanitize this data in your hooks before using it. Never use it for authorization decisions or to grant privileges without proper verification.
</Callout>

### Configuration

First, enable the additional data feature in your Better Auth configuration:
...
When initiating OAuth sign-in or account linking, you can pass additional data:
...
The additional data is available in your hooks through the `getOauthState` function:
    ...
    after: [
      {
        matcher: () => true,
        handler: async (ctx) => {
          // Only available during OAuth callback
          if (ctx.path === "/callback/:id") {
            ...
            if (additionalData) {
              ...
              // Example: Validate referral code format
...
<Callout type="info">
The additional data is **not** stored in the database. It's only available during the OAuth flow through request state. This makes it perfect for passing temporary data that should be processed in hooks but not persisted.
</Callout>

### Use Cases

Some common use cases for passing additional data:

**Safe Use Cases** (low security risk):
1. **Analytics**: Track the source or campaign that led to the OAuth sign-in - this is typically used for non-critical analytics
2. **UI State**: Pass UI preferences or display settings that don't affect security

**Use Cases Requiring Validation** (must verify on server):
1. **Referral Tracking**: Pass referral codes - **must verify the code exists in your database before crediting**
2. **User Preferences**: Pass initial preferences - **validate against allowed values before storing**
3. **Custom Metadata**: Any temporary data - **always validate and sanitize before use**

**Never Use For** (security risk):
- Authorization decisions (e.g., granting admin rights)
- Bypassing authentication or verification
- Directly setting sensitive user fields without verification
- Financial transactions or calculations

### Server-Side Usage

You can also pass additional data when using the server-side API:
