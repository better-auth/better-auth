...
To check whether an account already holds a scope, use the `hasGrantedScope` helper instead of inspecting the raw array:
...
if (google && !hasGrantedScope(google.grantedScopes, "https://www.googleapis.com/auth/drive.file")) {
