...
* **Microsoft Entra ID**: because `email` is tenant-mutable and never verified, use `profile.oid` (immutable, stable within the tenant) as the identity anchor; treat `email` as a profile attribute only. See the ["nOAuth" advisory](https://www.descope.com/blog/post/noauth) for the account-takeover implications of trusting Entra's `email` claim.
