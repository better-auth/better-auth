...
Better Auth recognizes a returning OAuth account by `(providerId, accountId)` by default, or by `(issuer, accountId)` when issuer-scoped identity is enabled. The linked Better Auth user still requires an email address. Support for users without an email is tracked in [#9124](https://github.com/better-auth/better-auth/issues/9124).
