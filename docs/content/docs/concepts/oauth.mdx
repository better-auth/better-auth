...
**overrideUserInfoOnSignIn**: when a user signs in, by default only 4 core user fields are updated: name, email, emailVerified, and image. Any additional fields in the user profile remain unchanged. To modify this behavior, you can pass `true` to override all fields or an array of fields to override.
...
export const auth = betterAuth({
  ...
  socialProviders: {
    github: {
      overrideUserInfoOnSignIn: ["name"], //or true to override all fields
    },
  },
});
```
