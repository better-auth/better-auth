...
When dynamic base URL is enabled and your app runs on multiple subdomains, you can enable `crossSubDomainCookies` without specifying a static `domain`. Better Auth will automatically derive the cookie domain from the resolved host of each request:
...
You can still provide an explicit `domain` to override the automatic behavior:
...
<Callout type="info">
When using a static `baseURL` string, `crossSubDomainCookies` derives the domain from that string at init time. With dynamic base URL, the domain is computed per-request from the resolved host.
</Callout>
