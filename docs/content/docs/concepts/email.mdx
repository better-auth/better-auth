...
You can run custom code immediately after a user verifies their email using the `onEmailVerified` callback. This is useful for any side-effects you want to trigger, like granting access to special features or logging the event.

The `onEmailVerified` function runs automatically when a user's email is confirmed, receiving the `user` object and `request` details so you can perform actions for that specific user.
...
export const auth = betterAuth({
    emailVerification: {
        async onEmailVerified(user, request) {
