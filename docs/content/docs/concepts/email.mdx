...
By default, Better Auth uses Zod's email validation when an email address enters a built-in email flow. You can replace that syntax check with a synchronous or asynchronous `user.emailValidator` function. The function receives the email normalized to lowercase, matching how Better Auth stores and looks up email addresses. Return `true` to accept the address and `false` to reject it.

```ts title="auth.ts"
import { betterAuth } from "better-auth";
import * as z from "zod";

const customEmailSchema = z.email({ pattern: z.regexes.unicodeEmail });

export const auth = betterAuth({
  user: {
    emailValidator: (email) => customEmailSchema.safeParse(email).success,
  },
});
```

The validator checks address syntax only. Use email verification when you need to confirm that an address exists and belongs to the user.
