...
Users can enter the OTP code they received:

```ts title="verify.ts"
await authClient.verifyEmail.otp({
    email: user.email,
    otp: userInputCode  // e.g., "123456"
});
```

The client automatically calls the `/verify-email/otp` endpoint and updates the session.
