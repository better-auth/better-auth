...
By default, Better Auth uses Zod's email validation when an email address enters a built-in email flow. You can replace that syntax check with a synchronous or asynchronous `user.emailValidator` function. Return `true` to accept the address and `false` to reject it.
