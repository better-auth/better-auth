...
You can provide a callback function that will be executed after a user successfully verifies their email. This is useful for any side-effects you want to trigger after email verification.

- **onEmailVerified**: This function is triggered after the user's email has been successfully verified and the `emailVerified` field is set to `true`. It accepts a `user` object and a `request` object as parameters.
