...
By default, when `requireEmailVerification` is true, verification emails are automatically sent during sign-in attempts if the user's email isn't verified.

You can disable the automatic sending:
