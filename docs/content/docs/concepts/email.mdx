...
export default function VerifyEmailWithOTP() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (verifyError) {
      ...
      // Email verified successfully! Session is automatically updated.
...
- If `autoSignInAfterVerification` is enabled, the user is automatically signed in
- The session is automatically updated on the client
