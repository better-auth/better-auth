...
By default, rate limiting uses the client's IP address combined with the request path as the identifier. However, this may not be ideal in scenarios where multiple users share the same IP address (e.g., users behind NAT/CGNAT, corporate networks, or VPN users).

You can provide a custom `keyGenerator` function to generate rate limit identifiers based on other criteria.
...
export const auth = betterAuth({
    rateLimit: {
        keyGenerator: (request) => request.headers.get("x-fingerprint"),
    },
})
```
