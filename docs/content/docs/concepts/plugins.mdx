...
Some specs (like OAuth/OIDC discovery) require `/.well-known/*` endpoints to live at the origin root. You can expose those by adding `publicEndpoints` to your plugin.
...
- The main `auth.handler` auto-routes only `/.well-known/*` paths to public endpoints.
- If you register a public endpoint at any other root path, you must call it through `auth.publicHandler` or mount that handler separately.
- Public endpoints share the same middleware list as regular endpoints; path-matched middleware will only apply when its `path` matches the incoming public route.
