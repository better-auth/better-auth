...
<Callout type="info">
  Skip this for local plugins that aren't accessed through `getPlugin()`. Published packages should declare `@better-auth/core` as a peer dependency with the same supported version range as `better-auth`.
</Callout>
