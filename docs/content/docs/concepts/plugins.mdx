...
<Callout type="info">
  If you publish the plugin as a separate package, declare `@better-auth/core` as a peer dependency with the same supported version range as `better-auth`.
</Callout>
...
* `createAuthCookie`: Creates a cookie name and attributes using the configured cookie namespace and security policy. This includes inferred or explicit `__Secure-` and `__Host-` prefixes and their required attributes.
