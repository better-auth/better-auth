...
<Callout type="info">
	...
	you'll also need to create a client plugin.
</Callout>
...
<Callout>
	You don't have to make the plugin a function, but it's recommended to do so.
	This way, you can pass options to the plugin and it's consistent with the
	built-in plugins.
</Callout>
...
`references`: if the field is a reference to another table. (optional) It takes an object with the following properties: - `model`: The table name to reference. - `field`: The field name to reference. - `onDelete`: The action to take when the referenced record is deleted. (default: `cascade`)
...
<Callout type="warn">
	Don't store sensitive information in the `user` or `session` table. Create a
	new table if you need to store sensitive information.
</Callout>
