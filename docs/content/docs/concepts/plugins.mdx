...
Plugins are a key part of Better Auth, they let you extend the base functionality. You can use them to add new authentication methods, features, or customize behaviors.
...
```
<Callout>
    You don't have to make the plugin a function, but it's recommended to do so. This way you can pass options to the plugin and it is consistent with the built-in plugins.
</Callout>
...
Create Auth endpoints wraps around `createEndpoint` from Better Call. Inside the `ctx` object, it'll provide another object called `context` that gives you access better-auth specific contexts including `options`, `db`, `baseURL` and more.

**Context Object**

- `appName`: The name of the application. Defaults to "Better Auth".
- `options`: The options passed to the Better Auth instance.
- `tables`:  Core tables definition. It is an object that has the table name as the key and the schema definition as the value.
...
- `adapter`: This is the same as db, but it gives you `orm` like functions to interact with the database. (we recommend using this over `db` unless you need raw sql queries or for performance reasons)
...
- `createAuthCookie`: This is a helper function that lets you get a cookie `name` and `options` for either to `set` or `get` cookies. It implements things like `__secure` prefix and `__host` prefix for cookies based on
...
In addition to middleware, you can also hook into right before a request is made and right after a response is returned. This is mostly useful if you want to do something that affects all requests or responses.
