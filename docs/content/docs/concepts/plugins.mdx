...
Reusable plugins can register their ID and creator with Better Auth's plugin registry. This allows `getPlugin()` to infer the plugin's exact return type. The `@better-auth/i18n` types also use the registry to discover plugin `$ERROR_CODES`.
