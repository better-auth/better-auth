...
Reusable plugins can register their ID and creator with Better Auth's plugin registry. This enables type-safe discovery through `getPlugin()` and `hasPlugin()`. The `@better-auth/i18n` types also use the registry to discover plugin `$ERROR_CODES`.

Augment `BetterAuthPluginRegistry` in the server plugin module. The registry key must exactly match the plugin's `id`:
...
Better Auth can now infer the registered plugin instead of returning the generic plugin type:

```ts
const plugin = ctx.context.getPlugin("my-plugin");
// ReturnType<typeof myPlugin> | null

const enabled = ctx.context.hasPlugin("my-plugin");
// true when the auth configuration includes "my-plugin", otherwise boolean
```

This registration only affects TypeScript. Users must still include the plugin in their `plugins` array to enable it at runtime. Schema, endpoint, and client inference work independently. Client plugins use `$InferServerPlugin` for those types.

<Callout type="info">
  You can skip registration for a local plugin that will never be discovered by another plugin. If you publish the plugin as a separate package, declare `@better-auth/core` as a peer dependency. Use the same supported version range as `better-auth` because the module augmentation targets `@better-auth/core` directly.
</Callout>
