...
This hook enables custom password validation for email and password authentication. Passwords must still comply with the `emailAndPassword.minPasswordLength` and `emailAndPassword.maxPasswordLength` settings defined in your auth config.
...
export const auth = betterAuth({
    hooks: {
        before: createAuthMiddleware(async (ctx) => {
            if (ctx.path === '/sign-up/email') {
                const { passed, message } = validatePassword(ctx.body.password);
                if (!passed) {
