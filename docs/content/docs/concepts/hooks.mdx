...
Reader for the newly created session on the request. Populated by any flow that publishes a session (sign-in, sign-up, anonymous upgrade, device authorization). Call it from an after hook.
  ...
  hooks: {
    after: createAuthMiddleware(async (ctx) => {
      if (ctx.path.startsWith("/sign-up")) {
        const issuedSession = ctx.context.getIssuedSession();
        if (issuedSession) {
          ctx.context.runInBackground(sendAnalyticsEvent(issuedSession.user.id));
        }
      }
    }),
  },
      ...
      if (ctx.path.startsWith("/sign-up")) {
        ...
        if (issuedSession) {
          await ctx.context.runInBackgroundOrAwait(
            sendWelcomeEmail(issuedSession.user)
          );
        }
      }
    }),
