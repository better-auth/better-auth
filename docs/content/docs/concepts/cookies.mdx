...
* `session_only` to store the flag when `rememberMe` is disabled (session-only, no `max-age`)

Plugins may also use cookies to store data. For example, the Two Factor Authentication plugin uses the `two_factor_challenge` cookie to store the two-factor authentication state.
