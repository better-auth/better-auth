...
When using a [dynamic base URL](/docs/concepts/dynamic-base-url) (allowed hosts or function), you can omit the `domain` property. Better Auth will automatically derive the cookie domain from the resolved host on each request:
...
export const auth = betterAuth({
    ...
    advanced: {
        crossSubDomainCookies: {
            enabled: true,
            // domain is computed per-request from the resolved baseURL
        },
    },
})
```
