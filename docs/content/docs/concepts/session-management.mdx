...
By default, cookie cache uses `base64-hmac` encoding with an HMAC-SHA256 signature. Alternatively, you can use the `jwt` strategy, which employs JWE (JSON Web Encryption) with A256CBC-HS512 and HKDF key derivation.
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            ...
            strategy: "jwt" // or "base64-hmac" (default)
        }
    }
});
```
...
By default, the cookie cache is used until it reaches `maxAge` without checking the database. You can enable periodic refreshes by setting `freshCache` to `true` (refreshes every 60 seconds) or a custom number of seconds.
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            enabled: true,
            maxAge: 5 * 60, // 5 minutes
            freshCache: 60 // Refresh every 60 seconds
        }
    }
});
```
...
You can run your application without database queries for session validation by setting `freshCache` to `false`. This keeps sessions in signed cookies and avoids database lookups. Useful for serverless deployments or high-traffic applications.
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            ...
            freshCache: false // use cookie until expiry, no database checks
        }
    }
});
```

With this setup, the session data is stored in a signed cookie and the server validates sessions without hitting the database.

### Limitations

When using stateless sessions, keep in mind:

- Session revocation only takes effect after the cookie expires (up to `maxAge`)
- Session listing requires database queries and won't show cookie-only sessions
- User data changes only reflect after the cookie expires

For security-critical applications, consider using `freshCache` with a short interval to balance performance and security.

### Examples

<Accordions>
<Accordion title="Serverless/Stateless Setup">
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            enabled: true,
            maxAge: 60 * 60 * 24, // 1 day
            freshCache: false // no database queries
        }
    }
});
```
...
<Accordion title="Database with Periodic Refresh">
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            enabled: true,
            maxAge: 60 * 60 * 24, // 1 day
            freshCache: 60 // check database every 60 seconds
        }
    }
});
```
...
<Accordion title="Database with Fast Refresh">
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            enabled: true,
            maxAge: 60 * 60 * 24, // 1 day
            freshCache: true // check database every 60 seconds (default)
        }
    }
});
```
