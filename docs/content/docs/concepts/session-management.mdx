...
- **`jwt`**: Standard JWT. By default this uses HMAC-SHA256 (HS256) with your Better Auth secret. If you need JWKS-verifiable session cache cookies, install the `jwt()` plugin and set `session.cookieCache.jwt.keySource = "jwks"` to use its asymmetric signing keys instead.
