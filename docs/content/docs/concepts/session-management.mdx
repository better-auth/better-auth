...
By default cookie cache uses `base64-hmac` encoding with HMAC-SHA256 signature. You can also use `jwt` strategy which uses JWE (JSON Web Encryption) with A256CBC-HS512 and HKDF key derivation.
...
By default, the cookie cache is used until it reaches `maxAge` expiry without checking the database. You can enable periodic refreshes by setting `freshCache` to `true` (refreshes every 60 seconds) or a custom number of seconds.
