...
`GET /get-session` only validates and returns the current session. It does not update or delete the server-side session, and it does not emit `Set-Cookie` headers.
...
The default reactive `useSession` store uses the refresh endpoint automatically. SSR-specific adapters that perform session reads during rendering remain read-only. For an explicit refresh from a client or another writable response boundary, call:
