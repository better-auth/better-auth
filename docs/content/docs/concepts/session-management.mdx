...
You can run your application without database queries for session validation by enabling cookie cache with `freshCache` disabled. This is useful for serverless deployments or high-traffic applications.
...
export const auth = betterAuth({
    session: {
        cookieCache: {
            enabled: true,
            maxAge: 5 * 60, // 5 minutes
            freshCache: false // no database checks
        }
    }
});
```

With this setup, the session data is stored in a signed cookie and the server can validate sessions without hitting the database.
...
<Accordion title="Serverless/Stateless Setup" type="single">
<AccordionItem value="stateless">
...
<Accordion title="High-Security Setup" type="single">
<AccordionItem value="high-security">
...
<Accordion title="Balanced Setup" type="single">
