...
During initialization, Better Auth compares the schema with the tables it writes and reports missing tables, missing columns, and required columns it never fills, together with their fixes. Errors appear through your configured logger without waiting for an authentication request. Requests await the same check and fail if the schema does not match; validation does not automatically stop your server or build. Run [`npx auth@latest check-schema`](/docs/concepts/cli#check-schema) in your build pipeline to fail before deployment.
...
A `modelName` cannot reuse another Better Auth model's identifier. For example,
don't set `user.modelName` to `account`, even if
`account.modelName` is customized, because Better Auth still uses `account`
internally to resolve account fields and relations.
