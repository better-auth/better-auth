...
An account represents one authentication method linked to a user. Better Auth recognizes the provider-side identity by the unique pair of `issuer` and `providerAccountId`, while `id` identifies the local account row. Use `id` when an account API asks for an `accountId`.
...
The database enforces a unique compound index on `issuer` and `providerAccountId`. Providers with a trusted issuer use that authority, including the verified issuer for OpenID Connect. OAuth providers without an issuer use the synthetic `local:oauth:<providerId>` namespace. Credential accounts use `local:credential` and the linked user's stable `id`.
