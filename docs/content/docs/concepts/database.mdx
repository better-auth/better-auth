...
The scope field must be a required, server-owned user additional field
(`required: true`, `input: false`). Better Auth resolves it for every database
operation, injects it into scoped records, and adds it to reads, updates, and
deletes. User email uniqueness becomes composite over the scope field and
email. Account, session, and verification records are scoped automatically.
