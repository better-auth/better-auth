...
<Callout type="info">
  **Field Retrieval**: The `returned` property controls whether a field appears in query results (default: `true`). Set `returned: false` only when you want to store a field in the database but exclude it from being retrieved in normal operations (e.g., internal metadata or audit fields).
</Callout>
