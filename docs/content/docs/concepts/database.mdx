...
<Callout type="warn">
  MySQL does not support returning inserted rows.
  We work around this by using pre-generated IDs, then querying the inserted row.
  
  If you disable ID generation or use database-level ID generation,
  this workaround won't work, and alternative methods are prone to race conditions.
  We recommend avoiding database-level ID generation for these reasons for MySQL databases.
</Callout>
