...
The check runs outside production and keeps its verdict for the life of the process. The built-in Kysely adapter introspects the database once. `npx auth migrate` reports the same problems before it changes anything.
