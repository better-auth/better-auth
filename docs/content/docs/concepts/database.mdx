...
The database enforces a unique compound index on `issuer` and `providerAccountId`. Built-in and Generic OAuth providers with a trusted issuer use that authority, including the verified issuer for OpenID Connect. Providers without an issuer use the synthetic `local:<encoded providerId>` namespace, where the provider ID segment is percent-encoded. Persisted SSO providers use an immutable provider-instance namespace. Credential identities use `local:credential` and the linked user's stable `id`.
