...
An account represents one authentication method linked to a user. By default, Better Auth recognizes the provider-side identity by the unique pair of `providerId` and `accountId`, while `id` identifies the local account row. Use `id` when an account API asks for an `accountId`.
...
export const accountTableFields = [
	...
	},
	{
		...
		type: "string",
		description:
			"The trusted authority that issued the provider account identifier. Available when account.identityStrategy is set to issuer",
		isOptional: true,
	},
	{
		name: "accountId",
...
The database enforces a unique compound index on `providerId` and `accountId`. Set [`account.identityStrategy`](/docs/reference/options#account) to `"issuer"` to use issuer-scoped identity instead. That option adds the `issuer` field and changes the compound index to `issuer` and `accountId`.
