...
Better Auth uses `nanoId` by default to generate unique IDs for users, sessions, and other entities. If you want to customize how IDs are generated, you can now configure this in the <Link href="/docs/reference/options#advanced">Advanced</Link> section of the Better Auth options.
...
**Example Usage Of Custom ID Generation**
```ts title="auth.ts" 
import { betterAuth } from "better-auth";
import { db } from "./db";
...
You can also enable automatic database ID generation by setting the `useAutoGeneratedId` option to `true` in your adapter configuration.

**Example Usage Of Automatic Database IDs**
```ts title="auth.ts" 
import { betterAuth } from "better-auth";
import { db } from "./db";
...
export const auth = betterAuth({
   database: {
      db: db,
      useAutoGeneratedId: true
   }
})
```

Note: If `useAutoGeneratedId` is set to `true`, it takes precedence over `generateId` and will use a database-generated ID regardless of whether `generateId` is also defined.
