...
An account represents one authentication method linked to a user. `providerId` identifies the configured provider, `accountId` identifies the account at that provider, and `issuer` provides its namespace. Better Auth identifies an external account by `(issuer, accountId)`. The `id` field identifies the local account record and is used by account management APIs.
...
[`account.identityScope`](/docs/reference/options#identityscope) controls the value stored in `issuer` without changing the schema. The `"issuer"` scope stores the verified authority when available. The `"provider"` scope stores a deterministic namespace such as `local:oauth:google`. Credential accounts use `local:credential`.
