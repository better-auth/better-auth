...
export const accountTableFields = [
	...
	},
	{
		...
		type: "string[]",
		description:
			"The OAuth scopes the user has granted to this account (accumulated; see the OAuth scopes docs).",
		isOptional: true,
	},
	{
		name: "idToken",
...
Plugins can define their own tables in the database to store additional data. For example, the two factor authentication plugin adds dedicated method tables:

* `twoFactorMethod`: method metadata such as `kind`, `label`, and `verifiedAt`.
* `twoFactorTotp`: encrypted TOTP secrets keyed by method ID.
* `twoFactorRecoveryCode`: hashed recovery-code rows keyed by method ID.
* `trustedDevice`: trusted-device records keyed by user ID.
