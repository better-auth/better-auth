...
An account represents one authentication method linked to a user. Every generated Better Auth 1.7 schema recognizes the persisted identity by the unique pair of `issuer` and `accountId`, while `id` identifies the local account row. Use `id` when an account API asks for an `accountId`.
...
export const accountTableFields = [
	...
	},
	{
		name: "accountId",
		type: "string",
		description: "The stable account identifier within the provider namespace",
	},
	{
		name: "providerId",
		type: "string",
...
The database requires `issuer` and `accountId` and enforces a unique compound index across them for every account identity strategy. With the default `"issuer"` strategy, `issuer` contains the verified authority. With [`account.identityStrategy: "provider-id"`](/docs/reference/options#identitystrategy), it contains a deterministic `local:oauth:<encoded providerId>` namespace, or `local:credential` for credential accounts. The compatibility strategy changes the stored namespace value, not the generated schema.
