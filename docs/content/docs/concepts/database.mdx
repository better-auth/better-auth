...
The Mikro ORM adapter expects the `MikroORM` instance created via `MikroORM.init` or `MikroORM.initSync`. The adapter will not manage database schema for you, so use Mikro ORM CLI for that.

```ts title="auth.ts"
import { betterAuth } from "better-auth";
import { mikroOrmAdapter } from "better-auth/adapters/mikro-orm-adapter";
...
- Since the session token acts as the token, it should be both unique and sufficiently long for security.
