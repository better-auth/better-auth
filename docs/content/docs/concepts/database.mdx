...
The `fields` configuration maps Better Auth's default field names to your database column names. This is a **database-only mapping** - your JavaScript/TypeScript code will always use the original field names for type safety and consistency.

**Example with snake_case database columns:**
...
CREATE TABLE users (
  id TEXT PRIMARY KEY,
  email TEXT NOT NULL,
  email_verified BOOLEAN,  -- mapped from emailVerified
  created_at TIMESTAMP,    -- mapped from createdAt
  updated_at TIMESTAMP,    -- mapped from updatedAt
  ...
);
```

**In your application code (always use original names):**

```ts title="middleware.ts"
// ✅ Correct - use original field names
const session = await auth.api.getSession({ headers });
if (session?.user) {
  console.log(session.user.emailVerified);  // Use camelCase
  console.log(session.user.createdAt);
  console.log(session.user.updatedAt);
}

// ❌ Wrong - these properties don't exist in JavaScript
console.log(session.user.email_verified);  // undefined
console.log(session.user.created_at);      // undefined
```

<Callout type="info">
  **Why this design?** Field mapping only affects database column names, keeping your TypeScript code consistent with camelCase conventions while allowing snake_case or other naming conventions in your database. This ensures type safety and prevents breaking changes when migrating existing databases.
</Callout>
