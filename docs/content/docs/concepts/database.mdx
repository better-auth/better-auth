...
<Callout type="info">
  If you're using any database adapter (official or community), import `betterAuth` from `better-auth/minimal` instead of `better-auth`. The minimal export excludes Kysely and reduces bundle size. Direct database connections and built-in migrations require the full `better-auth` import. See [Bundle Size Optimization](/docs/guides/optimizing-for-performance#bundle-size-optimization) for details.
</Callout>
