...
The context object (`ctx`), passed as the second argument to the hook, contains useful information like the current `session`. You can use this to access the logged-in user's details.
    ...
    user: {
      update: {
        before: async (data, ctx) => {
          ...
          // Return the data to proceed with the update.
          return { data };
        },
      },
    },
...
Much like standard hooks, database hooks also provide a `ctx` object that offers a variety of useful properties, including the `session` object which contains information about the current user session. Learn more in the [Hooks Documentation](/docs/concepts/hooks#ctx).
