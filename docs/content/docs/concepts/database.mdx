...
Better Auth uses `nanoId` by default to generate unique IDs for users, sessions, and other entities. If you want to customize how IDs are generated, you can now configure this in the `advanced` section of the <Link href="/docs/reference/options">options</Link>.
