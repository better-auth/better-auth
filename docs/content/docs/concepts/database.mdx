...
- `returned`: Controls whether the field is included in SELECT queries (default: `true`). Set to `false` if you want the field stored in the database but not retrieved in read operations.
- `fieldName`: The actual column name in the database if it differs from the JavaScript property name.
...
export const auth = betterAuth({
  user: {
    additionalFields: {
      ...
      firstName: {
        type: "string",
        fieldName: "first_name", // Database column name
        returned: true, // Required to include field in SELECT queries
      },
      internalNotes: {
        type: "string",
        returned: false, // Stored but not retrieved in normal queries
      },
    },
  },
});
...
<Callout type="warn">
  **Important**: If you add a custom field with a `fieldName` that differs from the property name, you must set `returned: true` to ensure the field is included in SELECT queries. Without this, the field will be stored but not retrieved from the database.
</Callout>
...
#### Option 1: Disable ID Generation for All Tables

Setting `generateId` to `false` disables ID generation globally, letting your database handle all ID generation (useful for auto-increment/serial columns):
    ...
    database: {
      generateId: (options) => {
        ...
        if (options.model === "user" || options.model === "users") {
          return false; // or return undefined as any
        }
...
export const auth = betterAuth({
  ...
  advanced: {
    database: {
      ...
      generateId: (options) => {
        if (options.model === "user" || options.model === "users") {
          return undefined as any; // Let PostgreSQL serial generate it
        }
...
<Callout type="info">
  **Property Names**: In `databaseHooks`, always use JavaScript property names (e.g., `firstName`) not database column names (e.g., `first_name`). Better Auth handles the mapping internally based on your `fieldName` configuration.
</Callout>
