...
Jobs, scripts, and other work outside an HTTP request must call
`runWithEndpointContext` with a request (or headers) that `resolve` can
read. There is no unscoped adapter escape hatch: operations without a
