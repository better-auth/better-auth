---
title: Database
description: Learn about database adapters, migrations, secondary storage with Redis, core schema (user, session, identity, account, verification), custom tables, extending schemas, ID generation, database hooks, and plugin schemas.
---
...
An Identity links a Better Auth user to a stable identifier from an issuing authority. The unique pair of `issuer` and `providerAccountId` recognizes the same provider-side identity across provider configurations.
...
export const identityTableFields = [
	...
	},
	{
		name: "userId",
		type: "string",
		description: "The ID of the user that owns the Identity",
		isForeignKey: true,
		references: { model: "user", field: "id", onDelete: "restrict" },
	},
	{
		name: "issuer",
		type: "string",
		description: "The trusted authority that assigned the provider identifier",
	},
	{
		name: "providerAccountId",
		type: "string",
		description: "The stable identifier within the issuer namespace",
	},
	{
		name: "createdAt",
		type: "Date",
	...
	},
	{
		name: "updatedAt",
		type: "Date",
		description: "Timestamp of when the Identity was updated",
	},
];

<DatabaseTable name="identity" fields={identityTableFields} />

The database enforces a unique compound index on `issuer` and `providerAccountId`. Built-in and Generic OAuth providers with a trusted issuer use that authority, including the verified issuer for OpenID Connect. Providers without an issuer use the synthetic `local:oauth:<encoded providerId>` namespace, where the provider ID segment is percent-encoded. Persisted SSO providers use an immutable provider-instance namespace. Credential identities use `local:credential` and the linked user's stable `id`.

Identity ownership uses a restricted foreign key. Delete a User through Better Auth's User lifecycle API so dependent Sessions, Accounts, and Identities are removed in the correct order and their hooks run. Directly deleting a User with dependent Identity or Session rows is rejected by the database.

### Account

Table Name: `account`

An Account represents one provider configuration or credential method attached to an Identity. It owns configuration-specific credentials, grants, and token state. Use its `id` when an account API asks for an `accountId`.
...
export const accountTableFields = [
	...
	},
	{
		name: "identityId",
		type: "string",
		description: "The ID of the Identity authenticated through this account",
		isForeignKey: true,
		references: { model: "identity", field: "id", onDelete: "restrict" },
	},
	{
		name: "providerId",
		type: "string",
		description: "The public alias of the provider configuration",
	},
	{
		name: "providerInstanceId",
		type: "string",
		description: "The stable internal namespace of the provider instance",
	},
	{
		name: "accessToken",
		type: "string",
...
The database enforces a unique compound index on `identityId` and `providerInstanceId`. `providerId` is the public configuration alias used by account APIs and provider lookup, while `providerInstanceId` is a stable internal namespace that prevents credentials and lifecycle operations from crossing provider instances. Built-in, social, Generic OAuth, and credential Accounts use their provider alias for both fields. Persisted SSO providers use `sso:provider:<recordId>` as the instance ID, and providers declared in `defaultSSO` use `sso:config:<providerId>`.

The Account-to-Identity foreign key is also restricted. Unlink Accounts through Better Auth before deleting an Identity so account-owned records, credentials, and hooks are handled consistently.
...
Better Auth allows you to customize the table names and column names for the core schema. You can also extend the core schema by adding additional fields to the User, Session, Identity, and Account tables.
...
Database hooks allow you to define custom logic that runs during core database operations in Better Auth. You can create hooks for the User, Session, Identity, and Account models.
...
* **Purpose**: This hook runs before the corresponding User, Session, Identity, or Account is created, updated, or deleted.
...
* **Purpose**: This hook runs after the respective entity is created, updated, or deleted.
* **Behavior**: Use it for follow-up work that depends on the committed database result.

<Callout type="info">
  After hooks run after the database operation commits. If an after hook throws,
  the caller receives the error, but the committed operation remains in the
  database. Put validation that must prevent a write in a before hook.
</Callout>
