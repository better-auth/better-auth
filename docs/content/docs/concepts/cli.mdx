...
- `--adapter` - Specify the adapter type (`prisma`, `drizzle`, or `kysely`) without requiring a pre-configured adapter in your project.
- `--dialect` - Specify the database dialect/provider (`postgresql`, `mysql`, `sqlite`). Used alongside `--adapter` to provide complete database configuration.

### Generate Without a Database Connection

You can generate schemas without a configured database connection by using the `--adapter` and `--dialect` flags. This is useful for:

- Bootstrapping a new project before database setup
- CI/CD pipelines where database access isn't available
- Generating schemas for different adapters or dialects
...
# Generate SQL file for SQLite (Kysely)
npx @better-auth/cli@latest generate --adapter kysely --dialect sqlite
```

When these flags are provided, the CLI creates a mock adapter internally, allowing schema generation without loading your database configuration.
