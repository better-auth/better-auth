...
icon: conceptsCLI
title: CLI
description: Learn about the Better Auth CLI commands for generating, migrating, and checking database schemas, creating initial admins, initializing projects, generating secret keys, and gathering diagnostic info.
---
...
<Callout type="info">
  **Using PostgreSQL with a non-default schema?**
  ...
  Set `database.schemaName` to select the schema explicitly. Otherwise, the migrate command uses the connection's `search_path`. See the [PostgreSQL adapter documentation](/docs/adapters/postgresql#use-a-non-default-schema) for configuration details.
</Callout>

## Check Schema

The `check-schema` command verifies that your configured schema can hold what Better Auth writes. It does not create files or apply migrations. Run it before your build to fail early when a schema change was missed:

```bash title="Terminal"
npx auth@latest check-schema --config ./src/auth.ts && pnpm build
```

The command checks the live database for Kysely, the configured schema object for Drizzle, and the generated client model for Prisma. Drizzle and Prisma checks do not confirm that migrations have been applied to the database. If validation is unavailable for your adapter, the command fails instead of silently skipping it. It also runs when runtime validation is disabled with `advanced.database.validateSchema: false`.
