...
The `check` command runs all available checks. Currently, it checks whether your configured schema can hold what Better Auth writes. It does not create files or apply migrations. Run it as a separate CI or pre-deployment step to catch missed schema changes:
...
To run the schema check specifically, use `check schema`:
