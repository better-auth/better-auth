...
<Callout type="warn">
  When upgrading a Drizzle or Prisma project from 1.6, update the ORM schema used by your Better Auth configuration before running the migration, but do not apply that schema to the database. Drizzle users should make the adapter import the generated 1.7 schema. Prisma users should update `schema.prisma` and run `prisma generate`. Keep existing physical table and column mappings unchanged. The database must remain on the 1.6 schema until `migrate apply` performs the reviewed data and schema changes together.
</Callout>
...
<Callout type="info">
  Plain `migrate`, `--dry-run`, and `--plan &lt;file&gt;` remain available temporarily and print their replacement command.
</Callout>
