...
Use `--dry-run` to preview a migration:

```package-install title="Terminal"
npx auth@latest migrate --dry-run
```

Use `--json` in CI or with a coding agent:

```package-install title="Terminal"
npx auth@latest migrate --json > better-auth-migration-plan.json
```

Both commands leave the database unchanged. The JSON output includes the detected adapter and dialect, sorted schema changes, and any problems that need attention. A blocked plan exits with status `1`.

### Migrating a Better Auth 1.6 database

`migrate` detects a 1.6 database and guides you through the decisions it cannot derive from your configuration. Dynamic issuers are reviewed per account, and legacy OAuth clients require confirming how 1.6 stored their secrets; unsupported storage transitions stop for client rotation or re-registration. The command saves the answers to `better-auth-migration.json` and shows every irreversible action before changing the database.
...
In a non-interactive environment, a migration that still needs decisions exits with status `1` instead of guessing. For the complete upgrade order and feature-specific actions, see the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide).
