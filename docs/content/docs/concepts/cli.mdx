...
The JSON plan includes the adapter, dialect, sorted schema changes, blockers, and any release migration actions. A blocked plan exits with status `1`.
...
For non-interactive JSON application, also pass `--yes`; otherwise the command exits without changing the database. Plain `npx auth@latest migrate` remains an alias for `migrate apply` and prints a deprecation warning. For the current release migration procedure, including Drizzle and Prisma preparation, see the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide#migrate-from-16-to-17).
