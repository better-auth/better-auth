...
The JSON plan includes the adapter, dialect, effective and stored account identity scopes, sorted schema changes, blockers, and any release migration actions. A blocked plan exits with status `1`.
...
The generated `auth.ts` includes `account: { identityScope: "provider" }`. Existing configurations that omit this option continue to use issuer scope. `auth migrate plan` reports this compatibility choice.
