...
* `--adapter` - The schema generator to use instead of the adapter from your Better Auth config. Supported built-in values are `prisma`, `drizzle`, and `kysely`.
* `--dialect` - The database dialect/provider to use with `--adapter`. Prisma supports `postgresql`, `mysql`, `sqlite`, and `mongodb`. Drizzle supports `postgresql`, `mysql`, and `sqlite`; `postgresql` maps to `pg` internally.

### Generate Without a Database Connection

For Prisma and Drizzle, you can pass `--adapter` and `--dialect` to generate a schema without initializing the database adapter from your Better Auth config. The config file is still loaded so the generated schema includes your plugins and other schema customizations.
...
export const schemaGeneratorOptions = {
  adapter: {
    description: "The built-in schema generator. Database-free generation is available for Prisma and Drizzle; Kysely uses the database configured in your Better Auth config.",
    type: '"prisma" | "drizzle" | "kysely"',
    required: false,
  },
  dialect: {
    description: 'The database dialect for Prisma or Drizzle. "mongodb" is Prisma-only, and Drizzle maps "postgresql" to "pg".',
...
<Callout type="info">
  Kysely generates migration SQL by introspecting the configured database, so it still requires a working database connection.
</Callout>

If `--adapter` is omitted, the CLI uses the adapter configured in your Better Auth config file.
