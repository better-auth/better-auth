...
- `--force` - Force generation of schema file even if no changes are detected. This is useful when you need to regenerate migration files after deleting them.
...
- `--force` - Force migration by treating the database as empty, recreating all tables. Use this when you need to regenerate your database schema from scratch.

<Callout type="warn">
The `--force` flag will treat your database as if it has no existing tables. This is useful for development environments or when you need to regenerate your schema, but should be used with caution in production environments.
</Callout>
