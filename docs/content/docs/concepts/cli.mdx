...
The `upgrade` command updates `better-auth` and official `@better-auth/*` packages to the exact version of the CLI being run. The package specifier selects the release channel, so prerelease upgrades do not require an additional CLI option.
