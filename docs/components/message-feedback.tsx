...
import { submitFeedbackToInkeep, logEventToInkeep } from "@/lib/inkeep-analytics";
...
export function MessageFeedback({ messageId, userMessageId, content, className }: MessageFeedbackProps) {
  const [feedback, setFeedback] = useState<"positive" | "negative" | null>(null);
  const [copied, setCopied] = useState(false);
  const [isSubmittingFeedback, setIsSubmittingFeedback] = useState(false);
  const [showSuccessCheckmark, setShowSuccessCheckmark] = useState<"positive" | "negative" | null>(null);
      ...
      await submitFeedbackToInkeep(feedbackMessageId, type, [
        {
          ...
          details: type === "positive" ? "The response was helpful" : "The response was not helpful",
        },
      ]);
  ...
  return (
    <div className={cn("flex items-center gap-1 mt-3 pt-2 border-t border-fd-border/30", className)}>
      <button
        type="button"
        ...
        title={showSuccessCheckmark === "positive" ? "Feedback submitted!" : "Helpful"}
      >
      ...
      <button
        ...
        title={showSuccessCheckmark === "negative" ? "Feedback submitted!" : "Not helpful"}
      >
      ...
      <button
        ...
        className={cn(
          buttonVariants({
            size: "icon-sm",
            color: "ghost",
            className: "h-7 w-7 hover:bg-fd-accent hover:text-fd-accent-foreground transition-colors",
          })
        )}
