          ...
          <ul className="md:flex items-center divide-x w-max hidden shrink-0">
            {navMenu.map((menu, i) => (
              <NavLink key={menu.name} href={menu.path}>
                {menu.name}
                {menu.name === "changelogs" && (
                  <Link href={"/changelogs/1-2"} className="">
                    <span
                      className={cn(
                        "absolute group-hover:text-zinc-100 transition-colors top-0 right-0 text-[10.5px] font-mono tracking-[-0.06rem] w-7 flex justify-center items-center h-5 border rounded-none",
                        "animate-bg-shine border-l-[1px] border-b-[1px] border-r-0 bg-[length:200%_100%] duration-[220ms]",
                        "dark:bg-[linear-gradient(110deg,#09090B,45%,#27272A,55%,#09090B)] dark:text-muted-foreground dark:border-zinc-700/60",
