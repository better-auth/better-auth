...
function getApiKey(): string {
  const apiKey = process.env.INKEEP_ANALYTICS_API_KEY || process.env.INKEEP_API_KEY;
  if (!apiKey) {
    throw new Error("INKEEP_ANALYTICS_API_KEY or INKEEP_API_KEY environment variable is required");
  }
  return apiKey;
}
...
async function makeAnalyticsRequest(endpoint: string, data: any) {
  ...
  const { data: result, error } = await betterFetch(`${INKEEP_ANALYTICS_BASE_URL}${endpoint}`, {
    method: "POST",
    headers: {
  ...
  if (error) {
    throw new Error(`Inkeep Analytics API error: ${error.status} ${error.message}`);
  }

  return result;
}

export async function logConversationToAnalytics(conversation: InkeepConversation) {
  try {
...
export async function logConversationToInkeep(messages: InkeepMessage[]) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to log conversation: ${error.status} - ${error.message}`);
    }
...
export async function submitFeedbackToInkeep(messageId: string, type: "positive" | "negative", reasons?: Array<{ label: string; details?: string }>) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to submit feedback: ${error.status} - ${error.message}`);
    }
...
export async function logEventToInkeep(type: string, entityType: "message" | "conversation", messageId?: string, conversationId?: string) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to log event: ${error.status} - ${error.message}`);
    }
