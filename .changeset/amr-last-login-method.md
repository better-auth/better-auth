...
- `verifyTwoFactor` returns a discriminated union (`{ mode: "complete", ... } | { mode: "management", ... }`) instead of a collapsed shape. Callers that destructure `valid` must switch on `mode`: complete-mode `valid(ctx, factor)` finalizes the paused sign-in and appends the second factor to `session.amr`; management-mode `valid(ctx)` returns the existing session with no AMR mutation.
...
- Two-factor verify flow fixes:
  ...
  - `trustDevice: true` is now honored in management mode (step-up / enrollment on an already-authenticated session); it previously only fired in complete mode and was silently dropped for signed-in callers.
