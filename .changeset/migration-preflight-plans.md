...
When upgrading a populated Better Auth 1.6 SQL database, the CLI now requires an explicit account identity strategy before it backfills the required issuer namespace. The provider-id compatibility path writes deterministic provider namespaces, while the issuer path writes reviewed authorities; both enforce the 1.7 `(issuer, accountId)` index. The same migration can move OAuth clients and eligible consents, revoke legacy provider tokens, and retire legacy SCIM state before reprovisioning. Decisions that require review are saved in `better-auth-migration.json` with the `1.6-to-1.7` transition and effective identity namespaces, so the same choices can be inspected and replayed without silently changing under another configuration. The migration also repairs accounts left with empty MySQL issuers and keeps indexed identity fields within MySQL and SQL Server limits.
