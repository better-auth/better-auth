...
This raw-`sub` exposure also lets colluding `pairwise` clients correlate users, so a one-time dev warning is logged when a `pairwise` client is issued a JWT access token. Configure such clients to receive opaque access tokens, or silence it with `silenceWarnings.pairwiseJwtAccessToken` once you've accepted the trade-off.
